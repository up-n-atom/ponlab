...
void* sendMsg(__attribute__ ((unused))void *vagp)
{
	...
	while(1)
	{
		...
		snprintf(xMsg.sSerName, MAX_SERVER_NAME, "servd");
		snprintf(xMsg.sSLId, MAX_SL_NAME, "sl_app");
		xMsg.nOperType=OPT_ADD;
		...
		{
			a[nCnt].a=nCnt+1;
			snprintf(a[nCnt].b, MAX_VAL, "MsgAPI:%d", nCnt+2);
			a[nCnt].c=nCnt+3;
			a[nCnt].d=nCnt+3.3;
			a[nCnt].f.a=nCnt+4;
			snprintf(a[nCnt].f.b, MAX_VAL, "MsgAPI:%d", nCnt+5);
			a[nCnt].x=nCnt+4;
		}
		...
		snprintf(pc,100, "name:value,name:value,name:value,name:value,name:value,........,name:value");
