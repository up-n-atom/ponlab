			   ...
			   struct ltq_proc_entry *proc_entry)
{
	...
	memset(&proc_entry->proc_ops, 0, sizeof(struct proc_ops));

	if (proc_entry->single_callback) {
		proc_entry->proc_ops.proc_open = ltq_proc_single_open;
		proc_entry->proc_ops.proc_release = single_release;
	} else {
		proc_entry->proc_ops.proc_open = ltq_proc_open;
		proc_entry->proc_ops.proc_release = ltq_proc_release;
	}

	proc_entry->proc_ops.proc_read = seq_read;
	proc_entry->proc_ops.proc_lseek = seq_lseek;
	proc_entry->proc_ops.proc_write = proc_entry->write_callback;
	proc_create_data(proc_entry->name,
			 (S_IFREG | S_IRUGO),
			 parent_node, &proc_entry->proc_ops, proc_entry);
#else
