		...
		switch(iSel) {
			...
			case 2:
				...
				if(enum_nv[iIndex].value == *uiId) {
					if(sprintf_s(trVal, MAX_LEN_PARAM_VALUE, "%s", enum_nv[iIndex].name)<=0)
						return UGW_FAILURE;
...
int32_t help_objlistToStructConv(ObjList *pxTmpObj, void *pxStruct, x_LTQ_Param xParamDetail[], int32_t iSize)
{
	...
	FOR_EACH_PARAM(pxTmpObj, pxParam) {
		...
		for(iIndex = 0;iIndex < iSize;iIndex++) {
			...
			nLen=strnlen_s(pxParam->sParamName,MAX_LEN_PARAM_NAME);
			if(nLen <= 0 || nLen > MAX_LEN_PARAM_NAME)
				return UGW_FAILURE;
			...
			if(0 == strncmp(pxParam->sParamName,xParamDetail[iIndex].acParamName,
				...
				switch(xParamDetail[iIndex].uiType) {
					...
					case LTQ_PARAM_TYPE_UCHAR:
						*((char*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) = atoi(pxParam->sParamValue);
						iFlag = 1;
						break;

					case LTQ_PARAM_TYPE_SHORT_INT:
					case LTQ_PARAM_TYPE_USHORT_INT:
						*((unsigned short*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) =
					...
					case LTQ_PARAM_TYPE_UINT:
						*((int32_t*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) = atoi(pxParam->sParamValue);
						iFlag = 1;
						break;
					...
					case LTQ_PARAM_TYPE_ENUM:
						...
						*((int32_t*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) = nEnumVal;
						break;

					case LTQ_PARAM_TYPE_ULONG:
					case LTQ_PARAM_TYPE_LONG:
						*((long*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) =
							atol(pxParam->sParamValue);
						iFlag = 1;
						break;

					case LTQ_PARAM_TYPE_FLOAT:
						*((float*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) =
							atof(pxParam->sParamValue);
						iFlag = 1;
						break;

					case LTQ_PARAM_TYPE_STR:
						if(sprintf_s(((char*)(uintptr_t)pxStruct + xParamDetail[iIndex].unOffset),MAX_LEN_PARAM_VALUE, "%s", pxParam->sParamValue)<=0)
							return UGW_FAILURE;
						iFlag = 1;
						break;

					case LTQ_PARAM_TYPE_BOOL:
						*((bool*)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)) = (bool)(!strcasecmp(pxParam->sParamValue,"true")?1:0);
						iFlag = 1;
						break;
	...
	*((int32_t *)((uintptr_t)pxStruct + mapOff)) = xParamMap;
...
int32_t help_structToObjlistConv(ObjList *pxTmpObj, void *pxStruct, x_LTQ_Param xParamDetail[], int32_t iSize)
{
	...
	/* scan the list to get the required param offset and type info. */
		for(iIndex = 0;iIndex < iSize;iIndex++) {
			...
			if(*((uint32_t *)((uintptr_t)pxStruct + mapOff)) & iParamBit) {
				switch(xParamDetail[iIndex].uiType) {
					case LTQ_PARAM_TYPE_OBJ_LIST:
					//case LTQ_PARAM_TYPE_OBJ:
						break;
					...
					case LTQ_PARAM_TYPE_CHAR:
					case LTQ_PARAM_TYPE_UCHAR:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%c", *((char *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_SHORT_INT:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%hu", *((short *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_USHORT_INT:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%hd", *((unsigned short *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_INT:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%d", (int)*((int32_t *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_ENUM:
						nEnumVal = *((int32_t *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset));
						if(enumMapGet(xParamDetail[iIndex].nvLookUp, xParamDetail[iIndex].unLookUpSize,
					...
					case LTQ_PARAM_TYPE_UINT:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%u", *((uint32_t *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;

						break;

					case LTQ_PARAM_TYPE_ULONG:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%lu", *((unsigned long *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_LONG:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%ld", *((long *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_FLOAT:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%f", *((float *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)))<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_STR:
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%s", (char *)(uintptr_t)pxStruct + xParamDetail[iIndex].unOffset)<=0)
							return UGW_FAILURE;
						break;

					case LTQ_PARAM_TYPE_BOOL:
						bTmp = *((bool *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset));
						if(sprintf_s(sParamValue, MAX_LEN_PARAM_VALUE, "%d", bTmp)<=0)
							return UGW_FAILURE;
						break;
        ...
        if(NULL == pxTmpObj || NULL == pxStruct || NULL == xParamDetail)
          ...
          FOR_EACH_PARAM(pxTmpObj, pxParam) {
                        ...
                        switch(xParamDetail[iIndex].uiType) {
                                ...
                                case LTQ_PARAM_TYPE_CHAR:
                                case LTQ_PARAM_TYPE_UCHAR:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%c", *((char *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_SHORT_INT:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%hu", *((short *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_USHORT_INT:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%hd", *((unsigned short *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;
                                ...
                                case LTQ_PARAM_TYPE_INT:
                                case LTQ_PARAM_TYPE_ENUM:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%d", (int)*((int32_t *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_UINT:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%u", *((uint32_t *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_ULONG:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%lu", *((unsigned long *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_LONG:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%ld", *((long *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_FLOAT:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%f", *((float *)((uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)))<=0)
						return UGW_FAILURE;
                                        break;

                                case LTQ_PARAM_TYPE_STR:
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%s", (char *)(uintptr_t)pxStruct
                                                                        + xParamDetail[iIndex].unOffset)<=0)
						return UGW_FAILURE;
                                        break;
                                ...
                                case LTQ_PARAM_TYPE_BOOL:
                                        bTmp = *((bool *)((uintptr_t)pxStruct + xParamDetail[iIndex].unOffset));
                                        if(sprintf_s(pxParam->sParamValue,MAX_LEN_PARAM_VALUE, "%d", bTmp)<=0)
						return UGW_FAILURE;
                                        break;
...
int ubusMsgToPolld(IN MainOper eOperation, IN MsgHeader *pxDynMsg, UNUSED_ARG OUT MsgHeader *pxOutputMsg, IN PolldTimeInfo *pxTimerList)
{
	...
	help_objListToBlobPolld(&xBuf, pxDynMsg, NO_ARG_VALUE, NO_ARG_VALUE, pxTimerList);
	...
	*/
	...
	{
		nRet = ubus_invoke(pxPolldCtx, unId, "get", xBuf.head, polld_getValueCb, (void *)&xRespCode, CAL_UBUS_SERVD_TIMEOUT);
		if(nRet != UGW_SUCCESS)
		{
			       ...
			       IN char *psUbusObject, IN char *psUbusMethod)
{
	...
	if ((nRet = ubus_invoke(pxUbusCtx, unId, psUbusMethod, pxBlob->head, NULL,
	     (void *)&xRespCode, CAL_UBUS_SERVD_TIMEOUT)) != UGW_SUCCESS) {
				  ...
				  OUT ParamList *pxParam)
{
	...
	char sVal[MAX_LEN_PARAM_VALUE] = { '\0' };
        ...
        __blob_for_each_attr(attr, head, len) {
                ...
                {
                        case BLOBMSG_TYPE_INT32:
				if(sprintf_s(sVal, MAX_LEN_PARAM_VALUE, "%d", blobmsg_get_u32(attr))<=0)
					return UGW_FAILURE;
...
int32_t help_WsdDataToBlob(struct blob_buf *buf, WsdData *pxWsdData)
{
        memset(buf, 0x0, sizeof(struct blob_buf));
        blobmsg_buf_init(buf);
        help_objListToBlob(buf, &pxWsdData->xMsgHdr, NO_ARG_VALUE, NO_ARG_VALUE);
...
int32_t help_blobToWsdData(struct blob_attr *msg, WsdData *pxWsdData)
{
        ...
        help_blobToObjList(msg, &pxWsdData->xMsgHdr);
...
int32_t help_sendStatusToWeb(uint32_t unNotifyId, unsigned char ucType, char *pcMessage)
{
	...
	help_notifyListToBlob(&buf, &xNotifyData);
...
/*!     \brief  This function is to retrive timestamp and verion 
        \param[in] sVersion
        \return on UGW_SUCCESS/UGW_FAILURE
*/
int help_getCurrVer(char *sVersion)
{
        int32_t nFd = 0, nCnt1 = 0, nCnt2 = 0;
        char    sTmpBuf[MAX_LEN_PARAM_VALUE] = {0};
        char    sTmp[MAX_LEN_PARAM_VALUE] = {0};

        /* get version of new image and update in DB along with history. */
        nFd = open(HELP_IMG_VER_FILE, O_RDONLY);
        if (nFd < 0) {
                LOGF_LOG_INFO("Failed to extract new image version.\n");
                return UGW_FAILURE;
        }
        else {
                nCnt1 = read(nFd, sTmp, MAX_LEN_PARAM_VALUE - 2);
                close(nFd);
        }
        if(nCnt1 > 0) {
                sTmp[nCnt1 - 1] = '\0'; /* skip newline. */
        }
        nFd = open(HELP_IMG_TS_FILE, O_RDONLY);
        if (nFd < 0) {
                LOGF_LOG_INFO("Failed to extract new image version.\n");
                return UGW_FAILURE;
        }
        else {
                nCnt2 = read(nFd, sTmpBuf, sizeof(sTmpBuf) - 2);
                close(nFd);
        }
        if(nCnt2 > 0) {
                sTmpBuf[nCnt2 - 1] = '\0'; /* skip newline. */
        }
        if(nCnt1 > 0) {
                strcat_s(sTmp, MAX_LEN_PARAM_VALUE, "-");
                strncat_s(sTmp, MAX_LEN_PARAM_VALUE, sTmpBuf, sizeof(sTmpBuf));
        }
        else {
                if(sprintf_s(sTmp, MAX_LEN_PARAM_VALUE, "%s", sTmpBuf)<=0)
			return UGW_FAILURE;
        }

	if(sprintf_s(sVersion, MAX_LEN_PARAM_VALUE, "%s", sTmp)<=0)
		return UGW_FAILURE;
