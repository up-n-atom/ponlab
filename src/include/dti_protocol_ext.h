...
/** while image update set the use count */
#define DTI_IMAGE_CTRL_UPD_PENDING   -1
/**
...
/**
   ...
   from the user context to save the image. The control struct is marked
   for the image update (update pending)
...
/**
   Function type to release the image control struct.
   (clear the update pending marker)
...
\param
   pImageControl  - image control struct from the user context.
\param
...
*/
typedef IFX_int_t (*DTI_imageCntrlReleaseFunction_t)(
                     IFX_void_t *, DTI_ImageControl_t *, DTI_PacketError_t *);
...
typedef struct {
   ...
   /** callback function: image control release */
   DTI_imageCntrlReleaseFunction_t pCbFctImageCntrlRelease;
