...
/** \defgroup DRV_DSL_CPE_API Intel DSL CPE API Driver
    Lists the entire modules to the DSL CPE_API.
*/

/** \defgroup DRV_MEI Intel MEI Driver
...
/** \defgroup MEI_API_IOCTL Functions for common usage within Linux user space
...
#define DSL_DRV_MAX_SUPPORTED_ENTITIES 2
#define DSL_DRV_ENTITIES (g_LinesPerDevice * g_MaxDeviceNumber)

#define DSL_DRV_LINES_PER_DEVICE ((const DSL_uint8_t) g_LinesPerDevice)
#define DSL_DRV_DEVICE_NUMBER ((const DSL_uint8_t) g_MaxDeviceNumber)
#define DSL_DRV_BONDING_ENABLED (g_LinesPerDevice * g_MaxDeviceNumber > 1)
#define DSL_MAX_SUPPORTED_CHANNELS_PER_LINE 1
#define DSL_CHANNELS_PER_LINE ((const DSL_uint8_t) g_ChannelsPerLine)
...
#ifndef DSL_DRV_ATM_PTM_INTERFACE_ENABLE
   /* Legacy option was 'INCLUDE_DSL_ATM_PTM_INTERFACE_SUPPORT' */
#  define DSL_DRV_ATM_PTM_INTERFACE_ENABLE    1
#else
#  if ( (DSL_DRV_ATM_PTM_INTERFACE_ENABLE != 1) && (DSL_DRV_ATM_PTM_INTERFACE_ENABLE != 0) )
#     undef  DSL_DRV_ATM_PTM_INTERFACE_ENABLE
#     define DSL_DRV_ATM_PTM_INTERFACE_ENABLE   1
#     warning "invalid config: include ATM/PTM interface by default"
#  endif
#endif
...
typedef enum
{
   ...
   /** Debug output for notification related information's.
       The related output can be used to checked that DSL Subsystem behaves
       as expected according to the preparation/generation of inter-module
       related information. For example the PP subsystem needs to be informed
       about showtime entry and showtime exit. */
   DSL_DBG_NOTIFICATIONS = 15,
...
/** messages, warnings and errors are printed */
#define DSL_DBGDST_CONSOLE     0
/** verbose output (low level debug) */
#define DSL_DBGDST_LOGGER      1
...
typedef enum
{
   ...
   DSL_DBG_DST_CONSOLE = DSL_DBGDST_CONSOLE,
   /** output will be passed to the logger application */
   DSL_DBG_DST_LOGGER  = DSL_DBGDST_LOGGER
} DSL_debugDestinations_t;
...
typedef enum
{
   ...
   /** Line State: DISABLED.
       ...
       - line "tear down" has been performed within context of activated on- or
         off-chip bonding handling (only valid for VRX200, VRX300 or VRX500
         Family). DSL firmware was started in bonding mode but the CO indicated
         that bonding is not used (remote PAF_Enable status is false).
   ...
   /** Line State: BONDING_CLR
       ...
       - VRX200/VRX300/VRX500: Modem status: GHS_BONDING_CLR_STATE */
   ...
   /** Line State: ORDERLY_SHUTDOWN.
      ...
      Corresponds to the following device specific state
       - ADSL only platforms: Not supported
       - VRX200/VRX300/VRX500: Modem status: PRE_FAIL_STATE */
...
typedef enum
{
   ...
   /**
   ISDN hybrid detected (Annex-B). */
   DSL_HYBRID_TYPE_ISDN_B = 2,
   /**
   ISDN hybrid detected (Annex-B/J). */
   DSL_HYBRID_TYPE_ISDN_BJ = 3,
   /**
    *  Universal Hybrid (ISDN and POTS) for VRX500 Families. */
   DSL_HYBRID_TYPE_UNIVERSAL_VRX500 = 4,
   /**
   Delimiter only! */
   DSL_HYBRID_TYPE_LAST = 5
} DSL_HybridType_t;

/** Definition for backward compatibility only */
#define DSL_HYBRID_TYPE_ISDN DSL_HYBRID_TYPE_ISDN_B
...
typedef struct
{
   ...
   - minor version (minor): logical number, incremented for minor changes in
     versions
   - build step (build): logical number, incremented for each new version build
   - optional release indictor (rel): specifies different Intel internal
     indications */
...
typedef enum
{
   ...
   \note This value is only available for VRX200, VRX300 and VRX500
...
typedef struct
{
   ...
   \note In case of enabling SRA functionality (configuration parameter RA_MODE
         via IOCtl \ref DSL_FIO_G997_RATE_ADAPTATION_CONFIG_SET respective
         "g997racs" CLI command) it is* required to also switch on bitswap for
         the same direction. DSL CPE API will take care internally, which means
         that independent of the user configuration for bitswap it is enabled
         for the related direction.
    */
...
typedef enum
{
   ...
   /** 7  G.993.2 Profile 35b.*/
   DSL_BF_PROFILE_35B = 0x00000100,
...
typedef struct
{
   ...
   - VRX200, VRX300 and VRX500: ACK_ModemFSM_FailReasonGet::ErrorCode (System
     Error Code)
   ...
   Device specific mapping as follows
   - ADSL only platforms: Not used (always 0)
   - VRX200, VRX300 and VRX500: ACK_ModemFSM_FailReasonGet::SubErrorCode (System
     Sub Error Code)
   \note This value references to the reason for the last fail state
         transition */
   ...
   - VRX200, VRX300 and VRX500: ACK_ModemFSM_FailReasonGet::FW_FailCode
     (Firmware Failure Code)
   \note This value references to the reason for the last fail state
         transition */
...
{
   /**
   Automatic detection activation sequence
   Obsolete value - reserved for the  backward compatibility */
   DSL_ACT_SEQ_AUTO = 0,
   /**
...
typedef struct
{
   ...
   \note This parameter is only valid for NEAR-END direction and in case of
         NOT using VRX200, VRX300 or VRX500. */
...
typedef struct
{
   ...
   /**
   Specifies to activate or deactivate the firmware message event handling.

   With this setting the DSL Firmware message event (autonomous message)
   handling within DSL CPE API Driver will be activated. If enabled, the API
   will activate all available autonomous messages and will make use of events
   instead of using firmware message polling to get according states.

   This feature is only supported for VRX500 family operating with a minimum R5
   DSL Firmware Release. Currently supported functionalities and respective
   autonomous firmware messages in this case are the following
   - line state information via EVT_ModemFSM_StateGet

   In case of VRX500 family and using a DSL Firmware Release R5 (or higher)
   - The configuration will be enabled by default at initialization time
   - It can be disabled at run-time (to switch back to the FW message polling
   mode)
   In all other cases
   - The configuration will be disabled by default at initialization time
   - It *cannot* be enabled run-time

   \note In opposite to the other parameters within this function/structure,
   this configuration is not open instance specific, which means that it
   will be applied on all open instances automatically.
   \note On switching between FW versions which supports and not supports
   (or vice versa) this feature the API will be automatically reconfigured. */
...
typedef struct
{
   ...
   DSL_IN DSL_boolean_t bForceDownload;
...
typedef struct
{
   ...
   - VRX300 and VRX500 Family platforms: The firmware binary that will be
     provided with this pointer is the one and only firmware that will be
     used. */
   DSL_IN DSL_uint8_t *pFirmware;
   /**
   Size of firmware binary.
   ...
   - VRX200 Family platforms: Depending on the compilation/functionality
     ...
     + ON-chip bonding (Dual Port Mode): This parameter shall be used to provide
       ...
       parameter is NOT used and has to be set to DSL_NULL.
   - VRX300 and VRX500 Family platforms: This parameter is NOT used and has to
     be set to DSL_NULL */
   DSL_IN DSL_uint8_t *pFirmware2;
   /**
   Size of the 2nd firmware binary.
...
/**
   Structure used to read (GET) or write (SET) the configuration of a debug
   module destination. This is used to select destination for dedicated debug
   printout (console or logger) or get the current configuration status.
*/
typedef struct
{
   /**
   Specifies/returns the debug level. This value has to be one of the defined
   debug levels (\ref DSL_debugLevels_t).*/
   DSL_CFG DSL_debugDestinations_t nDbgDestination;
} DSL_DBG_ModuleDestinationData_t;

/**
   Structure used to read (GET) or write (SET) the configuration of a debug
   module destination.
   This structure has to be used for ioctl
   - \ref DSL_FIO_DBG_MODULE_DESTINATION_SET
   - \ref DSL_FIO_DBG_MODULE_DESTINATION_GET
*/
typedef struct
{
   /**
   ...
   /**
   Structure that contains debug configuration data */
   DSL_CFG DSL_DBG_ModuleDestinationData_t data;
} DSL_DBG_ModuleDestination_t;
#endif /* INCLUDE_DSL_CPE_DEBUG_LOGGER_SUPPORT */
...
typedef enum
{
   ...
   \note This criteria is used for ADSL and VDSL modes of VDSL capable
         platforms (VRX200, VRX300 and VRX500).
   \note Default is DSL_TRUE */
   ...
   DSL_REBOOT_CRITERIA_ESX = 0x00000010,
   /**
   30 consecutive severely errored seconds (SES).

   \note This criteria is used for ADSL mode of ADSL only platforms and ADSL
         mode of VDSL capable platforms (VRX200, VRX300 and VRX500).
   \note Default is DSL_FALSE */
   DSL_REBOOT_CRITERIA_SESX = 0x00000020,
   /**
   Negative SNR margin.
...
typedef struct {
   /**
   T1413 revision */
   DSL_T1413_Revision_t nT1413RevisionNum;
} DSL_T1413RevisionStatusData_t;

/**
   This structure is used to get T1.413 Revision Number
   This structure has to be used for ioctl
   - \ref DSL_FIO_T1413_XTUO_REVISION_GET
   - \ref DSL_FIO_T1413_XTUR_REVISION_GET
*/
typedef struct
{
   /**
   ...
   /**
   Structure that contains status data */
   DSL_IN_OUT DSL_T1413RevisionStatusData_t data;
} DSL_T1413RevisionStatus_t;
...
typedef struct {
   /**
   T1413 revision */
   DSL_uint16_t nT1413VendorRevisionNum;
} DSL_T1413VendorRevisionStatusData_t;

/** Maximal number of calibration breakpoints for US and DS*/
#define DSL_PSD_US_MAX_CALIB_POINTS            32
#define DSL_PSD_DS_MAX_CALIB_POINTS            16

/**
   Structure used for configuration of the PSD Calibration.
*/
typedef struct
{
   /**
...
/**
   Structure used for PSD Calibration configuration.
*/
typedef struct
{
   /**
   Number of breakpoints  */
   DSL_IN DSL_uint16_t NumBreakPts;
   /**
   Table contain a breakpoints */
   DSL_IN DSL_PSDCompBreak_t breakpoint[DSL_PSD_US_MAX_CALIB_POINTS];
} DSL_PSDCalibrationData_t;

/**
   Structure used for PSD Calibration configuration.
   This structure has to be used for ioctl
   - \ref DSL_FIO_PSD_CALIBRATION_CONFIG_SET
   - \ref DSL_FIO_PSD_CALIBRATION_CONFIG_GET
*/
typedef struct
{
   /**
   ...
   DSL_IN DSL_AccessDir_t nDirection;
   /**
   Structure that contains PSD Calibration data */
   DSL_IN DSL_PSDCalibrationData_t data;
} DSL_PSDCalibration_t;

/**
   This structure is used to get T1.413 Vendor Revision Number
   This structure has to be used for ioctl
   - \ref DSL_FIO_T1413_XTUO_VENDOR_REVISION_GET
   - \ref DSL_FIO_T1413_XTUR_VENDOR_REVISION_GET
*/
typedef struct
{
   /**
