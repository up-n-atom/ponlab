...
typedef struct DSL_DEV_Context
{
   ...
   /** FW optimized download flag */
   DSL_boolean_t bUseFWOptimizedDownload;

} DSL_DEV_Context_t;

typedef DSL_DEV_Context_t DSL_devCtx_t;

extern DSL_devCtx_t ifxDevices[DSL_DRV_MAX_SUPPORTED_ENTITIES];
...
/**
   ...
   \param pContext    Pointer to dsl library context structure, [I]
   \param pVerNum     Pointer to Chip HW Version number,
...
/**
   ...
   \param pContext    Pointer to dsl library context structure, [I]
   \param pVerNum     Pointer to Chip HW Version number (default DSL_NULL),
...
/**
   ...
   \param pEvt          Pointer to EVT ModemFSM  msg which should be parsed
                        instead of retriving one
...
DSL_Error_t DSL_DRV_DEV_LineStateGet(
   ...
   DSL_IN EVT_ModemFSM_StateGet_t *pEvt
);
