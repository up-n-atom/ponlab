...
/** Extention V1 - support additional infos:
  - Listener: pid, tid and operation count
  - Worker: pid, tid and operation run and idle count
*/
#define DTI_STAT_STATUS_EXT_V1	1

/** Extention V2 - support additional infos:
  - Agent: image control info
*/
#define DTI_STAT_STATUS_EXT_V2	1
...
struct ltq_dti_statistic_agent_s
{
	...
	/** Listener - operation heart-beat counter (in use) */
	unsigned int op_run_count;
};

struct ltq_dti_statistic_image_s
{
	/** image ID */
	int image_id;
...
struct ltq_dti_statistic_worker_thr_s
{
	...
	/** worker_thr - thread exit count */
	unsigned int close_cnt;
	/** worker_thr - operation heart-beat counter (if the thr is in operation) */
	unsigned int op_run_count;
	/** worker_thr - idle heart-beat counter (if the thr is in idle, resume) */
	unsigned int op_idle_count;
};
...
struct ltq_dti_statistic_connection_s
{
	...
	/** connection - data stream re-sync trigger (missing magic number) */
	unsigned int conn_resync_cnt;
	/** connection - close trigger (TCP/IP FIN received) */
...
struct ltq_dti_system_status_s
{
	...
	/** image handling */
	struct ltq_dti_statistic_image_s stat_image[DTI_MAX_NUM_OF_IMAGES];
	/** Worker Status */
