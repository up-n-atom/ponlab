                        ...
                        DTI_DeviceCtx_t      **ppDtiDevCtx)
{
   ...
   if (*ppDtiDevCtx != IFX_NULL)
   {
      ...
      DTI_PRN_USR_ERR_NL(DTI_GEN, IFXOS_PRN_LEVEL_WRN,
         ("WARNING: Generic Dev Module Setup - already done (devIfNum = %d)."DTI_CRLF,
         pDtiDevCtx->devIfNum));
   ...
   pDtiDevCtx = (DTI_DeviceCtx_t *)DTI_Malloc((IFX_uint_t)ctxSize);
   ...
   DTI_MemSet(pDtiDevCtx, 0x0, ctxSize);

   pGenDevCtx = (DTI_DEV_GenericDriverCtx_t*)&pDtiDevCtx[1];
