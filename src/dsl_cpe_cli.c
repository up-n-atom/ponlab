            ...
            if(node->rec.func)
            {
               ...
               if((node->rec.mask & DSL_CPE_MASK_LONG) == DSL_CPE_MASK_LONG)
                  ...
                  DSL_CPE_CLI_PrintHelp(node->rec.psHelp, node->rec.sCmdLong,
                     node->rec.sCmdShort, node->rec.mask, node->rec.printFunc, out);
            #else
               DSL_CPE_FPrintf (out, DSL_CLI_HELP_NOT_AVAILABLE);
            #endif /* DSL_CPE_DEBUG_DISABLE */
            }
   ...
   DSL_CPE_File_t *out)
{
   ...
   DSL_char_t *sDeviceHelp = "- DSL_uint32_t nDevice (optional, not used in the 'backward compatible' mode)" DSL_CPE_CRLF;
   DSL_char_t *sHelp = DSL_NULL;
   sHelp = (DSL_char_t*)DSL_CPE_Malloc(4096);
   if (sHelp == DSL_NULL)
   {
   ...
   DSL_CPE_File_t *out)
{
   DSL_CPE_Control_Context_t *pCtrlCtx = DSL_CPE_GetGlobalContext();
   DSL_char_t *sDeviceHelp = "- DSL_uint32_t nDevice (optional, not used in the 'backward compatible' mode)" DSL_CPE_CRLF;
   DSL_char_t *sDeviceHelpInput =   "Input Parameter" DSL_CPE_CRLF
