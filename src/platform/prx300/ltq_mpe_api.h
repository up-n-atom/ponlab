...
*******************************************************************************/
#ifndef __LTQ_MPE_API_H__201909_19_1213__
#define __LTQ_MPE_API_H__201909_19_1213__
...
#define MAX_MPE_TC_NUM     ( MAX_TM_NUM + MAX_WORKER_NUM)     /*!< Maximum Threads supported in MPE FW */
...
enum CBM_BUF_MACROS {
	...
	CBM_BUF_DBG_PRINT = 1000  /*!< Buffer length required for FW debug print */
};
...
struct dma_desc_1	{
	union {
		struct {
			...
			uint32_t ep: 4;  /*!< egress port EGP */
			uint32_t ip:4;   /*!< ingress port IGP */
...
struct session_action {
	...
	/*4th byte of DWORD */
	uint32_t redirect: 1;		    /*!< Redirect bit to control IGP and EGP content in the TX DMA Descriptor and ingress PMAC. Default = 1 > */
...
struct tc_hw_res {
	...
	uint32_t TcNum;				/*!< Real TC Num on which TM/WORKER in a core works  */
...
&nbsp;&nbsp; bit2 is for eva_SegCtl2 register\n
	...
	uint32_t max_works_pkt_num : 8;  /*!< Maximum packets can be handled by worker TC before it switches to handle other events.\n
                                       It is set by MPE HAL*/
	uint32_t vuni_ip: 4; /*!< VUNI input port for PON HGU use. It is set by MPE HAL*/
	uint32_t pon_ip:  4; /*!<  PON input port for PON HGU use. It is set by MPE HAL*/
...
#endif  /*__LTQ_MPE_API_H__201909_19_1213__ */
