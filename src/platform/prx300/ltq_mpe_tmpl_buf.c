...
static uint16_t ppa_out_header_len( struct uc_session_node *p_item, uint32_t *delta)
{
	...
	if( IsGreSession(p_item->flag2)) {
		...
		if ( ppa_is_IngressGreSession(p_item) ) {
			...
			if ((!IsBridgedSession(p_item->flag2) && is_lan) || ppa_is_EgressGreSession(p_item))
				*delta -= hdrlen;
			if (is_eogre) {
				*delta -= ETH_HLEN;
			}
...
int32_t ppa_form_session_tmpl(PPA_SESSMETA_INFO *metainfo)
{
	...
	/* Set subif id  and clear redirect flag if tx if is VANI */
	if (txif_info && (txif_info->flags & NETIF_VANI)) {
		swaHdr.redirect = 0;
	}
	 ...
	 */
	/* Configure priority tag to be removed in session */
	if ((metainfo->skb->vlan_proto == ETH_P_8021Q)
		&& ((metainfo->skb->vlan_tci & ((1 << 12) - 1)) == 0))
	...
	if( IsBridgedSession(p_item->flag2) || isEgGreTap) {
		...
		if ( ppa_is_EgressGreSession(p_item) ) {
			if ( IsBridgedSession(p_item->flag2) ) {
				...
				/* Routed: AnyWAN->EoGRE session full processing */
			...
			if( isIPv6 ) {
				...
				if (IsDsQosSession(p_item)) {
					if (!(pkt_mark & SESSION_PCE_CLASSIFICATION)) {
						...
						swaHdr.ig_traffic_class_en = (pkt_mark & SESSION_ING_TC_SET) >> ING_TC_SET_POS;

						swaHdr.new_traffic_class_en = (pkt_mark & SESSION_EGR_TC_SET) >> EGR_TC_SET_POS;
						...
						if (!(swaHdr.new_traffic_class_en) && (pkt_mark & SESSION_FLAG_TC_REMARK)) {
					...
					swaHdr.ig_traffic_class = ((pkt_mark & SESSION_INGRESS_TC) >> INGRESS_TC_BIT_POS) - 1;
					swaHdr.traffic_class = ((pkt_mark & TC_MASK) >> TC_START_BIT_POS) - 1;
				}
#endif /* CONFIG_INTEL_IPQOS_MPE_DS_ACCEL */
			}
		}
	...
	if( swaHdr.templ_len ) {
		p_swaHdr->templ_buf = ppa_alloc_dma(swaHdr.templ_len + NET_IP_ALIGN);
		if (!p_swaHdr->templ_buf) {
			ppa_free(p_swaHdr);
			return PPA_ENOMEM;
		}
		ppa_memset(p_swaHdr->templ_buf, 0, swaHdr.templ_len + NET_IP_ALIGN);
	}
	/* else	TODO: Handle when no buffer need to be inserted.
		 Required mainly for downstream */
	hdr = p_swaHdr->templ_buf + NET_IP_ALIGN;
...
hdr_done:
	memmove(p_swaHdr->templ_buf, p_swaHdr->templ_buf + NET_IP_ALIGN, p_swaHdr->templ_len);
