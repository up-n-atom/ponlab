...
int32_t hw_litepath_xmit(PPA_SKBUF *skb)
{
	...
	/* increase size and pad sk_buff up to minimal size 60B */
	...
	/*
	 * Make sure, Tx desc classid bit3 is not set. Otherwise PCE detects it
	 * as padded short packet (32B) and does not forward to the destination!
	 * Workaround: drop the packet qos priority to 7 for the higher.
	 */
	if (skb->priority > 7)
		skb->priority = 7;
	...
	if (dp_xmit(priv->dev, &(priv->subif_id), skb, skb->len, dp_flags) != 0) {
		pr_debug_ratelimited("dp_xmit fails for dp port: %d\n", priv->dp_port);
		lp_tx_err++;
		lp_tx_err_b += skb->len;
		return PPA_EPERM;
	}	
...
#define dbg printk
	...
	if (priv == NULL) {
		free_netdev(hw_lite0_netdev);
		hw_lite0_netdev = NULL;
		free_netdev(hw_lite0_netdev_phy);
		hw_lite0_netdev_phy = NULL;
		dbg("priv alloc fail\n");
		return -1;
	}
...
static const struct file_operations dbgfs_file_hwlitepath_pkt_ctrs_seq_fops = {
	.owner		= THIS_MODULE,
...
static int proc_read_hwlitepath_pkt_ctrs(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "\nRX Success: %llu - TX Success: %llu - TX Error[Tx: %llu, Looped back: %llu]\n",
			lp_rx, lp_tx, lp_tx_err, lblp_rx);
	seq_printf(seq, "\n\t\tHW Litepath bytes Counters:\n");
	seq_printf(seq, "\nRX Success: %llu - TX Success: %llu - TX Error[Tx: %llu, Looped back: %llu]\n",
			lp_rx_b, lp_tx_b, lp_tx_err_b, lblp_rx_b);
	seq_printf(seq,	"=====================================================================\n");
	...
	const char __user *buf, size_t count, loff_t *data)
{
	...
	if (tmp == 0) {
		...
		ctr = lp_tx = lp_rx = lblp_rx = lp_tx_err = lp_tx_b = lp_rx_b = lblp_rx_b = lp_tx_err_b = 0;
	} else {
...
static struct ppa_debugfs_files hwtlp_debugfs_files[] = {
	{ "stats", 0600, &dbgfs_file_hwlitepath_pkt_ctrs_seq_fops },
};

static void hw_litepath_proc_file_create(void)
{
	ppa_debugfs_create(ppa_debugfs_dir_get(), "hw_litepath",
		&dbgfs_ppa_hwltp, hwtlp_debugfs_files,
		ARRAY_SIZE(hwtlp_debugfs_files));
}

static void hw_litepath_proc_file_remove(void)
{
	ppa_debugfs_remove(dbgfs_ppa_hwltp,
		hwtlp_debugfs_files,
		ARRAY_SIZE(hwtlp_debugfs_files));
}
