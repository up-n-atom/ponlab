...
#define MAX_VAP_QOS_PORT              2      /*!< Maximum VAP QoS port number */
...
#define MAX_DUP_PER_SESS               8   /*!< Maximum duplication supported per session per PMAC port*/

/*Make sure MAX_ING_VLAN is multiple of 4 so that struct using is will remain aligned */
#define MAX_ING_VLAN                   8   /*!< Maximum ingress VLAN supported per session per PMAC port*/
...
enum TUNL_TYPE {
	...
	TUNL_VXLAN,		/*!< VXLAN over UDP Tunnel */
	TRAN_ESP,           /*!< IPSEC Transport ESP */
	TRAN_ESP_NATT,      /*!< IPSEC Transport ESP with NAT-T */
...
struct port_subif {
	...
	uint32_t vlan_tag; /*!< vlan tag to be added to egress packeta as specified in MCGID */
};
...
struct vap_entry {
	...
	uint32_t vlan_tag; /*!< vlan tag to be added to egress packet */
...
struct session_action {
	...
	uint32_t key_en              : 1; /*!< GRE key flag: 1-enabled, 0-disabled. \ref key is valid if key_en 1. It is used for GRE Downstream traffic now */
	uint32_t sess_mib_hit_en		: 1; /*!< session MIB hit table selector: 1- hit info is valid, 0-Not valid */

	/*3rd byte of DWORD */
	uint32_t tunnel_id: 4;		/*!< IPSEC tunnel ID in upstream direction, currently valid from 0 ~ 15  */
	uint32_t fw_csum_en: 1;		/*!< Enable to let FW calculate outer UDP checksum like in L2TP */
	uint32_t esp_ip_offset_en: 1; /*!< esp_ip_offset_en used if ESP template buffer is used */
	uint32_t esp_udp_offset_en: 1; /*!< esp_udp_offset_en used if ESP template buffer is used */
	uint32_t esp_pppoe_offset_en: 1; /*!< esp_pppoe_offset_en used if ESP template buffer is used */
	
	
	/*4th byte of DWORD */
	uint32_t reserve2: 1;		/*!< reserved > */
        uint32_t fcs_bd: 1;             /*!< FCS bit for DSL bonding case, set by MPE HAL */
	...
	uint32_t qmap                : 6; /*!< Use if MPE handles DS QOS, set by MPE HAL \n
	                                   *   Bit fields corresponds to DMA descriptor  \n
	                                   *   Bit 5- DEC, 4- ENC, 3- MPE2, 2- MPE1, 1:0- FLOW[7:6] \n
	                                   */
	...
	uint32_t pkt_loop            : 1; /*!< Set by MPE HAL. If set then CBM loopback to MPE (DS QOS) */
	uint32_t ig_traffic_class_en : 1; /*!< Set by MPE HAL, if set then apply ig_traffic_class (DS QOS)*/

	/*4th byte of DWORD */
	uint32_t tunnel_type         : 4; /*!< refer to \ref TUNL_TYPE */
	uint8_t dst_pmac_port_num    : 4;  /*!<  Number of destination PMAC Port  configured in the dst_pmac_port_list per ingress VLAN.  \n
	...
	/*DWORD */
	uint8_t esp_ip_offset;     /*!< Outer IP offset in template buffer when ESP encryption is added on tunnel packet.
                                    Applicable if esp_ip_offset_en is set */
	uint8_t esp_udp_offset;    /*!< Outer UDP offset in template buffer when ESP encryption is added on tunnel packet 
                                    Applicable if esp_udp_offset_en is set */
	uint8_t esp_pppoe_offset;  /*!< Outer PPPOE offset in template buffer when ESP encryption is added on tunnel packet
	                                Applicable if esp_pppoe_offset_en is set */
	...
	/*1st byte of DWORD */
	uint32_t traffic_class   : 4; /*!< For TMU queue selection. In DMA descriptor, it is egress traffic class */
	uint32_t ig_traffic_class: 4; /*!< For TMU queue selection. In DMA descriptor, it is ingress traffic class */

	/*2nd byte of DWORD */
	uint32_t mc_ing_vlan:1; /*!< Flag to indicate to lookup Multicast VLAN table(MVT) to get ingress VLAN index */
	uint32_t mc_sess_index:7; /*!< session index into 2D MVT table to get the table for current session */

	/* can be renamed and used if needed */
	uint32_t res_align2          : 16; /*!< reserved for DWORD alignment */
	...
	/* DWORD: */
	uint16_t ft_idx;	/*!< MPE FW compare table index provided by MPE HAL (same as MPE SE hash index) */

	/*If the VXLAN port is configured with some port number other than the default*/
	uint16_t udp_vxlan_port;

	/*DWORD  */
	/*First 3 bytes of DWORD -- Vni is 24 bit value */
	uint32_t vni            : 24;

	/*4th byte of DWORD */
	/*Enable this flag if the VXLAN port is configured*/
	uint32_t  vxlan_port_en  : 1;

	uint32_t vxlan_res      : 7;

	/*DWORD  */
	struct mc_session_action *mc_act;
...
/*!
  \brief This is Session Action definition for multicast sessions specific parameters.
 */
struct mc_session_action {
	/*DWORD */
	uint8_t mc_index_num[MAX_ING_VLAN][MAX_PMAC_PORT];		  /*!< To get 7 bits sub interface MC index even without PAE support.\n
					           ...
					           */
	/*DWORD */
	uint8_t mc_index_list[MAX_ING_VLAN][MAX_PMAC_PORT][MAX_DUP_PER_SESS];		  /*!< To get 7 bits sub interface MC index even without PAE support. */
	uint8_t mc_dst_pmac_port_num[MAX_ING_VLAN];  /*!<  Number of destination PMAC Port  configured in the dst_pmac_port_list per ingress VLAN.  \n
                                         * For unicast it is 1 and for multicast the maximum value is \ref MAX_PMAC_PORT \n
                                        */
	/* DWORDS: */
	uint8_t mc_dst_pmac_port_list[MAX_ING_VLAN][MAX_PMAC_PORT];    /*!< Destination PMAC Port ID: \n
                                                    * for unicast only read index 0. \n
                                                    * 0   --CPU  \n
                                                    * 1-6 --Ethernet LAN Ports \n
                                                    * 7-14--VRX318(DSL)/WIFI/LTE/CAPWAP other directpath's port \n
...
/*!
  \brief This is Multicast VLAN Table Entry Definition.
 */
struct mc_vlan_table {
	uint16_t vlan_id; /*!< Ingress VLAN Id, 0xffff means absense of the VLAN */
	uint16_t mcgid_count:4; /*!< reference count to indicate number of MCGIDs using this entry */
};
 ...
 */
struct hw_act_ptr {
	uint32_t act;    /*!< action table's base pointer (use in complementary mode) */
} ;
...
struct ipsec_info {
	...
	uint8_t  cd_size; 		 /*!< Command Descriptor Size in DWORD (4 bytes). If follow this implementation proposal, its size
								    is EIP97_CD_SIZE ,*/
	uint8_t mode;            /*!< Choose IPSEC configuration: ESP/AH encapsulation, tunnel or transport mode. \n
								  There are four values: ESP_TU, ESP_TR, AH_TU, AH_TR and default is ESP_TU.  \n
								  The value must be set in inbound tunnel by MPE HAL */
	/*padding variables*/
...
&nbsp;&nbsp; bit2 is for eva_SegCtl2 register\n
	...
	struct mc_vlan_table mc_vlan_tbl[MAX_MC_NUM][MAX_ING_VLAN]; /*!< Multicast VLAN Table (MVT), it maintains ingress VLAN to MCGIDs mapping per MC session */
	uint16_t vap_qos_ports[MAX_VAP_QOS_PORT]; /*!< ports for which VAP level QoS is supported */
	...
	uint8_t gre_len;            /*!<  GRE header length, applicable only in GRE encapsulation upstream */	
