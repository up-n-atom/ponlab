                        ...
                        IFX_uint_t           maxEventSize_byte)
{
   ...
   pCliEventCtx->pEventFifoBuffer = DTI_Malloc(pCliEventCtx->eventFifoBufferSize_byte);
   if (pCliEventCtx->pEventFifoBuffer == IFX_NULL)
   {
      DTI_PRN_USR_ERR_NL(DTI_CLI, DTI_PRN_LEVEL_ERR,
         ("Error: CLI Worker Event Control Create - event buffer allocation (size %d)"DTI_CRLF,
           pCliEventCtx->eventFifoBufferSize_byte));
   ...
   if (DTI_Var_Fifo_Init(
               ...
               &pCliEventCtx->pEventFifoBuffer[0],
               &pCliEventCtx->pEventFifoBuffer[eventBufferSize_ulong],
   ...
   else
   {
      ...
      if ((pIfControl->bRdyForCliSend == IFX_TRUE) && (pIfControl->pSendFunct != IFX_NULL || pIfControl->pSendFragFunct != IFX_NULL))
      {
         ...
         else
         {
            ...
            pDtiPacketOut        = (DTI_Packet_t *)pIfControl->pCliResponceBuffer;
...

{
   ...
   if (pCliEventCtx->bAutoCliMsgActive == IFX_TRUE)
   {
      ...
      pEventPacket = (DTI_Packet_t *)DTI_Var_Fifo_writeElement(
                              &pCliEventCtx->eventFifo,
...
*/
      ...
      pEventPacket = (DTI_Packet_t *)DTI_Var_Fifo_peekElement(
      ...
      {
         DTI_PRN_USR_ERR_NL(DTI_CLI, DTI_PRN_LEVEL_ERR,
            ("ERROR: CLI Auto Msg (P) - peek packet, <peek failed>."DTI_CRLF));
      ...
      /* cli msg available - peek the msg */
      pEventPacket = (DTI_Packet_t *)DTI_Var_Fifo_readElement(
                              &pCliEventCtx->eventFifo, &fifoElementSize_byte);
