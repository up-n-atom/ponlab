...
int MEI_fw_download(MEIOS_File_t *streamOut, int fd, int fileNum, char *pFileName)
{
   ...
   {
      MEI_strncpy_s(bufFileName, sizeof(bufFileName),
        pFileName, MEIOS_StrLen(pFileName));
   }
   else
   {
      if (fileNum == -1)
      {
         MEIOS_SNPrintf(bufFileName, sizeof(bufFileName),
                "%s.bin", MEI_FW_DL_FILE_BASE_NAME);
      }
      else
      {
         MEIOS_SNPrintf(bufFileName, sizeof(bufFileName),
                "%s%d.bin", MEI_FW_DL_FILE_BASE_NAME, fileNum);
      }
   }
