...
/* Collects the status of the DTI Agent and Listener Thread
   - configuration
   - IP configuration
   - Listener: statistics
*/
	...
	struct ltq_dti_system_status_s *p_system_status)
{
	...
	for (i = 0; i < DTI_MAX_NUM_OF_IMAGES; i++)
	{
		p_system_status->stat_image[i].image_id =
			p_agt_ctx->imageCntrl[i].imageId;
		p_system_status->stat_image[i].max_image_size =
			p_agt_ctx->imageCntrl[i].maxImageSize;
		p_system_status->stat_image[i].max_chunk_size =
			p_agt_ctx->imageCntrl[i].maxChunkSize;
		p_system_status->stat_image[i].use_count =
			p_agt_ctx->imageCntrl[i].useCount;
		p_system_status->stat_image[i].loaded_size =
			p_agt_ctx->imageCntrl[i].loadedSize;
		p_system_status->stat_image[i].image_size =
			p_agt_ctx->imageCntrl[i].imageSize;
	}
...
/* Collects the status of the DTI Worker Thread
   - configuration, current status
   - IP configuration
   - Worker: packet and thread statistics
*/
