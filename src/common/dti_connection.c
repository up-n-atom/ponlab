   ...
   */
   if ((bSwapPayloadIntern == IFX_TRUE) && (bSwapPayload == IFX_TRUE))
   {
      ...
      switch(pHeader->packetOptions & DTI_HDR_OPTION_MASK_PAYL_TYPE)
      {
         case DTI_e16Bit:
            {
               IFX_uint16_t *pPayload_16 = (IFX_uint16_t *)pDtiPacket->payload;
         ...
         case DTI_e32Bit:
            {
               IFX_uint32_t *pPayload_32 = (IFX_uint32_t *)pDtiPacket->payload;
                        ...
                        IFX_int_t               waitTime_ms)
{
   ...
   pPacketHdr = (DTI_PacketHeader_t *)pBufIn;
   IFXOS_PRN_USR_DBG_NL(DTI_CON, IFXOS_PRN_LEVEL_HIGH,
      ("WRN: Packet Read Resync."IFXOS_CRLF));
   ...
   if (nRemainBytes == 0)
   {
      ...
      else
      {
         ...
         IFXOS_PRN_USR_DBG_NL(DTI_CON, IFXOS_PRN_LEVEL_HIGH,
            ("INFO: Packet Read Resync Done."IFXOS_CRLF));
         DTI_packetShow (
            (DTI_Packet_t *)pBufIn, IFX_FALSE, IFX_FALSE, "DTI Resync", DTI_PRN_LEVEL_HIGH);
                        ...
                        DTI_Packet_t            **ppPacket)
{
   ...
   */
   pDtiPacket = (DTI_Packet_t *)pBufIn;
   ...
   if (nTotalRecvBytes >= sizeof(DTI_PacketHeader_t))
   {
      ...
      {
         IFXOS_PRN_USR_ERR_NL(DTI_CON, IFXOS_PRN_LEVEL_ERR,
            ("ERROR: Packet Recv - Header invalid MAGIC (magic=0x%08X)."IFXOS_CRLF,
            DTI_ntohl(pDtiPacket->header.magic)));
         DTI_packetShow (
            pDtiPacket, IFX_FALSE, IFX_FALSE, "DTI Sync Err", IFXOS_PRN_LEVEL_HIGH);
      ...
      else
      {
         ...
         {
            IFXOS_PRN_USR_ERR_NL(DTI_CON, IFXOS_PRN_LEVEL_ERR,
               ("ERROR: Packet Recv - To long for buffer."IFXOS_CRLF));
   ...
   if (((bIsInHostOrder == IFX_TRUE) ? pHeader->magic : DTI_ntohl(pHeader->magic)) == DTI_MAGIC)
   {
      if ( (bShowPayload) && (payloadSize > 0))
      {
         ...
         switch(ePayloadType)
         {
            ...
            case DTI_e8Bit:
               {
                  IFX_uint8_t *pPayload_8 = pDtiPacket->payload;
