...
DSL_Error_t DSL_CPE_SetEnv(const DSL_char_t *sName, const DSL_char_t *sValue)
{
   ...
   {
      DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
         "Can't set zero-length env variable!" DSL_CPE_CRLF));

      return DSL_ERROR;
   }

   if (vallen > DSL_CPE_MAX_ENV_VAR_LENGTH)
   {
      DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
         "Unsupported environment variable value length=%d, variable value: %s!" DSL_CPE_CRLF,
         vallen, sValue));
   ...
   while(1)
   {
      if (pEnvList == DSL_NULL)
      {
         pEnvList = (DSL_CPE_EnvList_t*)malloc(sizeof(DSL_CPE_EnvList_t));

         if (!pEnvList)
         {
            DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
               "Failed to allocate Env list entry!" DSL_CPE_CRLF));
            return DSL_ERROR;
         }

         memset(pEnvList, 0x0, sizeof(DSL_CPE_EnvList_t));

         pEnvListPrev->pNext = pEnvList;
      }

      if (pEnvList->pEnvEntry == DSL_NULL)
      {
         pEnvList->pEnvEntry = (DSL_char_t*)malloc(namelen + DSL_CPE_MAX_ENV_VAR_LENGTH + 2);

         if (!(pEnvList->pEnvEntry))
         {
            DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
               "Failed to allocate Env entry!" DSL_CPE_CRLF));
            return DSL_ERROR;
         }

         memset(pEnvList->pEnvEntry, 0x0, namelen + DSL_CPE_MAX_ENV_VAR_LENGTH + 2);
...
{
   DSL_CPE_EnvList_t *pEnvListHead = &g_EnvList, *pEnvListNext = DSL_NULL;
...
DSL_Error_t DSL_CPE_System(const DSL_char_t *sCommand)
{
   ...
   else
   {
      ...
      /** don't wait till environment variable is set */
...
static void DSL_CPE_NlSendMsg(DSL_char_t* pMsg)
{
   ...
   srcAddr.nl_groups = NL_DBG_MSG_GROUP;
   if (bind(nSockFd, (struct sockaddr*)&srcAddr, sizeof(srcAddr))) {
      return;
   }

   memset(&dstAddr, 0, sizeof(dstAddr));
   dstAddr.nl_family = AF_NETLINK;
   dstAddr.nl_groups = NL_DBG_MSG_GROUP;
