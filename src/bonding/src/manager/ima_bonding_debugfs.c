...
int dbgfs_read_queue(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "\t head index          |   %-10u     |   %-10u     |\n",
			TXPRIV->host_txin_ring.head, TXPRIV->host_txout_ring.head);
	seq_printf(seq, "\t tail index          |   %-10u     |   %-10u     |\n",
			TXPRIV->host_txin_ring.tail, TXPRIV->host_txout_ring.tail);
	seq_printf(seq, "\t used                |   %-10u     |   %-10u     |\n",
			TXPRIV->host_txin_ring.count, TXPRIV->host_txout_ring.count);
	seq_printf(seq, "\t free                |   %-10u     |   %-10u     |\n",
			TXPRIV->host_txin_ring.size - TXPRIV->host_txin_ring.count,
			TXPRIV->host_txout_ring.size - TXPRIV->host_txout_ring.count);
	...
	seq_printf(seq, "\t ---------------------------------------|------------------|\n");
	seq_printf(seq, "\t base                |   0x%-8x     |   0x%-8x     |\n",
			RXPRIV->host_rxout_ring.start,
			RXPRIV->host_rxout_ring.start);
	seq_printf(seq, "\t phys base           |   0x%-8x     |   0x%-8x     |\n",
			RXPRIV->host_rxout_ring.start_phys,
			RXPRIV->host_rxout_ring.start_phys);
	seq_printf(seq, "\t element size (in DW)|   %-10d     |   %-10d     |\n", 4, 4);
	seq_printf(seq, "\t size                |   %-10u     |   %-10u     |\n",
			RXPRIV->host_rxout_ring.size,
			RXPRIV->host_rxout_ring.size);
	seq_printf(seq, "\t head index          |   %-10u     |   %-10u     |\n",
			RXPRIV->host_rxout_ring.head, RXPRIV->host_rxout_ring.head);
	seq_printf(seq, "\t tail index          |   %-10u     |   %-10u     |\n",
			RXPRIV->host_rxout_ring.tail, RXPRIV->host_rxout_ring.tail);
	seq_printf(seq, "\t used                |   %-10u     |   %-10u     |\n",
			RXPRIV->host_rxout_ring.count, RXPRIV->host_rxout_ring.count);
	seq_printf(seq, "\t free                |   %-10u     |   %-10u     |\n",
			RXPRIV->host_rxout_ring.size - RXPRIV->host_rxout_ring.count,
			RXPRIV->host_rxout_ring.size - RXPRIV->host_rxout_ring.count);
	seq_printf(seq, "\t ---------------------------------------|------------------|\n");
...
int dbgfs_read_stats(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq,
			"(TX)> AAL5 Frames=> Transmitted:%llu (but unrecognized:%llu), Waiting:%d, BackPressure:%llu, "
			"Cell Buffers=> Malloc:%d, Free:%d, Lock:%d, SwQ:[ASM:%d, Non-ASM:%d], "
			"Shaper:[Enabled:%s, Rate:%d kbps, Threshold:%d, Quota:%d]\n",
			TXPRIV->stats.counters[PACKETS_SEGMENTED], TXPRIV->stats.counters[PACKETS_UNRECOGNIZED],
	...
	seq_printf(seq,
			...
			"(RX)> AAL5 Frames=> Accepted:%llu, Discarded: [Congestion:%llu, SID:%llu, CRC:%llu, Length:%llu, Alloc-failure:%llu, Size-Overflow:%llu, Unrecognized:%llu]\n",
			...
			RXPRIV->stats.discardedframecount[UNRECOGNIZED]
		);
