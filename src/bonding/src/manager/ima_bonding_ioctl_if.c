...
static long ima_bond_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	 ...
	 **************************************************************/
	case IMA_IMAP_QOS_SET_LINK_DATA:
	{
		struct st_link_data data;
		u32 ilvdelay0, ilvdelay1;

		ret = copy_from_user(&data, (void *)arg, sizeof(st_link_data_t));
		if (ret != 0)
			goto err_out;

		if (data.interDelayMode != IMA_IMAP_QOS_INTER_DELAY_ON) {
			ilvdelay0 = ilvdelay1 = 0;
		} else {
			ilvdelay0 = data.link0InterDelay*10;
			ilvdelay1 = data.link1InterDelay*10;
		}

		if (!set_tx_dsl_info(data.link0Rate, data.link1Rate, ilvdelay0, ilvdelay1)) {
			pr_crit( "Bonding Driver: Failed to apply line rate change (mbps): "
				"[0:%d, 1:%d] Interleave delay (us): [0:%d 1:%d]\n",
				data.link0Rate, data.link1Rate, ilvdelay0, ilvdelay1 );
			goto err_out;
		}
