...
/**************************
 * Set link data per link
 **************************/
#define IMA_IMAP_QOS_SET_LINK_DATA _IOWR(ATM_DRIVER_MAGIC_NUM, 0x33, int)
...
typedef enum {
	IMA_IMAP_QOS_INTER_DELAY_ON = 0, /* Enable the US interleaving delay compensation (default) */
	IMA_IMAP_QOS_INTER_DELAY_OFF = 1, /* Disable the US interleaving delay compensation */
	IMA_IMAP_QOS_INTER_DELAY_INCOMPLETE = 2, /* Data not yet ready */
	IMA_IMAP_QOS_INTER_DELAY_INCONSISTENT = 3 /* Error case - contradicting values */
} IMA_IMAP_QOS_InterDelayMode_t;

/* IMA_IMAP_QOS_SET_LINK_DATA */
typedef struct st_link_data {
	/* The Actual Data Rate per line, coded in bit/s */
	unsigned int link0Rate;
	unsigned int link1Rate;
	IMA_IMAP_QOS_BondingLinkMode_t bondingLinkMode; /* Unused for now */
	/* The Actual Interleaving Delay per line, coded in multiple
	   of 1/100 ms (for example: 25 means 0.25 ms) */
	unsigned int link0InterDelay;
	unsigned int link1InterDelay;
	IMA_IMAP_QOS_InterDelayMode_t interDelayMode; /* Unused for now */
} __attribute__((packed)) st_link_data_t;
