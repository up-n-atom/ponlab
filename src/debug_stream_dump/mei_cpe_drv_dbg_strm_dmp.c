...
#include "drv_mei_cpe_safec_wrapper.h"
/* ============================================================== */
/* Local Variables                                                */
/* ============================================================== */
   ...
   while(1)
   {
      ...
      switch(c)
      {
         ...
         case 'f':
            if (optarg)
            {
               MEI_strncpy_s(dumpFileName, sizeof(dumpFileName),
         ...
         case 's':
            if (optarg)
            {
               MEI_strncpy_s(string, sizeof(string),
                  optarg, strlen(optarg));
...
int MEI_debug_stream_dump_daemon(void)
{
   ...
   if (bDfeChannel & 0x80)
   {
      MEIOS_SNPrintf(buf, sizeof(buf), DBG_STRM_DMP_MEI_DEV_PATH "cntrl%d",
         bDfeChannel & ~0x80);
   }
   else
   {
      MEIOS_SNPrintf(buf, sizeof(buf), DBG_STRM_DMP_MEI_DEV_PATH "%d", bDfeChannel);
   }
