...
u32 lmac_reg_read(int fd, GSW_MAC_Cli_t *param)
{
	int ret = 0;
...
int lmac_reg_write(int fd, GSW_MAC_Cli_t *param)
{
...
u32 gsw_lmac_reg_read(int fd, GSW_MAC_Cli_t *param, u32 reg)
{
	int ret = 0;
	u32 offset = param->val[0];
...
u32 gsw_lmac_reg_write(int fd, GSW_MAC_Cli_t *param, u32 reg, u32 data)
{
	int ret = 0;
	u32 offset = param->val[0];
...
int lmac_cli_get_intf_mode(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_duplex_mode(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_ctrl0 = gsw_lmac_reg_read(fd, param, MAC_CTRL0(param->mac_idx));
	u32 val;
...
int lmac_cli_get_flowcon_mode(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_ctrl0 = gsw_lmac_reg_read(fd, param, MAC_CTRL0(param->mac_idx));
	int val;
...
int lmac_cli_get_txfcs(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_ctrl0 = gsw_lmac_reg_read(fd, param, MAC_CTRL0(param->mac_idx));
	u32 val;
...
int lmac_cli_get_int(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_ipg(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_preamble(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_ctrl1 = gsw_lmac_reg_read(fd, param, MAC_CTRL1(param->mac_idx));
	u32 val;
...
int lmac_cli_get_defermode(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_ctrl1 = gsw_lmac_reg_read(fd, param, MAC_CTRL1(param->mac_idx));
	u32 val;
...
int lmac_cli_get_lpi(int fd, GSW_MAC_Cli_t *param, u32 *mode_en, u32 *lpi_waitg, u32 *lpi_waitm)
{
...
int lmac_cli_get_jps(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_loopback(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_txer(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_testen = gsw_lmac_reg_read(fd, param, MAC_TESTEN(param->mac_idx));
	u32 val;
...
int lmac_cli_get_lpimonitor_mode(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_testen = gsw_lmac_reg_read(fd, param, MAC_TESTEN(param->mac_idx));
	u32 val;
...
int lmac_cli_get_pauseframe_samode(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_pauseframe_addr(int fd, GSW_MAC_Cli_t *param, u8 *mac_addr)
{
...
int lmac_cli_get_mac_pstat(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_mac_pisr(int fd, GSW_MAC_Cli_t *param)
{
...
int lmac_cli_get_rmon(int fd, GSW_MAC_Cli_t *param)
{
	...
	for (i = 0; i < max_mac; i++)
		sprintf(buf + strlen(buf), "%18s %d\t", "LMAC", i);
...
void lmac_help(void)
{
	...
	for (i = 0; i < num_of_elem; i++) {
		if (!strcmp(lmac_cfg[i].cmdname, "rmon")) {
			...
			printf("switch_cli lmac   <0/1/2..>        %15s             %s\n",
	...
	for (i = 0; i < num_of_elem; i++) {
		...
		if ((!strcmp(lmac_cfg[i].cmdname, "r")) ||
			...
			printf("switch_cli lmac   <0/1/2..>  get   %15s\n",
			       lmac_cfg[i].cmdname);
		}
	}
...
int lmac_main(u32 argc, char *argv[], int fd)
{
	int idx = 0;
	u32 start_arg = 0;
	u32 max_mac;
	...
	if (!strcmp(argv[start_arg], "get")) {
		...
		for (i = 0; i < num_of_elem; i++) {
			...
			if (!strcmp(argv[start_arg], lmac_cfg[i].cmdname)) {
				...
				if (!argv[start_arg]) {
					printf("switch_cli lmac   <0/1/2..>        %s   %s\n", lmac_cfg[i].cmdname, lmac_cfg[i].help);
					goto end;
				}
