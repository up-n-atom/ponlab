...
static char* get_name(char *name, char *p)
{
    ...
    if (*nameend == ':') {
        if ((nameend - namestart) < IFNAMSIZ) {
            memcpy_s(name, IFNAMSIZ, namestart, nameend - namestart);
...
static void get_dev_fields(char *bp, INTERFACE *ife, int procnetdev_vsn)
{
    ...
    sscanf_s(bp, ss_fmt[procnetdev_vsn],
...
static int if_readlist_proc(char *target, INTERFACE *ife)
{
    ...
    fh = scapi_getFilePtr(PATH_PROCNET_DEV, "r");
    if (!fh) {
	err = -1;
        goto error;
    }
    /* */
    if (!fgets(buf, sizeof(buf) - 1, fh)) { /* To escape 1st two lines of /proc/net/dev entries*/
        err = -1;
        fclose(fh);
        goto error;
    }
