...
{
	struct ltc2945_dev *ltc2945 =  dev_get_drvdata(dev);
	struct regmap *regmap = ltc2945->regmap;
	u32 res = ltc2945->res;
			      ...
			      unsigned long val)
{
	...
	case LTC2945_MIN_POWER_THRES_H:
		...
		if (control & CONTROL_MULT_SELECT) {
			/* 25 mV * 25 uV = 0.625 uV resolution. */
			val = DIV_ROUND_CLOSEST(val * res, 625);
		} else {
			/*
			 * 0.5 mV * 25 uV = 0.0125 uV resolution.
			 ...
			 */
			val = DIV_ROUND_CLOSEST(val* res, 25) * 2;
		}
		break;
	case LTC2945_VIN_H:
	case LTC2945_MAX_VIN_H:
	...
	case LTC2945_MIN_SENSE_THRES_H:
		 ...
		 * in mOhm.
		 */
		val = DIV_ROUND_CLOSEST(val * res, 25);
		break;
	default:
		return -EINVAL;
	}
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	ret = regmap_bulk_read(regmap, LTC2945_ADIN_H, &vol2, 2);
	if (ret < 0)
		return ret;

	ret = regmap_bulk_read(regmap, LTC2945_SENSE_H, &curr1, 2);
	if (ret < 0)
		return ret;

	vol2 *= 2;
	curr1 = DIV_ROUND_CLOSEST(curr1 * res, 25);
...
static int ltc2945_probe(struct i2c_client *client)
{
	...
	struct ltc2945_dev *ltc2945;
	struct regmap *regmap;

	ltc2945= devm_kzalloc(dev, sizeof(*ltc2945), GFP_KERNEL);
	if (!ltc2945)
		return -ENOMEM;
	...
	if (device_property_read_u32(dev, "ltc2945,resistor", &ltc2945->res)) {
		dev_info(dev,
			 "resistor is not specified, the default value 1mOhm is specified\n");
		ltc2945->res = 1;
	}
	ltc2945->regmap = regmap;
