...
#define CADENCE_QSPI_AUTOSUSPEND_TIMEOUT 3000

unsigned long cadence_qspi_init_timeout(const unsigned long timeout_in_ms)
{
	return jiffies + msecs_to_jiffies(timeout_in_ms);
}

unsigned long cadence_qspi_check_timeout(const unsigned long timeout)
{
...
static void cadence_qspi_work(struct work_struct *work)
{
	struct struct_cqspi *cadence_qspi = container_of(work,
							 struct struct_cqspi,
							 work);
	struct device *dev = &cadence_qspi->pdev->dev;
	unsigned long flags;
...
static int cadence_qspi_of_get_pdata(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(np, "lantiq,qspi-physize", &prop)) {
		dev_info(&pdev->dev,
			 "missing lantiq,qspi-physize; default to 128 kB\n");
		prop = 128;
	}
	pdata->qspi_ahb_size = prop * 1024; /* translate to kBytes */
	...
	for_each_child_of_node(np, nc) {
		...
		if (of_device_is_compatible(nc, "mxl,spinand"))
...
static int cadence_qspi_probe(struct platform_device *pdev)
{
	...
	pr_debug("%s %s %s\n", __func__, pdev->name, pdev->id_entry->name);
	...
	pm_runtime_set_autosuspend_delay(&pdev->dev,
					 CADENCE_QSPI_AUTOSUSPEND_TIMEOUT);
	...
	init_completion(&cadence_qspi->transfer_complete);
...
#ifdef CONFIG_PM
static int cadence_qspi_runtime_suspend(struct device *dev)
{
	struct spi_master *master = dev_get_drvdata(dev);
	struct struct_cqspi *priv = spi_master_get_devdata(master);

	clk_disable_unprepare(priv->fpi_clk);
	clk_disable_unprepare(priv->clk);

	return 0;
}

static int cadence_qspi_runtime_resume(struct device *dev)
{
	struct spi_master *master = dev_get_drvdata(dev);
	struct struct_cqspi *priv = spi_master_get_devdata(master);
	int ret;
	...
	ret = clk_prepare_enable(priv->fpi_clk);
	if (ret) {
		dev_err(dev, "Cannot enable fpi clock.\n");
		clk_disable_unprepare(priv->clk);
		return ret;
	}
...
static const struct dev_pm_ops cadence_qspi__dev_pm_ops = {
	SET_RUNTIME_PM_OPS(cadence_qspi_runtime_suspend,
			   cadence_qspi_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(cadence_qspi_suspend, cadence_qspi_resume)
};
