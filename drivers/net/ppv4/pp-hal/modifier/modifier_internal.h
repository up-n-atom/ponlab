...
enum mod_eu_si_mux_sel {
	...
	EU_SI_SEL_HDR_L4_OFF,
	EU_SI_SEL_LAST = EU_SI_SEL_HDR_L4_OFF,
};
...
enum mod_sce_mux_sel {
	...
	SCE_SI_SEL_HDR_L4_OFF      = 0x26,
	/* define only for fetching L2TP diff and L4 CSUM in one command */
	SCE_SI_SEL_RSRV            = 0x28,
	SCE_SI_SEL_L2TP_DIFF       = 0x29,
	SCE_SI_SEL_L4_CSUM         = 0x2A,
	SCE_SI_SEL_L3_HDR_SZ       = 0x2C,
...
#define MOD_PKT_OFF_L4_EXT EU_SI_SEL_HDR_L4_OFF
 ...
 * @return s32 0 on success, error code otherwise
 */
static inline s32 mod_alu_sub(enum mod_eu_type alu, struct mod_recipe *rcp,
			      s32 src, u32 s_sz, s32 val, u32 v_sz, s32 dst)
{
	return mod_alu_cmd(alu, rcp, src, s_sz, val, v_sz, dst,
			   EU_ALU_DEC_2S_A);
}

/**
 * @brief subtract register field by a value, done if condition met
 * @param alu ALU engine, EU_ALU0 or EU_ALU1
 * @param rcp current recipe
 * @param src source field register
 * @param s_sz source field size
 * @param val register holding the value to subtract
 * @param v_sz value field size
 * @param dst destination register to save the result
 * @param cond the condition
 * @param cond_op when to do the condition
 * @return s32 0 on success, error code otherwise
 */
static inline s32 mod_alu_sub_cond(enum mod_eu_type alu, struct mod_recipe *rcp,
				   s32 src, s32 s_sz, s32 val, s32 v_sz,
				   s32 dst, enum mod_cmd_fmt_alu_cond_type cond,
				   enum mod_cmd_fmt_alu_cond_op cond_op)
{
	return mod_alu_cmd_cond(alu, rcp, src, s_sz, val, v_sz, dst,
				EU_ALU_DEC_2S_A, cond, cond_op);
}
...
/**
 * @brief copy random number to dpu register
 * @param alu ALU engine, EU_ALU0 or EU_ALU1
 * @param rcp current recipe
 * @param dst destination register
 * @param sz field size register (max size is 4 bytes)
 * @return s32 0 on success, error code otherwise
 */
static inline s32 mod_alu_rand(enum mod_eu_type alu, struct mod_recipe *rcp,
			       s32 dst, u32 sz)
{
	return mod_alu_cmd(alu, rcp, DPU_RCP_REG_INVALID, sz,
			   DPU_RCP_REG_INVALID, sz, dst, EU_ALU_RAND);
}
