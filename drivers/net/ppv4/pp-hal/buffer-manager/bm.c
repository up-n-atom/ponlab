...
s32 bm_init(const struct pp_bm_init_param * const cfg)
{
	...
	/* Get config */
#ifdef CONFIG_X86_64
	db->bm_pop_push_addr = cfg->bm_pop_push_base;
#else
	db->bm_pop_push_addr = cfg->bm_pop_push_phys;
#endif
...
#ifdef CONFIG_X86_64
	*buff = PP_REG_RD64(db->bm_pop_push_addr + BM_POP_ADDR_OFFSET(policy));
#else
	/* This flow is not recommended for none-debug flows as it uses
	 * the qos uc mcdma for doing the 64bit access to HW
	 */
	...
	if (pool) {
		/* pool is at offset 56bits */
		*pool = (*buff >> 56) & 0xFF;
	}
	/* buffer pointer is 36bits */
	*buff &= BM_POP_NULL_ADDRESS;
...
s32 bm_push_buffer(u32 policy, u32 pool, u64 buff)
{
	...
	if (buff >= BM_POP_NULL_ADDRESS) {
		pr_err("invalid buffer pointer %#llx\n", buff);
		return -EINVAL;
	}
	...
	buff |= ((u64)pool << 56);
#ifdef CONFIG_X86_64
	PP_REG_WR64(db->bm_pop_push_addr + BM_POP_ADDR_OFFSET(policy), buff);
	return 0;
#else
	/* This flow is not recommended for none-debug flows as it uses
	 * the qos uc mcdma for doing the 64bit access to HW
