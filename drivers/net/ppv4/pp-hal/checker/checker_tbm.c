 ...
 * The crawler has 24 bits timer which has same clock as the PP clock.
 ...
 * 4. 1MHz        : 512bps-2Gbps
 *
 * Also, to support quick clock changes, we use the prescaler instead of
 * changing the credits for all TBMs.
 * to support that, we reduce the TBM clock during init, when we are at the
 * highest clock, so when the clock is reduced we increase the TBM
 * clock to compensate over the clock reduction w/o changing a single credit.
 */
 ...
 *        we set it to the maximum supported in HW so the crawler
...
/**
 * @brief The default prescaler for the TBM internal clock
 *        2 means the internal TBM clock will be reduced by 2 ^ 2
 *        we set it to 2 cause we want to be able to increase this clock
 *        in order to fix the rates w/o changing the credits.
 *        e.g. Regular working PP clock is 600MHz, BW is set to 100Mbps
 *             to keep same rate at 300MHz clock w/o changing the credits
 *             we double the TBM clock by scaling it up by 2 ^ 1
 */
#define TBM_DFLT_PRESCALER                         2U
...
struct tbm_database {
	...
	spinlock_t lock;
	/*! base clock rate for bandwidth calculations */
	u32 base_clk;
...
static u64 __tbm_max_rate_calc(u32 clk_MHz)
{
	...
	clk_hz       = MHZ_TO_HZ(clk_MHz) >> TBM_DFLT_PRESCALER;
...
static inline s32 __tbm_mantissa_calc(struct tbm_cfg *tbm, u64 rate)
{
	...
	clk_hz       = MHZ_TO_HZ(db->base_clk) >> TBM_DFLT_PRESCALER;
	max_mantissa = buf_fld_max_val_get(&tbm_flds[TBM_FLD_MANTISSA]);
	exp          = db->exp_init;

	pr_debug("rate %llu, clk_hz %u, prescaler %u, max_mantissa %u, exp %d\n",
		 rate, clk_hz, TBM_DFLT_PRESCALER, max_mantissa, exp);
...
static inline u64 __tbm_rate_calc(struct tbm_cfg *tbm, u32 clk_MHz)
{
	...
	rate =   MHZ_TO_HZ(clk_MHz) >> TBM_DFLT_PRESCALER;
	rate *=  tbm->mantissa;
	rate >>= tbm->exponent;
...
s32 chk_dual_tbm_get(u16 idx, struct pp_dual_tbm *dtbm)
{
	...
	dtbm->cir      = __tbm_rate_calc(&ctbm, db->base_clk);
	dtbm->pir      = __tbm_rate_calc(&ptbm, db->base_clk);
...
static void __dtbm_del(struct dual_tbm_db_info *db_info)
{
...
s32 chk_dual_tbm_free(u16 idx)
{
	...
	if (unlikely(__dtbm_is_used(idx))) {
		db_info->stopped = true;
		__tbm_unlock();
		pr_debug("cannot free, TBM %u is used by %u sessions, blocking new session attachements\n",
...
s32 chk_dual_tbm_session_attach(u16 idx)
{
	...
	if (db_info->stopped) {
		__tbm_unlock();
		pr_debug("cannot attached new sessions to tbm %u, entry is in stopped state\n",
			 idx);
		return -EPERM;
	}
...
s32 chk_dual_tbm_session_detach(u16 idx)
{
	...
	if (db_info->stopped && !db_info->n_sessions)
		__dtbm_del(db_info);
...
/**
 * @brief Update the tbm database defaults base on the clock frequency
 * @param clk the new clock in MHz
 */
static inline void __tbm_set_db_defaults(u32 clk_MHz)
{
	db->base_clk = clk_MHz;
	db->max_rate = __tbm_max_rate_calc(db->base_clk);
	db->exp_init = __tbm_exp_init_calc(db->max_rate);

	pr_debug("max_rate is %llu\n", db->max_rate);
	pr_debug("exp_init is %u\n", db->exp_init);
}
...
static s32 __tbm_db_init(struct pp_chk_init_param *init_args)
{
	...
	__tbm_set_db_defaults(init_args->hw_clk);

	pr_debug("max_rate %llu Bps, exp_init %u\n", db->max_rate,
		 db->exp_init);
	return 0;
}
...
static void __tbm_crawler_init(void)
{
	 ...
	 */
	reg = PP_FIELD_MOD(PP_CHK_TBM_TIMER_TIMER_MSB_MSK, TBM_DFLT_PRESCALER,
			   reg);
