...
struct cntr_db_info {
	...
	u32 n_sessions;        /*! number of associated sessions,
				...
				*  counter is not 0
				*/
	bool stopped;          /*! counter is stopped, cannot attach new
				*  sessions, will be deleted when last
				*  session removed
				*/
...
static void __sgc_del(u8 grp, u16 cntr)
{
	struct grp_db_info  *grp_info = &db->groups[grp];
	struct cntr_db_info *cntr_info = &grp_info->cntrs[cntr];

	/* reset the counter */
	__sgc_set(cntr_info, 0, 0);
	/* return the counter back the free list tail */
	list_add_tail(&cntr_info->node, &grp_info->free);
	cntr_info->stopped = false;
	grp_info->used_cntrs--;
}
...
s32 chk_sgc_free(u32 owner, u8 grp, u16 cntr)
{
	...
	if (unlikely(__sgc_is_used(cntr_info))) {
		cntr_info->stopped = true;
		__sgc_unlock();
		pr_debug("cannot free, sgc[%u][%04u] is used by %u sessions, blocking new session attachements\n",
...
s32 chk_sgc_session_attach(u8 grp, u16 cntr)
{
	...
	if (cntr_info->stopped) {
		__sgc_unlock();
		pr_debug("cannot attached new sessions to sgc %u, entry is in stopped state\n",
			 cntr);
		return -EPERM;
	}
...
s32 chk_sgc_session_detach(u8 grp, u16 cntr)
{
	...
	if (cntr_info->stopped && !cntr_info->n_sessions)
		__sgc_del(grp, cntr);
