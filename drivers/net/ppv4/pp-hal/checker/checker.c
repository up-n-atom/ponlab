...
static struct chk_dbg_stats rdbgstats;
...
dma_addr_t chk_tbm_prescaler_phys_addr_get(void)
{
	return pp_virt_to_phys((void *)(ulong)PP_CHK_TBM_TIMER_REG);
}

dma_addr_t chk_crawler_prescaler_phys_addr_get(void)
{
	return pp_virt_to_phys((void *)(ulong)PP_CHK_MBX_CRWLR_TIMER_INIT_REG);
}
...
{
	if (PP_IS_EXCEPTION_ID_VALID(id))
		return true;
...
s32 chk_session_dsi_evict(u32 id)
{
	dma_addr_t phys;
 ...
 */
static inline void __chk_crawler_init(struct pp_chk_init_param *init_param)
{
	...
	/* Configure the crawler to iterate over 8K sessions in ~32ms */
	cfg.timer_init_val =
	US_TO_HW_CYCLES(init_param->hw_clk, CRWLR_DFLT_SCAN_TIME_US) /
...
s32 chk_scan_time_get(u32 *n_sessions, u32 *time)
{
	...
	/* use 2 times timer values to make sure it will also work
	 * with high load
	 */
	timer = 2 * PP_REG_RD32(PP_CHK_MBX_CRWLR_TIMER_INIT_REG);
	reg   = PP_REG_RD32(PP_CHK_MBX_CRWLR_CTRL_REG);
	...
	*time       = HW_CYCLES_TO_US(pp_dev_hw_clk_get(), cycles);

	pr_debug("reg %#x, pace %u, timer %u, n_sessions %u, cycles %u, time %u[us]\n",
...
static s32 __chk_exception_session_si_set(u32 id, struct pp_si *alt_si)
{
	...
	for (i = 0 ; i < PP_SI_TOP_WORDS_CNT ; i++, sram_addr++)
...
s32 chk_nf_set(u16 queue, enum pp_nf_type nf)
{
	...
	case PP_NF_IPSEC_LLD:
		/* Nothing to do for IPSEC_LLD NF */
		break;
	case PP_NF_REMARKING:
		/* Nothing to do for Remarking NF */
		break;
			      ...
			      struct pp_port_cfg *eg_info)
{
	...
	ret = pp_si_port_info_set(PP_PORT_INVALID, NULL, cfg->port, eg_info,
				  &alt_si);
	if (ret) {
...
{
	if (ptr_is_null(alt_si))
		return -EINVAL;

	if (!__chk_is_exp_sess_id_valid(id))
		return -EINVAL;
	if (!test_bit(id, db->excp_sess_bmap))
		return -ENOENT;
...
s32 chk_exceptions_pkts_get(u64 *pkts)
{
	...
	*pkts = 0;
	for_each_set_bit(id, db->excp_sess_bmap, CHK_NUM_EXCEPTION_SESSIONS) {
		chk_exception_stats_get(id, &stats);
		*pkts += stats.packets;
	}
...
static void __chk_dbg_stats_get(struct chk_dbg_stats *stats)
{
	stats->sync = PP_REG_RD32(PP_CHK_SYNC_CNT_REG);
	stats->sync_first = PP_REG_RD32(PP_CHK_SYNC_FIRST_CNT_REG);
	stats->to_uc = PP_REG_RD32(PP_CHK_SEND2UC_CNT_REG);
	stats->uc_drop = PP_REG_RD32(PP_CHK_DROP_UC_CNT_REG);
	stats->drop = PP_REG_RD32(PP_CHK_DROP_FLAG_CNT_REG);
	stats->dynamic = PP_REG_RD32(PP_CHK_DYNAMIC_CNT_REG);
	stats->ip_opt = PP_REG_RD32(PP_CHK_IP_OPT_CNT_REG);
	stats->ttl_exp = PP_REG_RD32(PP_CHK_TTL_EXP_CNT_REG);
	stats->tcp_ctrl = PP_REG_RD32(PP_CHK_TCP_CTRL_CNT_REG);
	stats->frags = PP_REG_RD32(PP_CHK_FRG_CNT_REG);
	stats->mtu = PP_REG_RD32(PP_CHK_MTU_CNT_REG);
	stats->tcp_ack = PP_REG_RD32(PP_CHK_TCP_ACK_CNT_REG);
	stats->multicast = PP_REG_RD32(PP_CHK_MULTICAST_CNT_REG);
	stats->mtu_df = PP_REG_RD32(PP_CHK_MTU_STW_DF_CNT_REG);
	stats->exception = PP_REG_RD32(PP_CHK_EXCEPTION_CNT_REG);
}

s32 chk_dbg_stats_get(struct chk_dbg_stats *stats)
{
	if (ptr_is_null(stats))
		return -EINVAL;

	__chk_dbg_stats_get(stats);
	U32_STRUCT_DIFF(&rdbgstats, stats, stats);
...
u32 chk_num_sessions_get(void)
{
	return db->n_sessions;
}
...
static void __chk_cntr_cache_init(void)
{
	...
	reg = PP_FIELD_MOD(PP_CNTR_CACHE_POLICY_REPLACEMENT_POLICY_MSK,
...
s32 __chk_ddr_allocate(void)
{
	...
	db->dsi_base_virt = pp_dma_alloc(db->dsi_sz, &db->dsi_base_phys,
					 PP_DMA_NONECOHERENT);
	if (!db->dsi_base_virt) {
	...
	pp_cache_writeback(db->dsi_base_virt, db->dsi_sz);
...
{
	if (db->dsi_base_virt)
		pp_dma_free(db->dsi_base_virt, db->dsi_sz);
}
