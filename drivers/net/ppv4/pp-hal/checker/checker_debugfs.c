...
void exception_set(char *cmd_buf, void *data)
{
	...
	ret = pp_port_get(chk_cfg.port, &port_info);
	if (ret) {
		pr_err("Failed to get port%u configuration, ret %d\n",
		       chk_cfg.port, ret);
		return;
	}
	...
	if (chk_exception_session_set(&chk_cfg, &port_info)) {
...
enum crawler_scan_opts {
	crawler_scan_opt_help = 1,
	crawler_scan_opt_start_id,
};

static const match_table_t crawler_scan_tokens = {
	{crawler_scan_opt_help,     "help"},
	{crawler_scan_opt_start_id, "id=%u"},
	{ 0 },
};
 ...
 * @brief Crawler file help
 */
static void crawler_scan_start_help(void)
{
	pr_info("\n");
	pr_info(" Brief: Start crawler scan\n");
	pr_info(" Usage: echo [option=value] ... > crawler_scan\n");
	pr_info(" Options:\n");
	pr_info("   help - prints this help message\n");
	pr_info("   id   - first session to scan from, 0 - %u\n",
		chk_num_sessions_get() - 1);
}

/**
 * @brief Crawler configuration get/set
 */
static void crawler_scan_start(char *args, void *data)
{
	substring_t substr[MAX_OPT_ARGS];
	enum crawler_scan_opts opt;
	char *tok;
	int val, ret = 0;
	u32 start_id = 0, tries = 0;
	...
	while ((tok = strsep(&args, " \t\n")) != NULL) {
		if (!*tok)
			continue;
		opt = match_token(tok, crawler_scan_tokens, substr);
		switch (opt) {
		case crawler_scan_opt_help:
			crawler_scan_start_help();
			return;
		case crawler_scan_opt_start_id:
			if (match_int(&substr[0], &val))
				goto opt_parse_err;
			start_id = (u32)val;
			break;
	...
	inact_bmap_sz = BITS_TO_LONGS(chk_num_sessions_get()) * sizeof(ulong);
	inact_bmap = pp_dma_alloc(inact_bmap_sz, &inact_bmap_phys, 0);
	if (!inact_bmap) {
	...
	scan_time = get_jiffies_64();
	ret = chk_crawler_start(start_id, CHK_CRWLR_MAX_ITR_SIZE,
				inact_bmap_phys, inact_bmap_sz);
	chk_crawler_idle_wait(&tries);
	scan_time = get_jiffies_64() - scan_time;

	if (unlikely(ret))
		pr_info("failed to start crawler scan\n");

	pr_info("Scan time: %u [ms], tries %u", jiffies_to_msecs(scan_time),
		tries);
	pr_info("Inactive sessions: %*pbl", chk_num_sessions_get(), inact_bmap);

	pp_dma_free(inact_bmap, inact_bmap_sz);
	return;
...
PP_DEFINE_DEBUGFS(crawler_scan, NULL, crawler_scan_start);
...
struct chk_full_stats {
	struct chk_stats main;
	struct chk_dbg_stats dbg;
};

s32 chk_global_stats_get(void *stats, u32 num_stats, void *data)
{
	struct chk_full_stats *s = (struct chk_full_stats *)stats;
	s32 ret;

	ret = chk_stats_get(&s->main);
	if (ret)
		return ret;

	return chk_dbg_stats_get(&s->dbg);
}

/**
 * @brief Dump checker global stats
 */
s32 chk_global_stats_show(char *buf, size_t sz, size_t *n, void *stats,
			   u32 num_stats, void *data)
{
	struct chk_full_stats *st = (struct chk_full_stats *)stats;

	if (unlikely(ptr_is_null(buf) || ptr_is_null(n) || ptr_is_null(stats)))
		return -EINVAL;

	pr_buf_cat(buf, sz, *n, " Checker global stats:\n");
	pr_buf_cat(buf, sz, *n, " =====================\n");
	pr_buf_cat(buf, sz, *n, "Rx            = %u\n", st->main.pkts_rcvd_from_cls);
	pr_buf_cat(buf, sz, *n, "Exception Cnt = %u\n", st->main.exception_pkt_cnt);
	pr_buf_cat(buf, sz, *n, "Dflt session  = %u\n", st->main.dflt_session_pkt_cnt);
	pr_buf_cat(buf, sz, *n, "Accelerated   = %u\n", st->main.accelerated_pkts);

	pr_buf_cat(buf, sz, *n, "Sync          = %u\n", st->dbg.sync);
	pr_buf_cat(buf, sz, *n, "Sync First    = %u\n", st->dbg.sync_first);
	pr_buf_cat(buf, sz, *n, "TX to UC      = %u\n", st->dbg.to_uc);
	pr_buf_cat(buf, sz, *n, "TX to UC Drop = %u\n", st->dbg.uc_drop);
	pr_buf_cat(buf, sz, *n, "Drop          = %u\n", st->dbg.drop);
	pr_buf_cat(buf, sz, *n, "Dynamic       = %u\n", st->dbg.dynamic);
	pr_buf_cat(buf, sz, *n, "IP Option     = %u\n", st->dbg.ip_opt);
	pr_buf_cat(buf, sz, *n, "TTL Expired   = %u\n", st->dbg.ttl_exp);
	pr_buf_cat(buf, sz, *n, "TCP Control   = %u\n", st->dbg.tcp_ctrl);
	pr_buf_cat(buf, sz, *n, "TCP ACK       = %u\n", st->dbg.tcp_ack);
	pr_buf_cat(buf, sz, *n, "IP Fragments  = %u\n", st->dbg.frags);
	pr_buf_cat(buf, sz, *n, "MTU Exceed    = %u\n", st->dbg.mtu);
	pr_buf_cat(buf, sz, *n, "MTU Exceed DF = %u\n", st->dbg.mtu_df);
	pr_buf_cat(buf, sz, *n, "Multicast     = %u\n", st->dbg.multicast);
	pr_buf_cat(buf, sz, *n, "Exception     = %u\n", st->dbg.exception);

	return 0;
}

s32 chk_global_stats_diff(void *pre, u32 num_pre, void *post, u32 num_post,
			  void *delta, u32 num_delta, void *data)
{
	struct chk_full_stats *pre_it, *post_it, *delta_it;
	...
	post_it  = post;
	delta_it = delta;
	for_each_struct_mem(pre, pre_it, post_it++, delta_it++)
		U32_STRUCT_DIFF(pre_it, post_it, delta_it);

	return 0;
}

void chk_global_pps_seqfile(struct seq_file *f)
{
	pp_pps_show_seq(f, sizeof(struct chk_full_stats), 1,
			chk_global_stats_get, chk_global_stats_diff,
			chk_global_stats_show, NULL);
}

void chk_global_stats_seqfile(struct seq_file *f)
{
	pp_stats_show_seq(f, sizeof(struct chk_full_stats),
			  1, chk_global_stats_get,
			  chk_global_stats_show, NULL);
}

void chk_global_reset(char *cmd_buf, void *data)
{
	u32 reset;

	if (kstrtou32(cmd_buf, 10, &reset)) {
		pr_err("failed to parse '%s', not a number\n", cmd_buf);
		return;
	}
...
PP_DEFINE_DEBUGFS(chk_global, chk_global_stats_seqfile, chk_global_reset);
PP_DEFINE_DEBUGFS(chk_global_pps, chk_global_pps_seqfile, chk_global_reset);

static struct debugfs_file chk_dbg_files[] = {
	{"crawler",                &PP_DEBUGFS_FOPS(crawler)},
	{"crawler_scan",           &PP_DEBUGFS_FOPS(crawler_scan)},
	...
	{"global",                 &PP_DEBUGFS_FOPS(chk_global)},
	{"pps",                    &PP_DEBUGFS_FOPS(chk_global_pps)},
};
