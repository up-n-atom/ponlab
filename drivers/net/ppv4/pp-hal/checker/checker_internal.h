...
struct dual_tbm_db_info {
	...
	bool allocated;
	/*! specify if the tbm is in delete process and cannot be attached to
	 *  a new sessions, when no sessions attached the entry will be deleted
	 */
	bool stopped;
...
/**
 * @brief Get number of supported sessions
 * @return u32 number of sessions
 */
u32 chk_num_sessions_get(void);

/**
 * @brief Get checker's debug stats
 * @param stats
 * @return s32 0 on success, error code otherwise
 */
