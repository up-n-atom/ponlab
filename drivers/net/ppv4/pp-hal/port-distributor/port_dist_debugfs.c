...
/**
 * @brief Dump port distributor stats
 */
static void dbg_global_stats_show(struct seq_file *f)
{
	struct pp_stats stat;

	if (port_dist_stats_get(&stat))
		return;

	seq_printf(f, "global Rx: %u packets\n" ,(u32)stat.packets);
}

PP_DEFINE_DEBUGFS(port_dist_global_stats, dbg_global_stats_show, NULL);
...
static struct debugfs_file port_dist_dbg_files[] = {
	...
	{ "global", &PP_DEBUGFS_FOPS(port_dist_global_stats) },
};
