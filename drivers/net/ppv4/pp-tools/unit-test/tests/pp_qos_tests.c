...
#define QOS_TEST_NUM_RING_ADDR   (7)
...
static int qos_pre_test(void *data)
{
	...
	for (ring_idx = 0; ring_idx < QOS_TEST_NUM_RING_ADDR; ring_idx++) {
		alloc_sz = PPV4_QOS_DESC_SIZE * QOS_NUM_RING_DESCRIPTORS;
		ring_addr_virt[ring_idx] =
			pp_dma_alloc(alloc_sz, &ring_addr_phys[ring_idx], 0);
...
static int qos_post_test(void *data)
{
	...
	for (ring_idx = 0; ring_idx < QOS_TEST_NUM_RING_ADDR; ring_idx++) {
		pp_dma_free(ring_addr_virt[ring_idx],
			    PPV4_QOS_DESC_SIZE * QOS_NUM_RING_DESCRIPTORS);
	}
...
#define EWSP_PORT_NUM   7
#define EWSP_QUEUES_NUM 8
/* TBD: test this functionality */
...
static int enhanced_wsp_test(void *data)
{
	...
	PPUT_FAIL_RETURN(
		qdev->ewsp_db.num_queues == QOS_ENHANCED_WSP_MAX_QUEUES_PRX,
		"expected %u ewsp queues, found %u\n",
		QOS_ENHANCED_WSP_MAX_QUEUES_PRX, qdev->ewsp_db.num_queues);

	/* Make sure only first 6 ports are assigned with 6 ewsp queues each */
	for (port = 0; port < ARRAY_SIZE(ports); port++) {
		for (queue = 0; queue < EWSP_QUEUES_NUM; queue++) {
			if ((port == 0 && (queue >= 0 && queue <= 5)) ||
			    (port == 1 && (queue >= 0 && queue <= 5)) ||
			    (port == 2 && (queue >= 0 && queue <= 5)) ||
			    (port == 3 && (queue >= 0 && queue <= 5)) ||
			    (port == 4 && (queue >= 0 && queue <= 5)) ||
			    (port == 5 && (queue >= 0 && queue <= 5))) {
	...
	/* Remove port0 queue0, verify port6 is assigned with 1 ewsp queue */
	...
	/* Remove port0 queue7, verify port6 is assigned with 2 ewsp queue */
	...
	/* Move port6 queue7 to port0 */
	pp_qos_queue_conf_get(qdev, queues[6][7], &confq);
	confq.blocked = 1;
	confq.queue_child_prop.parent = ports[0];
	queues[0][7] = queues[6][7];
	...
	DUMP_QOS_TREE(qdev);
	for (port = 0; port < EWSP_PORT_NUM; port++)
