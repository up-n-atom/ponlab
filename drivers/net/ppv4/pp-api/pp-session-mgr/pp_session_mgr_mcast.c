...
/**
 * @brief return mcast_db from smgr_db
 */
static inline struct smgr_mcast_database *get_mcast_db(void)
{
	...
	return (struct smgr_mcast_database *)smgr_db->mcast_db;
}
/**
...
{
	struct smgr_mcast_database *db = get_mcast_db();
...
static inline struct pp_mcast_grp_info *__mcast_db_ent(u16 id)
{
	...
	if (ptr_is_null(db) || !__mcast_is_group_valid(id))
		return NULL;
...
s32 smgr_mcast_sess_lookup(struct sess_info *sess)
{
	...
	if (SESS_ARGS_IS_FLAG_ON(sess, PP_SESS_FLAG_MCAST_GRP_BIT)) {
		__mcast_lock();
		grp_db = __mcast_db_ent(sess->args->mcast.grp_idx);
		if (unlikely(ptr_is_null(grp_db)) ||
			unlikely(grp_db->base_sess != db->invalid_sess)) {
			__mcast_unlock();
			return -EEXIST;
		}
		__mcast_unlock();
		...
		grp_db = __mcast_db_ent(sess->args->mcast.grp_idx);
		if (unlikely(ptr_is_null(grp_db)) ||
			unlikely(grp_db->dst_sess[sess->args->mcast.dst_idx] !=
			     db->invalid_sess)) {
			__mcast_unlock();
			return -EEXIST;
		}
...
s32 smgr_mcast_init(struct device *dev, u32 invalid_sess_id)
{
	...
	if (!db) {
		pr_err("Failed to alloc %zu mcast memory\n", sizeof(*db));
		return -ENOMEM;
	}
	/* updating smgr_db to hold mcast_db */
	smgr_db->mcast_db = db;
...
void smgr_mcast_exit(void)
{
	...
	devm_kfree(dev, smgr_db->mcast_db);
	smgr_db->mcast_db = NULL;
}
