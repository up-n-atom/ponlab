...
#ifdef MTLK_WAVE_700
/* IEEE P802.11be/D1.31, December 2021 Sec 36.5 (RU: 242, 484, 996, 2*996, 4*996) */
/* Mode 5 (11be) EHT, MCS 0...13, CBW 0/1/2/3/4, CP 0/1/2. */
static const uint16 bitrates_11be[NUM_RATE_MCS_BE * 5 * HE_CP_TYPES_NUM] = {
 /* NSS = 1 */
  /*  20     20     20      40     40     40      80     80     80     160    160    160    320    320    320     CBW */
  /* SCP    MCP    LCP     SCP    MCP    LCP     SCP    MCP    LCP     SCP    MCP    LCP    SCP    MCP    LCP   *//* MCS*/
      86,    81,    73,    172,   163,   146,    360,   340,   306,    721,   681,   613,   1441,  1361,  1225,   /*  0 */
     172,   163,   146,    344,   325,   293,    721,   681,   613,   1441,  1361,  1225,   2882,  2722,  2450,   /*  1 */
     258,   244,   219,    516,   488,   439,   1081,  1021,   919,   2162,  2042,  1838,   4324,  4083,  3675,   /*  2 */
     344,   325,   293,    688,   650,   585,   1441,  1361,  1225,   2882,  2722,  2450,   5765,  5444,  4900,   /*  3 */
     516,   488,   439,   1032,   975,   878,   2162,  2042,  1838,   4324,  4083,  3675,   8647,  8167,  7350,   /*  4 */
     688,   650,   585,   1376,  1300,  1170,   2882,  2722,  2450,   5765,  5444,  4900,  11529, 10889,  9800,   /*  5 */
     774,   731,   658,   1549,  1463,  1316,   3243,  3063,  2756,   6485,  6125,  5513,  12971, 12250, 11025,   /*  6 */
     860,   813,   731,   1721,  1625,  1463,   3603,  3403,  3063,   7206,  6806,  6125,  14412, 13611, 12250,   /*  7 */
    1032,   975,   878,   2065,  1950,  1755,   4324,  4083,  3675,   8647,  8167,  7350,  17294, 16333, 14700,   /*  8 */
    1147,  1083,   975,   2294,  2167,  1950,   4804,  4537,  4083,   9607,  9074,  8166,  19215, 18148, 16333,   /*  9 */
    1290,  1219,  1097,   2581,  2438,  2194,   5404,  5104,  4594,  10809, 10208,  9188,  21618, 20417, 18375,   /* 10 */
    1434,  1354,  1219,   2868,  2708,  2438,   6004,  5671,  5104,  12010, 11342, 10208,  24019, 22685, 20416,   /* 11 */
    1549,  1463,  1316,   3097,  2925,  2633,   6485,  6125,  5513,  12971, 12250, 11025,  25941, 24500, 22050,   /* 12 */
    1721,  1625,  1463,   3441,  3250,  2925,   7206,  6806,  6125,  14412, 13611, 12250,  28824, 27222, 24500,   /* 13 */
};
#endif
...
_hw_rate_params_to_rate (uint32 mode, uint32 cbw, uint32 scp, uint32 mcs, uint32 nss)
{
  ...
  case PHY_MODE_AX: /* CBW 0/1/2/3 */
    ...
    if ((scp < HE_CP_MODES_NUM) && (mcs < NUM_RATE_MCS_AX) && (nss > 0) && (nss <= PHY_STAT_RATE_NSS_MAX)) {
      ...
      idx = cp_type + (HE_CP_TYPES_NUM * (cbw + (PHY_STAT_RATE_CBW_NUM_AX * mcs)));
      rate = bitrates_11ax[nss - 1][idx];
    }
    break;

#ifdef MTLK_WAVE_700
  case PHY_MODE_11BE: /* CBW 0/1/2/3/4 */
    /* MCS14/15 are "special" versions of MCS0, so override the value to use MCS0 bitrates */
    if ((mcs == 14) || (mcs == 15))
    {
        mcs = 0;
    }
    /* scp means cp_mode */
    if ((scp < HE_CP_MODES_NUM) && (mcs < NUM_RATE_MCS_BE) && (nss > 0) && (nss <= PHY_STAT_RATE_NSS_MAX)) {
      uint8 cp_type = he_cp_mode_to_cp_type_table[scp];
      idx = cp_type + (HE_CP_TYPES_NUM * (cbw + (PHY_STAT_RATE_CBW_NUM * mcs)));
      rate = bitrates_11be[idx] * nss;
    }
    break;
#endif
...
mtlk_bitrate_hw_params_to_rate (uint32 mode, uint32 cbw, uint32 scp, uint32 mcs, uint32 nss)
{
  ...
  if (MTLK_BITRATE_INVALID == rate) {
    ILOG1_DDDDD("Rate is not found: mcs %2d, mode %d, cbw %d, scp %d, nss %d", mcs, mode, cbw, scp, nss);
  }
  return rate;
}
 ...
 *          sm    - CBW [ 0 - 20MHz, 1 - 40MHz, 2 - 80MHz, 3 - 160MHz, 4 - 320MHz]
*/
int __MTLK_IFUNC
mtlk_bitrate_get_value (int index, int sm, int scp, BOOL is_eht)
{
  int phy; /* PHY mode */
  int mcs;
  int value;
...
#ifdef MTLK_WAVE_700
      /* select BE mode in case of EHT and BW >= 80 MHz */
      phy = (sm > 1) ? (is_eht ? PHY_MODE_11BE : PHY_MODE_AC) : PHY_MODE_N;
#else
...
/* cp: FW enum CyclicPrefixMode_e */
he_cp_types_e __MTLK_IFUNC wave_get_tx_he_gi_from_cp (int cp)
{
  if (cp < HE_CP_MODES_NUM)
    return he_cp_mode_to_cp_type_table[cp];

  return HE_CP_TYPES_NUM;
}

he_cp_types_e __MTLK_IFUNC wave_get_rx_he_gi_from_cp (int cp)
{
  /* HE CP [bits 1..2] */
  int cp_he = (cp & (BIT(1) | BIT(2))) >> 1;

  /* cp_he: 0 = short, 1 = medium, 2 = long; FW enum HeCp_e */
  if ((HE_CP_TYPE_SHORT != cp_he) &&
      (HE_CP_TYPE_MEDIUM != cp_he) &&
      (HE_CP_TYPE_LONG != cp_he)) {
        WLOG_D("Unexpected RX HE CP value %d", cp_he);
        return HE_CP_TYPES_NUM;
  }

  /* Our he_cp_types_e mimics FW HeCp_e, so just return value if valid */
  return cp_he;
}

BOOL __MTLK_IFUNC
wave_short_gi_from_tx_data_rate_info (mtlk_wssa_drv_peer_rate_info1_t *ri)
{
  /* scp: 0 = long, 1 = short; FW enum Scp_e */
  if (ri->PhyMode < PHY_MODE_AX)
    return ri->Scp;

  if (HE_CP_TYPE_SHORT == wave_get_tx_he_gi_from_cp(ri->Scp))
    return TRUE;
...
BOOL __MTLK_IFUNC
wave_short_gi_from_rx_data_rate_info (mtlk_wssa_drv_peer_rate_info1_t *ri)
{
  /* non-HE SCP [bit 0] */
  /* scp: 0 = long, 1 = short; FW enum Scp_e */
  if (ri->PhyMode < PHY_MODE_AX)
  ...
  if (HE_CP_TYPE_SHORT == wave_get_rx_he_gi_from_cp(ri->Scp))
    return TRUE;
