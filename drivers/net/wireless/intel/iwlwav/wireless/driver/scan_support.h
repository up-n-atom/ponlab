...
typedef struct scan_support
{
  ...
  devicePhyRxStatusDb_t orig_chandef_stats;
  struct mtlk_scan_request req; /* sched scan stuff can get saved here, too */
  mtlk_atomic_t fg_scan_end_event; /* To handle channel setting right after initial scan */
...
static __INLINE
unsigned is_bg_scan_running(const struct scan_support *ss)
{
  return (ss->flags & SDF_RUNNING) &&
...
static __INLINE
unsigned is_bg_break(const struct scan_support *ss)
{
  return (ss->flags & SDF_RUNNING) &&
         (ss->flags & SDF_BACKGROUND) &&
...
#ifdef MTLK_WAVE_700
void wave_scan_support_fill_chan_bonding_320mhz (wave_chan_bonding_320mhz_t *out_data, unsigned center_freq, unsigned primary_freq);
#endif
