...
_mtlk_df_fw_request_firmware(void *ctx, const char *fname)
{
  ...
  result = firmware_request_nowarn(&fw_entry, fname, ctx);

  if (result)
  {
    if (result == -ENOENT) {
      ILOG0_S("('%s') not located on the filesystem", fname);
    } else {
      WLOG_SD("Failed to request Firmware ('%s') result %d", fname, result);
    }
