...
static const  mtlk_ability_id_t _radio_mbss_master_abilities[] = {
  ...
  WAVE_RADIO_REQ_MBSS_VAP_DEL_SEC_VAP,
};
...
static const mtlk_ability_id_t _radio_general_abilities[] = {
  ...
  WAVE_RADIO_REQ_SET_FILS_BEACON_FLAG,
  WAVE_RADIO_REQ_GET_FILS_BEACON_FLAG,
...
static const mtlk_ability_id_t _radio_hw_data_abilities[] = {
  ...
  WAVE_RADIO_REQ_GET_PROP_PHY_CAP,
...
wave_radio_abilities_register(mtlk_core_t *core)
{
  ...
  if (wave_radio_get_is_zwdfs_radio(radio)) {
  ...
  } else if (wave_radio_get_is_zwdfs_operational_radio(radio) || wave_radio_get_is_zwdfs_background_supp(radio)) {
    ...
    if (MTLK_ERR_OK == res) {
      ...
      /* _radio_zwdfs_abilities is needed for wav700 so that serializer calls to be requested on operational radio */
