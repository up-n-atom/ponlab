...
__mtlk_mc_mcsta_compare_func (mcast_sta_t *item, const IEEE_ADDR *sta_addr, const IEEE_ADDR *host_addr, uint32 vap_id)
{
  if (item->vap_id != vap_id)
    return FALSE;
  if (0 != ieee_addr_compare(&item->sta_addr, sta_addr))
    return FALSE;
  if (host_addr) {
    if (0 != ieee_addr_compare(&item->host_addr, host_addr))
      return FALSE;
  }
...
int __MTLK_IFUNC
mtlk_mc_parse (mtlk_core_t* core, mtlk_nbuf_t *nbuf, const IEEE_ADDR *sta_addr,
               uint32 vap_id, BOOL *is_igmp_or_mld)
{
  int res = MTLK_ERR_OK;
  ...
  BOOL handle_mcast_groups = TRUE;

  ASSERT(core != NULL);
  ASSERT(nbuf != NULL);
  ASSERT(nbuf->data != NULL);
  ASSERT(is_igmp_or_mld != NULL);

  *is_igmp_or_mld = FALSE;
  if (!core || !nbuf || !nbuf->data)
    return MTLK_ERR_PARAMS;

  /* If multicast module is available, it will handle mcast groups */
  if (mtlk_core_mcast_module_is_available(core)) {
    handle_mcast_groups = FALSE;
  } else if (mtlk_mmb_fastpath_available(mtlk_vap_get_hw(core->vap_handle))) {
    /* If fastpath available and multicast module unavailable,
     * no need to handle mcast groups. In some of these cases, mcast group
  ...
  case ETH_P_IPV6:
    {
      ...
      if ((htons(*hopopt_data) == 0x0502) &&  /* Router Alert option in hop-by-hop option header */
...
mtlk_mc_transmit (mtlk_core_t* nic, mtlk_core_handle_tx_data_t *tx_data, uint32 nbuf_flags, int bridge_mode, sta_entry *src_sta)
{
    ...
    if (sta) {
      do {
        ...
        if (mac80211_sta->ml_sta_info.is_ml) {
          if (sta->info.MainVapId != mtlk_vap_get_id_fw(nic->vap_handle)) {
            ILOG2_DD("Multilink : mainvapid is not match to vapid %d : %d ",
                                    sta->info.MainVapId, mtlk_vap_get_id_fw(nic->vap_handle));
            sta = mtlk_stadb_iterate_next(&iter);
            continue;
          }
          tmp_mac =  mac80211_sta->ml_sta_info.mld_mac;
        }
	else
        ...
        mtlk_osal_copy_eth_addresses(ether_header->h_dest, tmp_mac);
        /* send packet */
    ...
    if (sta) {
      do {
        ...
        if ((sta->peer_ap || mtlk_sta_is_4addr(sta)) && (sta != src_sta)){
          ...
          if (mac80211_sta->ml_sta_info.is_ml) {
            if (sta->info.MainVapId != mtlk_vap_get_id_fw(nic->vap_handle)) {
              ILOG2_DD("Multilink : mainvapid is not match to vapid %d : %d ", sta->info.MainVapId, mtlk_vap_get_id_fw(nic->vap_handle));
              sta = mtlk_stadb_iterate_next(&iter);
              continue;
            }
          }
  ...
  wave_core_qos_adjust_be_priority(tx_data->dst_sta, tx_data->nbuf);
#endif
...
static mcast_ip4_src_t *
_mtlk_mc_find_ip4_src_gid (mcast_ip4_grp_t *grp, const struct in_addr *src_addr, unsigned gid)
{
  mcast_ip4_src_t *src = NULL;

  if (grp) {
    MTLK_LIST_FIND_ITEM(src, grp->ip4_src_list,
        (src->ip4_src_addr.s_addr == src_addr->s_addr) && (src->info.grp_id == gid));
  }
...
static mcast_ip6_src_t *
_mtlk_mc_find_ip6_src_gid (mcast_ip6_grp_t *grp, const struct in6_addr *src_addr, unsigned gid)
{
  mcast_ip6_src_t *src = NULL;
  if (grp) { /* Find source */
    MTLK_LIST_FIND_ITEM(src, grp->ip6_src_list, (ipv6_addr_equal(&src->ip6_src_addr, src_addr))
...
mtlk_mc_update_group_id_sta (mtlk_core_t *nic, uint32 grp_id, mc_action_t action, mtlk_mc_addr_t *mc_addr, sta_entry *sta)
{
  ...
  if (wave_is_mld_sta(sta))
    sta_addr = wave_get_sta_mld_addr(sta);
  else
  ...
  case MTLK_MC_STA_JOIN_GROUP:
    ...
    if (mc_addr->type == MTLK_IPv4) {
      ...
      mcsta = __mtlk_mc_find_mcsta(group->mcsta, sta_addr, NULL, sta->secondary_vap_id);
      if (!mcsta) {
...
_mtlk_mc_update_sta_status (mtlk_core_t *nic, mtlk_core_ui_mc_update_sta_db_t *req, mcast_group_info_t *group_info)
{
  ...
  for (i = 0; i < req->macs_num; i++, mac_list++) {
    ...
    if ((MULTI_AP_BACKHAUL == multi_ap_mode) ||
          ((MULTI_AP_EXT_HYBRID == multi_ap_mode) && (df_user != sec_df_user))) {
...
mtlk_mc_update_stadb (mtlk_core_t *nic, mtlk_core_ui_mc_update_sta_db_t *req)
{
  ...
  switch (req->mc_addr.type) {
    case MTLK_IPv4: {
       ...
       * if entry is not exists and (sta_num == 0 or action is DELETE) --> do nothing */
      src = _mtlk_mc_find_ip4_src_gid(grp, &req->mc_addr.src_ip.ip4_addr, req->grp_id);
      if (!src) {
        if ((MTLK_MC_STADB_DEL == req->action) || (0 == req->macs_num)) goto end;
    ...
    case MTLK_IPv6: {
      ...
      /* STEP 2: Get/create mcast source entry.
       * if entry is not exists and (sta_num == 0 or action is DELETE --> do nothing */
      src = _mtlk_mc_find_ip6_src_gid(grp, &req->mc_addr.src_ip.ip6_addr, req->grp_id);
      if (!src) {
        if ((MTLK_MC_STADB_DEL == req->action) || (0 == req->macs_num)) goto end;
...
#ifdef WAVE_DCDP_MCAST_GID_FROM_SKB_SUPPORTED
  if (mtlk_core_mcast_module_is_available(core)) {
    group_id = dc_dp_get_skb_gid(nbuf); /* a negative value in error case */
    if ((group_id >= 0) && (group_id < MC_GROUPS)) /* non-negative value if found the SKB extension for MC GID */
      return group_id;
  }
#endif
