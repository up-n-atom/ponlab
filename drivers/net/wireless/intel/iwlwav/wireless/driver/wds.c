...
wds_beacon_process (mtlk_handle_t master_wds_handle, const void *data,  uint32 data_size)
{
  ...
  if ((peer->state == WDS_STATE_DISCONNECTED) && mtlk_osal_atomic_get(&peer->can_connect)) {
    ...
    if (beacon_data->is_he)
      bonding_core = bonding_beacon;
#ifdef MTLK_WAVE_700
    if ((wave_radio_is_gen7(radio)) && (beacon_data->is_eht))
      bonding_core = bonding_beacon;
#endif
    /* exclude bonding from check in case of non HT/VHT beacon or no secondary
     * channel */
    ...
    if (bonding_core ^ bonding_beacon) {
      ILOG1_DD("Beacon on wrong bonding!  Our bonding %d, Beacon's bonding %d", bonding_core, bonding_beacon);
      peer->numof_beacons_wrong_bond++;
    }
    if (channel_core ^ beacon_data->channel) {
      ILOG1_DD("Beacon on wrong channel! Our channel %d, Beacon's channel %d", channel_core, beacon_data->channel);
    ...
    action = WDS_BEACON_CONNECT;
...
#ifdef MTLK_WAVE_700
      if (wave_radio_is_gen7(radio) && beacon_data->is_eht) {
          is_eht_cap_changed = memcmp(&peer->beacon_data.eht_cap, &beacon_data->eht_cap,
                                      sizeof(beacon_data->eht_cap));
        if (is_eht_cap_changed) {
            mtlk_dump(1, &peer->beacon_data.eht_cap, sizeof(peer->beacon_data.eht_cap), "peer:");
            mtlk_dump(1, &beacon_data->eht_cap, sizeof(beacon_data->eht_cap), "beacon:");
        }
      }
#endif
      if ((peer->beacon_data.basic_rate_set     ^ beacon_data->basic_rate_set)     |
        ...
        if (wave_radio_is_gen7(radio)) {
          ILOG1_DD("WDS: EHT                : %d -> %d", peer->beacon_data.is_eht, beacon_data->is_eht);
          ILOG1_S( "WDS: EHT Cap Info       : %s", is_eht_cap_changed ? "changed" : "not changed");
        }
#endif
...
void __MTLK_IFUNC wds_peer_connect (wds_t *wds, sta_entry *sta, const IEEE_ADDR *addr, uint8 is_ht, uint8 is_vht,
#ifdef MTLK_WAVE_700
                                    uint8 is_eht,
#endif
                                    uint8 is_he)
{
  ...
  ILOG1_DY("CID-%04x: WDS peer connect %Y", mtlk_vap_get_oid(wds->vap_handle), addr->au8Addr);
