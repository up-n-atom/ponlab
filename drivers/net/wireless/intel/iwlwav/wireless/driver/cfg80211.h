...
int __MTLK_IFUNC wv_cfg80211_send_vendor_radar_event(struct wireless_dev *wdev, void *data, int data_len);
void __MTLK_IFUNC wv_cfg80211_set_dfs_state_bit_map(struct wiphy *wiphy, struct mtlk_chan_def *mcd, uint8 rbm, enum nl80211_dfs_state state);
int __MTLK_IFUNC wv_cfg80211_get_prop_phy_cap (struct wiphy *wiphy, struct net_device *ndev);
