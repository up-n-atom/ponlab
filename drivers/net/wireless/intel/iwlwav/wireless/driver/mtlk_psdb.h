...
typedef struct psdb_gain_per_freq
{
  uint32        freq;
  uint32        gain;
} psdb_gain_per_freq_t;
...
typedef struct psdf_table_data {
    ...
    union {
        ...
        psdb_gain_per_freq_t  gain_per_freq_entries[0];
...
typedef struct {
  ...
  uint16            rx_ant_sel_mask; /* Rx_AntSelection_Mask */
...
#ifdef MTLK_WAVE_700
    PSDB_PHY_MODE_11BE = 8,
    PSDB_PHY_MODE_BF_11BE = 9
#endif
} psdb_phy_mode_t;
...
typedef enum psdb_phy_cw_e {
    ...
    PSDB_PHY_CW_BE_160,
    PSDB_PHY_CW_BE_320,
    PSDB_PHY_CW_BF_BE_20,
    PSDB_PHY_CW_BF_BE_40,
    PSDB_PHY_CW_BF_BE_80,
    PSDB_PHY_CW_BF_BE_160,
    PSDB_PHY_CW_BF_BE_320,
#endif
    PSDB_PHY_CW_LAST
} psdb_phy_cw_t;
...
#define PSDB_RATES_MU     (NUM_RATE_MCS_AC * PHY_STAT_RATE_NSS_MAX)
#define PSDB_RATES_BFAC   (NUM_RATE_MCS_AC * PHY_STAT_RATE_NSS_MAX)
#define PSDB_RATES_BF     (NUM_RATE_MCS_AX * PHY_STAT_RATE_NSS_MAX)
/*
...
*/
#define PSDB_RATES_GEN7    (NUM_RATE_MCS_BE * PHY_STAT_RATE_NSS_MAX)
#define PSDB_RATES_BF_BE   (PSDB_RATES_GEN7)
#define PSDB_RATES_BE      (MTLK_HW_BAND_MAX * PSDB_RATES_GEN7)
#define PSDB_RATES_TOTAL   (PSDB_RATES_B + PSDB_RATES_AG +  PSDB_RATES_N + PSDB_RATES_AC + PSDB_RATES_MU + PSDB_RATES_BFAC + PSDB_RATES_AX + PSDB_RATES_BF + PSDB_RATES_BF_BE + PSDB_RATES_BE)
...
typedef struct psdb_rate_pw_table {
  union {
    ...
    struct {
      ...
      mtlk_rate_pw_entry_t  rates_mu[PSDB_RATES_MU];
      mtlk_rate_pw_entry_t  rates_bfac[PSDB_RATES_BFAC];
      mtlk_rate_pw_entry_t  rates_11ax[PSDB_RATES_AX];
      mtlk_rate_pw_entry_t  rates_bf[PSDB_RATES_BF];
      mtlk_rate_pw_entry_t  rates_bfbe[PSDB_RATES_BF_BE];
      mtlk_rate_pw_entry_t  rates_gen7_2G[PSDB_RATES_GEN7];
      mtlk_rate_pw_entry_t  rates_gen7_5G[PSDB_RATES_GEN7];
      mtlk_rate_pw_entry_t  rates_gen7_6G[PSDB_RATES_GEN7];
    };
  };
...
/* Power limit damage table (in half dBs) */
typedef struct psdb_pw_limit_damage_table {
    uint8   pw_limits[MTLK_HW_BAND_MAX];
} psdb_pw_limit_damage_table_t;

typedef struct psdb_ant_gain_per_freq_table
{
  uint32                nof_entries;
  psdb_gain_per_freq_t  entries[0];
} psdb_ant_gain_per_freq_table_t;
...
struct hw_psdb {
    ...
    psdb_pw_limit_damage_table_t *pw_limit_damage_table; /* Pointer to Power Limit Damage table */
    psdb_ant_gain_per_freq_table_t  *ant_gain_per_freq; /* Pointer to Antenna Gain table */
...
static __INLINE psdb_ant_gain_per_freq_table_t *
wave_psdb_get_ant_gain_per_freq_table (hw_psdb_t *psdb)
{
  psdb_ant_gain_per_freq_table_t *agpf = NULL;

  if (psdb) {
    agpf = psdb->ant_gain_per_freq;
  }
 ...
 * Note: The chan nums 223,231 are not allowed                                      */
#define INVALID_6G_CHAN_NUM_223 223
#define INVALID_6G_CHAN_NUM_231 231
...
int psdb_get_hw_limits (mtlk_core_t *core, const char *country, int chan_freq, psdb_pw_limits_t *pw_limits);
...
int __MTLK_IFUNC mtlk_psdb_get_rate_power_list (mtlk_core_t *core, mtlk_clpb_t *clpb, mtlk_hw_band_e bandId);
int __MTLK_IFUNC mtlk_psdb_get_damage_power_limit (mtlk_core_t *core, mtlk_clpb_t *clpb, mtlk_hw_band_e bandId, uint8 max_ant_gain);
int __MTLK_IFUNC mtlk_psdb_get_ant_gain (mtlk_core_t *core, mtlk_clpb_t *clpb, int chan, mtlk_hw_band_e bandId);

uint32 __MTLK_IFUNC mtlk_psdb_country_to_regd_code (mtlk_core_t *core, const char *country);
BOOL   __MTLK_IFUNC mtlk_psdb_country_is_supported (mtlk_core_t *core, const char *country);
...
uint8 __MTLK_IFUNC wave_psdb_get_hw_radio_band_cfg(mtlk_hw_t* hw);

uint16 __MTLK_IFUNC wave_psdb_ant_mask_to_ant_sel_mask(uint8 ant_mask);
