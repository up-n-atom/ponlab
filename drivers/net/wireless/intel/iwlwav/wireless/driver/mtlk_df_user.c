...
#if defined(CONFIG_WAVE_DEBUG) && defined(CONFIG_WIRELESS_EXT)
/*********************************************************************
...
struct _mtlk_df_ui_slow_ctx_t
{
  ...
  /* Used to prevent re-arming timer in case sync_cancel and set are executed in parallel from different contexts */
  mtlk_osal_spinlock_t    stat_timer_lock;
...
struct _mtlk_df_user_t {
  ...
  mtlk_dlist_t       vlan_pending_list;
...
typedef struct _pending_list_entry_t {
  mtlk_dlist_entry_t lentry;
  IEEE_ADDR addr;                 /* MAC address of backhaul STA*/
  char vlan_ifname[IFNAMSIZ + 1]; /* The name of the vlan/backhaul interface with the secondary df that the backhaul STA waits */
} pending_list_entry_t;
...
void __MTLK_IFUNC mtlk_df_user_error (mtlk_slid_t caller_slid)
{
...
mtlk_error_t mtlk_df_user_push_backhaul_sta_to_pending_list (mtlk_df_user_t *df_user, sta_entry *backhaul_sta)
...
{
  pending_list_entry_t *list_entry;
  MTLK_ASSERT(df_user != NULL);
  MTLK_ASSERT(backhaul_sta != NULL);

  list_entry = mtlk_osal_mem_alloc(sizeof(pending_list_entry_t), WAVE_MEM_TAG_VLAN_PENDING_LIST);
  if (NULL == list_entry) {
    return MTLK_ERR_NO_MEM;
  }
  list_entry->addr = *mtlk_sta_get_addr(backhaul_sta);
  wave_memcpy(list_entry->vlan_ifname, sizeof(list_entry->vlan_ifname),
    backhaul_sta->vlan_ifname, sizeof(backhaul_sta->vlan_ifname));
  mtlk_dlist_push_back(&df_user->vlan_pending_list, &list_entry->lentry);
  return MTLK_ERR_OK;
}

BOOL mtlk_df_user_pop_backhaul_sta_from_pending_list (mtlk_df_user_t *df_user, const char *vlan_ifname, IEEE_ADDR *addr)
/* Find a STA pending vlan/bkh interface name 'vlan_name' and pop it from the pending list */
{
  mtlk_dlist_entry_t *head, *entry;
  MTLK_ASSERT(df_user != NULL);
  MTLK_ASSERT(vlan_ifname != NULL);
  MTLK_ASSERT(addr != NULL);

  mtlk_dlist_foreach(&df_user->vlan_pending_list, entry, head) {
    pending_list_entry_t *list_entry = MTLK_LIST_GET_CONTAINING_RECORD(entry, pending_list_entry_t, lentry);
    if (0 == strcmp(list_entry->vlan_ifname, vlan_ifname)) {
      *addr = list_entry->addr;
      mtlk_dlist_remove(&df_user->vlan_pending_list, entry);
...
static void _mtlk_df_user_cleanup_vlan_pending_list (mtlk_dlist_t *list)
{
  mtlk_dlist_entry_t *entry;
  while (NULL != (entry = mtlk_dlist_pop_front(list))) {
    pending_list_entry_t *list_entry = MTLK_LIST_GET_CONTAINING_RECORD(entry, pending_list_entry_t, lentry);
    mtlk_osal_mem_free(list_entry);
  }
  mtlk_dlist_cleanup(list);
}
...
__mtlk_df_user_is_secondary (mtlk_df_user_t *df_user)
{
  MTLK_ASSERT(NULL != df_user);
...
{
  return __mtlk_df_user_is_secondary(df_user);
}

static __INLINE BOOL
__mtlk_df_user_is_stats_available(mtlk_df_user_t *df_user)
{
  MTLK_ASSERT(NULL != df_user);
  return df_user->is_stats_available;
}
...
mtlk_df_user_ndev_to_df_user(struct net_device *ndev)
{
  ...
  if (ndev->ieee80211_ptr == NULL)
    ...
    df_user = _mtlk_df_user_internal_ndev_to_df_user(ndev);
  else
    /* ndev priv is mac80211 sdata */
    df_user = wv_ieee80211_ndev_to_dfuser(ndev);

#if 0 /* have to be checked by the caller */
  if (__UNLIKELY(!df_user)) {
    ELOG_S("%s: df_user is NULL", ndev->name);
  }
#endif
...
#endif
  MTLK_INIT_STEPS_LIST_ENTRY(df_user, STAT_TIMER_LOCK)
  MTLK_INIT_STEPS_LIST_ENTRY(df_user, STAT_TIMER)
  MTLK_INIT_STEPS_LIST_ENTRY(df_user, VLAN_PENDING_LIST)
...
/*
 *  Translates given DCDP/FW (absolute) vap id input to driver (relative) vap id.
 */
uint8
wave_get_drv_vap_id_from_dcdp_vap_id (mtlk_vap_handle_t vap_handle, uint8 vap_id)
{
#ifdef WAVE_VAPID_CONVERT_IN_DRV
  /* In case of LGM or PUMA or WAVE700 CTB,
   * there is no translation of VAPID involved in DCDP
   * So, Convert absolute vapid to relative vapid */
  wave_radio_t *radio = wave_vap_radio_get(vap_handle);
  mtlk_hw_t *hw = mtlk_vap_get_hw(vap_handle);
  if (wave_radio_is_5g_in_zwdfs_cdb_cfg(radio) || mtlk_hw_type_is_gen7(hw)) {
    return vap_id & wave_hw_get_vap_id_mask(hw, wave_vap_radio_id_get(vap_handle));
  }
#endif
  return vap_id;
}
...
/*
 * Translates given driver (relative) vap id input to DCDP/FW (absolute) vap id.
 */
static uint8
_wave_get_dcdp_vap_id_from_drv_vap_id (mtlk_vap_handle_t vap_handle, uint8 vap_id)
{
#ifdef WAVE_VAPID_CONVERT_IN_DRV
  /* In case of (LGM CDB) and (LGM or PUMA with WAVE700 CTB),
   * there is no translation of VAPID involved in DCDP
   * So, Convert absolute vapid to relative vapid */
  wave_radio_t *radio = wave_vap_radio_get(vap_handle);
  mtlk_hw_t *hw = mtlk_vap_get_hw(vap_handle);
  if (wave_radio_is_5g_in_zwdfs_cdb_cfg(radio) || mtlk_hw_type_is_gen7(hw))
      return wave_vap_id_fw_get(wave_vap_ep_bit_get(vap_handle), vap_id);
#endif
  return vap_id;
}

uint8
wave_df_user_get_dcdp_vap_id (mtlk_df_user_t* df_user)
{
  ...
  return _wave_get_dcdp_vap_id_from_drv_vap_id(vap_handle, vap_id);
}
...
_mtlk_df_user_priv_subxface_start (mtlk_df_user_t* df_user)
{
  ...
  dc_dp_subif_id.vap_id = wave_df_user_get_dcdp_vap_id(df_user);
  ...
  if (mtlk_mmb_dcdp_path_available(hw)) {
    ...
    if (mtlk_hw_type_is_gen6_d2(hw)) {
      wave_radio_t   *radio;
      radio = wave_vap_radio_get(mtlk_df_get_vap_handle(df_user->df));
      if (wave_radio_get_is_zwdfs_radio(radio)) {
        ILOG0_S("%s: ZWDFS radio - DirectConnect registration not required",
            mtlk_df_user_get_name(df_user));
        return MTLK_ERR_OK;
      }
    }
...
#if MTLK_USE_DIRECTCONNECT_DP_API
void __MTLK_IFUNC
mtlk_dcdp_entries_disconnect (mtlk_df_t* df)
{
  mtlk_df_user_t *df_user = mtlk_df_get_user(df);
  MTLK_ASSERT(df_user != NULL);
  if (dc_dp_disconn_if(NULL, &df_user->dcdp.subif_id, NULL, 0) != DC_DP_SUCCESS) {
    ELOG_D("Unable to remove dcdp entries for sub interface id: %d", df_user->dcdp.subif_id.port_id);
  }
}
#endif
                                              ...
                                              const mtlk_coc_power_cfg_t *power_params)
{
  ...
  intvec[7] = power_params->ant_mask_params.mask[4];
...
{
  WAVE_CHECK_AND_SET_INTVEC_LENGTH(max_length, 6, *intvec_length);
  ...
  intvec[4] = stats->FastRcvryWithinTimer;
  intvec[5] = stats->FullRcvryWithinTimer;
}
...
_mtlk_df_user_get_coc_power_params_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_coc_power_cfg_t *power_params)
{
  ...
  if (intvec_length == 1) { /* enable/disable auto coc with one parameter */
    res = MTLK_ERR_OK;
  } else {
    if (intvec_length >= MTLK_COC_PW_ANT_CFG_PARAMS) {
      num_tx_ant = intvec[1];
      num_rx_ant = intvec[2];
...
static int
_mtlk_df_user_set_tid_link_spreading_cfg_params_by_intves (const uint32 *intvec, uint16 intvec_length, mtlk_core_str_tid_link_spreading_cfg_t *tid_spreading_cfg)
{
  if ((1 <= intvec_length) && (intvec_length <= 3)) {
     tid_spreading_cfg->enable = intvec[0];
     tid_spreading_cfg->tid_spreading_mode = TID_SPREAD_INVALID;

     if (intvec_length > 1) {
         tid_spreading_cfg->tid_spreading_mode = intvec[1];
         if (intvec_length > 2) {
           tid_spreading_cfg->ratio = intvec[2];
         }
     }
     return MTLK_ERR_OK;
  } else {
...
{
  if ((intvec_length < 1) || (intvec_length > 4)) {
    ELOG_D("Incorrect vector length (%u), must be 1..4", intvec_length);
    return MTLK_ERR_PARAMS;
  }
  ...
  if (intvec_length > 3)
    amsdu_num_cfg->amsdu_ehtnum = intvec[3];
...
{
  if (intvec_length != WAVE_PIE_CFG_PARAM_COUNT) {
    ELOG_DD("Incorrect vector length (%u), must be %u", intvec_length, WAVE_PIE_CFG_PARAM_COUNT);
    return MTLK_ERR_PARAMS;
  }

  pie_cfg->drop_thresh                   = intvec[0];
  pie_cfg->max_burst                     = intvec[1];
  pie_cfg->minQueueSizeDefault           = intvec[2];
  pie_cfg->minQueueSizePs                = intvec[3];
  pie_cfg->weigth                        = intvec[4];
  pie_cfg->pie_convert_p[0].drop_thresh  = intvec[5];
  pie_cfg->pie_convert_p[0].denominator  = intvec[6];
  pie_cfg->pie_convert_p[1].drop_thresh  = intvec[7];
  pie_cfg->pie_convert_p[1].denominator  = intvec[8];
  pie_cfg->pie_convert_p[2].drop_thresh  = intvec[9];
  pie_cfg->pie_convert_p[2].denominator  = intvec[10];
  pie_cfg->pie_convert_p[3].drop_thresh  = intvec[11];
  pie_cfg->pie_convert_p[3].denominator  = intvec[12];
  pie_cfg->pie_convert_p[4].drop_thresh  = intvec[13];
  pie_cfg->pie_convert_p[4].denominator  = intvec[14];
  pie_cfg->pie_convert_p[5].drop_thresh  = intvec[15];
  pie_cfg->pie_convert_p[5].denominator  = intvec[16];
  pie_cfg->pie_convert_p[6].drop_thresh  = intvec[17];
  pie_cfg->pie_convert_p[6].denominator  = intvec[18];
  pie_cfg->pie_convert_p[7].drop_thresh  = intvec[19];
  pie_cfg->pie_convert_p[7].denominator  = intvec[20];
  pie_cfg->pie_convert_p[8].drop_thresh  = intvec[21];
  pie_cfg->pie_convert_p[8].denominator  = intvec[22];
  pie_cfg->pie_convert_p[9].drop_thresh  = intvec[23];
  pie_cfg->pie_convert_p[9].denominator  = intvec[24];
  pie_cfg->pie_convert_p[10].drop_thresh = intvec[25];
  pie_cfg->pie_convert_p[10].denominator = intvec[26];
  pie_cfg->decay_val                     = intvec[27];
  pie_cfg->min_drop_prob                 = intvec[28];
  pie_cfg->max_drop_prob                 = intvec[29];
  pie_cfg->alpha[0]                      = intvec[30];
  pie_cfg->alpha[1]                      = intvec[31];
  pie_cfg->alpha[2]                      = intvec[32];
  pie_cfg->alpha[3]                      = intvec[33];
  pie_cfg->beta[0]                       = intvec[34];
  pie_cfg->beta[1]                       = intvec[35];
  pie_cfg->beta[2]                       = intvec[36];
  pie_cfg->beta[3]                       = intvec[37];
  pie_cfg->dq_thresh[0]                  = intvec[38];
  pie_cfg->dq_thresh[1]                  = intvec[39];
  pie_cfg->dq_thresh[2]                  = intvec[40];
  pie_cfg->dq_thresh[3]                  = intvec[41];
  pie_cfg->target_latency[0]             = intvec[42];
  pie_cfg->target_latency[1]             = intvec[43];
  pie_cfg->target_latency[2]             = intvec[44];
  pie_cfg->target_latency[3]             = intvec[45];
  pie_cfg->t_update                      = intvec[46];
  pie_cfg->user_mode                     = intvec[47];
...
{
  WAVE_CHECK_AND_SET_INTVEC_LENGTH(max_length, WAVE_PIE_CFG_PARAM_COUNT, *intvec_length);

  intvec[0]  = pie_cfg->drop_thresh;
  intvec[1]  = pie_cfg->max_burst;
  intvec[2]  = pie_cfg->minQueueSizeDefault;
  intvec[3]  = pie_cfg->minQueueSizePs;
  intvec[4]  = pie_cfg->weigth;
  intvec[5]  = pie_cfg->pie_convert_p[0].drop_thresh;
  intvec[6]  = pie_cfg->pie_convert_p[0].denominator;
  intvec[7]  = pie_cfg->pie_convert_p[1].drop_thresh;
  intvec[8]  = pie_cfg->pie_convert_p[1].denominator;
  intvec[9]  = pie_cfg->pie_convert_p[2].drop_thresh;
  intvec[10]  = pie_cfg->pie_convert_p[2].denominator;
  intvec[11] = pie_cfg->pie_convert_p[3].drop_thresh;
  intvec[12] = pie_cfg->pie_convert_p[3].denominator;
  intvec[13] = pie_cfg->pie_convert_p[4].drop_thresh;
  intvec[14] = pie_cfg->pie_convert_p[4].denominator;
  intvec[15] = pie_cfg->pie_convert_p[5].drop_thresh;
  intvec[16] = pie_cfg->pie_convert_p[5].denominator;
  intvec[17] = pie_cfg->pie_convert_p[6].drop_thresh;
  intvec[18] = pie_cfg->pie_convert_p[6].denominator;
  intvec[19] = pie_cfg->pie_convert_p[7].drop_thresh;
  intvec[20] = pie_cfg->pie_convert_p[7].denominator;
  intvec[21] = pie_cfg->pie_convert_p[8].drop_thresh;
  intvec[22] = pie_cfg->pie_convert_p[8].denominator;
  intvec[23] = pie_cfg->pie_convert_p[9].drop_thresh;
  intvec[24] = pie_cfg->pie_convert_p[9].denominator;
  intvec[25] = pie_cfg->pie_convert_p[10].drop_thresh;
  intvec[26] = pie_cfg->pie_convert_p[10].denominator;
  intvec[27] = pie_cfg->decay_val;
  intvec[28] = pie_cfg->min_drop_prob;
  intvec[29] = pie_cfg->max_drop_prob;
  intvec[30] = pie_cfg->alpha[0];
  intvec[31] = pie_cfg->alpha[1];
  intvec[32] = pie_cfg->alpha[2];
  intvec[33] = pie_cfg->alpha[3];
  intvec[34] = pie_cfg->beta[0];
  intvec[35] = pie_cfg->beta[1];
  intvec[36] = pie_cfg->beta[2];
  intvec[37] = pie_cfg->beta[3];
  intvec[38] = pie_cfg->dq_thresh[0];
  intvec[39] = pie_cfg->dq_thresh[1];
  intvec[40] = pie_cfg->dq_thresh[2];
  intvec[41] = pie_cfg->dq_thresh[3];
  intvec[42] = pie_cfg->target_latency[0];
  intvec[43] = pie_cfg->target_latency[1];
  intvec[44] = pie_cfg->target_latency[2];
  intvec[45] = pie_cfg->target_latency[3];
  intvec[46] = pie_cfg->t_update;
  intvec[47] = pie_cfg->user_mode;
}
...
static int
_mtlk_df_user_get_intvec_by_mu_group_config_params (const uint32 *intvec, uint16 intvec_length, mtlk_mu_group_config_params_t *mu_grp_params)
{
  if (intvec_length != WAVE_DF_USER_MU_GROUP_CFG_PARAM_COUNT) {
    ELOG_DD("Incorrect vector length (%u), must be %u", intvec_length, MTLK_DF_USER_RTS_MODE_PARAM_COUNT);
    return MTLK_ERR_PARAMS;
  }

  if ((intvec[0] >= WAVE_NUM_FORMATION_TYPES)) {
    ELOG_D("Formation type must be from 0 to %d", WAVE_NUM_FORMATION_TYPES - 1);
    return MTLK_ERR_PARAMS;
  }

  if ((intvec[1] > MAX_UINT8) || (intvec[2] > MAX_UINT8)) {
    ELOG_D("Number of STAs in group must be from <= %d", MAX_UINT8);
    return MTLK_ERR_PARAMS;
  }

  mu_grp_params->formation_type = intvec[0];
  mu_grp_params->min_stas       = intvec[1];
  mu_grp_params->max_stas       = intvec[2];
...
#ifdef MTLK_WAVE_700
static int
_mtlk_df_user_fill_logger_fifo_mux_cfg (mtlk_logger_fifo_mux_cfg_t *mux_cfg, uint32 data)
{
  if ((data != LOGGER_HWFIFO_MUX_B1_B0) &&
      (data != LOGGER_HWFIFO_MUX_B2_B0) &&
      (data != LOGGER_HWFIFO_MUX_B1_B2)) {
    ELOG_D("Incorrect logger hwfifo mux mode %u", data);
    return MTLK_ERR_VALUE;
  }

  mux_cfg->mode = (uint8)data;
  return MTLK_ERR_OK;
}
#endif
...
_mtlk_df_user_get_intvec_by_amsdu_params (uint32 *intvec, uint16 max_length, uint16 *intvec_length, const mtlk_amsdu_num_cfg_t *amsdu_num_cfg)
{
  ...
  intvec[3] = amsdu_num_cfg->amsdu_ehtnum;
}
...
static int
_mtlk_df_user_get_emulate_params_by_intvec (const uint32 *intvec, uint32 intvec_length, iwpriv_emulate_params_t *emulate_params)
{
  int res = MTLK_ERR_PARAMS;
  ...
  if (intvec[1]) {
    if ((intvec[1] >= MTLK_MIN_RBM && intvec[1] <= MTLK_MAX_RBM))
      emulate_params->rbm = intvec[1];
    else {
      ELOG_V("Error in params, usage: s11hEmulatRadar <unused_param> [radar bit map: 0x1 to 0xFF] [dfs band: ZWDFS - ZWDFS Band]");
      return res;
    }
  }
  else {
    emulate_params->rbm = MTLK_MAX_RBM;
  }
  emulate_params->dfs_band = intvec[2];
...
#define MTLK_DF_USER_RETRY_LIMIT_COUNT 3
...
_mtlk_df_user_get_intvec_by_retry_limit(uint32 *intvec, uint16 max_length, uint16 *intvec_length, wave_retry_limit_cfg_t *retry_limit_cfg)
{
    ...
    intvec[2] = retry_limit_cfg->retry_limit.retry_limit_proberesp;
}
...
_mtlk_df_user_get_retry_limit_by_intvec (const uint32 *intvec, uint16 intvec_length, wave_retry_limit_cfg_t *retry_limit_cfg)
{
    if (intvec_length != MTLK_DF_USER_RETRY_LIMIT_COUNT) {
      ELOG_D("Need exactly 3 arguments (length=%u)", intvec_length);
      return MTLK_ERR_PARAMS;
    }
    ...
    if (intvec[2] < MIN_RETRY_LIMIT || intvec[2] > MAX_RETRY_LIMIT) {
      ELOG_DDD("Error in retry limit probe response : %d valid range %d ... %d",intvec[2],MIN_RETRY_LIMIT,MAX_RETRY_LIMIT);
      return MTLK_ERR_PARAMS;
    }

    retry_limit_cfg->retry_limit.retry_limit_mgmt      = intvec[0];
    retry_limit_cfg->retry_limit.retry_limit_data      = intvec[1];
    retry_limit_cfg->retry_limit.retry_limit_proberesp = intvec[2];
    return MTLK_ERR_OK;
}
...
{
  if ((data != 0) && (data!=1) && (data != 3) && (data != 6) && (data != 9) && (data != 12)) {
...
static int
_mtlk_df_user_get_radar_fifo_params_by_intvec (const uint32 *intvec, uint32 intvec_length, iwpriv_radar_fifo_params_t *radar_fifo_params)
{
  int res = MTLK_ERR_PARAMS;
  int i;

  if (intvec_length > 3 || intvec_length == 0) {
    ELOG_D("Incorrect vector length (%u)", intvec_length);
    return res;
  }

  for (i = 0; i < intvec_length; i++) {
    if ((intvec[i] != 0 && intvec[i] != 1)) {
      ELOG_V("Incorrect input");
      return res;
    }
  }

  radar_fifo_params->enable_dump = intvec[0];

  if (intvec_length > 1) {
    radar_fifo_params->immediate = intvec[1];

    if (intvec_length == 3) {
      radar_fifo_params->dfs_band = intvec[2];
    }
  }
...
#define _MTLK_STATIC_PLAN_COMMON_CFG_TABLE_ \
  ...
  _ENTRY_(  0,    15,  dl_HeCp                          )\
  _ENTRY_(  0,    15,  dl_HeLtf                         )\
  _ENTRY_(  0,     2,  ul_HeCp                          )\
...
#define _MTLK_STATIC_PLAN_USER_CFG_TABLE_GEN6_ \
...
#define _MTLK_STATIC_PLAN_USER_CFG_TABLE_ \
  /* Per user phase  parameters */ \
  _ENTRY_(  0,   511,  uspStationIndexes                )\
  _ENTRY_(  0,    59,  dl_PsduRatePerUsp                )\
  _ENTRY_(  0,   511,  tid_alloc_bitmap                 )\
  _ENTRY_(  0,    59,  ul_PsduRatePerUsp                )\
  _ENTRY_(  0,     1,  dl_bfType                        )\
  _ENTRY_(  0,    15,  dl_subBandPerUsp                 )\
  _ENTRY_(  0,     9,  dl_startRuPerUsp                 )\
  _ENTRY_(  0,     15, dl_ruSizePerUsp                  )\
  /* Trigger frame RCR per User */ \
  _ENTRY_(  0,     7,  tfStartingSS                     )\
  _ENTRY_(  0,     3,  tfMpduMuSpacingFactor            )\
  _ENTRY_(  0,     7,  tfPadding                        )\
  _ENTRY_(  0,   255,  targetRssi                       )\
  _ENTRY_(  0,     1,  ul_ldpc                          )\
  _ENTRY_(  0,    59,  ul_psduRate                      )\
  _ENTRY_(  0,    15,  ul_SubBand                       )\
  _ENTRY_(  0,     9,  ul_StartRU                       )\
  _ENTRY_(  0,     15, ul_ruSize                        )\
  _ENTRY_(  0,   255,  SsAllocation                     )\
  _ENTRY_(  0,     1,  codingType_BCC_OR_LDPC           )\

#endif

#define _MTLK_STATIC_PLAN_USER_MRU_CFG_TABLE_ \
  /* Per user MRU  parameters */ \
  _ENTRY_(  0,   15,   dl_mruPerUsp                     )\
  _ENTRY_(  0,   15,   ul_mru                           )\
...
static mtlk_df_user_cfg_limit_t     _mtlk_static_plan_user_cfg_tab_gen6[] = {
  _MTLK_STATIC_PLAN_USER_CFG_TABLE_GEN6_
};
#ifdef MTLK_WAVE_700
...
static mtlk_df_user_cfg_limit_t     _mtlk_static_plan_user_mru_cfg_tab[] = {
  _MTLK_STATIC_PLAN_USER_MRU_CFG_TABLE_
};
...
static void
_mtlk_df_user_dump_static_plan_user_cfg (BOOL is_gen7, uint32 *params, int i)
{
  ...
  if (is_gen7)
    ...
    _mtlk_df_user_dump_limit_params(params, _mtlk_static_plan_user_cfg_tab_gen6, ARRAY_SIZE(_mtlk_static_plan_user_cfg_tab_gen6));
}

static void
_mtlk_df_user_dump_static_plan_user_mru_cfg (uint32 *params, int i)
{
  mtlk_osal_emergency_print("--- Static Planner User MRU Config[%d] ---", i);
  _mtlk_df_user_dump_limit_params(params, _mtlk_static_plan_user_mru_cfg_tab, ARRAY_SIZE(_mtlk_static_plan_user_mru_cfg_tab));
}

#define _ENTRY_(a, b, name) { { \
        size_t szof = sizeof(cfg->name); \
        size_t offs = (size_t)((uintptr_t)(void*)&cfg->name - (uintptr_t)(void*)cfg); \
...
static int
_mtlk_df_user_fill_static_plan_user_cfg (BOOL is_gen7, UMI_MU_PLAN_PER_USER_CONFIGURATION *cfg, uint32 *params, int size)
{
    /* Filling the table if input values are valid */
    if (is_gen7 && mtlk_df_user_cfg_param_limits_valid(_mtlk_static_plan_user_cfg_tab, params, size)) {
        _MTLK_STATIC_PLAN_USER_CFG_TABLE_
        return MTLK_ERR_OK;
    } else if (!is_gen7 && mtlk_df_user_cfg_param_limits_valid(_mtlk_static_plan_user_cfg_tab_gen6, params, size)) {
           _MTLK_STATIC_PLAN_USER_CFG_TABLE_GEN6_
           return MTLK_ERR_OK;
    } else {
        return MTLK_ERR_PARAMS;
...
static int
_mtlk_df_user_fill_static_plan_user_mru_cfg (UMI_MU_PLAN_PER_USER_MRU_CONFIG *cfg, uint32 *params, int size)
{
    /* Filling the table if input values are valid */
    if (mtlk_df_user_cfg_param_limits_valid(_mtlk_static_plan_user_mru_cfg_tab, params, size)) {
        _MTLK_STATIC_PLAN_USER_MRU_CFG_TABLE_
        return MTLK_ERR_OK;
    } else {
        return MTLK_ERR_PARAMS;
...
static int
_mtlk_df_user_fill_static_plan_config_by_intvec (mtlk_df_user_t *df_user, UMI_STATIC_PLAN_CONFIG *config, uint32 *data, unsigned length)
{
  UMI_MU_PLAN_COMMON_CONFIGURATION      *common_cfg;
  UMI_MU_PLAN_PER_USER_CONFIGURATION    *user_cfg;
  UMI_MU_PLAN_PER_USER_MRU_CONFIG       *user_mru_cfg;
  int   i, nof_users, size, size_mru, remain;
  int   res;
  mtlk_vap_handle_t vap_handle          = mtlk_df_get_vap_handle(mtlk_df_user_get_df(df_user));
  BOOL is_gen7                          = mtlk_hw_type_is_gen7(mtlk_vap_get_hw(vap_handle));
  ...
  size = ARRAY_SIZE(_mtlk_static_plan_user_cfg_tab_gen6);
#ifdef MTLK_WAVE_700
  if (is_gen7)
    size = ARRAY_SIZE(_mtlk_static_plan_user_cfg_tab);
#endif
  if (remain < (nof_users * size)) {
    ELOG_DDD("Wrong number of UserConfig params: (%d) < (%d * %d)",
             remain, nof_users, size);
    return MTLK_ERR_PARAMS;
  }

  for (i = 0; i < nof_users; i++) {
    user_cfg = &config->perUserParameters[i];
    _mtlk_df_user_dump_static_plan_user_cfg(is_gen7, data, i);
    res = _mtlk_df_user_fill_static_plan_user_cfg(is_gen7, user_cfg, data, size);
    data += size;
    if (MTLK_ERR_OK != res) {
  ...
  remain -= (nof_users * size);
  /* Fill User MRU config(s) */
  size_mru = ARRAY_SIZE(_mtlk_static_plan_user_mru_cfg_tab);
  if (remain == (nof_users * size_mru)) {
    for (i = 0; i < nof_users; i++) {
      user_mru_cfg = &config->perUserMruParams[i];
      _mtlk_df_user_dump_static_plan_user_mru_cfg(data, i);
      res = _mtlk_df_user_fill_static_plan_user_mru_cfg(user_mru_cfg, data, size_mru);
      if (MTLK_ERR_OK != res) {
        ELOG_D("Invalid StaticPlanner MRU UserConfig[%d]", i);
        return res;
      }
      data += size_mru;
    }
  } else if (remain) {
    ELOG_DDD("Wrong number of MRU UserConfig params: (%d) != (%d * %d)",
              remain, nof_users, size_mru);
    return MTLK_ERR_PARAMS;
  }
...
#if 0 // Support dwell time < window slice
...
_mtlk_df_user_fill_fixed_ltf_and_gi_by_intvec(uint32 *intvec, uint16 intvec_length, UMI_FIXED_LTF_AND_GI_REQ *fixed_ltf_and_gi)
{
    if (intvec_length != 2) {
      ...
      if((intvec[1] & 0x7F) < 6) /* TODO: Use constants here from UMI ? */
      {
...
#define MIN_MGMT_FRAME_TX_POWER -20
#define MAX_MGMT_FRAME_TX_POWER 0

static int
_wave_df_user_fill_mgmt_frame_pwr_ctrl_cfg (mtlk_df_user_t *df_user, wave_mgmt_frame_pwr_ctrl_cfg_t *cfg, int data)
{
  if ((data >= MIN_MGMT_FRAME_TX_POWER &&
       data <= MAX_MGMT_FRAME_TX_POWER))
  {
    cfg->mgmt_frame_pwr_ctrl = (int8)data;
    return MTLK_ERR_OK;
  }
  else
  {
    ELOG_DDD("Incorrect Management Frame Tx Power %d should be %d - %d", data, MIN_MGMT_FRAME_TX_POWER, MAX_MGMT_FRAME_TX_POWER);
    return MTLK_ERR_VALUE;
  }
}

static int
_wave_df_user_validate_fill_eth_addr (IEEE_ADDR *mac_addr, struct sockaddr *sa)
{
  if (!mtlk_osal_is_valid_ether_addr(sa->sa_data)) {
    ELOG_V("Incorrect MAC address provided (zero, multicast or broadcast)");
    return MTLK_ERR_PARAMS;
...
#define CSI_PARAM_LENGTH 15      /* SA_HDR(2) + STAMAC(6) + Br-lanMAC(6) + Enable(1) */
#define CSI_AUTO_PARAM_LENGTH (CSI_PARAM_LENGTH + 1) /* Csi Rate(1) */
#define CSI_MAX_RATE 30
#define CSI_MIN_RATE 1

static int
_wave_df_user_fill_csi_enable_cfg (uint32 *intvec, uint16 intvec_length, wave_csi_enable_t *cfg)
{
  char *data = (char *) intvec;
  struct sockaddr *sa = (struct sockaddr *)data;

  if (intvec_length != CSI_PARAM_LENGTH) {
    ELOG_D("Need exactly 3 arguments (length=%u)", intvec_length);
    return MTLK_ERR_PARAMS;
  }
  ...
  if (!mtlk_osal_is_zero_address(sa->sa_data) &&
      (MTLK_ERR_OK != _wave_df_user_validate_fill_eth_addr (&cfg->sta_mac, sa))) {
    ELOG_V("Error in STA MAC addr");
    return MTLK_ERR_PARAMS;
  }
  ...
  if (MTLK_ERR_OK != _wave_df_user_validate_fill_eth_addr (&cfg->brlan_mac, sa)) {
    ELOG_V("Error in br-lan MAC addr");
    return MTLK_ERR_PARAMS;
  }

  data += IEEE_ADDR_LEN + sizeof(sa->sa_family);

  if ((*data != 0) && (*data != 1)) {
    ELOG_V("Error in CSI enable flag should be 0/1");
    return MTLK_ERR_PARAMS;
  }
  cfg->is_enable = *data;
...
static int
_wave_df_user_fill_csi_auto_rate_cfg (uint32 *intvec, uint16 intvec_length, wave_csi_auto_rate_t *cfg)
{
  char *data = (char *) intvec;
  struct sockaddr *sa = (struct sockaddr *)data;

  if (intvec_length != CSI_AUTO_PARAM_LENGTH) {
    ELOG_D("Need exactly 3 arguments (length=%u)", intvec_length);
    return MTLK_ERR_PARAMS;
  }
  ...
  if (MTLK_ERR_OK != _wave_df_user_validate_fill_eth_addr (&cfg->sta_mac, sa)) {
    ELOG_V("Error in STA MAC addr");
    return MTLK_ERR_PARAMS;
  }
  ...
  if(*data > CSI_MAX_RATE || *data < CSI_MIN_RATE) {
    ELOG_DD("Error in CSI Rate - should be %d - %d", CSI_MIN_RATE, CSI_MAX_RATE);
    return MTLK_ERR_PARAMS;
  }
  cfg->csi_rate = *data;
...
_mtlk_df_user_fill_fixed_rate_thermal_by_intvec (int32 *intvec, uint16 intvec_length, wave_thermal_cfg_t *fixed_rate_thermal)
{
  ...
  if ((intvec[0] != 0) && (intvec[0] != 1)) {
    ELOG_D("Incorrect vector configuration: <enable mode %d>", intvec[0]);
    return res;
  } else if (intvec[0] == 0) {
    ELOG_V("Disable fixed rate thermal");
    return MTLK_ERR_OK;
  }

  if ((intvec[1] < MIN_RSSI) || (intvec[1] > 0)) {
    ELOG_D("Incorrect RX threshold %d", intvec[1]);
    return MTLK_ERR_VALUE;
  } else {
    fixed_rate_thermal->thermal_cfg.thermal_enable          = (int8) intvec[0];
    fixed_rate_thermal->thermal_cfg.thermal_threshold       = (int8) intvec[1];
    fixed_rate_thermal->thermal_cfg.thermal_power_reduc_val = (int8) intvec[2];
    res = MTLK_ERR_OK;
  }

  if ((intvec[2] < 0) || (intvec[2] > MAX_POWER_DBM)) {
    ELOG_D("Incorrect value for Power reduction value : %d", intvec[2]);
    res = MTLK_ERR_PARAMS;
  }
...
static void
_mtlk_df_user_get_intvec_by_fixed_rate_thermal (uint32 *intvec, uint16 max_length, uint16 *intvec_length, const
wave_thermal_cfg_t *fixed_rate_thermal)
{
  WAVE_CHECK_AND_SET_INTVEC_LENGTH(max_length, 3, *intvec_length);
  intvec[0] = (uint32)fixed_rate_thermal->thermal_cfg.thermal_enable;
  intvec[1] = (uint32)fixed_rate_thermal->thermal_cfg.thermal_threshold;
  intvec[2] = (uint32)fixed_rate_thermal->thermal_cfg.thermal_power_reduc_val;
}
...
_mtlk_df_user_iwpriv_get_param(mtlk_df_user_t* df_user, uint32 param_id, char* data, mtlk_iwpriv_params_t params, uint16* length)
{
  ...
  _DF_USER_GET_PARAM_MAP_START(df_user, param_id, res)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_FILS_BEACON_FLAG, WAVE_RADIO_REQ_GET_FILS_BEACON_FLAG, FALSE, mtlk_fils_beacon_flag_cfg_t, fils_beacon_flag_cfg)
      MTLK_CFG_GET_ITEM(fils_beacon_flag_cfg, fils_beacon_flag, *(int*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_MGMT_FRM_PWR_CTRL, WAVE_CORE_REQ_GET_MGMT_FRAME_PWR_CTRL, FALSE, wave_mgmt_frame_pwr_ctrl_cfg_t, mgmtframepwr_cfg)
      MTLK_CFG_GET_ITEM(mgmtframepwr_cfg, mgmt_frame_pwr_ctrl, *(int*)data);

    /* Multicast 3-Address Configuration */
    _DF_USER_GET_ON_PARAM(PRM_ID_ALLOW_3ADDR_MCAST, WAVE_CORE_REQ_GET_ALLOW_3ADDR_MCAST, FALSE, wave_3addr_mcast_mode_t, cfg)
      MTLK_CFG_GET_ITEM(cfg, allow_3addr_mcast, *(uint32*)data);

    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_PCIE_SPEED, WAVE_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg, pcie_auto_cfg)
      MTLK_CFG_GET_ITEM(core_cfg, pcie_auto_cfg, *(uint32*)data);
#ifdef MTLK_WAVE_700
    /* Logger HW FIFO Mux Config IW processing */
    _DF_USER_GET_ON_PARAM(PRM_ID_LOGGER_FIFO_MUX_CFG, WAVE_RADIO_REQ_GET_LOGGER_FIFO_MUX_CFG, FALSE, mtlk_logger_fifo_mux_cfg_t, logger_fifo_mux_cfg)
      MTLK_CFG_GET_ITEM(logger_fifo_mux_cfg, mode, *(uint32*)data);
#endif
    ...
    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_MSCS_ENABLE, WAVE_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg, mscs_enable)
      MTLK_CFG_GET_ITEM(core_cfg, mscs_enable, *(uint32*)data);
...
_mtlk_df_user_iwpriv_set_param(mtlk_df_user_t* df_user, uint32 param_id, char* data, mtlk_iwpriv_params_t params)
{
  ...
  _DF_USER_SET_PARAM_MAP_START(df_user, param_id, res)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_MULTI_AP_MODE, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, multi_ap_mode, *(int *)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_MU_GROUP_CONFIG, WAVE_RADIO_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC_VOID(master_core_cfg, mu_group_config_params, _mtlk_df_user_get_intvec_by_mu_group_config_params,
                                     ((uint32*)data, length, &master_core_cfg->mu_group_config_params));
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_DYNAMIC_WMM, WAVE_RADIO_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM(master_core_cfg, dynamic_wmm, *(uint32 *)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_DYNAMIC_EDCA, WAVE_RADIO_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM(master_core_cfg, dynamic_edca, *(uint32 *)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_INTRA_VAP_MCAST, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, intra_vap_mcast, *(uint32 *)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_MSCS_ENABLE, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, mscs_enable, *(uint32 *)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_FILS_BEACON_FLAG, WAVE_RADIO_REQ_SET_FILS_BEACON_FLAG, FALSE,  mtlk_fils_beacon_flag_cfg_t, fils_beacon_flag_cfg)
      MTLK_CFG_SET_ITEM(fils_beacon_flag_cfg, fils_beacon_flag, *(int*)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_11H_EMULATE_RADAR_DETECTION, WAVE_RADIO_REQ_SET_SCAN_AND_CALIB_CFG, FALSE, mtlk_scan_and_calib_cfg_t, scan_and_calib_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(scan_and_calib_cfg, emulate_params,
                                _mtlk_df_user_get_emulate_params_by_intvec, ((uint32*)data, length, &scan_and_calib_cfg->emulate_params), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_STATIC_PLAN_CONFIG, WAVE_RADIO_SET_STATIC_PLAN, FALSE, mtlk_static_plan_cfg_t, static_plan_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(static_plan_cfg, config, _mtlk_df_user_fill_static_plan_config_by_intvec,
                              (df_user, &static_plan_cfg->config, (uint32*)data, length), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_VW_TEST_MODE, WAVE_RADIO_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM(master_core_cfg, vw_test_mode, *(uint32*)data);
    ...
    /* Management Frame Power Control */
    _DF_USER_SET_ON_PARAM(PRM_ID_MGMT_FRM_PWR_CTRL, WAVE_CORE_REQ_SET_MGMT_FRAME_PWR_CTRL, FALSE, wave_mgmt_frame_pwr_ctrl_cfg_t, mgmtframepwr_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(mgmtframepwr_cfg, mgmt_frame_pwr_ctrl, _wave_df_user_fill_mgmt_frame_pwr_ctrl_cfg,
                               (df_user, mgmtframepwr_cfg, *(int*)data), res);

    /* Channel State Information */
    _DF_USER_SET_ON_PARAM(PRM_ID_CSI_ENABLE, WAVE_CORE_REQ_SET_CSI_ENABLE, FALSE, wave_csi_enable_cfg_t, csi_enable_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(csi_enable_cfg, csi_params, _wave_df_user_fill_csi_enable_cfg,
                               ((uint32*)data, length, &csi_enable_cfg->csi_params), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_CSI_SEND_QOS_NULL, WAVE_CORE_REQ_SET_CSI_SEND_QOS_NULL, FALSE, wave_csi_send_qos_null_cfg_t, csi_send_qos_null_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(csi_send_qos_null_cfg, sta_mac, _mtlk_df_user_fill_ether_address,
                               (&csi_send_qos_null_cfg->sta_mac, (struct sockaddr*)data), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_CSI_AUTO_RATE, WAVE_CORE_REQ_SET_CSI_AUTO_RATE, FALSE, wave_csi_auto_rate_cfg_t, csi_auto_rate_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(csi_auto_rate_cfg, csi_auto_params, _wave_df_user_fill_csi_auto_rate_cfg, ((uint32*)data, length, &csi_auto_rate_cfg->csi_auto_params), res);
    ...
    /* Multicast 3-Address Configuration */
    _DF_USER_SET_ON_PARAM(PRM_ID_ALLOW_3ADDR_MCAST, WAVE_CORE_REQ_SET_ALLOW_3ADDR_MCAST, FALSE, wave_3addr_mcast_mode_t, cfg)
      MTLK_CFG_SET_ITEM(cfg, allow_3addr_mcast, !!(*(uint32*)data)); /* 0 or 1 */

    _DF_USER_SET_ON_PARAM(PRM_ID_PCIE_SPEED, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, pcie_auto_cfg, *(uint32*)data);

#ifdef MTLK_WAVE_700
    /* Logger HW FIFO Mux Config IW processing */
    _DF_USER_SET_ON_PARAM(PRM_ID_LOGGER_FIFO_MUX_CFG, WAVE_RADIO_REQ_SET_LOGGER_FIFO_MUX_CFG, FALSE, mtlk_logger_fifo_mux_cfg_t, logger_fifo_mux_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(logger_fifo_mux_cfg, mode, _mtlk_df_user_fill_logger_fifo_mux_cfg,
                               (logger_fifo_mux_cfg, *(uint32*)data), res);

#ifdef BEST_EFFORT_TID_SPREADING
    /* STR TID to link spreading configuration */
    _DF_USER_SET_ON_PARAM(PRM_ID_STR_TID_SPREADING, WAVE_CORE_REQ_SET_STR_TID_LINK_SPREADING, FALSE, mtlk_str_tid_link_spreading_cfg_t, tid_spreading_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(tid_spreading_cfg, cfg, _mtlk_df_user_set_tid_link_spreading_cfg_params_by_intves,
                                ((uint32 *)data, length, &tid_spreading_cfg->cfg), res);
#endif
#endif
...
/************************* DEBUG SET APIs *************************/
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_RADAR_FIFO_DUMP, WAVE_RADIO_REQ_SET_SCAN_AND_CALIB_CFG, FALSE, mtlk_scan_and_calib_cfg_t, scan_and_calib_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(scan_and_calib_cfg, radar_fifo_params, _mtlk_df_user_get_radar_fifo_params_by_intvec, ((uint32*)data, length,
                                &scan_and_calib_cfg->radar_fifo_params), res);
                          ...
                          mtlk_core_general_stats_t *core_status)
{
  ...
  linux_stats->rx_errors      = core_status->rx_errors;
  linux_stats->rx_dropped     = core_status->rx_drops;
  /* TBD */
}
...
__mtlk_df_user_set_stats_timer (mtlk_df_user_t *df_user, uint32 value)
{
  if (__mtlk_df_user_is_stats_available(df_user)) {
    mtlk_osal_lock_acquire(&df_user->slow_ctx->stat_timer_lock);
    mtlk_osal_timer_set(&df_user->slow_ctx->stat_timer, value);
    mtlk_osal_lock_release(&df_user->slow_ctx->stat_timer_lock);
  }
}

static void
_mtlk_df_user_cancel_stat_timer (mtlk_df_user_t* df_user)
{
  mtlk_osal_lock_acquire(&df_user->slow_ctx->stat_timer_lock);
                        ...
                        mtlk_clpb_t  *pclpb)
{
  ...
  MTLK_ASSERT(NULL != hw);
  MTLK_ASSERT(__mtlk_df_user_is_stats_available(df_user));
...
mtlk_df_user_start(mtlk_df_t *df, mtlk_df_user_t *df_user)
{
  ...
  MTLK_START_TRY(df_user, MTLK_OBJ_PTR(df_user))
    ...
    if ((wave_hw_mmb_get_stats_poll_period(hw) == 0) &&
         __mtlk_df_user_is_stats_available(df_user)) {
      wave_hw_mmb_set_stats_poll_period(hw, poll_period);
    }
    MTLK_START_STEP_IF(__mtlk_df_user_is_stats_available(df_user),
                       df_user, STAT_POLL, MTLK_OBJ_PTR(df_user),
...
mtlk_df_on_rcvry_isol (mtlk_df_t *df)
{
  ...
  /* Stop stat timer */
  _mtlk_df_user_cancel_stat_timer(mtlk_df_get_user(df));
...
mtlk_df_on_rcvry_restore (mtlk_df_t *df)
{
  ...
  /* Start stat timer */
  __mtlk_df_user_set_stats_timer(mtlk_df_get_user(df), wave_hw_mmb_get_stats_poll_period(hw));
}
...
static int mtlk_df_ui_sta_list(mtlk_seq_entry_t *s, void *data)
{
  ...
  mtlk_aux_seq_printf(s, "\n"
      ...
      "\n"
      "------------------+-----+-----+----------------------------\n"
      "MAC               | SID | AID | VAP | AUTH | 4 ADDR | NSS |\n"
      "------------------+-----+-----+-----+------+--------+------\n");
  ...
  while(NULL != (stadb_stat = mtlk_clpb_enum_get_next(clpb, &size))) {
    ...
    if (STAT_ID_STADB == stadb_stat->type) {
      ...
      mtlk_aux_seq_printf(s, MAC_PRINTF_FMT " | %-3u | %-3u | %-3u | %-3u  | %-3u    | %-3u |\n",
...
static void
_mtlk_df_print_power_array (mtlk_seq_entry_t *s, char *msg, uint16 *pw_array, int size, bool limit_enable, int limit_value)
{
  int i;

  mtlk_aux_seq_printf(s, "%s", msg);
  for (i = 0; i < size; i++) {
    if ((limit_enable == TRUE) && ((int)pw_array[i] > limit_value)) { 
     _mtlk_df_print_power_in_dbm(s, limit_value);
    } else {
...
static const char* _phy_mode_to_string (uint8 phy_mode)
{
  static const char* modes[] = { "802.11a/g", "802.11b", "802.11n", "802.11ac", "802.11ax",
                                 "802.11ax_su_ext", "802.11ax_mu_trig", "802.11ax_mu_dl", "802.11be",
                                 "11be/bf", "802.11be_mu_trig", "802.11be_mu_dl", "unknown", "unknown",
                                 "unknown", "unknown", "11ax/bf","11ac/bf"};
...
#ifdef MTLK_WAVE_700
#define _DF_USER_MAX_CBW_SUPPORTED_     (CW_320)
#else
...
static int mtlk_df_ui_tx_power(mtlk_seq_entry_t *s, void *data)
{
  ...
  unsigned  pw_size, pw_size_tmp, *tx_damage_power_limit, *antenna_gain;
  uint16 power_cfg_limit;
  ...
  /* 2. PSDB damage power limit */
  if (NULL == (tx_damage_power_limit = mtlk_clpb_enum_get_next(clpb, NULL))) {
    mtlk_aux_seq_printf(s, "Tx damage power limit not present\n");
    goto finish;
  }

  /* 3. Antenna Gain */
  if (NULL == (antenna_gain = mtlk_clpb_enum_get_next(clpb, NULL))) {
    mtlk_aux_seq_printf(s, "Antenna Gain not present\n");
    goto finish;
  }
  ...
  _mtlk_df_print_power_line(s, "Current Array Gain:  ", " dB", tx_pw_data->cur_ant_gain);
  _mtlk_df_print_power_line(s, "Antenna Gain:        ", " dB\n", *antenna_gain);
  ...
  if (*tx_damage_power_limit != 0) {
    tx_pw_data->power_cfg = hw_mmb_get_tx_power_target(tx_pw_data->power_cfg, *tx_damage_power_limit + *antenna_gain);
  }
  // Take into account antenna gain for total Tx power limit
  power_cfg_limit = tx_pw_data->power_cfg - *antenna_gain;
  _mtlk_df_print_power_line(s, "Total Tx power Configured limit:", " dBm", power_cfg_limit);
  _mtlk_df_print_power_line(s, "Tx power limit damage:          ", " dBm", *tx_damage_power_limit);

  /* For Gen7 in 6G only when BW selected is 320MHz*/
  if (_DF_USER_GET_PW_SIZE_(tx_pw_data->cur_cbw) == PHY_STAT_RATE_CBW_NUM) 
  mtlk_aux_seq_printf(s,"\nSingle Chain Tx Power Calibration Data:\n"
                        "Channel Width       20MHz  40MHz  80MHz 160MHz 320MHz\n");
  else
  mtlk_aux_seq_printf(s,"\nSingle Chain Tx Power Calibration Data:\n"
                        "Channel Width       20MHz  40MHz  80MHz 160MHz\n");

  _mtlk_df_print_power_array(s, min_power_interpretation, &tx_pw_data->power_hw.pw_min_ant[0], pw_size, FALSE, 0);
  _mtlk_df_print_power_array(s, max_power_interpretation, &tx_pw_data->power_hw.pw_max_ant[0], pw_size, FALSE, 0);
  ...
  _mtlk_df_print_power_array(s, min_power_interpretation, &tx_pw_data->pw_min_brd[0], pw_size, FALSE, 0);
  _mtlk_df_print_power_array(s, max_power_interpretation, &tx_pw_data->pw_max_brd[0], pw_size, FALSE, 0);

  mtlk_aux_seq_printf(s,"\nOpen loop:          %s\n", tx_pw_data->open_loop ? "On " : "Off");

  /* 3. PSDB rate power list */
  /* For Gen7 in 6G only*/
  if (_DF_USER_GET_PW_SIZE_(tx_pw_data->cur_cbw) == PHY_STAT_RATE_CBW_NUM) 
  mtlk_aux_seq_printf(s,"\nTx power targets per rate:\n"
                        " PhyMode   MCS NSS  20MHz  40MHz  80MHz 160MHz 320MHz\n");
  else
  mtlk_aux_seq_printf(s,"\nTx power targets per rate:\n"
...
#ifdef MTLK_WAVE_700
    BOOL is_11be = (PHY_MODE_11BE == entry->phy_mode);
    BOOL is_11beBf = (PHY_MODE_BF_11BE == entry->phy_mode);

#endif
#define _IS_QAM256_OR_HIGHER(phy_mode,mcs) (((phy_mode) >= PHY_MODE_AC) && ((mcs) >= 8))
    BOOL target_pw_based_on_UEVM = _IS_QAM256_OR_HIGHER(entry->phy_mode, entry->mcs);
#undef _IS_QAM256_OR_HIGHER
    ...
    if ((MTLK_HW_BAND_2_4_GHZ == tx_pw_data->cur_band) && ((PHY_MODE_11AX_MU_DL == entry->phy_mode))) continue;
    if ((MTLK_HW_BAND_2_4_GHZ == tx_pw_data->cur_band) && ((PHY_MODE_11AC_BF_DL == entry->phy_mode))) continue;

    /*
    * Power Adaptation Table in W700 is per band so we have 3 tables, print only the desired
    */
    if ((entry->band != tx_pw_data->cur_band) && (is_11be)) continue; 
    ...
    for (i = 0; i < pw_size; i++) {
      ...
      if (is_11b) {
        ...
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[i + 1] - *antenna_gain;
        power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[i + 1] - *antenna_gain);
      }

      /* Check HW power limit damage */
      if ((*tx_damage_power_limit != 0) && (power_cfg != 0)) {
        uint16 power_cfg_tmp = power_cfg;
        power_cfg = hw_mmb_get_tx_power_target(power_cfg, *tx_damage_power_limit);
        if (power_cfg_tmp != power_cfg) {
          ILOG1_DD("Power Limit reduced from %u to %u to avoid damage", power_cfg_tmp, power_cfg);
        }
      }
    ...
    } else if ((!is_11beBf) && (!is_11be)) {
      /* In not 11be/11bebf modes, skip printing power values of 320MHz */
      pw_size_tmp = MIN(CW_320, pw_size);
    } else {
      pw_size_tmp = pw_size;
    }
...
static int mtlk_df_ui_debug_general(mtlk_seq_entry_t *s, void *data)
{
  ...
  while(NULL != (stadb_stat = mtlk_clpb_enum_get_next(clpb, &size))) {
    ...
    if (STAT_ID_STADB == stadb_stat->type) {
      ...
      mtlk_aux_seq_printf(s, MAC_PRINTF_FMT " | %3u | %-22llu | %-22llu | %-16u\n",
                          MAC_PRINTF_ARG(stadb_stat->u.general_stat.addr.au8Addr),
                          stadb_stat->u.general_stat.sta_sid,
...
_mtlk_df_ui_print_chandef_info(mtlk_seq_entry_t *s, const char *prefix, struct mtlk_chan_def *cd)
{
  ...
  low_chan_num = freq2lowchannum(_wave_get_cf1_or_cf2(cd), cd->width);
...
#ifdef BEST_EFFORT_TID_SPREADING
static int
_wave_df_ui_ml_vap_str_tid_spread_cfg (mtlk_seq_entry_t *s, void *data)
{
  mtlk_df_t *df = mtlk_df_proc_seq_entry_get_df(s);
  mtlk_vap_handle_t vap_handle = mtlk_df_get_vap_handle(df);
  wave_ml_vap_str_tid_spreading_info_t *info;

  info = wave_vap_manager_get_str_tid_spreading_info(vap_handle);

  if (info) {
    mtlk_aux_seq_printf(s, "tid spreading      : %d\n", info->active);
    mtlk_aux_seq_printf(s, "dynamic mode       : %d\n", info->tid_spreading_mode);
    mtlk_aux_seq_printf(s, "static split ratio : %d\n", info->tid_split_ratio);
  } else {
    mtlk_aux_seq_printf(s, "non ml vap\n");
  }
...
static int
_wave_df_ui_ml_str_sta_tid_spread_cfg (mtlk_seq_entry_t *s, void *data)
{
  mtlk_error_t res;
  mtlk_clpb_t *clpb = NULL;
  mtlk_stadb_stat_t *stadb_stat;
  ...
  get_stadb_status_req.get_str_sta_tid_spread_cfg = TRUE;
  get_stadb_status_req.get_hostdb = FALSE;
  get_stadb_status_req.use_cipher = FALSE;
  res = _mtlk_df_user_invoke_core(df, WAVE_CORE_REQ_GET_STADB_STATUS, &clpb,
                                  &get_stadb_status_req, sizeof(get_stadb_status_req));
  ...
  while (NULL != (stadb_stat = mtlk_clpb_enum_get_next(clpb, &size))) {
    ...
    if (STAT_ID_ML_STR_STA == stadb_stat->type) {
      ...
      mtlk_aux_seq_printf(s, "low_eff_rate_percent  : %u\n", stadb_stat->u.tid_spread_stat.cfg.low_eff_rate_tid_percent);
      mtlk_aux_seq_printf(s, "high_rate_tid_load    : %u\n", stadb_stat->u.tid_spread_stat.cfg.curr_load_high_rate_tid);
      mtlk_aux_seq_printf(s, "low_rate_tid_load     : %u\n", stadb_stat->u.tid_spread_stat.cfg.curr_load_low_rate_tid);
      mtlk_aux_seq_printf(s, "total_load            : %u\n", stadb_stat->u.tid_spread_stat.cfg.tid_flip);
      mtlk_aux_seq_printf(s, "-----------------------------------------------------------\n");
    } else {
      res = MTLK_ERR_UNKNOWN;
      goto delete_clpb;
...
const char * _wave_acl_policy_to_str (enum wave_acl_policy policy)
{
  switch (policy) {
    case WAVE_ACL_POLICY_ACCEPT:
      return "ACCEPT";
    case WAVE_ACL_POLICY_DENY:
      return "DENY";
    default:
      return "ACL DISABLED";
  }
...
static int _mtlk_df_ui_get_acl (mtlk_seq_entry_t *s, void *data)
{
  ...
  res = _mtlk_df_user_invoke_core(df, WAVE_CORE_REQ_GET_ACL, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_GET_ACL, FALSE);
  ...
  MTLK_CLPB_TRY(acl_data, acl_data_size)
    mtlk_aux_seq_printf(s, "ACL Policy: %s\n", _wave_acl_policy_to_str(acl_data->acl_policy));
    mtlk_aux_seq_printf(s, "ACL MAC List:\n");
...
#ifdef MTLK_WAVE_700
      mtlk_aux_seq_printf(s,"      EHT support                   : %d\n", (unsigned)peer->is_eht);
#endif
...
static int _mtlk_df_ui_mgmt_data_tx(struct file *file, const char __user *buffer,
                                    unsigned long count, void *data)
{
...
static int _mtlk_df_ui_data_tx (struct file *file, const char __user *buffer,
                                unsigned long count, void *data)
{
  mtlk_df_user_t *df_user = mtlk_df_get_user(mtlk_df_proc_entry_get_df(data));
  mtlk_nbuf_t * nbuf = NULL;
  char *input_hex = NULL, *tx_data = NULL;
  unsigned long data_len;

  if (!count) goto error;
  input_hex = mtlk_osal_mem_alloc(count, MTLK_MEM_TAG_USER_DF);
  if (!input_hex) goto error;

  tx_data = mtlk_osal_mem_alloc(count >> 1, MTLK_MEM_TAG_USER_DF);
  if (!tx_data) goto error;

  if (copy_from_user(input_hex, buffer, count))
    goto error;

  input_hex[count - 1] = '\0';
  data_len = wave_remove_spaces(input_hex, count) >> 1;
  if (hex2bin(tx_data, input_hex, data_len)) {
    ELOG_V("Unable to convert an ASCII hexadecimal string to its binary representation");
    goto error;
  }

  nbuf = mtlk_df_nbuf_alloc(data_len);
  if (!nbuf) goto error;

  nbuf->len = data_len;
  nbuf->dev = df_user->ndev;
  wave_memcpy(nbuf->data, data_len, tx_data, data_len);

  mtlk_dump(1, nbuf->data, data_len, "Driver inject TX data dump:");

  _mtlk_df_user_start_skb_tx(nbuf, df_user);
  mtlk_osal_mem_free(input_hex);
  mtlk_osal_mem_free(tx_data);
  return count;
...
_mtlk_df_ui_eeprom(mtlk_seq_entry_t *s, void *data)
{
  ...
  if((MTLK_ERR_OK == res) && (MTLK_INVALID_HANDLE != hdata)) {
    ...
    mtlk_aux_seq_write(s, eeprom_cfg->eeprom_raw_data, eeprom_cfg->eeprom_total_size);
...
{ "utest",              _DEBUG_,  mtlk_df_ui_utest                , NULL },
  ...
  { "mgmt_data_tx",     _DEBUG_,  NULL,  _mtlk_df_ui_mgmt_data_tx }, /* EAPOL through mgmt ring */
  ...
  { "acl_list",         _NODBG_, _mtlk_df_ui_get_acl              , NULL },
#ifdef BEST_EFFORT_TID_SPREADING
...
proc_file_info_t * __MTLK_IFUNC wave_get_proc_table (int *size)
{
  *size = MTLK_ARRAY_SIZE(_proc_files_list);
  return _proc_files_list;
}
...
int __MTLK_IFUNC
mtlk_df_reg_file (mtlk_df_user_t* df_user, const struct _proc_file_info *info)
{
...
void __MTLK_IFUNC
mtlk_df_unreg_file (mtlk_df_user_t* df_user, const struct _proc_file_info *info)
{
  mtlk_df_proc_fs_node_t *df_node;
  ...
  if (!df_user->is_secondary_df) {
    MTLK_CLEANUP_BEGIN(df_user, MTLK_OBJ_PTR(df_user))
      MTLK_CLEANUP_STEP(df_user, PRE_PROC_INIT, MTLK_OBJ_PTR(df_user),
        MTLK_NOACTION, ());
      MTLK_CLEANUP_STEP(df_user, VLAN_PENDING_LIST, MTLK_OBJ_PTR(df_user),
                      _mtlk_df_user_cleanup_vlan_pending_list, (&df_user->vlan_pending_list));
      MTLK_CLEANUP_STEP(df_user, STAT_TIMER, MTLK_OBJ_PTR(df_user),
                        mtlk_osal_timer_cleanup, (&df_user->slow_ctx->stat_timer));
      MTLK_CLEANUP_STEP(df_user, STAT_TIMER_LOCK, MTLK_OBJ_PTR(df_user),
                        mtlk_osal_lock_cleanup, (&df_user->slow_ctx->stat_timer_lock));
#ifdef MTLK_PER_RATE_STAT
...
_mtlk_df_user_init(mtlk_df_user_t *df_user,  const char *name)
{
  ...
  if (!df_user->is_secondary_df) {
    MTLK_INIT_TRY(df_user, MTLK_OBJ_PTR(df_user))
...
#endif
      MTLK_INIT_STEP(df_user, STAT_TIMER_LOCK, MTLK_OBJ_PTR(df_user),
                      mtlk_osal_lock_init, (&df_user->slow_ctx->stat_timer_lock));
      ...
      MTLK_INIT_STEP_VOID(df_user, VLAN_PENDING_LIST, MTLK_OBJ_PTR(df_user),
                      mtlk_dlist_init, (&df_user->vlan_pending_list));
...
mtlk_df_user_create (mtlk_df_t *df, const char *name, struct net_device *ndev_p, BOOL is_secondary)
{
  mtlk_df_user_t* df_user;
  mtlk_vap_handle_t vap_handle = mtlk_df_get_vap_handle(df);
  BOOL ndev_allocated = FALSE;
  MTLK_ASSERT(vap_handle);

  if (is_secondary && ndev_p == NULL) {
    ELOG_S("%s: secondary ndev is NULL", name);
    return NULL;
  }
  ...
  if (df_user == NULL){
    ELOG_S("%s: failed to allocate df_user", name);
    return NULL;
  }
  memset (df_user, 0, sizeof(mtlk_df_user_t));

  if (mtlk_vap_is_master_ap(vap_handle) && ndev_p == NULL) {
    struct ieee80211_hw *hw = wave_vap_ieee80211_hw_get(vap_handle);
    ILOG0_S("%s: creating master netdev", name);
    ndev_p = ieee80211_if_alloc(hw, name, 0, NL80211_IFTYPE_AP, NULL);
    if (ndev_p == NULL) {
      ELOG_S("%s: failed to allocate net device for master df user", name);
      goto free_df_user;
    }
    ndev_allocated = TRUE;
  }
  ...
  /* set "secondary" flag */
  df_user->is_secondary_df = is_secondary;
  ...
  /* Statistics are not available on ZWDFS radio, nor on secondary DF user */
  df_user->is_stats_available = !df_user->is_secondary_df &&
                                !wave_radio_get_is_zwdfs_radio(wave_vap_radio_get(vap_handle));
...
free_ndev:
  if (ndev_allocated) free_netdev(ndev_p);
free_df_user:
                                      ...
                                      void *mc_stream, uint32_t flags)
{
  ...
  if (__mtlk_df_user_is_secondary(df_user))
...
mtlk_df_user_group_init (mtlk_df_user_t* df_user)
{
  ...
  if (mtlk_mmb_fastpath_available(mtlk_vap_get_hw(vap_handle))) {
    ...
    if (DC_DP_FAILURE == dc_dp_register_mcast_module(ndev, THIS_MODULE, mtlk_df_ui_group_handler, flags)) {
      WLOG_S("%s: can't register multicast callback", df_user->name);
      return;
    }
...
mtlk_df_mcast_notify_sta_connected (mtlk_df_t *df)
{
  ...
  mtlk_df_user_t* df_user = mtlk_df_get_user(df);
  if (df_user->dcdp.mcast_registered == TRUE) {
    struct net_device *ndev = mtlk_df_user_get_ndev(df_user);
...
mtlk_df_ui_dp_prepare_and_xmit (mtlk_vap_handle_t vap_handle, mtlk_core_handle_tx_data_t *data, BOOL wds, uint16 sta_sid, int mc_index, uint32 nbuf_flags)
{
...

    {
      ...
      dc_dp_subif.vap_id = _wave_get_dcdp_vap_id_from_drv_vap_id(vap_handle, vap_id);
      dc_dp_subif.port_id = port_id;
...
{
#if DC_DP_API_VERSION_CODE >= DC_DP_API_VERSION(4,0,0)
  uint32_t subif = 0, port_id = 0;

  if (__UNLIKELY(NULL == subif_id))
...
#if DC_DP_API_VERSION_CODE >= DC_DP_API_VERSION(4,0,0)
    if (mtlk_core_dp_get_subif_cb(df_user, mac_addr, &subif, port_id) == MTLK_ERR_OK) {
      subif_id->subif = subif;
      return DC_DP_SUCCESS;
    }
#else
    if (subif && mtlk_core_dp_get_subif_cb(df_user, mac_addr, subif, port_id) == MTLK_ERR_OK) {
      return DC_DP_SUCCESS;
    }
#endif
    else {
    ...
    uint32_t flags)
{
  ...
  if (0 == dc_dp_get_mark_pkt(skb, 0)) {
    ...
    dc_dp_subif.port_id = port_id;
     ...
     /* API to SET subif params in DCDP */
     dc_dp_set_subif_param(&dc_dp_subif);
     ILOG2_D("DCDP subif : 0x%08x",dc_dp_subif.subif);
...
_mtlk_df_dcdp_print_hwcmode (const char *msg, uint32 hw_cmode)
{
  ...
  if (hw_cmode & DC_DP_F_DCCNTR_MODE_4MSG_MODE)
    strlcat(buf, "4_UMT_COUNTER_MODE|", sizeof(buf));
...
static mtlk_error_t
_mtlk_df_dcdp_get_peripheral_config (mtlk_hw_t *hw, mtlk_dcdp_dev_t *dp_dev, BOOL zwdfs)
{
  mtlk_error_t res = MTLK_ERR_OK;
  MTLK_ASSERT(dp_dev);
  MTLK_ASSERT(hw);
...
#else
  {
    enum dc_dp_dev_type dev_type = DC_DP_DEV_WAV_500;

    if (wave_hw_radio_band_cfg_is_single(hw)) {
#if defined(MTLK_WAVE_700) && defined(WAVE_DCDP_CTB_SUPPORTED)
      if (mtlk_hw_type_is_gen7(hw) && !wave_hw_mmb_card_is_asic(hw))
        dev_type = DC_DP_DEV_WAV_700;
      else
#endif
        dev_type = (mtlk_is_band_supported(HANDLE_T(hw), 0, MTLK_HW_BAND_2_4_GHZ) ? DC_DP_DEV_WAV_600_24G :
                    mtlk_is_band_supported(HANDLE_T(hw), 0, MTLK_HW_BAND_5_2_GHZ) ? DC_DP_DEV_WAV_600_5G :
                    mtlk_is_band_supported(HANDLE_T(hw), 0, MTLK_HW_BAND_6_GHZ)   ? DC_DP_DEV_WAV_600_6G :
                    DC_DP_DEV_WAV_500);
    } else {
...
#endif
        {
          dev_type = zwdfs ? DC_DP_DEV_WAV_600_5G : DC_DP_DEV_WAV_600_CDB;
        }
      }
#if defined(MTLK_WAVE_700) && defined(WAVE_DCDP_CTB_SUPPORTED)
...
mtlk_df_dcdp_datapath_dev_init (mtlk_hw_t *hw, mtlk_dcdp_dev_t *dp_dev)
{
  ...
  dp_dev->dp_port_id[2]  = DC_DP_FAILURE;


  /* We need 3 different ports per band for CTB support */
  dev_port = 3 * mtlk_hw_mmb_get_card_idx(hw);
  dp_dev->dp_dev_port[0] = dev_port + 0;
  dp_dev->dp_dev_port[1] = dev_port + 1;
  dp_dev->dp_dev_port[2] = dev_port + 2;
...
mtlk_df_dcdp_datapath_dev_unregister (mtlk_hw_t *hw, mtlk_dcdp_dev_t *dp_dev, BOOL is_recovery)
{
    ...
    if (MTLK_DC_DP_MODE_UNREGISTERED != dp_dev->dp_mode) {
      ...
      do {
        if (dp_dev->is_registered[i]) {
          MTLK_ASSERT(dp_dev->dp_ndev[i]);
          ILOG0_S("Deregistering network device %s from DirectConnect...", dp_dev->dp_ndev[i]->name);
          if (DC_DP_FAILURE == dc_dp_register_dev(THIS_MODULE, dp_dev->dp_port_id[i], dp_dev->dp_ndev[i], &dp_cb, NULL,
                                                  &dp_dev->dp_devspec[i], DC_DP_F_DEREGISTER)) {
            ELOG_S("Can't deregister network device %s from DirectConnect", dp_dev->dp_ndev[i]->name);
            res = MTLK_ERR_UNKNOWN;
          }
          dp_dev->is_registered[i] = FALSE;
        }
      } while (i--);
...
static uint32 _wave_hw_num_radios(mtlk_hw_t *hw)
{
  wave_radio_descr_t *radio_descr = NULL;
  uint32 num_radios = WAVE_NUM_RADIO_INVALID;
  MTLK_ASSERT(hw);

  radio_descr = (wave_radio_descr_t*)wave_card_radio_descr_get(hw);
  num_radios = (uint32)radio_descr->num_radios;
#ifdef MTLK_WAVE_700
  if (mtlk_hw_type_is_gen7(hw) && wave_hw_mmb_card_is_asic(hw) &&
     (num_radios != WAVE_CARD_RADIO_NUM_MAX_G7) &&
      (num_radios != WAVE_CARD_RADIO_NUM_MAX_G6)) { //TODO do it better for Dual band Wave700 Product 
    return WAVE_NUM_RADIO_INVALID;
  }
#endif
  return num_radios;
}
...
/* Register and initialize datapath device */
{
  ...
  mtlk_ndev_t             *netdev[WAVE_DCDP_MAX_NDEVS] = { NULL, NULL, NULL };
  uint32                  num_radios = WAVE_NUM_RADIO_INVALID;
  int                     i = 0;
  BOOL                    is_gen7_hw = mtlk_hw_type_is_gen7(hw);
  BOOL                    zwdfs = FALSE;
  ...
  num_radios = _wave_hw_num_radios(hw);
  if (num_radios == WAVE_NUM_RADIO_INVALID) {
     ILOG0_V("mtlk_df_dcdp_datapath_dev_register failed due to invlaid number of radios");
     return MTLK_ERR_UNKNOWN;
  }

  for(i = 0; i < num_radios; i++) {
    netdev[i] = wave_hw_ndev_get(hw, i);
    zwdfs = wave_radio_is_zwdfs_gen6_d2(wave_hw_radio_get(hw, i));
  }
  ...
  if (MTLK_DC_DP_MODE_UNREGISTERED == dp_dev->dp_mode)
  {
    uint32 flags;
    uint32 port_id = 0;
    uint32 dcdp_reg_flag = DC_DP_F_QOS;
    uint32 extra_flag[WAVE_CARD_RADIO_NUM_MAX] = {0, DC_DP_F_SHARED_RES, DC_DP_F_SHARED_RES};
    uint32 all_flags;
    /* below flags will be set only for CTB */
#if defined(WAVE_DCDP_CTB_SUPPORTED) && defined(MTLK_WAVE_700)
    wave_radio_t *radio = NULL;
    uint32 dc_dp_band_flag[MTLK_HW_BAND_MAX] = {DC_DP_F_FAST_WLAN_5G,
                                                DC_DP_F_FAST_WLAN_2_4G,
                                                DC_DP_F_FAST_WLAN_6G};
    if (is_gen7_hw) {
      for (i=0; i < num_radios; i++) {
        radio = wave_hw_radio_get(hw,i);
        extra_flag[i] |= (dc_dp_band_flag[wave_radio_band_get(radio)]| DC_DP_F_FAST_WLAN_EXT);
      }
      MTLK_ASSERT(dp_init->rd_pool_size > MGMT_RX_RING_SIZE);
    } else {
#endif
    ...
    if (DC_DP_FAILURE != dp_dev->dp_port_id[0]) {
      ...
      if ((flags & DC_DP_F_FASTPATH) && (!zwdfs) && 
          ((num_radios > 1) || is_gen7_hw)) {
        flags |= DC_DP_F_MULTI_PORT;
      }
      for (i = 0; i < num_radios; i++) {
        if (wave_radio_is_zwdfs_gen6_d2(wave_hw_radio_get(hw, i))) {
          break;
        }
        TRACE_PARAM_INT(dp_dev->dp_dev_port[i]);
        /* Create radio device for LitePath interface */
        if (DC_DP_F_LITEPATH & flags) {
          mtlk_ndev_t *radio_ndev;
          ILOG0_D("Create radio device for LitePath interface %d", i);
          radio_ndev = litepath_dev_create(netdev[i]);
          if (NULL == radio_ndev) {
            res = MTLK_ERR_NO_MEM;
            goto finish;
          }
          dp_dev->dp_ndev[i] = dp_dev->dp_radio_ndev[i] = radio_ndev;
        } else {
          dp_dev->dp_ndev[i] = netdev[i];
        }
        dev_port = dp_dev->dp_dev_port[i];
         all_flags =  flags | extra_flag[i] | DC_DP_F_FAST_WLAN;
         ILOG0_DDD("Registering DirectConnect dev_port %u, port_id %d, flags 0x%x",
                    dev_port, port_id, all_flags);
         dp_dev->dp_port_id[i] = dc_dp_alloc_port(THIS_MODULE, dev_port, dp_dev->dp_ndev[i], port_id, all_flags);
        if ((DC_DP_F_FASTPATH & flags)) {
          port_id =  dp_dev->dp_port_id[0];
        }
        TRACE_PARAM_INT(dp_dev->dp_port_id[i]);
        if (DC_DP_FAILURE == dp_dev->dp_port_id[i]) {
          ELOG_D("Can't allocate DirectConnect port %d", i);
          res = MTLK_ERR_UNKNOWN;
          goto finish;
        }
      }
    ...
    res = _mtlk_df_dcdp_get_peripheral_config(hw, dp_dev, zwdfs);
    if (MTLK_ERR_OK != res)
      goto finish;
    ...
    if (wave_mmb_dcdp_4umt_cntr_mode(hw))
      dp_dev->dp_resources.num_dccntr = MTLK_DCDP_DCCNTR_NUM_4UMT;
    else
    ...
    memset(dp_dev->dp_resources.dccntr, 0, dp_dev->dp_resources.num_dccntr * sizeof(struct dc_dp_dccntr));
    ...
    */
    ...
    dp_dev->dp_resources.dccntr[0].soc2dev_enq_phys_base     = dp_init->dccntr[0].soc2dev_enq_phys_base;
    dp_dev->dp_resources.dccntr[0].soc2dev_enq_base          = dp_init->dccntr[0].soc2dev_enq_base;
    dp_dev->dp_resources.dccntr[0].dev2soc_ret_enq_phys_base = dp_init->dccntr[0].dev2soc_ret_enq_phys_base;
    dp_dev->dp_resources.dccntr[0].dev2soc_ret_enq_base      = dp_init->dccntr[0].dev2soc_ret_enq_base;
    /* RX_OUT + TX_OUT */
    dp_dev->dp_resources.dccntr[0].soc2dev_ret_deq_phys_base = dp_init->dccntr[0].soc2dev_ret_deq_phys_base;
    dp_dev->dp_resources.dccntr[0].soc2dev_ret_deq_base      = dp_init->dccntr[0].soc2dev_ret_deq_base;
    dp_dev->dp_resources.dccntr[0].dev2soc_deq_phys_base     = dp_init->dccntr[0].dev2soc_deq_phys_base;
    dp_dev->dp_resources.dccntr[0].dev2soc_deq_base          = dp_init->dccntr[0].dev2soc_deq_base;

    if (is_gen7_hw) {
      dp_dev->dp_resources.dccntr[0].dev2soc_dccntr_timer    = 10; //10usec: UMT message interval
    }

#ifdef WAVE_DCDP_4UMT_SUPPORTED
    if (wave_mmb_dcdp_4umt_cntr_mode(hw)) {
       dp_dev->dp_resources.dccntr[1] = dp_dev->dp_resources.dccntr[0];
       dp_dev->dp_resources.dccntr[1].soc_write_dccntr_mode     |= DC_DP_F_DCCNTR_MODE_4MSG_MODE;

       dp_dev->dp_resources.dccntr[1].soc2dev_enq_phys_base     = dp_init->dccntr[1].soc2dev_enq_phys_base;
       dp_dev->dp_resources.dccntr[1].soc2dev_enq_base          = dp_init->dccntr[1].soc2dev_enq_base;
       dp_dev->dp_resources.dccntr[1].dev2soc_ret_enq_phys_base = dp_init->dccntr[1].dev2soc_ret_enq_phys_base;
       dp_dev->dp_resources.dccntr[1].dev2soc_ret_enq_base      = dp_init->dccntr[1].dev2soc_ret_enq_base;
       /* RX_OUT + TX_OUT */
       dp_dev->dp_resources.dccntr[1].soc2dev_ret_deq_phys_base = dp_init->dccntr[1].soc2dev_ret_deq_phys_base;
       dp_dev->dp_resources.dccntr[1].soc2dev_ret_deq_base      = dp_init->dccntr[1].soc2dev_ret_deq_base;
       dp_dev->dp_resources.dccntr[1].dev2soc_deq_phys_base     = dp_init->dccntr[1].dev2soc_deq_phys_base;
       dp_dev->dp_resources.dccntr[1].dev2soc_deq_base          = dp_init->dccntr[1].dev2soc_deq_base;

       if (is_gen7_hw) {
         dp_dev->dp_resources.dccntr[1].dev2soc_dccntr_timer    = 10; //10usec: UMT message interval
       }
     }
#endif /* WAVE_DCDP_4UMT_SUPPORTED */

     for (i = 0; i < dp_dev->dp_resources.num_dccntr; i++) {
       TRACE_PARAM_INT(i);
       TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].soc_write_dccntr_mode);
       /* Wave TX_IN, filled */
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_enq_phys_base);
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_enq_base);
       TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].soc2dev_enq_dccntr_len);
       /* Wave TX_OUT, filled */
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_ret_deq_phys_base);
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_ret_deq_base);
       TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].soc2dev_ret_deq_dccntr_len);
       /* Wave RX_OUT, filled*/
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_deq_phys_base);
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_deq_base);
       TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].dev2soc_deq_dccntr_len);
       /* Wave RX_IN, filled */
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_ret_enq_phys_base);
       TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_ret_enq_base);
       TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].dev2soc_ret_enq_dccntr_len);
    }
...
#if defined(MTLK_WAVE_700) && !defined(WAVE_DCDP_CTB_SUPPORTED)
    if (num_radios == WAVE_CARD_RADIO_NUM_MAX_G7) {
      ELOG_V("DCDP API not available to support CTB, will use internal software path");
...
#endif //defined MTLK_WAVE_700 and not defined WAVE_DCDP_CTB_SUPPORTED
    ...
    */
    for (i=0; i < num_radios; i++) {
      ...
      dcdp_reg_flag = DC_DP_F_QOS;
      wave_strcopy(dp_dev->dp_devspec[i].dev_name, dp_dev->dp_ndev[i]->name, sizeof(dp_dev->dp_devspec[i].dev_name));
      dp_dev->dp_devspec[i].dev_ctx = NULL;
      TRACE_PARAM_STR(dp_dev->dp_devspec[i].dev_name);
      TRACE_PARAM_PTR(dp_dev->dp_devspec[i].dev_ctx);
      if((i > 0) && (flags & DC_DP_F_FASTPATH) && (!zwdfs)) {
        /* DC_DP_F_SHARED_RES to be set for radios(idx > 0) for FastPath */
        dcdp_reg_flag |= DC_DP_F_SHARED_RES;
      }
      /* try to register network device */
      dp_dev->is_registered[i] = FALSE;
      ILOG0_S("Registering network device %s in DirectConnect...", dp_dev->dp_ndev[i]->name);
#ifdef WAVE_DCDP_CTB_SUPPORTED
      dp_dev->dp_resources.num_additional_tx_ring = DCDP_NO_ADDITIONAL_TX_IN_RING;
      if (DC_DP_SUCCESS != dc_dp_register_dev(THIS_MODULE, dp_dev->dp_port_id[i], dp_dev->dp_ndev[i],
                                               &dp_cb, &dp_dev->dp_resources, &dp_dev->dp_devspec[i], dcdp_reg_flag)) {
#else
      if (DC_DP_SUCCESS != dc_dp_register_dev(THIS_MODULE, dp_dev->dp_port_id[i], dp_dev->dp_ndev[i],
                                               &dp_cb, &dp_dev->dp_resources, &dp_dev->dp_devspec[i], dcdp_reg_flag)) {
#endif
        ELOG_S("Can't register device %s in DirectConnect!", dp_dev->dp_ndev[i]->name);
        ...
        dp_dev->is_registered[i] = TRUE;
        ILOG0_S("Network device %s registered in DirectConnect", dp_dev->dp_ndev[i]->name);
      }
    ...
    for (i = 0; i < dp_dev->dp_resources.num_dccntr; i++) {
      TRACE_PARAM_INT(i);
      TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].dev_write_dccntr_mode);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_deq_phys_base);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_deq_base);
      TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].soc2dev_deq_dccntr_len);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_ret_enq_phys_base);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].soc2dev_ret_enq_base);
      TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].soc2dev_ret_enq_dccntr_len);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_enq_phys_base);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_enq_base);
      TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].dev2soc_enq_dccntr_len);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_ret_deq_phys_base);
      TRACE_PARAM_PTR(dp_dev->dp_resources.dccntr[i].dev2soc_ret_deq_base);
      TRACE_PARAM_INT(dp_dev->dp_resources.dccntr[i].dev2soc_ret_deq_dccntr_len);
    }
    TRACE_PARAM_INT(dp_dev->dp_resources.rings.dev2soc.size);
...
finish:
  ...
  if (MTLK_ERR_OK != res) {
    for (i = (num_radios -1); i >= 0; i--) {
	  if ((NULL != dp_dev->dp_ndev[i]) && (DC_DP_FAILURE != dp_dev->dp_port_id[i])) {
        /* De-register DirectConnect device */
        if (dp_dev->is_registered[i]) {
          if (DC_DP_FAILURE == dc_dp_register_dev(THIS_MODULE, dp_dev->dp_port_id[i], dp_dev->dp_ndev[i],
                                                          &dp_cb, NULL, &dp_dev->dp_devspec[i], DC_DP_F_DEREGISTER))
          {
            ELOG_S("Can't deregister network device %s from DirectConnect", dp_dev->dp_ndev[i]->name);
          }
          dp_dev->is_registered[i] = FALSE;
        ...
        dp_dev->dp_port_id[i] = DC_DP_FAILURE;
        dp_dev->dp_ndev[i] = NULL;
        ...
        if (use_internal_sw_path) {
          WLOG_S("%s: continue with internal slow path", netdev[i]->name);
          res = MTLK_ERR_OK;
        }
      }
	}
    /* make sure buflist is released after netdev deregistration */
    mtlk_df_dcdp_release_buflist(dp_dev);
    dp_dev->dp_mode = MTLK_DC_DP_MODE_UNREGISTERED;
    dp_dev->dp_port_flags = 0;
  }
