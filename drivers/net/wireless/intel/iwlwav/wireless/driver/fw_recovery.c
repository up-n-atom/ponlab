...
struct wave_rcvry_task_ctx {
  ...
  atomic_t                must_not_wait_on_rcvry;
...
static uint32 _rcvry_timer_handler (mtlk_osal_timer_t *timer, mtlk_handle_t data)
{
  ...
  _mtlk_df_user_invoke_core_async(mtlk_vap_get_df(master_vap_handle), WAVE_RCVRY_RESET, &rcvry_ctx, sizeof(typeof(rcvry_ctx)), NULL, 0);
  return 0;
}
...
static inline char *__rcvry_type_to_string (wave_rcvry_type_e rcvry_type)
{
  switch (rcvry_type) {
    ...
    case RCVRY_TYPE_FULL:
      return "FULL";
    case RCVRY_TYPE_UNRECOVERABLE_ERROR:
...
#ifdef MTLK_WAVE_700
    if (_chipid_is_gen7(pcard->chip_id) && strstr(pcard->info[i].name, "test_bus"))
      sprintf_written = mtlk_snprintf(buff_tmp, (size -= sprintf_written), "%s\n%X\n", pcard->info[i].name, pcard->info[i].io_size * 4);
    else
#endif
...
#define __MAX_RCVRY_STAT_NUMBER 9999
  ...
  if (stats->FastRcvryWithinTimer > __MAX_RCVRY_STAT_NUMBER)
    stats->FastRcvryWithinTimer = __MAX_RCVRY_STAT_NUMBER;
  if (stats->FullRcvryWithinTimer > __MAX_RCVRY_STAT_NUMBER)
    stats->FullRcvryWithinTimer = __MAX_RCVRY_STAT_NUMBER;
...
#define __RCVRY_STATS_WITHIN_TIMER " recovery processed in current timer slot"
  ...
  written = mtlk_snprintf(buff_tmp += written, (size -= written), "        %u", stats->FastRcvryWithinTimer);
  if (written < 0 || (size_t)written >= size)
    return MTLK_ERR_UNKNOWN;
  written = mtlk_snprintf(buff_tmp += written, (size -= written), __RCVRY_STATS_NUM_OF "FAST" __RCVRY_STATS_WITHIN_TIMER "\n");
  if (written < 0 || (size_t)written >= size)
    return MTLK_ERR_UNKNOWN;

  written = mtlk_snprintf(buff_tmp += written, (size -= written), "        %u", stats->FullRcvryWithinTimer);
  if (written < 0 || (size_t)written >= size)
    return MTLK_ERR_UNKNOWN;
  written = mtlk_snprintf(buff_tmp += written, (size -= written), __RCVRY_STATS_NUM_OF "FULL" __RCVRY_STATS_WITHIN_TIMER);
  if (written < 0 || (size_t)written >= size)
    return MTLK_ERR_UNKNOWN;
  *(buff_tmp += written) = '\n';
...
void wave_get_cur_rcvry_stats(const mtlk_hw_t *hw_ctx, uint32 *fast_cnt, uint32 *full_cnt)
{
  ...
  mtlk_osal_lock_acquire(&rcvry_ctx->lock);
  *fast_cnt = rcvry_ctx->rcvry_fast_cnt;
  *full_cnt = rcvry_ctx->rcvry_full_cnt;
  mtlk_osal_lock_release(&rcvry_ctx->lock);
}
...
static void _rcvry_card_delete (wave_rcvry_card_cfg_t *pcard)
{
  ...
  mtlk_osal_lock_cleanup(&pcard->rcvry_ctx.lock);
  mtlk_osal_mutex_cleanup(&pcard->rcvry_ctx.fw_dump_mutex);
...
BOOL __MTLK_IFUNC wave_rcvry_is_enabled(mtlk_core_t *core)
{
  wave_rcvry_task_ctx_t *rcvry_ctx = _rcvry_task_ctx_get(mtlk_vap_get_hw(core->vap_handle));

  if (rcvry_ctx != NULL)
    return _wv_rcvry_is_enabled(rcvry_ctx);
  else
    return FALSE;
}
...
int wave_rcvry_card_add (void *mmb_base, void *hw_ctx)
{
  ...
  mtlk_osal_lock_acquire(&proc_descr->lock);
  for (i = 0; i < MTLK_MAX_HW_ADAPTERS_SUPPORTED; i++) {
    /* Sanity check if hw_ctx is already registered */
    pcard = proc_descr->p_card[i];
    if (pcard && (pcard->hw_ctx == hw_ctx)) {
      ELOG_V("Attempt to register the same hw_ctx twice");
      mtlk_osal_lock_release(&proc_descr->lock);
  ...
  if (i == MTLK_MAX_HW_ADAPTERS_SUPPORTED) {
  ...
  /* Save chip id for fw dump creation*/
  pcard->chip_id = hw_mmb_get_chip_id(pcard->hw_ctx);
...
#ifdef MTLK_WAVE_700
    if (mtlk_hw_type_is_gen7(pcard->hw_ctx) && strstr(info->name, "test_bus")) {
      dump_total_size += (info->io_size * 4);
  ...
  res = mtlk_osal_lock_init(&rcvry_ctx->lock);
  if (MTLK_ERR_OK != res)
    goto WAVE_RCVRY_CARD_ADD_UNINIT;

  return res;

WAVE_RCVRY_CARD_ADD_UNINIT:
  wave_rcvry_card_uninit(hw_ctx);
WAVE_RCVRY_CARD_ADD_ERR:
  _rcvry_card_delete(pcard);
  return res;
}

void wave_rcvry_card_uninit (mtlk_hw_t *hw)
{
  wave_rcvry_card_cfg_t *pcard = _rcvry_card_hw_ctx_get(hw);

  if (pcard) {
    /* Recovery stats are stored in HW context, therefore this proc file
     * have to be unregistered prior HW context freeing */
...
_wv_pcard_get_wdev(wave_rcvry_card_cfg_t *pcard){
  ...
  if (pcard) {
    ...
    wdev = mtlk_core_get_wdev(master_vap_handle);
  }
  return wdev;
}
...
static int _rcvry_task_ctx_init (mtlk_hw_t *hw_ctx, wave_rcvry_task_ctx_t *rcvry_ctx)
{
  ...
  mtlk_rmlock_acquire(&rcvry_ctx->rcvry_rmlock);
  mtlk_osal_lock_acquire(&rcvry_ctx->lock);
...
static void _rcvry_task_ctx_cleanup (wave_rcvry_task_ctx_t *rcvry_ctx)
{
  ...
  mtlk_rmlock_release(&rcvry_ctx->rcvry_rmlock);
  if (MTLK_ERR_OK != mtlk_rmlock_wait(&rcvry_ctx->rcvry_rmlock))
    ELOG_V("Recovery remove lock wait failed");
...
MTLK_INIT_STEPS_LIST_BEGIN(wave_rcvry_task_ctx)
  MTLK_INIT_STEPS_LIST_ENTRY(wave_rcvry_task_ctx, RCVRY_TASK_CTX_RMLOCK)
...
void wave_rcvry_cleanup_finalize (mtlk_hw_t *hw_ctx)
{
  ...
  MTLK_CLEANUP_BEGIN(wave_rcvry_task_ctx, MTLK_OBJ_PTR(rcvry_ctx))
    ...
    MTLK_CLEANUP_STEP(wave_rcvry_task_ctx, RCVRY_TASK_CTX_RMLOCK, MTLK_OBJ_PTR(rcvry_ctx),
                      mtlk_rmlock_cleanup, (&rcvry_ctx->rcvry_rmlock));
...
int wave_rcvry_init_finalize (mtlk_hw_t *hw_ctx)
{
  ...
  MTLK_INIT_TRY(wave_rcvry_task_ctx, MTLK_OBJ_PTR(rcvry_ctx))
    MTLK_INIT_STEP(wave_rcvry_task_ctx, RCVRY_TASK_CTX_RMLOCK, MTLK_OBJ_PTR(rcvry_ctx),
                   mtlk_rmlock_init, (&rcvry_ctx->rcvry_rmlock));
    ...
    MTLK_INIT_STEP(wave_rcvry_task_ctx, RCVRY_TASK_CTX_WSS_PARENT_GET, MTLK_OBJ_PTR(rcvry_ctx),
                   wave_hw_mmb_get_prop, (hw_ctx, MTLK_HW_WSS, &hw_wss, sizeof(typeof(hw_wss))));
...
static BOOL _rcvry_is_running (wave_rcvry_task_ctx_t *rcvry_ctx)
{
  ...
  if (RCVRY_TYPE_UNRECOVERABLE_ERROR == __rcvry_type_global_get()) {
    WLOG_S("%s Recovery faild", __rcvry_type_to_string(RCVRY_TYPE_UNRECOVERABLE_ERROR));
    return TRUE;
  }
...
static inline void _rcvry_deinit_vifs (wave_rcvry_radio_ctx_t *radio_ctx)
{
  wave_radio_t *radio = wave_vap_radio_get(radio_ctx->master_vap_handle);
  wave_radio_recovery_deinit_vifs(radio);
}
...
static int _rcvry_step_vap_deactivation (wave_rcvry_task_ctx_t *rcvry_ctx, uint16 oid)
{
  ...
  for (i = 0; i < rcvry_ctx->max_radios_number; i++) {
    ...
    /* Mark vifs as uninitialized after all vaps have been deactivated */
    _rcvry_deinit_vifs(&rcvry_ctx->radio_ctx[i]);
  }
...
static int _rcvry_step_fw_restoration (wave_rcvry_card_cfg_t *pcard, uint16 oid)
{
  ...
  if (MTLK_ERR_OK == res) {
     ...
     /* Add HW RT-Logger if exists */
     if (_chipid_is_gen7(pcard->chip_id)) {
        res = mtlk_mmb_fw_rcvry_rtlog_init(pcard->hw_ctx);
        if (MTLK_ERR_OK != res) {
           ELOG_DD("CID-%04x: Master Recovery FW Restoration failed in logger init: res=%d", oid, res);
           return res;
        }
     }
#endif
...
static inline int _rcvry_step_fw_reconfiguration_mld (wave_rcvry_radio_ctx_t *radio_ctx, wave_rcvry_type_e rcvry_type)
{
  wave_rcvry_vap_info_t **vap_info_ptr_temp = radio_ctx->vap_info;
  wave_rcvry_vap_info_t *vap_info_temp = *vap_info_ptr_temp++;
  int res = MTLK_ERR_OK;
  ...
  /* Configure VAPs */
  while (vap_info_temp) {
    res = core_on_rcvry_configure_mld(mtlk_vap_get_core(vap_info_temp->vap_handle),
                                      (uint32)vap_info_temp->was_connected);
    if (MTLK_ERR_OK != res)
      return res;
...
static int _rcvry_step_fw_reconfiguration (wave_rcvry_card_cfg_t *pcard, uint16 oid)
{
  ...
  ILOG1_D("CID-%04x: Recovery Create MLD FW Reconfiguration Step started", oid);

  for (i = 0; i < rcvry_ctx->max_radios_number; i++) {
    res = _rcvry_step_fw_reconfiguration_mld(&rcvry_ctx->radio_ctx[i], rcvry_type);
    if (MTLK_ERR_OK != res)
      break;
  }
  return res;
...
static int _rcvry_task_master (mtlk_handle_t pcard_handle, const void *payload, uint32 size)
{
  ...
  switch (rcvry_type)
  {
    ...
    case RCVRY_TYPE_UNRECOVERABLE_ERROR:
    {
      ...
      res = wave_rcvry_nl_send_msg_unrecoverable_error(pcard);
      /* If panic is enabled, panic */
      if (_wave_rcvry_krnl_panic_enabled(pcard)){
        char msg[128];
        mtlk_snprintf(msg, sizeof(msg), "Unrecoverable Error on card '%u'",pcard->card_idx);
                                          ...
                                          uint32 radio_idx)
{
  ...
  while (vap_info_temp) {
    ...
    if (mtlk_vap_is_master(vap_info_temp->vap_handle) && (WAVE_RCVRY_PRIMARY_VAP_RADIO_IDX == radio_idx)) {
      mtlk_rmlock_acquire(&pcard->rcvry_ctx.rcvry_rmlock);
      ...
      core_schedule_recovery_task(vap_info_temp->vap_handle, _rcvry_task_slave, (mtlk_handle_t)pcard,
                                  &vap_info_temp, sizeof(typeof(vap_info_temp)));
...
static int _rcvry_task_schedule (wave_rcvry_card_cfg_t *pcard, wave_rcvry_task_ctx_t *rcvry_ctx)
{
  uint32 i;
  ...
  /* acquiring mutex for first time from atomic context */
  if (mtlk_osal_mutex_try_acquire(mutex)) {
    ELOG_V("Failed to acquire initial mutex for recovery serializer sync");
    return MTLK_ERR_UNKNOWN;
  }
...
static int _rcvry_task_initiate (wave_rcvry_card_cfg_t *pcard)
{
  ...
  /* Schedule Master and Slave Recovery tasks */
  res = _rcvry_task_schedule(pcard, rcvry_ctx);
...
int wave_rcvry_task_initiate (mtlk_hw_t *hw_ctx, BOOL is_dut)
{
  ...
  if (pcard && _rcvry_is_initialized(&pcard->rcvry_ctx)) {
    rcvry_ctx = &pcard->rcvry_ctx;
    if ((mtlk_hw_type_is_gen7(hw_ctx)) && (is_dut))
      rcvry_ctx->cfg.rcvry_mode = RCVRY_MODE_ENABLED_DONT_PANIC;
...
static int _wave_rcvry_nl_process_msg_dump_send (wave_rcvry_card_cfg_t *pcard)
{
  ...
  for (i = 0; i < pcard->nof_files; i++, info++) {
    ...
    if (mtlk_hw_type_is_gen7(pcard->hw_ctx) && strstr(info->name, "test_bus")) {
	  ...
	  wave_ccr_write_phy_td_tb_page_num(mtlk_hw_mmb_get_ccr(pcard->hw_ctx), radio_id, page_num);
...
static inline BOOL __rcvry_mac_fatal_pending_get (wave_rcvry_card_cfg_t *pcard)
{
...
BOOL wave_rcvry_mac_fatal_pending_get (const mtlk_hw_t *hw_ctx)
{
  ...
  if (pcard)
    return __rcvry_mac_fatal_pending_get(pcard);
...
static void _rcvry_process_mac_code (wave_rcvry_card_cfg_t *pcard, const APP_FATAL *data)
{
  const char *const names[] = { /* APP_FATAL fields */
        "uCoreNum", "uTimeStamp", "uCauseRegOrLineNum", "uStatusReg",
        "uEretReg", "uEfaReg", "FileId", "GroupId", "OriginId", "FWinterface", "uFirstFreePdOffset" };
  ...
  MTLK_STATIC_ASSERT((sizeof(APP_FATAL)/sizeof(uint32) == ARRAY_SIZE(names)));

  ELOG_DDDDDD("CID-%02x: MAC event: From FW_CORE%d: FW Error Code: [GroupID: %u, FileID: %u, Line: %u], TS 0x%x",
               pcard->card_idx,
  ...
  ELOG_D("CID-%02x: APP_CODE", pcard->card_idx);
...
void wave_rcvry_process_mac_fatal (mtlk_hw_t *hw_ctx, const APP_FATAL *data)
{
  ...
  _rcvry_process_mac_code(pcard, data);
}

static void _rcvry_process_mac_ex_code (wave_rcvry_card_cfg_t *pcard, const APP_FATAL *data)
{
  ELOG_DDDDDDD("CID-%02x: MAC event: From FW_CORE%d: FW Error Code: Cause 0x%x, ERET 0x%x, EFA 0x%x, Status 0x%x, TS 0x%x",
               pcard->card_idx,
               MAC_TO_HOST32(data->uCoreNum),
               MAC_TO_HOST32(data->uCauseRegOrLineNum),
               MAC_TO_HOST32(data->uEretReg),
               MAC_TO_HOST32(data->uEfaReg),
...
void wave_rcvry_process_mac_exception (mtlk_hw_t *hw_ctx, const APP_FATAL *data)
{
  ...
  _rcvry_mac_fatal_pending_set(pcard, TRUE);
  _rcvry_process_mac_ex_code(pcard, data);
}

static BOOL
_wave_rcvry_will_not_end (wave_rcvry_type_e rcvry_type)
{
  return (rcvry_type == RCVRY_TYPE_DBG) ||
         (rcvry_type == RCVRY_TYPE_IGNORE) ||
         (rcvry_type == RCVRY_TYPE_UNRECOVERABLE_ERROR) ||
         (rcvry_type == RCVRY_TYPE_DUT);
}

/* Several mac80211 ops can wait until recovery finishes. In case we are calling
 * mac80211 functions during these waits from recovery, most likely they will try to acquire the
 * the same mutexes, resulting in recovery process to wait while mac80211 ops waits
 * on recovery finish, which results in timeout and long recovery time.
 * Avoid that be not waiting on recovery finish in case recovery is calling mac80211 */
void __MTLK_IFUNC
wave_disable_rcvry_finish_wait (mtlk_core_t *core)
{
  wave_rcvry_card_cfg_t *pcard = _rcvry_card_hw_ctx_get(mtlk_vap_get_hw(core->vap_handle));

  if (pcard)
    atomic_set(&pcard->rcvry_ctx.must_not_wait_on_rcvry, 1);
}

void __MTLK_IFUNC
wave_enable_rcvry_finish_wait (mtlk_core_t *core)
{
  wave_rcvry_card_cfg_t *pcard = _rcvry_card_hw_ctx_get(mtlk_vap_get_hw(core->vap_handle));

  if (pcard)
    atomic_set(&pcard->rcvry_ctx.must_not_wait_on_rcvry, 0);
}

static BOOL
_wave_must_not_wait_on_rcvry (mtlk_core_t *core)
{
  wave_rcvry_card_cfg_t *pcard = _rcvry_card_hw_ctx_get(mtlk_vap_get_hw(core->vap_handle));

  if (pcard && atomic_read(&pcard->rcvry_ctx.must_not_wait_on_rcvry))
    return TRUE;
...
mtlk_error_t
wave_rcvry_wait_completion_if_needed (mtlk_core_t *core)
{
  mtlk_osal_timestamp_t start_time = mtlk_osal_timestamp();
  BOOL waited = FALSE;
  mtlk_osal_msec_t wait_time_ms = 0;

  while (wave_in_fw_recovery(core)) {
    if (_wave_rcvry_will_not_end(wave_rcvry_type_current_get(mtlk_vap_get_hw(core->vap_handle))))
      return MTLK_ERR_NOT_READY;

    if (_wave_must_not_wait_on_rcvry(core))
      return MTLK_ERR_NOT_READY;
    ...
    if ((wait_time_ms = mtlk_osal_time_passed_ms(start_time)) > WAVE_RCVRY_RETRY_TIMEOUT_MS)
      break;
  ...
  if (waited)
    ILOG0_D("Waited %d ms for recovery procedure to complete", wait_time_ms);

  if (wait_time_ms > WAVE_RCVRY_RETRY_TIMEOUT_MS)
    return MTLK_ERR_NOT_READY;
...
BOOL __MTLK_IFUNC
wave_in_fw_recovery (mtlk_core_t *core)
{
  wave_rcvry_card_cfg_t *pcard = _rcvry_card_hw_ctx_get(mtlk_vap_get_hw(core->vap_handle));
  wave_rcvry_task_ctx_t *rcvry_ctx;
  ...
  /* Also 'Disabled' recovery (RCVRY_TYPE_NONE) runs serializer tasks
   * to created dumps, and this must be handled to avoid deadlocks */
  if (_rcvry_is_initialized(rcvry_ctx) &&
     ((__rcvry_type_current_get(rcvry_ctx) != RCVRY_TYPE_UNDEF) &&
      (__rcvry_type_current_get(rcvry_ctx) != RCVRY_TYPE_DUT)))
    return TRUE;

  return _wv_rcvry_is_enabled(rcvry_ctx) &&
         (__rcvry_mac_fatal_pending_get(pcard) ||
          mtlk_core_is_hw_halted(core));
}

BOOL __MTLK_IFUNC
wave_in_complete_recovery (mtlk_core_t *core)
{
  return wave_rcvry_type_current_get(mtlk_vap_get_hw(core->vap_handle)) == RCVRY_TYPE_UNRECOVERABLE_ERROR;
}
