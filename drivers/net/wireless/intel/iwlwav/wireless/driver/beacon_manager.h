...
#ifdef MTLK_WAVE_700
  uint8      *mbssid_ptr;
  uint32     mbssid_dma_addr;
  uint32     mbssid_alloc_size;
#endif
  uint16     head_len;
  uint16     tail_len;
} beacon_template_t;
...
int wave_beacon_template_get_beacon_data(mtlk_core_t *core, struct sk_buff *beacon, struct sk_buff *probe_resp,
