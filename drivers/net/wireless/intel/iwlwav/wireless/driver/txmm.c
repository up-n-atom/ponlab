                                   ...
                                   uint32            timeout_ms)
{
  ...
  if (!mtlk_hw_type_is_gen7(mtlk_vap_get_hw(master_vap))) {
  ...
  res = mtlk_vap_get_core_vft(master_vap)->get_prop(master_vap,
                                                    ...
                                                    &is_pending,
                                                    sizeof(is_pending));
  if ((res != MTLK_ERR_OK) || is_pending || mtlk_core_is_hw_halted(mtlk_vap_get_core(master_vap))) {
...
#else
mtlk_txmm_msg_init_objpool(mtlk_txmm_msg_t *msg,
...
#else
mtlk_txmm_data_t* __MTLK_IFUNC
mtlk_txmm_msg_init_with_empty_data_objpool(mtlk_txmm_msg_t *msg,
                                           mtlk_txmm_t     *obj,
                                           int             *err,
                                           mtlk_slid_t      caller_slid)
...
#else
  int               r   = mtlk_txmm_msg_init_objpool(msg, caller_slid);
#endif /* CPTCFG_IWLWAV_ENABLE_OBJPOOL */
