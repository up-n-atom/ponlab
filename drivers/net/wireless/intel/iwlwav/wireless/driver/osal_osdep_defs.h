...
/* Have to override ARRAY_SIZE macro to solve the Klocwork CWARN.SIZEOF.NOPOINTER problem */
#define MTLK_ARRAY_SIZE(arr) (sizeof(arr) / sizeof(typeof((arr)[0])) + __must_be_array(arr))

#undef ARRAY_SIZE
#define ARRAY_SIZE(arr)     MTLK_ARRAY_SIZE(arr)
                           ...
                           mtlk_data_direction_e  direction)
{
  ...
  if (addr) {
#ifdef CONFIG_DMA_MIXCOHERENT
    dma_unmap_single_attrs(dev_ctx, addr, size, direction, DMA_ATTR_NON_CONSISTENT);
#else
...
#define wave_osal_mem_dma_alloc_noncoherent(drv_ctx, size, dma_addr)  _wave_osal_mem_dma_alloc_noncoherent((drv_ctx), (size), (dma_addr), (MTLK_SLID))
...
#ifdef CONFIG_DMA_MIXCOHERENT
static __INLINE void*
_wave_osal_mem_dma_alloc_noncoherent (void *dev_ctx, uint32 size, dma_addr_t *dma_addr, mtlk_slid_t caller_slid)
{
  void *out_buf;
  uintptr_t guard_offset;
  size_t full_size = mem_leak_get_full_allocation_size(size);
  void *buf = dma_alloc_attrs(dev_ctx, full_size, dma_addr, GFP_ATOMIC | GFP_DMA, DMA_ATTR_NON_CONSISTENT);
  if (!buf)
    return NULL;
  set_memory_uc((unsigned long)buf, PAGE_ALIGN(full_size) >> PAGE_SHIFT);
  out_buf = mem_leak_handle_allocated_buffer(buf, size, caller_slid);
  guard_offset = out_buf - buf;
  (*dma_addr) += guard_offset;
  return out_buf;
}
#endif
...
#ifdef CONFIG_DMA_MIXCOHERENT
static __INLINE void*
wave_osal_mem_dma_alloc_noncoherent (void *dev_ctx, uint32 size, dma_addr_t *dma_addr)
{
  void *addr = dma_alloc_attrs(dev_ctx, size, dma_addr, GFP_ATOMIC | GFP_DMA, DMA_ATTR_NON_CONSISTENT);
  if (!addr)
    return NULL;
...
#ifdef CONFIG_DMA_MIXCOHERENT
/* Free memory previously allocated by wave_osal_mem_dma_alloc_noncoherent(). */
static __INLINE void
wave_osal_mem_dma_free_noncoherent (void *dev_ctx, uint32 size, void *buffer, dma_addr_t dma_addr)
{
  if (buffer) {
    /* Unlike with CPU memory allocators, calling dma_free_attrs() with a NULL buffer is not safe. */
    #ifdef CPTCFG_IWLWAV_ENABLE_OBJPOOL
      void *buf = mem_leak_handle_buffer_to_free(buffer);
      uintptr_t guard_offset = buffer - buf;
      size_t full_size = mem_leak_get_full_allocation_size(size);
      dma_addr -= guard_offset;
      set_memory_wb((unsigned long)buf, PAGE_ALIGN(full_size) >> PAGE_SHIFT);
      dma_free_attrs(dev_ctx, full_size, buf, dma_addr, DMA_ATTR_NON_CONSISTENT);
    #else
      set_memory_wb((unsigned long)buffer, PAGE_ALIGN(size) >> PAGE_SHIFT);
      dma_free_attrs(dev_ctx, size, buffer, dma_addr, DMA_ATTR_NON_CONSISTENT);
    #endif
  }
}
#endif
...
#endif
#ifdef __KLOCWORK__ /* workaround to not always return OK */
  if (0 != atomic_read(&spinlock))
...
static __INLINE int
mtlk_osal_mutex_try_acquire (mtlk_osal_mutex_t* mutex)
{
  return down_trylock(&mutex->sem);
}
...
{
  /* Like ktime_get_ns(), but does not stop when suspended */
...
static __INLINE mtlk_osal_msec_t
mtlk_osal_timestamp_ms (void)
{
  return mtlk_osal_timestamp_to_ms(mtlk_osal_timestamp());
}
...
static __INLINE mtlk_osal_usec_t
mtlk_osal_timestamp_us (void)
{
  return mtlk_osal_timestamp_to_us(mtlk_osal_timestamp());
}
...
static __INLINE uint32
mtlk_osal_time_passed_sec(mtlk_osal_timestamp_t tm)
{
