...
_mtlk_wss_adopt_contexts (mtlk_wss_t *wss)
{
  ...
  new_entries = (mtlk_wss_stats_entry_t **)mtlk_osal_mem_alloc(new_nof_entries * sizeof(typeof(new_entries[0])),
                                                               MTLK_MEM_TAG_WSS);
  if (new_entries == NULL) {
    res = MTLK_ERR_NO_MEM;
    goto FINISH;
  ...
  memset(new_entries, 0, new_nof_entries * sizeof(typeof(new_entries[0])));

  if (wss->parent && wss->parent->nof_entries) {
    /* Copy all reader entries from the parent */
    wave_memcpy(new_entries, new_nof_entries * sizeof(typeof(new_entries[0])),
                wss->parent->entries, wss->parent->nof_entries * sizeof(typeof(new_entries[0])));
  }
                     ...
                     uint32                    nof_stats)
{
  ...
  memset(hctrls, 0, sizeof(typeof(hctrls[0])) * nof_stats);
