...
#define INDEX_CHAN_NUM_BW_HIGH 1
  ...
  while ((ssize_t)data_size >= (ssize_t)tpc_struct_size) {
    ...
    for(idx = 0; idx < EEPROM_GEN6_MAX_ANT; idx++) {
      ...
      ILOG1_DDDD("TPC data for chan %u, bw %u, antenna %u, fw_chan_bw 0x%x", chan, bw, idx,
                     ...
                     mtlk_eeprom_cis_data_t *parsed_cis)
{
  ...
  ILOG0_DDDCD("EEPROM Card ID: subversion 0x%04x, type 0x%02X, revision 0x%02X (%c) production 0x%02X",
            card_id->dev_opt_mask.s.subversion, card_id->type, card_id->revision, card_id->revision, card_id->production);
