...
*/
static __INLINE int
_buffer_read (mtlk_ccr_t *ccr, uint8 chip, uint32 addr, uint8 alen, uint8 *buffer, int len)
{
  int shift;
...
static __INLINE int
gen7_eeprom_write (mtlk_ccr_t *ccr, uint32 offset, uint8 const *buffer, int len)
{
  /* Need 13 bit word address to write data from 8K-EEPROM on W700. Hence require 2 byte address */
  return _page_write(ccr, EEPROM_CHIP_ID, offset, 2, buffer, len);
}

static __INLINE int
gen6_eeprom_write (mtlk_ccr_t *ccr, uint32 offset, uint8 const *buffer, int len)
{
  return _page_write(ccr, (EEPROM_CHIP_ID | (((offset) >> 8) & 7)), offset, 1, buffer, len);
}
...
/**
  ...
  \param buffer Pointer to the source buffer
  \param len    Size of source buffer
  \param is_gen7 Flag to validate W700 chip
...
int __MTLK_IFUNC
eeprom_gpio_write (mtlk_ccr_t *ccr, uint32 offset, void const *buffer, int len, BOOL is_gen7)
{
  int ret = MTLK_ERR_OK;
  ...
  while (len > 0)
  {
    ...
    if (is_gen7)
      ret = gen7_eeprom_write(ccr, offset, buffer, size);
    else
      ret = gen6_eeprom_write(ccr, offset, buffer, size);
...
static __INLINE int
gen7_eeprom_read (mtlk_ccr_t *ccr, uint32 offset, uint8 *buffer, int len)
{
  /* Need 13 bit word address to read data from 8K-EEPROM on W700. Hence require 2 byte address */
  return _buffer_read(ccr, EEPROM_CHIP_ID, offset, 2, buffer, len);
}

static __INLINE int
gen6_eeprom_read (mtlk_ccr_t *ccr, uint32 offset, uint8 *buffer, int len)
{
...
*/
int __MTLK_IFUNC
eeprom_gpio_read (mtlk_ccr_t *ccr, uint32 offset, void *buffer, int len, BOOL is_gen7)
{
  int ret;
  MTLK_ASSERT(NULL != ccr);
  MTLK_ASSERT(NULL != buffer);
  ...
  if (is_gen7)
    ret = gen7_eeprom_read(ccr, offset, buffer, len);
  else
    ret = gen6_eeprom_read(ccr, offset, buffer, len);
...
/**
  ...
  \param is_gen7 Flag to check W700 chip
  \param storage nv memory type

  \return
    MTLK_ERR_...
*/
static __INLINE int
_size_get (mtlk_ccr_t *ccr, uint32 *size, BOOL is_gen7, uint8 storage)
{
  uint8  testData, savedData; /* both of same size */
  uint32 eeprom_size, offset;
  uint32 data_size = sizeof(testData);
  uint32 max_eeprom_size = GET_EEPROM_SIZE(is_gen7, storage);

  /* Test eeprom size */
  for(eeprom_size = max_eeprom_size; eeprom_size >= MTLK_MIN_EEPROM_SIZE; eeprom_size >>= 1)
  {
    /* Try to read/write data by last data offset */
    offset = eeprom_size - data_size;

    /* Save eeprom data from tested cell */
    eeprom_gpio_read(ccr, offset, &savedData, data_size, is_gen7);

    /* Test eeprom cell */
    testData = EEPROM_TEST_VALUE;
    eeprom_gpio_write(ccr, offset, &testData, data_size, is_gen7);

    testData = 0;
    eeprom_gpio_read(ccr, offset, &testData, data_size, is_gen7);

    /* Restore eeprom data to tested cell */
    eeprom_gpio_write(ccr, offset, &savedData, data_size, is_gen7);
...
*/
int __MTLK_IFUNC
eeprom_gpio_init (mtlk_ccr_t *ccr, uint32 *size, BOOL is_gen7, uint8 storage)
{
  int ret = MTLK_ERR_OK;
  ...
  if (MTLK_ERR_OK == ret)
  {
    ...
    ret = _size_get(ccr, size, is_gen7, storage);
  }
