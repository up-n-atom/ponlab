...
mtlk_get_cal_offset (mtlk_eeprom_type_e ee_type, uint8 fw_num, BOOL is_gen7, uint32 *offset)
{
  *offset = 0;
  ...
  /* Need to use offset 0 incase of gen6 for band0 cal, on both FILE and GPIO type
     Incase of gen7 need to use offset 0 only FILE type */
  if ((!is_gen7 && fw_num == WAVE_FW_NUM_SECB_CAL_FILE) ||
      (is_gen7 && (fw_num == WAVE_FW_NUM_SECB_CAL_FILE) && (ee_type == EEPROM_TYPE_FILE))) {
    return TRUE;
  }

  /* Use different offset value on gen7 for 3 cal file(band0/1/2) with GPIO type */
  if (is_gen7 && (ee_type == EEPROM_TYPE_GPIO)) {
    switch (fw_num) {
      ...
      case WAVE_FW_NUM_SECB_CAL_FILE_2:
        *offset = MTLK_WAV700_EEPROM_5G_OFFSET;
        break;
      case WAVE_FW_NUM_SECB_CAL_FILE_4:
        *offset = MTLK_WAV700_EEPROM_6G_OFFSET;
        break;
      default:
        return FALSE;
    }
    ILOG2_DD("offset : %u fw_num : %u", *offset, fw_num);
    return TRUE;
  }
  return FALSE;
}
...
#define MTLK_EEPROM_VER_7       7

static __INLINE BOOL
__mtlk_eeprom_ver_validate (uint8 ver, uint32 chip_id)
{

  /* Currently, need to support both ver6 and ver7 on W700. In future need to use only ver7 on W700 */
  if (_chipid_is_gen7(chip_id)) {
    if ((ver == MTLK_EEPROM_VER_6) || (ver == MTLK_EEPROM_VER_7)) {
      return TRUE;
    }
  }

  if (_chipid_is_gen6(chip_id)) {
    if (ver == MTLK_EEPROM_VER_6) {
      return TRUE;
    }
  }
  return FALSE;
...
_eeprom_parse (mtlk_eeprom_t *raw_eeprom, mtlk_eeprom_data_t *parsed_eeprom, int eeprom_size, uint32 chip_id)
{
  ...
  ret = __mtlk_eeprom_ver_validate (ver1, chip_id);
  if (ret) {
                       ...
                       BOOL is_gen7, uint8 storage)
{
  ...
  *bytes_read = 0;
  ...
  (void)mtlk_hw_get_prop(hw_api, MTLK_HW_PROP_CCR, &ccr, sizeof(typeof(ccr)));
  res = eeprom_gpio_init(ccr, &eeprom_size, is_gen7, storage);
  if (MTLK_ERR_OK != res)
  {
  ...
  /* Buffer size to read EEPROM GPIO must be at-least the maximum eeprom size
   * excluding the signed CAL header size.
   */
  MTLK_ASSERT((buffer_size + MTLK_SIGNED_CAL_HEADER_SIZE) >= eeprom_size);

  if(buffer_size > eeprom_size)
    buffer_size = eeprom_size;

  res = eeprom_gpio_read(ccr, 0, buffer, buffer_size, is_gen7);
  if (MTLK_ERR_OK != res)
  {
                       ...
                       mtlk_card_type_t card_type, mtlk_eeprom_type_e ee_type)
{
  ...
  is_gen7 = mtlk_hw_type_is_gen7(hw_api->hw);
  *bytes_read = 0;

  /* GEN7 : allocate 8K memory to read cal data from 8K EEPROM */
  if (is_gen7 && (ee_type == EEPROM_TYPE_GPIO))
    max_eeprom_size = MTLK_EEPROM_W700_TOTAL_SIZE;

  *buffer = mtlk_osal_mem_alloc(max_eeprom_size, MTLK_MEM_TAG_EEPROM);
  ...
  {
    ILOG0_SD("CAL load from EEPROM (%s) completed, reading finished, %d bytes read.",
	         _mtlk_eeprom_type_to_string(ee_type), *bytes_read);
  }
...
uint8 __MTLK_IFUNC
mtlk_get_eeprom_type (mtlk_eeprom_data_t *eeprom_data)
{
  return eeprom_data->ee_type;
}
...
mtlk_get_eeprom_raw_data (mtlk_eeprom_data_t *eeprom_data, const uint8 **raw_data, uint32 *raw_data_size, uint32 offset)
{
  ...
  *raw_data = (uint8*)(eeprom_data->raw_data + offset);
...
mtlk_eeprom_read_and_parse(mtlk_hw_api_t *hw_api, mtlk_eeprom_data_t *ee_data, uint32 chip_id)
{
  ...
  result = mtlk_hw_get_prop(hw_api, MTLK_HW_PROP_CARD_TYPE, &card_type, sizeof(typeof(card_type)));
  if (MTLK_ERR_OK != result)
  {
    return MTLK_ERR_NOT_SUPPORTED;
  }

  if (mtlk_hw_type_is_gen6_d2_or_gen7(hw_api->hw))
  {
  ...
  /* Read EEPROM data into new allocated buffer */
    ...
    if ((_chipid_is_gen7(chip_id)) && (ee_type == EEPROM_TYPE_GPIO)) {
      offset = MTLK_WAV700_EEPROM_2_4G_OFFSET;
    }
...
mtlk_eeprom_get_raw_data (mtlk_eeprom_data_t *eeprom_data, mtlk_eeprom_cfg_t *eeprom_cfg)
{
  ...
  if (!eeprom_data || !eeprom_cfg)
    return MTLK_ERR_PARAMS;

  if (eeprom_data->raw_data_size <= sizeof(eeprom_cfg->eeprom_raw_data))
  {
    wave_memcpy(eeprom_cfg->eeprom_raw_data, sizeof(eeprom_cfg->eeprom_raw_data), eeprom_data->raw_data, eeprom_data->raw_data_size);
    eeprom_cfg->eeprom_total_size = eeprom_data->raw_data_size;
  }
  else
  {
    ELOG_DD("Buffer size (%zu) too small, required size %u", sizeof(eeprom_cfg->eeprom_raw_data), eeprom_data->raw_data_size);
    eeprom_cfg->eeprom_total_size = 0;
