...
_mtlk_dut_core_start (mtlk_core_t *core)
{
  ...
  res = wave_core_cfg_get_dynamic_mu_type(core, &dynamic_mu_type);
  /* disable DL/UL ofdma on dut mode */
  if (MTLK_ERR_OK == res) {
    res = wave_core_cfg_send_dynamic_mu_type(core, &dynamic_mu_type);
    if (res != MTLK_ERR_OK) {
      ILOG0_DD("CID-%04x: failed to send dynamic_mu_type on dut mode res=%d",
