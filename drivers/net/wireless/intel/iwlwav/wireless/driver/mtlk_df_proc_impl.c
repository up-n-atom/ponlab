...
static int
_mtlk_df_proc_nonseekable_open(struct inode *inode,
                   struct file *file)
{
...
struct proc_ops _mtlk_df_proc_entry_fops = {
#if LINUX_VERSION_IS_LESS(5,6,0)
  .owner   = THIS_MODULE,
#endif
  .proc_open    = _mtlk_df_proc_nonseekable_open,
  .proc_read    = _mtlk_df_proc_read,
  .proc_write   = _mtlk_df_proc_write
};
 ...
 * seq_release.
 */
struct proc_ops _mtlk_df_proc_seq_entry_fops = {
#if LINUX_VERSION_IS_LESS(5,6,0)
  .owner   = THIS_MODULE,
#endif
  .proc_open    = _mtlk_df_proc_seq_entry_open_ops,
  .proc_read    = seq_read,
  .proc_lseek   = seq_lseek,
  .proc_release = _mtlk_df_proc_seq_entry_release_ops
};
...
void __MTLK_IFUNC
mtlk_df_proc_node_remove_entry_w_ops(const char *name,
                                     mtlk_df_proc_fs_node_t *parent_node, void *sta)
{
  struct mtlk_proc_ops* proc_ops;
  ...
  mtlk_df_proc_node_remove_entry(name, parent_node);

  /* There could be only one proc entry for dir */
  mtlk_dlist_foreach(&parent_node->proc_ops_list, entry, head) {
    proc_ops = MTLK_LIST_GET_CONTAINING_RECORD(entry, struct mtlk_proc_ops, list_entry);
    if (proc_ops->df == sta) {
      ILOG1_SS("Proc ops are deleted %s/%s", parent_node->name, name);
      mtlk_dlist_remove(&parent_node->proc_ops_list, entry);
      mtlk_osal_mem_free(proc_ops);
      break;
    }
  }
