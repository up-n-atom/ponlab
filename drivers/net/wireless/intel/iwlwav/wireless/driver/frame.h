...
#ifdef MTLK_WAVE_700
/* Definitions as per IEEE 802.11BE D1.01 version */
#define IE_EHT_CAPABILITIES_MIN_LEN 11
#endif
#define ML_PROBE_TYPE_POS 4
...
typedef enum {
  ...
  IE_VHT_OPERATING_MODE_NOTIFICATION = 199,
...
typedef struct {
  ...
  int8      max_rssi;
  uint8     all_rssi[WAVE_STAT_MAX_ANTENNAS];
  int8      noise; /* dBm */
  // From IFs
  uint16    capability;
  uint16    beacon_interval;
  ...
  uint8     dtim_period;
  uint8     erp_info;
  uint8     is_hidden_ssid;
#ifdef MTLK_WAVE_700
  uint8     *eht_cap_info;
  unsigned  eht_cap_info_len;
  uint8     is_eht;
#endif
  uint8     has_probe_ml_ie;
} bss_data_t;
...
#define ML_PROBE_IE_FOUND(bss) ((bss).has_probe_ml_ie != 0)
...
#define WLAN_EID_HE_CAP_EXT                             WLAN_EID_EXT_HE_CAPABILITY
#define WLAN_EID_HE_OPER_EXT                            WLAN_EID_EXT_HE_OPERATION
#define WLAN_EID_MAX_CHAN_SW_TIME_EXT                   WLAN_EID_EXT_MAX_CHANNEL_SWITCH_TIME
#define WLAN_EID_HE_6GHZ_CAP_EXT                        WLAN_EID_EXT_HE_6GHZ_CAPA
#define WLAN_EID_EXTENSION_ID_OFFSET                    2
#define WLAN_ELEM_ID_EXTENSION_LEN                      1
#ifdef MTLK_WAVE_700
#define WLAN_EID_EHT_OPER_EXT                           WLAN_EID_EXT_EHT_OPERATION
#define WLAN_EID_MULTILINK_EXT                          WLAN_EID_EXT_EHT_MULTI_LINK
#define WLAN_EID_EHT_CAP_EXT                            WLAN_EID_EXT_EHT_CAPABILITY
#endif
...
uint32  mtlk_frame_oper_rateset_to_net_mode(uint32 oper_rateset, mtlk_hw_band_e band, BOOL is_ht, BOOL is_vht, BOOL is_he, BOOL is_eht);
