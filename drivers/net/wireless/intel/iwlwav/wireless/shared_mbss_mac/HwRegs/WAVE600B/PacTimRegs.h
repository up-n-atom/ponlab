...
Module:				pacTim
SOC Revision:		latest
Generated at:       2022-03-29 07:45:59
Purpose:
...
typedef union
{
	...
	struct
	{
		uint32 tsfNavLoadTime : 24; //TIM  time, reset value: 0x0, access type: RW
		uint32 reserved0 : 5;
		uint32 tsfNavLoadHwEnableRd : 1; //TSF NAV load RD, reset value: 0x0, access type: RO
		uint32 tsfNavLoadHwEnableWr : 1; //TSF NAV load WR, reset value: 0x0, access type: WO
		uint32 reserved1 : 1;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 tsfNavLoadDuration : 16; //TIM  tsf nav load duration, reset value: 0x0, access type: RW
		uint32 reserved0 : 16;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 navEnable : 1; //TIM  nav enable, reset value: 0x0, access type: RW
...
typedef union
{
	...
	struct
	{
		uint32 navTimer : 16; //TIM  nav timer, reset value: 0x0, access type: RO
		uint32 reserved0 : 16;
	} bitFields;
} RegPacTimNavTimer_u;
...
typedef union
{
	...
	struct
	{
		uint32 rtsClrRxNavTimerEnable : 1; //Enable reset Rx NAV timer in case RTS duration timer expired, reset value: 0x1, access type: RW
		uint32 reserved0 : 31;
	} bitFields;
