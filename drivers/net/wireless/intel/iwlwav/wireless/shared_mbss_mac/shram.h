...
/***************************************************************************/
/***        General defines used at many places later on                 ***/
/***************************************************************************/

#define FW_CORE0			(0) // wave700, wave600 and wave500: Lower mac 0
#define FW_CORE1			(1) // wave700, wave600: Lower mac 1; wave500: Upper mac
#define FW_CORE2			(2) // wave700  Lower mac 2, wave600: Upper mac; wave500: N/A
#define FW_CORE3			(3) //wave700 Upper mac

#define MAX_NUM_OF_FW_CORES_G6 (3)
#define MAX_NUM_OF_FW_CORES_G7 (4)
#define MAX_NUM_OF_FW_CORES    MAX_NUM_OF_FW_CORES_G7
...
#define MHI_MAN_STAT_SIZE_MAX    (20 * 1024)
    ...
    #define UMI_IND_COUNT       26
#endif

#if !defined(BEACON_IND_COUNT)
    #define BEACON_IND_COUNT    (MAX_NUM_OF_FW_CORES-1)   /* this is the max number of LMACs for all supported generations*/
#endif
...
// NUM_MAN_INDS must be a power of two value WLANRTSYS-55045
#define NUM_MAN_INDS            (BSS_IND_COUNT + STA_IND_COUNT + RSN_IND_COUNT + UMI_IND_COUNT + BEACON_IND_COUNT)
#endif

#if ((NUM_MAN_INDS & 0x1) == 0x1)
	#error NUM_MAN_INDS must be an Even value WLANRTSYS-55045
#endif
...
#define VECTOR_AREA_VECTOR_AREA_CALFILE_INFO_EXTENSION_ID		    (11)
#define VECTOR_AREA_RINGS_EXTENSION_ID								(12)
...
//TODO:check with driver guys how they did it, so it is consistent
typedef struct 
{
	uint32			u32Core_info[MAX_NUM_OF_FW_CORES]; /* LMAC0, LMAC1, LMAC2 (W700 only), UMAC */
...
#define BAND_CONFIGURATION_MODE_TRI_BAND				(3)
...
#define TEST_PLATFORM_TYPE_ASIC							(2)
...
typedef struct 
{
	uint32			u32BandConfiguration; /* 0x0 - single band / 0x1 - dual band / 0x2 - single band + scan on other band / 0x3 - Tri band */
	...
	uint32			masterVapIDBand[MAX_NUM_OF_FW_CORES-1]; /* this is the max number of LMACs for all supported generations */
...
typedef struct
{	
	uint32 rxInHdsCountSubAddress; // not in use
	uint32 txInHdsCountSubAddress; // not in use
	uint32 rxOutHdsCountSubAddress;
	uint32 txOutHdsCountSubAddress;
} __MTLK_PACKED VECTOR_AREA_FLOW_CONTROL_FOR_4UMT;

typedef struct
{
    VECTOR_AREA_EXTENSION_HEADER sHeader;
	VECTOR_AREA_FLOW_CONTROL_FOR_4UMT sData;
} __MTLK_PACKED VECTOR_AREA_RINGS_EXTENSION;

typedef struct 
{
	uint32			u32rxAntMask[MAX_NUM_OF_FW_CORES-1]; /* this is the max number of LMACs for all supported generations */
    uint32			u32txAntMask[MAX_NUM_OF_FW_CORES-1]; /* this is the max number of LMACs for all supported generations */
} __MTLK_PACKED VECTOR_AREA_CALFILE_INFO_EXTENSION_DATA;

typedef struct
{	
    VECTOR_AREA_EXTENSION_HEADER sHeader;
	VECTOR_AREA_CALFILE_INFO_EXTENSION_DATA sData;
} __MTLK_PACKED VECTOR_AREA_CALFILE_INFO_EXTENSION;
...
typedef struct 
{
    ...
    uint32                                                  pcie_link_ready;
    VECTOR_AREA_CALFILE_INFO_EXTENSION                      sCalFileInfoExt;
	VECTOR_AREA_RINGS_EXTENSION								sFlowContolFor4Umt;
...
/*********************************************************************************************/
//WAVE700
#define MTLK_BITRATE_STAT_PSDU_PHYMODE_G7        MTLK_BFIELD_INFO(8, 4)
#define MTLK_BITRATE_PSDU_BW_G7                  MTLK_BFIELD_INFO(12, 3)
#define MTLK_BITRATE_PSDU_SCP_G7                 MTLK_BFIELD_INFO(15, 1)
#define MTLK_BITRATE_PSDU_SCP_HE_G7              MTLK_BFIELD_INFO(0, 2)
/*********************************************************************************************
* PMCU
*********************************************************************************************/
