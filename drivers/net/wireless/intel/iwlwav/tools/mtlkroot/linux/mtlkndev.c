...
void __MTLK_IFUNC
mtlk_rtlog_check_iph_csum(struct iphdr *iph)
{
    uint16  csum_old, csum_new;

    csum_old = iph->check;
    mtlk_rtlog_set_iph_csum(iph);
    csum_new = iph->check;
#ifdef CONFIG_WAVE_DEBUG
    if (csum_old != csum_new) {
        mtlk_osal_emergency_print("%s: Change iph->csum from %04X to %04X", __FUNCTION__, (int)csum_old, (int)csum_new);
    }
#endif
}
...
_mtlk_ndev_send_capwap_packet (struct net_device *dev, void *buff, size_t data_len, void *hdr, size_t hdr_len, uint8 is_wifi)
{
    ...
    iph->tot_len = htons(ip_len);
    iph->id      = htons(ip_ident);
    mtlk_rtlog_set_iph_csum(iph); /* IPH checksum validation enabled */
...
mtlk_rtlog_forward_log_packet(struct sk_buff *skb)
{
    ...
    if (rtlog_netdev) {
        ...
        /* Save sent PID handle in order to set it after setting initial rtlog0 net device configuration */
#ifdef CONFIG_TI_PACKET_PROCESSOR
        pid_handle = skb->dev->pid_handle;
#endif

        skb->dev = rtlog_netdev;

        /* Set PID handle and parent PID Handle in order to create session in ingress hook */
#ifdef CONFIG_TI_PACKET_PROCESSOR
        skb->dev->pid_handle = pid_handle;
        skb->dev->vpid_block.parent_pid_handle = pid_handle;
#endif
        ...
        /* Currently we have only wlan0/stream0 */
        {
            ...
            MTLK_UNREFERENCED_PARAM(udp_id);
            ILOG2_DDD("LOG pck: IP id 0x%04x, udp_len 0x%04x (%d)", udp_id, udp_len, udp_len);
            ...
            if (1512 < udp_len /* HW limited length */ ||
              ...
              dev_kfree_skb(skb);
                ...
                mtlk_rtlog_check_iph_csum(&skb_pck_hdr->iph);
                send_ready_udp_packet(skb);
            }
        }
