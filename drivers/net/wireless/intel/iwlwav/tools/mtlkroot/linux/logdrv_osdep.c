...
static const struct proc_ops logger_debug_fops = {
#if LINUX_VERSION_IS_LESS(5,6,0)
  .owner      = THIS_MODULE,
#endif
  .proc_open       = logger_debug_open,
  .proc_read       = seq_read,
  .proc_lseek      = seq_lseek,
  .proc_release    = single_release,
  .proc_write      = logger_debug_write
};

#ifdef CONFIG_WAVE_RTLOG_REMOTE
static struct proc_ops logger_stat_fops = {
#if LINUX_VERSION_IS_LESS(5,6,0)
  .owner      = THIS_MODULE,
#endif
  .proc_open       = logger_stat_open,
  .proc_read       = seq_read,
  .proc_lseek      = seq_lseek,
  .proc_release    = single_release,
};
...
static const struct proc_ops logger_rtlog_fops = {
#if LINUX_VERSION_IS_LESS(5,6,0)
  .owner      = THIS_MODULE,
#endif
  .proc_open       = logger_rtlog_open,
  .proc_read       = seq_read,
  .proc_lseek      = seq_lseek,
  .proc_release    = single_release,
  .proc_write      = logger_rtlog_write
};
...
rtlog_netdev_init(void)
{
    ...
    // Set PID handle and parent PID handle in order to register to ingress hook
    // vpid is allocated dynamically
#ifdef CONFIG_TI_PACKET_PROCESSOR
    rtlog_netdev->pid_handle = PP_WiFi_PORT1_PID_NUM;
    rtlog_netdev->vpid_block.parent_pid_handle = PP_WiFi_PORT1_PID_NUM;
#endif
