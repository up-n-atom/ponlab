...
static void fifo_entry_timeout(unsigned long data);
#else
static void fifo_entry_timeout(struct timer_list *data);
#endif
...
#else
	timer_setup(&f_entry->timer, fifo_entry_timeout, 0);
#endif
...
#else
	struct mac_fifo_entry *f_entry  = from_timer(f_entry, data, timer);
#endif
