...
static irqreturn_t mac_isr(int irq, void *dev_id)
{
	...
	int i = 0, mac_int_sts = 0, lmac_int_sts = 0, link_int_sts = 0;
	...
	if (mac_int_sts) {
		pr_debug("Link status change: 0x%04X\n", mac_int_sts);
	...
	for (i = 0; i < max_mac; i++) {
		...
		link_int_sts = gswss_get_int_stat(adap_ops, LINK);

		/* Check LINK i Interrupts */
		if (link_int_sts & (1 << (GSWIPSS_IER1_LINK2_POS + i))) {
			/* Clear link related interrupts. First */
			gswss_set_interrupt(adap_ops, LINK,
					    pdata->mac_idx, 0);
			GSWSS_RGWR(GET_ADAP_PDATA(adap_ops), GSWIPSS_ISR1,
				   (1 << (GSWIPSS_ISR1_LINK2_POS + i)));
			task_sched = 1;
		}
...
static int mac_probe(struct platform_device *pdev)
{
	...
	if (ret < pdata->ptp_clk - pdata->ptp_clk / 10000 ||
	    ret > pdata->ptp_clk + pdata->ptp_clk / 10000) {
		/* Ignore difference within 100ppm.
		 * If difference is larger than 100ppm,
		 * change ptp_clk used for time update calculation to freq
		 * reported by CGU driver */
	...
	if (speed == GSW_PORT_SPEED_10)
		...
		load = "1 Gbps";
	else if (speed == GSW_PORT_SPEED_10000)
		load = "10 Gbps";
	else if (speed == GSW_PORT_SPEED_2500)
		load = "2.5 Gbps";
	else
		load = "AUTO";
