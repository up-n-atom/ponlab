...
static int cli_mdio_rd(void *pdev, u32 dev_adr, u32 phy_id, u32 phy_reg)
{
#ifdef __KERNEL__
	struct mac_prv_data *priv_data = GET_MAC_PDATA(pdev);
	...
	phy_reg &= 0xFFFF;
	if (dev_adr)
		phy_reg |= MII_ADDR_C45 | (dev_adr << 16);
	return mdiobus_read(priv_data->mii, phy_id, phy_reg);
#else
	return xgmac_mdio_single_rd(pdev, dev_adr, phy_id, phy_reg);
#endif
}
...
static int cli_mdio_wr(void *pdev)
{
	...
	dev_adr = pdata->val[0] & 0x1F;
	phy_reg = pdata->val[2] & 0xFFFF;
	if (dev_adr)
		phy_reg |= MII_ADDR_C45 | (dev_adr << 16);
	return mdiobus_write(priv_data->mii, pdata->val[1], phy_reg,
			     pdata->val[3]);
#else
...
static const struct mac_api_cfg xgmac_cfg[] = {
	...
	{MAC_CLI_SET_MDIO_RD, 0, 0, 0, cli_mdio_rd, 0},
...
struct mac_api_cfg ss_cfg[] = {
	...
	{GSWSS_ADAP_MACSEC_RST, gswss_macsec_reset, 0, 0, 0, 0},
