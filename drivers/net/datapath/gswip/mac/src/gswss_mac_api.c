...
#define GSWSS_MAC_REG_POLL_INT_IN_US		1
#define GSWSS_MAC_REG_POLL_TIMEOUT_IN_US	5
...
int gswss_adap_reset(void *pdev, u32 reset)
{
	...
	mac_dbg("GSWSS: Resetting ADAP %d Module reset: %u\n",
		pdata->mac_idx, reset);
...
int gswss_set_1g_intf(void *pdev, u32 macif)
{
	...
	/* It takes time for hardware to complete the interface changing
	 * operation. Poll the MAC_IF_CFG register to check the interface
	 * changing operation is completed.
	 * Delay should be before the polling.
	 */
	if (read_poll_timeout_atomic(GSWSS_MAC_RGRD,
				     mac_if_cfg_n,
				     mac_if_cfg_n == mac_if_cfg,
				     GSWSS_MAC_REG_POLL_INT_IN_US,
				     GSWSS_MAC_REG_POLL_TIMEOUT_IN_US,
				     true,
				     pdata,
				     MAC_IF_CFG(pdata->mac_idx))) {
		mac_dbg("GSWSS: MACIF 1G set to %d failed\n", macif);
		ret = -1;
	}
...
int gswss_set_fe_intf(void *pdev, u32 macif)
{
	...
	if (read_poll_timeout_atomic(GSWSS_MAC_RGRD,
				     ...
				     pdata,
				     MAC_IF_CFG(pdata->mac_idx))) {
		mac_dbg("GSWSS: MACIF FE set to %d failed\n", macif);
		ret = -1;
	}
...
int gswss_set_2G5_intf(void *pdev, u32 macif)
{
	...
	if (read_poll_timeout_atomic(GSWSS_MAC_RGRD,
				     ...
				     pdata,
				     MAC_IF_CFG(pdata->mac_idx))) {
		mac_dbg("GSWSS: MACIF 2G5 set to %d failed\n", macif);
		ret = -1;
	}
...
int gswss_get_txtstamp_fifo(void *pdev, u32 record_id,
			    struct mac_fifo_entry *f_entry)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u32 mac_txtstamp;
...
int gswss_get_phy_mode(void *pdev)
{
        struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
        u16 phy_mode = 0;
...
int gswss_get_stat_duplex_mode(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u16 phy_stat = 0;
	u32 val;

	phy_stat = GSWSS_MAC_RGRD(pdata, PHY_STAT(pdata->mac_idx));
	val = MAC_GET_VAL(phy_stat, PHY_STAT, FDUP);
...
u8 gswss_get_stat_speed(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u16 phy_stat = 0;
	u8 speed_msb, speed_lsb, speed;
	...
	phy_stat = GSWSS_MAC_RGRD(pdata, PHY_STAT(pdata->mac_idx));
	speed_msb = MAC_GET_VAL(phy_stat, PHY_STAT, SPEEDMSB);
	speed_lsb = MAC_GET_VAL(phy_stat, PHY_STAT, SPEEDLSB);
	speed = ((speed_msb << 2) | speed_lsb);
...
int gswss_get_stat_linkstatus(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u16 phy_stat = 0;
	int linkst;
	...
	phy_stat = GSWSS_MAC_RGRD(pdata, PHY_STAT(pdata->mac_idx));
	linkst = MAC_GET_VAL(phy_stat, PHY_STAT, LSTAT);

	if (linkst == 0)
		mac_dbg("\tGSWSS: Link status is  DOWN\n");
	else if (linkst == 1)
		mac_dbg("\tGSWSS: Link status is UP\n");
...
u32 gswss_get_stat_flowctrl_tx(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u16 phy_stat = 0;
	u32 flow_ctrl_tx;
	...
	phy_stat = GSWSS_MAC_RGRD(pdata, PHY_STAT(pdata->mac_idx));
	flow_ctrl_tx = MAC_GET_VAL(phy_stat, PHY_STAT, TXPAUEN);
...
u32 gswss_get_stat_flowctrl_rx(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u16 phy_stat = 0;
	u32 flow_ctrl_rx;
	...
	phy_stat = GSWSS_MAC_RGRD(pdata, PHY_STAT(pdata->mac_idx));
	flow_ctrl_rx = MAC_GET_VAL(phy_stat, PHY_STAT, RXPAUEN);
...
int gswss_get_eee_cap(void *pdev)
{
	struct mac_prv_data *pdata = GET_MAC_PDATA(pdev);
	u32 aneg_eee;

	aneg_eee = GSWSS_MAC_RGRD(pdata, ANEG_EEE(pdata->mac_idx));
	return (int)MAC_GET_VAL(aneg_eee, ANEG_EEE, EEE_CAPABLE);
}
