...
GSW_return_t GSW_Debug_RMON_Port_Get(void *cdev, GSW_Debug_RMON_Port_cnt_t *parm)
{
	...
	u32 r_frameyellowRed = 0, r_unicastyellowRed = 0, r_multicastyellowRed = 0,
	    t_frameyellowRed = 0, t_unicastyellowRed = 0, t_multicastyellowRed = 0;
	u32 rgbcl = 0, rbbcl = 0, tgbcl = 0;
	u64 rgbch = 0, rbbch = 0, tgbch = 0;
	u32 rgbcl_yellowRed = 0, tgbcl_yellowRed = 0;
	u64 rgbch_yellowRed = 0, tgbch_yellowRed = 0;
	...
	if (Portmode == GSW_RMON_CTP_PORT_RX || Portmode == GSW_RMON_BRIDGE_PORT_RX) {
		...
		if (IS_VRSN_32(gswdev->gipver)) {
			count = RMON_COUNTER_OFFSET_GSWIP3_2;
		}
	} else {
		count = 14;
...
//One-time populate.
	...
	for (RmonCntOffset = 0; RmonCntOffset < count; RmonCntOffset++) {
		...
		if (Portmode == GSW_RMON_CTP_PORT_RX ||
			...
			case 0x0E: /* Receive Extended Vlan Discard Frame Count */
				...
				if (Portmode == GSW_RMON_CTP_PORT_RX) {
					GSW_CTP_portAssignment_t ctp_port_assign;
					GSW_QoS_queuePort_t qos_qport;
					u32 mac_id, mac_upbound;
					int ret;

					parm->nRxFCSErrorPkts = data1; /* Receive MAC/FCS Error frame Count */

					/* Overwrite nRxFCSErrorPkts with xgmac rmon rx_crc_error
					 * in case of pce bypass
					 */
					...
					mac_id = gswdev->ctpportconfig_idx[PortId].AssociatedLogicalPort;
					if (mac_id >= gswdev->tpnum)
						break;

					memset(&qos_qport, 0, sizeof(qos_qport));
					qos_qport.nPortId = mac_id;
					ret = GSW_QoS_QueuePortGet(cdev, &qos_qport);
					memset(&ctp_port_assign, 0, sizeof(ctp_port_assign));
					ctp_port_assign.nLogicalPortId = mac_id;
					ret |= GSW_CTP_PortAssignmentGet(cdev, &ctp_port_assign);

					if (!ret && qos_qport.bEnableIngressPceBypass &&
					    ctp_port_assign.nFirstCtpPortId == PortId) {
						if (VRSN_CMP(gswdev->gipver, LTQ_GSWIP_3_2) >= 0 &&
						    (mac_id >= MAC_2 && mac_id <= mac_upbound)) {
							/* Get from XGMAC */
							struct mac_rmon *rmon = NULL;
							struct mac_ops *ops = NULL;

							ops = gsw_get_mac_ops(0, mac_id);
							if (ops) {
								rmon = kzalloc(sizeof(*rmon), GFP_ATOMIC);
								if (rmon) {
									ops->rmon_get(ops, rmon);
									parm->nRxFCSErrorPkts = rmon->cnt[RMON_RX_CRC_ERR];
								}
								kfree(rmon);
							}
						}
			...
			case 0x16: /* Receive Good Byte Count for Yellow & Red (Low) */

				//if (Portmode == GSW_RMON_CTP_PORT_RX)
				rgbcl_yellowRed = data;


				//if (Portmode == GSW_RMON_CTP_PORT_RX)
				rgbch_yellowRed = data1; /* Receive Good Byte Count for Yellow & Red (High) */


				break;

			case 0x18: /* Receive Unicast Frame Count for Yellow & Red */

				//if (Portmode == GSW_RMON_CTP_PORT_RX)
				parm->nRxUnicastPktsYellowRed = r_unicastyellowRed = data;

				//if (Portmode == GSW_RMON_CTP_PORT_RX)
				parm->nRxMulticastPktsYellowRed = r_multicastyellowRed = data1; /* Receive Multicast Frame Count1 */

				break;

			case 0x1a: /* Receive Broadcast Frame count for Yellow & Red*/

				//if (Portmode == GSW_RMON_CTP_PORT_RX)
				parm->nRxBroadcastPktsYellowRed = r_frameyellowRed = data;
		...
		if (Portmode == GSW_RMON_CTP_PORT_TX ||
			...
			case 0x0E: /* Transmit Good byte Count for yellow & Red (Low)*/
				tgbcl_yellowRed = data;
				tgbch_yellowRed = data1; /* Transmit Good Byte Count for yellow & Red (High) */
				break;

			case 0x10: /* Transmit Unicast Count for Yellow & Red */
				parm->nTxUnicastPktsYellowRed = t_unicastyellowRed = data;
				parm->nTxMulticastPktsYellowRed = t_multicastyellowRed = data1; /* Transmit Multicast Frame Count for yellow & red */
				break;

			case 0x12: /* Transmit BroadCast Count for Yellow & Red */
				parm->nTxBroadcastPktsYellowRed = t_frameyellowRed = data;
				break;
			}
		}
	...
	if (IS_VRSN_32(gswdev->gipver)) {
		if (Portmode == GSW_RMON_CTP_PORT_RX) {
			...
			for (RmonCntOffset = 0x12; RmonCntOffset < 0x14; RmonCntOffset++) {
				bmtable.adr.rmon.counterOffset = RmonCntOffset;
				gsw_bm_table_read(cdev, &bmtable);
				data = (bmtable.value[1] << 16) | (bmtable.value[0] & 0xFFFF);
				...
				case 0x13: /* Receive Discard (Tail-Drop) Frame Count */
	...
	if (Portmode == GSW_RMON_CTP_PORT_RX ||
		...
		if (IS_VRSN_32(gswdev->gipver)) {
			parm->nRxGoodBytesYellowRed = (u64)(((rgbch_yellowRed & 0xFFFFFFFF) << 32) | (rgbcl_yellowRed & 0xFFFFFFFF));
			parm->nRxGoodPktsYellowRed = r_frameyellowRed + r_unicastyellowRed + r_multicastyellowRed;
		}
	}
	...
	if (Portmode == GSW_RMON_CTP_PORT_TX ||
		...
		if (IS_VRSN_32(gswdev->gipver)) {
			parm->nTxGoodBytesYellowRed = (u64)(((tgbch_yellowRed & 0xFFFFFFFF) << 32) | (tgbcl_yellowRed & 0xFFFFFFFF));
			parm->nTxGoodPktsYellowRed = t_frameyellowRed + t_unicastyellowRed + t_multicastyellowRed;
		}
	}
...
GSW_return_t GSW_Debug_TunnelTemplateTableStatus(void *cdev, GSW_debug_t *parm)
{
	...
	if (parm->nCheckIndexInUse) {
		for (j = 0; j < gswdev->num_of_pbbtunnel; j++) {
			if (gswdev->tunnelTemplate_idx[j].IndexInUse) {
				printk("tunnelTemplate Table Index  %d = InUse\n", j);
			}
		}
	...
	if (parm->nForceSet) {
		gswdev->tunnelTemplate_idx[parm->nTableIndex].IndexInUse = 1;
		printk("tunnelTemplate Table Index  %d is Forced to InUSe\n", parm->nTableIndex);
	} else {
		printk("tunnelTemplate Table Index %d:\n\n", parm->nTableIndex);
		printk("IndexInUse					= %d\n", gswdev->tunnelTemplate_idx[parm->nTableIndex].IndexInUse);
	}
...
#if defined(WIN_PC_MODE) && WIN_PC_MODE
GSW_return_t GSW_MacsecCfg(void *cdev, GSW_MACSec_cfg_t *macsec_cfg)
{
	u8 *argv[10];
	int i = 0;
	ethsw_api_dev_t *gswdev = GSW_PDATA_GET(cdev);
...
#if defined(WIN_PC_MODE) && WIN_PC_MODE
GSW_return_t GSW_PmacbrCfg(void *cdev, GSW_MAC_cfg_t *pmac_cfg)
{
	u8 *argv[10];
	int i = 0;
	ethsw_api_dev_t *gswdev = GSW_PDATA_GET(cdev);
	...
	if (VRSN_CMP(gswdev->gipver, LTQ_GSWIP_3_2) < 0) {
		pr_err("Pmac Br Cfg Supported only in GSWIP3.2 and above\n");
		return GSW_statusErr;
	}

	for (i = 0; i < pmac_cfg->argc; i++)
		argv[i] = pmac_cfg->argv[i];

	pmac_br_main(pmac_cfg->argc, argv);
