...
int phy_ethtool_get_eee(struct phy_device *phydev, struct ethtool_eee *data)
{
	...
	if (val & MDIO_EEE_2_5GT)
		data->supported |= SUPPORTED_2500baseX_Full;
	...
	if (val < 0)
		return val;
	if (val & MDIO_EEE_2_5GT)
		data->advertised |= SUPPORTED_2500baseX_Full;
...
int phy_ethtool_set_eee(struct phy_device *phydev, struct ethtool_eee *data)
{
	...
	cap2 = phy_read_mmd(phydev, MDIO_MMD_PCS, MDIO_PCS_EEE_ABLE2);
	if (cap2 > 0)
		cap2 &= MDIO_EEE_2_5GT;
	...
	if (cap2 > 0) {
		old_adv2 = phy_read_mmd(phydev, MDIO_MMD_AN, MDIO_AN_EEE_ADV2);
		if (old_adv2 < 0)
			return old_adv2;
	}
	...
	if (data->eee_enabled) {
		...
		adv2 &= ~(phydev->eee_broken_modes & MDIO_EEE_2_5GT);
	}
	...
	if (old_adv2 != adv2) {
		ret = phy_write_mmd(phydev, MDIO_MMD_AN, MDIO_AN_EEE_ADV2,
				    adv2);
		if (ret < 0)
			return ret;
		changed++;
