...
static int check_pppoe_addr_valid(struct net_device *dev, struct pppoe_addr *pa)
{
	...
	if ((!pa->dev[0]) || is_zero_ether_addr(pa->remote) || (pa->sid == 0))
		return ret;
	...
	if (pppoe_netdev) {
		...
		if (!po) {
			pr_err("Cannot find the pppoe addr in hash table\n");
			goto err;
		}
		if (!po->pppoe_dev || !(po->pppoe_pa.remote[0] |
			...
			pr_err("no pppoe device or remote address is zero\n");
			goto err;
		}
		ret = 0;
	}
...
static int pppoe_post_connect(struct ppp_channel *chan, struct net_device *dev,
			      bool is_connect)
{
	...
	/* derive features from lower device */
	if (po->pppoe_dev->hw_enc_features & NETIF_F_HW_ESP) {
		if (is_connect) {
			dev->features |= NETIF_F_HW_ESP;
			dev->hw_enc_features |= NETIF_F_HW_ESP;
			dev->vlan_features |= NETIF_F_HW_ESP;
			dev->xfrmdev_ops = po->pppoe_dev->xfrmdev_ops;
		} else {
			dev->features &= ~NETIF_F_HW_ESP;
			dev->hw_enc_features &= ~NETIF_F_HW_ESP;
			dev->vlan_features &= ~NETIF_F_HW_ESP;
...
static const struct ppp_channel_ops pppoe_chan_ops = {
	...
	.post_connect = pppoe_post_connect,
};
