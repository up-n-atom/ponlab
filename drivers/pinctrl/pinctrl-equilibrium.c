			     ...
			     unsigned int offset)
{
	...
	eqbr_cfg_bit(gctrl->membase + GPIO_EXINTCR1, offset, type->edge_type);
...
static int gpiolib_reg(struct eqbr_pinctrl_drv_data *drvdata)
{
	...
	for (i = 0; i < drvdata->nr_gpio_ctrls; i++) {
		...
		ret = bgpio_init(&gctrl->chip, dev, 4,
...
static void eqbr_pin_dbg_show(struct pinctrl_dev *pctldev,
			      struct seq_file *s, unsigned int pin)
{
	...
	if (!(bank->aval_pinmap & BIT(offset))) {
		seq_puts(s, "pin reserved by chip");
		return;
	}

	pinmux = readl(bank->membase + (offset * 4));

	if (!pinmux)
		seq_puts(s, "pinmux: GPIO ");
	else
		seq_printf(s, "pinmux: %u ", pinmux);
}
			       ...
			       unsigned int selector, unsigned int group)
{
	...
	struct eqbr_pmx_mux *mux;
	struct eqbr_grp_vals *vals;
	...
	mux = grp->data;
	if (!mux) {
		dev_err(pctldev->dev, "%s: mux setting is empty!\n", func->name);
		return -EINVAL;
	}

	vals = mux->vals;
	for (i = 0; i < mux->nvals; i++) {
		unsigned int val;

		regmap_read(vals[i].map, vals[i].reg, &val);
		val &= ~vals[i].mask;
		val |= vals[i].val;
		regmap_write(vals[i].map, vals[i].reg, val);
		dev_dbg(pctldev->dev, "write reg: 0x%x, mask: 0x%x, val: 0x%x, set val: 0x%x\n",
			vals[i].reg, vals[i].mask, vals[i].val, val);
	}

	for (i = 0; i < grp->num_pins; i++) {
		if (i >= mux->mux_nr)
			pinmux = mux->pinmux[mux->mux_nr - 1];
		else
			pinmux = mux->pinmux[i];
		eqbr_set_pin_mux(pctl, pinmux, grp->pins[i]);
	}
...
static int funcs_utils(struct device *dev, struct eqbr_pmx_func *funcs,
		       unsigned int *nr_funcs, unsigned int op)
{
...
static int eqbr_build_groups(struct eqbr_pinctrl_drv_data *drvdata)
{
	struct device *dev = drvdata->dev;
	struct device_node *node = dev->of_node;
	const char *name = "topmux", *pinmux_name = "pinmux";
	unsigned int *pinmux;
	struct eqbr_pmx_mux *pmx_mux;
	struct eqbr_grp_vals *vals;
	...
	for_each_child_of_node(node, np) {
		...
		group.num_pins = of_property_count_u32_elems(np, "intel,pins");
		if (group.num_pins < 0) {
			dev_err(dev, "No intel,pins in the group: %s\n", group.name);
			of_node_put(np);
			return -EINVAL;
		}
		...
		pmx_mux = devm_kzalloc(dev, sizeof(*pmx_mux), GFP_KERNEL);
		if (!pmx_mux)
			goto alloc_err;

		if (of_property_read_u32_array(np, "intel,pins", group.pins, group.num_pins)) {
		...
		cnt = of_property_count_u32_elems(np, pinmux_name);
		pinmux = devm_kcalloc(dev, cnt, sizeof(*pinmux), GFP_KERNEL);
		if (!pinmux)
			goto alloc_err;

		if (of_property_read_u32_array(np, pinmux_name, pinmux, cnt)) {
			dev_err(dev, "Group %s: Read pinmux id failed\n", group.name);
			of_node_put(np);
			return -EINVAL;
		}
		pmx_mux->pinmux = pinmux;
		pmx_mux->mux_nr = cnt;

		rows = pinctrl_count_index_with_args(np, name);
		if (rows >= 0 && drvdata->map) {
			vals = devm_kcalloc(dev, rows,
					    sizeof(*vals), GFP_KERNEL);
			if (!vals)
				goto alloc_err;
			...
			for (i = 0; i < rows; i++) {
				...
				if (pinctrl_parse_index_with_args(np, name, i, &pinctrl_spec))
					break;

				/* format: register offset, mask, value */
				if (pinctrl_spec.args_count != 3) {
					dev_err(dev, "invalid count %d for %s @ %d\n",
						pinctrl_spec.args_count, group.name, i);
					break;
				}
				vals[i].map  = drvdata->map;
				vals[i].reg  = pinctrl_spec.args[0];
				vals[i].mask = pinctrl_spec.args[1];
				vals[i].val  = pinctrl_spec.args[2];
				pmx_mux->nvals += 1;
				dev_dbg(dev, "add reg: 0x%0x, mask: 0x%x, val: 0x%x\n",
					vals[i].reg, vals[i].mask, vals[i].val);
			}
		}
...
static int eqbr_pinctrl_probe(struct platform_device *pdev)
{
	...
	/* attach topmux register if any */
	drvdata->map = syscon_regmap_lookup_by_phandle_optional(dev->of_node,
								"pinctrl,syscon");
...
static int __init eqbr_pinctrl_drv_register(void)
{
	return platform_driver_register(&eqbr_pinctrl_driver);
}
subsys_initcall(eqbr_pinctrl_drv_register);
