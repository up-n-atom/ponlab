...
#if defined(CONFIG_MTD_UBI_GLUEBI_ROOTFS_DEV)
...
#if defined(CONFIG_MTD_UBI_GLUEBI_ROOTFS_DEV)
static char *rootfsname;
module_param(rootfsname, charp, 0);
MODULE_PARM_DESC(rootfsname, "Specify rootfs using gluebi volume name");
#endif
/**
...
#if defined(CONFIG_MTD_UBI_GLUEBI_ROOTFS_DEV)
	if (rootfsname) {
		pr_debug("Gluebi: Found kernel commandline option 'rootfsname=%s'\n",
			 rootfsname);

		if (!strncmp(mtd->name, rootfsname, vi->name_len)) {
			ROOT_DEV = MKDEV(MTD_BLOCK_MAJOR, mtd->index);
			pr_info("Gluebi: mtd '%s' set to be root filesystem\n",
				mtd->name);
			pr_debug("rootdev: 0x%08x, mtd block: %08x, mtd index: %08x\n",
