...
#define HSNAND_PARA0_BCH_BE	BIT(28)
...
static unsigned char ebu_read_byte(struct nand_chip *chip)
{
	return ebu_nand_readb(chip);
}
...
static void ebu_select_chip(struct nand_chip *chip, int select)
{
	struct ebu_nand_controller *ebu_host = nand_get_controller_data(chip);
	...
	switch (select) {
		case -1:
			...
			pm_runtime_mark_last_busy(ebu_host->dev);
			pm_runtime_put_autosuspend(ebu_host->dev);
		break;
		case 0:
			pm_runtime_get_sync(ebu_host->dev);
...
static int ebu_dev_ready(struct nand_chip *chip)
{
	struct ebu_nand_controller *ebu_host = nand_get_controller_data(chip);

	return readl(ebu_host->ebu + EBU_WAIT) & EBU_WAIT_RDBY;
}

static void ebu_cmd_ctrl(struct nand_chip *chip, int cmd, unsigned int ctrl)
{
	if (cmd == NAND_CMD_NONE)
		return;

	if (ctrl & NAND_CLE)
		ebu_nand_writeb(chip, HSNAND_CLE_OFFS | HSNAND_CS_OFFS, cmd);
	else if (ctrl & NAND_ALE)
		ebu_nand_writeb(chip, HSNAND_ALE_OFFS | HSNAND_CS_OFFS, cmd);
}
#ifdef LEGACY_NO_SUPPORT
			 ...
			 const u8 *buf, u32 len)
{
	...
	if (virt_to_phys((u8 *)buf) > 0xFFFFFFFF) {
		tmp_buf = kzalloc(len, GFP_KERNEL | GFP_DMA);
		if (!tmp_buf)
			return -ENOMEM;

		if (virt_to_phys(tmp_buf) > 0xFFFFFFFF) {
			dev_err(ebu_host->dev, "Couldn't allocate DMA buffer lower than \
					32 Bits., buf=0x%p\n", tmp_buf);
			ret = -EINVAL;
			goto free_buf;
		}
	...
	if (dir == DMA_DEV_TO_MEM) {
		...
		if (tmp_buf)
			memcpy(tmp_buf, (u8 *)buf, len);
	}

	if (tmp_buf)
		sg_init_one(&sg, tmp_buf, len);
	else
		sg_init_one(&sg, (u8 *)buf, len);

	nents = dma_map_sg(chan->device->dev, &sg, 1, dir);
	if (nents != 1) {
	...
	tx = dmaengine_prep_slave_sg(chan, &sg, nents, dir, flags);
	if (!tx) {
		dev_err(ebu_host->dev, "Could not prepare DMA S/G list\n");
		return -ENXIO;
	}

	tx->callback = callback;
	tx->callback_param = (void *)ebu_host;
	...
	if (dir == DMA_DEV_TO_MEM && tmp_buf)
		memcpy((u8 *)buf, tmp_buf, len);
			     ...
			     int page, u32 cmd)
{
	...
	writel(cmd | (page & 0xFF) << HSNAND_CTL1_ADDR_SHIFT,
			ebu_host->hsnand + HSNAND_CTL1);
	writel((page & 0xFF0000) >> 8 | (page & 0xFF00) >> 8 |
	       HSNAND_CTL2_CYC_N_V5, ebu_host->hsnand + HSNAND_CTL2);
				     ...
				     int oob_required, int page)
{
	...
	ret = readl_poll_timeout(int_sta, stat, stat & HSNAND_INT_STA_WR_C,
								200, 10000);
	if (ret)
		return ret;
...
static int ebu_nand_attach_chip(struct nand_chip *chip)
{
	...
	bch_alg = -1;
	for (i = bch_alg_start; i < (1 + (bch_alg_start >> 2)) * 4; i++) {
		if (ecc_strength_ds == ecc_strength[i]) {
			bch_alg = i;
			break;
		}
	}

	chip->ecc.strength = ecc_strength_ds;
	if (bch_alg < 0)
		return -EINVAL;
	...
	ebu_host->nd_para0 = pagesize | pg_per_blk | HSNAND_PARA0_BYP_EN_NP |
			     ...
			     HSNAND_PARA0_TYPE_ONFI | HSNAND_PARA0_BCH_BE |
			     (bch_alg << 29);
...
#ifdef CONFIG_PM
static int ebu_runtime_suspend(struct device *dev)
{
    struct ebu_nand_controller *ebu_host = dev_get_drvdata(dev);
...
static int ebu_runtime_resume(struct device *dev)
{
    struct ebu_nand_controller *ebu_host = dev_get_drvdata(dev);

    return clk_prepare_enable(ebu_host->clk);
}
#endif

static const struct dev_pm_ops ebu_dev_pm_ops = {
    SET_RUNTIME_PM_OPS(ebu_runtime_suspend, ebu_runtime_resume, NULL)
};
...
static int ebu_nand_probe(struct platform_device *pdev)
{
	...
	writel(EBU_BUSCON_CMULT_V4 | EBU_BUSCON_RECOVC(1) |
	       ...
	       EBU_BUSCON_SETUP_EN, ebu_host->ebu + EBU_BUSCON(cs));

	/* Need check carefully, it should be NAND physical address */
	writel(0x17400051, ebu_host->ebu + EBU_ADDR_SEL(0));
	writel(0x17C00051, ebu_host->ebu + EBU_ADDR_SEL(cs));
	...
	ebu_host->chip.legacy.read_byte = ebu_read_byte;
	ebu_host->chip.legacy.read_buf = ebu_read_buf;
	ebu_host->chip.legacy.write_buf = ebu_write_buf;
	ebu_host->chip.legacy.select_chip = ebu_select_chip;
	ebu_host->chip.legacy.dev_ready = ebu_dev_ready;
	ebu_host->chip.legacy.cmd_ctrl = ebu_cmd_ctrl;
	ebu_host->chip.legacy.chip_delay = 30;
