...
static void ptn5150_check_state(struct ptn5150_info *info)
{
	...
	unsigned int port_status, reg_data, polarity;
	int ret;
	bool host = false;
	bool dev = false;
	...
	polarity = FIELD_GET(PTN5150_REG_CC_POLARITY, reg_data);
	vbus.intval = FIELD_GET(PTN5150_REG_CC_VBUS_DETECTION, reg_data);
	...
	case PTN5150_UFP_ATTACHED:
		host = true;
		flipped.intval = polarity == POLARITY_CC2;
		dev_dbg(info->dev, "cc is %s\n", flipped.intval ? "reversed" : "normal");
		extcon_set_property(info->edev, EXTCON_USB_HOST,
				    EXTCON_PROP_USB_TYPEC_POLARITY, flipped);
		break;
	default:
		break;
	}
	...
	extcon_set_property(info->edev, EXTCON_USB_HOST,
			    EXTCON_PROP_USB_VBUS, vbus);
	gpiod_set_value_cansleep(info->vbus_gpiod, !vbus.intval);
