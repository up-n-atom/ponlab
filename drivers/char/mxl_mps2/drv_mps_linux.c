...
extern irqreturn_t tep2atom_irq(int32_t irq, void * pDev);
...
int32_t mps_open (struct inode * inode, struct file * file_p)
{
	...
	/* a trick: VMMC driver passes the first parameter as a value So, if inode value is
	   ...
	   kernel space. */
	if (((uintptr_t) inode > 0) && ((uintptr_t) inode < LQ_MPS_TOTAL_FD)) {
		from_kernel = 1;
		num = (uintptr_t) inode;
	}
	else {
	...
	if ((SUCCESS) == mps_common_open (pDev, pMBDev, from_kernel))
	{
		if (!from_kernel) {
...
int32_t mps_close (struct inode * inode, struct file * file_p)
{
	...
	if (NULL != pMBDev) {
		...
		TRACE_MPS (MPS2, DBG_LEVEL_HIGH, ("MPS2 ERROR: pMBDev pointer is NULL!\n"));
		return -ENODEV;
	}
}
...
static uint32_t mps_poll (struct file *file_p, poll_table * wait)
{
	...
	if (mpsexcpt) {
		mpsexcpt = 0;
		TRACE_MPS (MPS2, DBG_LEVEL_LOW, ("waking up sleeping process\n"));
		/* upstream queue */
 ...
 * \ingroup API
 */
long mps_ioctl (struct file *file_p, uint32_t nCmd, ulong arg)
{
	...
	/* a trick: VMMC driver passes the first parameter as a value of
	   ...
	   kernel space. */
	if (((uintptr_t) file_p > 0) && ((uintptr_t) file_p <= LQ_MPS_TOTAL_FD)) {
		from_kernel = 1;
	...
	switch (nCmd)
	{
		case FIO_MPS_MB_READ:
			...
			if (from_kernel) {
				...
				if (0 < copy_from_user (&rw_struct, (void *) arg, sizeof (mps_message))) {
					TRACE_MPS (MPS2, DBG_LEVEL_HIGH,
				...
				if (CHECK_PTR(rw_struct.header.Hd.param_attr, 0)) {
					if (0 < copy_to_user ((uint32_t *)(uintptr_t)UserBuf[0], (uint32_t *)(uintptr_t)rw_struct.data[0], rw_struct.data[1])) {
						TRACE_MPS (MPS2, DBG_LEVEL_HIGH,
								(KERN_ERR "[%s %s %d]: copy_to_user error\r\n", __FILE__,
								 __func__, __LINE__));
					}
					mps_bufman_free ((void *)(uintptr_t)rw_struct.data[0]);
				}

				/* ... and finally restore the buffer pointer and copy mps_message
				   back! */
				memcpy(rw_struct.data, UserBuf, 16);
				if (0 < copy_to_user ((void *) arg, &rw_struct, sizeof (mps_message)))
				{
					TRACE_MPS (MPS2, DBG_LEVEL_HIGH, (KERN_ERR "[%s %s %d]: copy_to_user error\r\n", __FILE__, __func__, __LINE__));
				}
			}
			break;
		case FIO_MPS_MB_WRITE:
			...
			else
			{
				...
				if (0 < copy_from_user (&rw_struct, (void *) arg, sizeof (mps_message)))
				{
					TRACE_MPS (MPS2, DBG_LEVEL_HIGH, (KERN_ERR "[%s %s %d]: copy_from_user error\r\n", __FILE__, __func__, __LINE__));
				}
				...
				if (CHECK_PTR(rw_struct.header.Hd.param_attr, 0)) {
					...
					if (0 < copy_from_user ((uint32_t *)(uintptr_t)rw_struct.data[0], (uint32_t *)(uintptr_t)UserBuf[0], rw_struct.data[1])) {
						TRACE_MPS (MPS2, DBG_LEVEL_HIGH, (KERN_ERR "[%s %s %d]: copy_from_user error\r\n", __FILE__, __func__, __LINE__));
					}
				}
				if ((retvalue =
		...
		case FIO_MPS_GETVERSION:
			if (from_kernel) {
				...
				if (0 < copy_to_user ((void *) arg, mps_device_version, strlen (mps_device_version)))
				{
					TRACE_MPS (MPS2, DBG_LEVEL_HIGH, (KERN_ERR "[%s %s %d]: copy_to_user error\r\n", __FILE__, __func__, __LINE__));
				}
			}
...
static int32_t mps_get_mailbox_proc (struct seq_file *s)
{
	...
	seq_printf (s, " (wr:0x%08x, rd: 0x%08x)\n",
			(unsigned int)(uintptr_t) mps_dev.upstrm_fifo.pend +
			(unsigned int)(uintptr_t) * mps_dev.upstrm_fifo.pwrite_off,
			(unsigned int)(uintptr_t) mps_dev.upstrm_fifo.pend +
			(unsigned int)(uintptr_t) * mps_dev.upstrm_fifo.pread_off);
	for (i = 0; i < mps_dev.upstrm_fifo.size; i += 16)
	{
		seq_printf (s, "   0x%08x: %08x %08x %08x %08x\n",
				(unsigned int)(uintptr_t) (mps_dev.upstrm_fifo.pend + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.upstrm_fifo.pend + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.upstrm_fifo.pend + 1 + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.upstrm_fifo.pend + 2 + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.upstrm_fifo.pend + 3 + (i / 4)));
	}

	seq_printf (s, "\n * VOICE * DO");
	seq_printf (s, " (wr: 0x%08x, rd: 0x%08x)\n",
			(unsigned int)(uintptr_t) mps_dev.dwstrm_fifo.pend +
			(unsigned int)(uintptr_t) * mps_dev.dwstrm_fifo.pwrite_off,
			(unsigned int)(uintptr_t) mps_dev.dwstrm_fifo.pend +
			(unsigned int)(uintptr_t) * mps_dev.dwstrm_fifo.pread_off);
	for (i = 0; i < mps_dev.dwstrm_fifo.size; i += 16)
	{
		seq_printf (s, "   0x%08x: %08x %08x %08x %08x\n",
				(unsigned int)(uintptr_t) (mps_dev.dwstrm_fifo.pend + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.dwstrm_fifo.pend + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.dwstrm_fifo.pend + 1 + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.dwstrm_fifo.pend + 2 + (i / 4)),
				(unsigned int)(uintptr_t)*(mps_dev.dwstrm_fifo.pend + 3 + (i / 4)));
	}
...
static void mps_proc_entrycreate ( struct proc_dir_entry *parent_node, struct proc_entry *proc_entry)
{
	memset(&proc_entry->ops, 0, sizeof(struct proc_ops));
	proc_entry->ops.proc_open	= 	mps_proc_open;
...
static int mps_internal_init(void)
{
	int32_t result;
	int32_t i;
#if !defined(SYSTEM_ATOM)
	unsigned int virt;
#endif
	...
	{
		TRACE_MPS (MPS2, DBG_LEVEL_HIGH, ("MPS2: cannot create proc entry\n"));
	}
#endif /* */
	return SUCCESS;
}
...
int ltq_mps_probe(struct platform_device *pdev)
{
	...
	struct resource      *res = NULL, mbx_res;
	int uiFlag;
	uint32_t mps_irq;
	int tep2atom = 0, error = 0;

	/* Baseaddress of the MPS register block. */
	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (res) {
		mps_reg_base = (uintptr_t)devm_ioremap_resource(&pdev->dev, res);
		if (IS_ERR((void *)mps_reg_base))
			return PTR_ERR((void *)mps_reg_base);
	}

	tep2atom = platform_get_irq(pdev, 0);
	if (tep2atom < 0) {
		dev_err(&pdev->dev, "failed to get keypad irq\n");
		return -ENXIO;
	}
	/* Note: Currently U-Boot is not clearing the pending IRQ
	 * of ICC communication between TEP and U-Boot.
	 * So here we are ensuring that IRQ line should be cleared
	 * before registering tep2atom IRQ handler irrespective of
	 * TEP and U-Boot communication.
	 */
	mps_irq = *MPS_REG_GIRDR;
	*MPS_REG_GICR = mps_irq;

	error = devm_request_irq(&pdev->dev, tep2atom , tep2atom_irq, 0, "tep2atom", &mps_dev);
	if (error) {
	...
	pdn = of_parse_phandle(pdev->dev.of_node, "mxl,mbx", 0);
	if (pdn) {
		if (of_address_to_resource(pdn, 0, &mbx_res) == 0) {
//			mps_ram_base = (uintptr_t)devm_ioremap_resource(&pdev->dev, &mbx_res);
			mps_ram_base = (uintptr_t)devm_ioremap(&pdev->dev, mbx_res.start, resource_size(&mbx_res));
			uiFlag = 1;
			if (IS_ERR((void *)mps_ram_base)) {
				of_node_put(pdn);
				return PTR_ERR((void *)mps_ram_base);
			}
		}
		of_node_put(pdn);
	}
...
#ifdef VPE0
		if (!mps_ir4) {
			TRACE_MPS ( MPS2, DBG_LEVEL_HIGH, (KERN_ERR "Failed to find MPS2 AD0 irq in device tree.\n"));
		}
#endif
		if (!mps_reg_base) {
			TRACE_MPS ( MPS2, DBG_LEVEL_HIGH, (KERN_ERR "Failed to find MPS2 reg base in device tree.\n"));
		}
		if (!mps_ram_base) {
			TRACE_MPS ( MPS2, DBG_LEVEL_HIGH, (KERN_ERR "Failed to find MPS2 mbx base in device tree.\n"));
		}

		return -ENXIO;
	}

	if (mps_internal_init()) {
		TRACE_MPS ( MPS2, DBG_LEVEL_HIGH, (KERN_ERR "MPS2 Internal init failed !! .\n"));
		return -ENXIO;
	}

	if (icc_init_module()) {
		TRACE_MPS ( MPS2, DBG_LEVEL_HIGH, (KERN_ERR "ICC init failed !! .\n"));
		return -ENXIO;
	}

	TRACE_MPS ( MPS2, DBG_LEVEL_HIGH, (KERN_ERR "ALL OK .\n"));
	return 0;
}

static const struct of_device_id ltq_mps_match[] = {
  { .compatible = "mxl,lgm-mps2" },
  {},
};
MODULE_DEVICE_TABLE(of, ltq_mps_match);
...
static struct irqaction mps_irqaction = {
      .handler        = mps_ad0_irq,
      .flags          = 0, /* for disable nested interrupts */
...
*/
static int __init mps_init_module(void)
{
	...
	TRACE_MPS (MPS2, DBG_LEVEL_HIGH,
			("%s%s, (c) 2017 - 2020 Intel Corporation, (c) 2020 - 2022 MaxLinear, Inc.  \n", MPS_INFO_STR,
			 mps_device_version));

	sprintf (mps_dev_name, MPS_DEV_NAME);

	pdev = platform_device_register_simple("mxl_mps2", -1, NULL, 0);
	if (IS_ERR(pdev))
		return PTR_ERR(pdev);

	return platform_driver_register(&ltqmps_driver);
}
...
{
#ifdef SYSTEM_ATOM
	tep2atom_disable_mailbox_int();
	tep2atom_disable_all_int();
#else
	...
	TRACE_MPS (MPS2, DBG_LEVEL_HIGH, (KERN_INFO "Lantiq MPS2 driver: cleanup done\n"));
}

arch_initcall(mps_init_module);
