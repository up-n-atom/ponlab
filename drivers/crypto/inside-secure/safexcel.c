...
static int eip197_load_firmwares(struct safexcel_crypto_priv *priv)
{
	...
	if (priv->version == EIP197D_MRVL)
		...
		dir = "eip197b";
	else if (priv->version == EIP197_MXL)
		dir = "";
	else
		return -ENODEV;
...
static int safexcel_hw_init(struct safexcel_crypto_priv *priv)
{
	...
	if (priv->flags & SAFEXCEL_HW_EIP197)
		writel(EIP197_DRBG_CTRL_ENABLE, priv->base + EIP197_DRBG_CTRL);
...
finalize:
	...
	if (!priv->ring[ring].busy) {
		...
		cpu_latency_qos_update_request(&priv->pm_qos_req, 0);
	}
...
requests_left:
	...
	if (!priv->ring[ring].requests) {
		...
		cpu_latency_qos_update_request(&priv->pm_qos_req,
...
static int safexcel_probe(struct platform_device *pdev)
{
	...
	cpu_latency_qos_add_request(&priv->pm_qos_req,
				    PM_QOS_CPU_LATENCY_DEFAULT_VALUE);
...
err_reg_clk:
	cpu_latency_qos_remove_request(&priv->pm_qos_req);
