 ...
 * @width:	Number of bits for this field (or 0 if not avail)
 * @shift:	Bit offset within @reg of this field
 */
 ...
 * @sel_3p3_1p8:	Where to find SEL_3P3_1P8 (voltage select), if any
 *
...
struct sdhci_arasan_soc_ctl_map {
	...
	struct sdhci_arasan_soc_ctl_field	sel_3p3_1p8;
};
...
/* Force HS clock in HS timing */
#define SDHCI_ARASAN_QUIRK_FORCE_HS_TIMING_CLOCK BIT(3)
};
...
static const struct sdhci_arasan_soc_ctl_map intel_lgm_sdxc_soc_ctl_map = {
	...
	.sel_3p3_1p8 = { .reg = 0x1b4, .width = 1, .shift = 8 },
};
				   ...
				   u32 val)
{
	...
	/*
	 * Silently return errors for width == 0 so caller doesn't have
	...
	if (sdhci_arasan->soc_ctl_map->hiword_update)
		ret = regmap_write(soc_ctl_base, reg,
				   HIWORD_UPDATE(val, GENMASK(width - 1, 0),
						 shift));
	else
		ret = regmap_update_bits(soc_ctl_base, reg,
					 GENMASK(shift + width - 1, shift),
					 val << shift);
...
static void sdhci_arasan_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (sdhci_arasan->quirks &
	    SDHCI_ARASAN_QUIRK_FORCE_HS_TIMING_CLOCK) {
		/* some cards no longer respond to command if we switch
		 * to high speed timing with low clock. To overcome this
		 * we increase the clock to the minimum HS during timing
		 * switch.
		 */
		if (host->timing > MMC_TIMING_LEGACY &&
		    clock <= PHY_CLK_TOO_SLOW_HZ) {
			pr_debug("%s: force clock at HS timing change\n",
...
/**
 * sdhci_arasan_voltage_switch_extension - Additional steps for voltage switch
 *
 * This is used to perform additional needed steps during voltage switching.
 * It will be called by sdhci_voltage_switch()
 *
 * @host:		Pointer to sdhci host
 */
static void sdhci_arasan_voltage_switch_extension(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_arasan_data *sdhci_arasan = sdhci_pltfm_priv(pltfm_host);
	const struct sdhci_arasan_soc_ctl_map *soc_ctl_map =
	...
	/* set 3.3v/1.8v selector bit */
	fld = &soc_ctl_map->sel_3p3_1p8;
	if (!fld->width)
		return;

	if (sdhci_readw(host, SDHCI_HOST_CONTROL2) & SDHCI_CTRL_VDD_180)
		vdd_180 = 1;

	regmap_update_bits(sdhci_arasan->soc_ctl_base, fld->reg,
			   GENMASK(fld->shift + fld->width, fld->shift),
			   vdd_180 << fld->shift);
}
...
static const struct sdhci_ops sdhci_arasan_ops = {
	...
	.voltage_switch = sdhci_arasan_voltage_switch_extension,
};
...
/**
 * sdhci_intel_lgm_emmc_sdcardclk_set_phase - Set the SD Output Clock Tap Delays
 *
 * @hw:			Pointer to the hardware clock structure.
 * @degrees:		The clock phase shift between 0 - 359.
 *
...
static int sdhci_arasan_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(np, "no-mmc-hs"))
