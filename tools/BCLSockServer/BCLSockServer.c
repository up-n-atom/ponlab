...
void BCL_SclRemoveEOL(MT_UBYTE* cDataArray) 
{
    ...
    if (nLastChar > MT_BUFFERSIZE) {
        ...
        // TODO: signal to caller that an error occurred
        return;
    }
    if (cCR != NULL) {
        *cCR = '\0'; 
...
void BCL_HandlerRx(MT_UBYTE* cDataArray, int lDataLength) 
{
    ...
    // Remove SCL CR from end of command before calling BCL interpreter
...
//void BCL_RxSocketsrv(void) 
{
    ...
    /* Non blocking socket.
       The accept() and recv() are not blocking, but immediately get the status and continue in the function flow */
...
*   Algorithm: 
            ...
            If there is old active task:
               ...
               (local variable) in addition to the global lNewMsgSocket variable.
...
MT_THREAD_RET_T BCL_RxMsgSocket (void* arg)
{
    ...
    while(!BCL_nExit)
    {   
        /* Get the free space in Rx buffer (allow for null terminator)*/
        unRxFreeSpace = MT_BUFFERSIZE - 1; 
        ...
        if (lCurrentMsgSocket == SOCKET_INVALID)
        {
          ...
          while (lResultStatus == -2 && lCurrentMsgSocket == lNewMsgSocket && lCurrentMsgSocket != SOCKET_INVALID && !BCL_nExit) 
          {
              ...
              else if (lResultStatus != -1) 
              {
                  ...
                  {
                      /* Discard buffer contents and start again */
                      mt_print(1,"\n\n\nBuffer is full and does not contain a valid command (received %s)\n\n\n   ", cDataArray);                    
                  }
...
#endif /* _STANDALONE */
{
    ...
    BCL_Exit();
    /* close all sockets to insure exit from sleeping */
    close(BCL_nListenSocket);
    close(lCurrentMsgSocket);
    close(lNewMsgSocket);
