...
static uintmax_t asciiToUintmax_t(char *pAscii, char delim, int *error_occured) {
	char  ucDigit;
	uintmax_t unValue = 0;
	eNUMBER_TYPE type;
...
MT_UINTPTR asciiToUintptr(char * pAscii, char delim, int *error_occured)
{
	uintmax_t value = asciiToUintmax_t(pAscii, delim, error_occured);
	if(value > UINTPTR_MAX)
		*error_occured = MT_TRUE;
	return (MT_UINTPTR)value;
}
