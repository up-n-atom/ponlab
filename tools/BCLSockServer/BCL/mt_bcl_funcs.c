...
MT_UINTPTR pitemlist(MT_UBYTE item)
{
    if (item >= MT_BCL_ITEM_LISTS_NUM)
    {
        error("Illegal value for Itemlist.");
    ...
    return (MT_UINTPTR)(MT_BCL_item_list[item]);
}
...
MT_UINT32 item(MT_UBYTE size)
{
    ...
    else
    {
        ...
        {
            if (((MT_UINTPTR)IL_cur_ptr & 0x1) == 0)
        ...
        {
            if (((MT_UINTPTR)IL_cur_ptr & 0x3) == 0)
...
MT_UINT32 ditem (MT_UBYTE mem_size,MT_UINTPTR address, MT_UBYTE offset, MT_UBYTE count)
{
...
{
	MT_UINTPTR itemList = (MT_UINTPTR)pAddress;
...
MT_UINT32 WriteStr(void)
{
    ...
    Copy2address = (char *)asciiToUintptr(params, ' ', &numConvErr);
