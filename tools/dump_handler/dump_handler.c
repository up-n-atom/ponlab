...
static void _zip_fw_files (const char *storage_path, int card_idx,
  ...
  sprintf_res = sprintf_s(system_cmd, sizeof (system_cmd),
                          ...
                          "/usr/bin/wc -l` -eq 1 ] && "
                          "/bin/ls -l %s/fw_dump_*tar.gz 2>/dev/null | "
                          "/usr/bin/head -n 1 | /usr/bin/awk '{print $5, $NF}'",
...
save_dumps:
  ...
  if (get_free_space (storage_path, &free_space_in_fs) == MTLK_ERR_OK){
    ...
    if (free_space_in_fs <= (LEAVE_FREE_SPACE_ON_FS_IN_KB * 1024)){
      ...
      printf("Dump Handler: Not enough space left on storage device, removing dump file\n");
...
_print_help (const char *app_name)
{
  ...
  sprintf_res = sprintf_s(version, sizeof(version), "%s: %s\n%s %u %s",
                          "Firmware dump evacuation application version",
                          DUMP_HANDLER_VERSION,
                          "Debug: echo",
                          LOG_LOCAL_OID,
...
static BOOL _rcvry_happend(char *dump_file_path){
  ...
  int sprintf_res = sprintf_s(sys_cmd, sizeof(sys_cmd), "/usr/bin/head -n 1 %s  2>/dev/null | /bin/grep %s > /dev/null",
...
#if CONFIG_USE_DWPAL_DAEMON
int dump_handler_dwpald_term_cond_get(void)
{
  return g_dh_config.terminate;
}

static void dump_handler_dwpald_term_cond_set(BOOL val)
{
  g_dh_config.terminate = val;
}
...
static int fw_dump_ready_event_handler(char *ifname, int drv_event_id, void *data, size_t len)
{
  ...
  if (MTLK_ERR_FILEOP == res) {
    dump_handler_dwpald_term_cond_set(TRUE);
  }
...
again:
  ...
  ret = dwpald_start_blocked_listen(dump_handler_dwpald_term_cond_get);
...
static int check_if_usb_mounted(char * mnt_path, char * storage_path, BOOL * found_mount)
{
  ...
  *found_mount = FALSE;
  sprintf_res = sprintf_s(sys_cmd, sizeof (sys_cmd), "/bin/ls -d %s 2>/dev/null | /usr/bin/head -n 1",
                          mnt_path);
