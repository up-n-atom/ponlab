...
signal_callback_handler(int signum)
{
  ...
  if (write(app_data->stop_pipe_fd[1], "x", 1) != 1) {
    ELOG_V("Failed to signal the notification thread to stop");
  }
}
...
_mtlk_irba_packet_processor (void* param, void* packet)
{
  ...
  mtlk_osal_mutex_acquire(&app_data->lock);
  cdev_minor = hdr->cdev_minor; /*fix unaligned access */
  data_size = hdr->data_size;  /*fix unaligned access */
  h = mtlk_mhash_find_first_irba(&app_data->db, &cdev_minor, &ctx);
  while (h) {
    mtlk_irba_t *irba = MTLK_CONTAINER_OF(h, mtlk_irba_t, hentry);
    _mtlk_irb_hash_on_evt(&irba->hash, &hdr->evt, data, &data_size, HANDLE_T(irba));
    h = mtlk_mhash_find_next_irba(&app_data->db, &cdev_minor, &ctx);
  }
  mtlk_osal_mutex_release(&app_data->lock);
}
