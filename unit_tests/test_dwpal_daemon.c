...
static int dpal_ext_hostap_injected_event(char *ifname, char *op_code, char *msg, size_t len)
{
	...
	expected_len = sprintf_s(expected_msg_buff, sizeof(expected_msg_buff), "<3>%s %s %s", op_code, ifname, expected_msg);
...
static int dpald_hostap_injected_event_1(char *ifname, char *op_code, char *msg, size_t len)
{
	...
	expected_len = sprintf_s(expected_msg_buff, sizeof(expected_msg_buff), "<3>%s %s %s", op_code, ifname, INJECTED_EVENT_1_MSG);
...
static int dpald_hostap_injected_event_2(char *ifname, char *op_code, char *msg, size_t len)
{
	...
	expected_len = sprintf_s(expected_msg_buff, sizeof(expected_msg_buff), "<3>%s %s %s", op_code, ifname, INJECTED_EVENT_2_MSG);
...
static int dpald_hostap_injected_event_3(char *ifname, char *op_code, char *msg, size_t len)
{
	...
	expected_len = sprintf_s(expected_msg_buff, sizeof(expected_msg_buff), "<3>%s %s %s", op_code, ifname, INJECTED_EVENT_3_MSG);
...
static size_t num_debug_hap_events = ARRAY_SIZE(debug_hap_events);
...
static dwpald_driver_nl_event drv_events[] = {
	...
	{ LTQ_NL80211_VENDOR_EVENT_UNRECOVERABLE_ERROR , driver_event },
	{ LTQ_NL80211_VENDOR_EVENT_CHAN_DATA , driver_event },
};

static size_t num_drv_events = ARRAY_SIZE(drv_events);
