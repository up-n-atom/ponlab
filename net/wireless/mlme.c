	...
	for (bandid = 0; bandid < NUM_NL80211_BANDS; bandid++) {
		...
		for (i = 0; i < sband->n_channels; i++) {
			...
			if (time_after_eq(jiffies, timeout)) {
				...
				/* In case SB DFS active, set and propagate to AVAILABLE */
				if ((radar_event == NL80211_RADAR_NOP_FINISHED) &&
					cfg80211_any_wiphy_oper_chan(wiphy, c)) {
					radar_event = NL80211_RADAR_CAC_FINISHED;
					c->dfs_state = NL80211_DFS_AVAILABLE;
				}
...
void __cfg80211_radar_event(struct wiphy *wiphy,
			    struct cfg80211_chan_def *chandef, bool offchan,
                      ...
                      const u8 *buf, size_t len, gfp_t gfp)
{
	...
	struct cfg80211_mgmt_registration *reg;
	int res = 0;
	struct cfg80211_rx_info rx_info = {
		.freq = freq,
		.sig_dbm = 0,
		.snr_db = 0,
		.buf = buf,
	...
	list_for_each_entry(reg, &wdev->mgmt_registrations, list) {
		res = nl80211_send_mgmt(rdev, wdev, reg->nlportid, &rx_info, gfp);
		break;
	}
				...
				enum nl80211_radar_event event)
{
	...
	nl80211_radar_notify(rdev, chandef, event, netdev, 0, GFP_KERNEL);
}
