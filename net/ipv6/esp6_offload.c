...
#if IS_ENABLED(CONFIG_MXL_VPN)
static void skb_len_restore(struct sk_buff *skb)
{
	u32 ext_id;
	unsigned int *ext;

	ext_id = mxl_skb_ext_get_ext_id("mxl_vpn");
	ext = mxl_skb_ext_find(skb, ext_id);
	if (!ext)
		return;

	skb->len = *ext;
	pr_debug("%s: restoring skb len from 0x%p. len=%d\n",
		 __func__, ext, skb->len);
	mxl_skb_ext_del(skb, ext_id);
}
#endif
