...
static void ieee80211_restart_work(struct work_struct *work)
{
	...
	flush_work(&local->radar_detection.radar_detected_work);
					   ...
					   const char *requested_name)
{
	...
	mutex_init(&local->radar_detection.radar_detctx_mtx);
	INIT_WORK(&local->radar_detection.radar_detected_work,
...
int ieee80211_register_hw(struct ieee80211_hw *hw)
{
	...
	if (WARN_ON(ieee80211_hw_check(hw, DATA_OFFLOAD) &&
				!ieee80211_hw_check(hw, CONNECTION_MONITOR) &&
				!local->ops->get_connection_alive))
		return -EINVAL;

	if (WARN_ON(ieee80211_hw_check(hw, DATA_OFFLOAD) &&
				(!local->ops->data_tx ||
				 !local->ops->sta_statistics)))
		return -EINVAL;

	if (WARN_ON(ieee80211_hw_check(hw, DATA_OFFLOAD) &&
				(!local->ops->get_et_sset_count ||
				 !local->ops->get_et_stats ||
				 !local->ops->get_et_strings)))
		return -EINVAL;

	if (WARN_ON(ieee80211_hw_check(hw, DATA_OFFLOAD) &&
				(!local->ops->set_default_multicast_key)))
		return -EINVAL;
	...
	if (ieee80211_hw_check(&local->hw, SUPPORTS_SSID_LIST))
		local->ext_capa[3] |= WLAN_EXT_CAPA4_SSID_LIST;
	if (ieee80211_hw_check(&local->hw, SUPPORTS_WNM_NOTIF))
		local->ext_capa[5] |= WLAN_EXT_CAPA6_WNM_NOTIF;
	if (ieee80211_hw_check(&local->hw, SUPPORTS_OP_MODE_NOTIF))
		local->ext_capa[7] |= WLAN_EXT_CAPA8_OPMODE_NOTIF;
	...
	local->hw.wiphy->cac_time_debug = -1;
...
{
	WARN_ONCE(0, "Have pending ack frames!\n");
	kfree_skb(p);
	return 0;
}
...
void ieee80211_free_hw(struct ieee80211_hw *hw)
{
	...
	mutex_destroy(&local->radar_detection.radar_detctx_mtx);
