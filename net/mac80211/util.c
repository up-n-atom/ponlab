                ...
                struct ieee802_11_elems *elems, bool calc_crc, u32 crc)
{
        ...
        if (elen >= 4 && elems->vendor_events_filter && elems->vendor_events_filter_len >= 3) {
                ...
                if (found) {
                        ...
                        (pos[0] == ((WLAN_OUI_WFA >> 16) & 0xff)) &&
                        (pos[1] == ((WLAN_OUI_WFA >> 8)  & 0xff)) &&
                        (pos[2] == ((WLAN_OUI_WFA >> 0)  & 0xff))) {
                /* Wi-Fi Alliance OUI */

                if (elen >= 5 && pos[3] == WLAN_OUI_TYPE_WFA_MULTI_AP) {
                        elems->multi_ap = pos;
				  ...
				  struct ieee80211_elems_parse_params *params)
{
	...
	case WLAN_EID_EXT_HE_6GHZ_CAPA:
		...
		break;
	case WLAN_EID_EXT_BSS_COLOR_CHG_ANN:
		if (len >= sizeof(*elems->cca_ie))
			elems->cca_ie = data;
		break;
...
void ieee80211_dfs_cac_restart(struct ieee80211_hw *hw){
        struct ieee80211_sub_if_data *sdata;
        mutex_lock(&hw_to_local(hw)->mtx);
        list_for_each_entry(sdata, &hw_to_local(hw)->interfaces, list) {
                cancel_delayed_work(&sdata->deflink.dfs_cac_timer_work);
                if (sdata->wdev.cac_started) {
                        ieee80211_queue_delayed_work(&sdata->local->hw,
                                                &sdata->deflink.dfs_cac_timer_work,
                                                msecs_to_jiffies(sdata->wdev.cac_time_ms));
                        drv_prepare_cac_start(sdata->local, sdata,
                                               sdata->wdev.cac_time_ms);
                }
        }
        mutex_unlock(&hw_to_local(hw)->mtx);
}
EXPORT_SYMBOL(ieee80211_dfs_cac_restart);

void ieee80211_dfs_radar_detected_work(struct work_struct *work)
{
	struct ieee80211_radar_detection *detection =
		container_of(work, struct ieee80211_radar_detection, radar_detected_work);
	struct ieee80211_local *local =
		container_of(detection, struct ieee80211_local, radar_detection);
	struct cfg80211_chan_def chandef = local->hw.conf.chandef;
	struct ieee80211_chanctx *ctx;
	int num_chanctx = 0;
	u8 radar_bit_map = 0xff;
	...
	if (num_chanctx > 1)
		...
		mutex_lock(&local->radar_detection.radar_detctx_mtx);
		if (detection->radar_bit_map) {
			radar_bit_map = detection->radar_bit_map;
			chandef = detection->chandef;

			detection->radar_bit_map = 0;
			memset(&detection->chandef, 0, sizeof(struct cfg80211_chan_def));
		}
		mutex_unlock(&local->radar_detection.radar_detctx_mtx);
		rtnl_lock();
		cfg80211_radar_event(local->hw.wiphy, &chandef, radar_bit_map, GFP_KERNEL);
		drv_handle_radar_debug(local, &chandef, radar_bit_map);
		rtnl_unlock();
	}
}
                                        ...
                                        u8 radar_bit_map)
{
        ...
        trace_api_radar_detected_rbm(local, radar_bit_map);

        if (chandef) {
                if (!radar_bit_map)
                        radar_bit_map = 0xff;
                mutex_lock(&local->radar_detection.radar_detctx_mtx);
                        if (local->radar_detection.radar_bit_map)
                                local->radar_detection.radar_bit_map |= radar_bit_map;
                        else
                                local->radar_detection.radar_bit_map = radar_bit_map;

                        memcpy(&local->radar_detection.chandef, chandef,
                                                sizeof(struct cfg80211_chan_def));
                mutex_unlock(&local->radar_detection.radar_detctx_mtx);
        }

        schedule_work(&local->radar_detection.radar_detected_work);
}
EXPORT_SYMBOL(ieee80211_radar_detected_rbm);
...
void ieee80211_radar_detected(struct ieee80211_hw *hw)
{
	...
	ieee80211_radar_detected_rbm(hw, NULL, 0);
}
	...
	case NL80211_CHAN_WIDTH_320:
		if (band == NL80211_BAND_6GHZ) {
                        if (!c->center_freq2) {
                                ...
                                c->center_freq2 = c->center_freq1;
                                if (c->center_freq1 > c->chan->center_freq)
                                        c->center_freq1 -= 40;
                                else
                                        c->center_freq1 += 40;
                        }
