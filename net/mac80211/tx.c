...
/* misc utils */
static bool control_port_protocol_frame(struct ieee80211_sub_if_data *sdata, __be16 proto, u32 ctrl_flags)
{
	return (proto == sdata->control_port_protocol || (ctrl_flags & IEEE80211_TX_CTRL_PORT_CTRL_PROTO));
}
		     ...
		     struct sta_info *sta, struct sk_buff *skb)
{
	...
	if (likely(sta)) {
		...
		if (sdata->vif.type == NL80211_IFTYPE_AP_VLAN) {
			...
			tx->sta = sta_info_get_bss(sdata, hdr->addr1, INCLUDE_SEARCH_IN_MLDB);
		}
			    ...
			    struct sta_info **sta_out)
{
	...
	case NL80211_IFTYPE_ADHOC:
		...
		sta = sta_info_get_bss(sdata, skb->data, INCLUDE_SEARCH_IN_MLDB);
		break;
					   ...
					   u64 *cookie)
{
	...
	case NL80211_IFTYPE_STATION:
		...
		if (tdls_peer) {
			...
			hdrlen = 24;
			    ...
			    !control_port_protocol_frame(sdata, cpu_to_be16(ethertype), ctrl_flags)) {
			fc |= cpu_to_le16(IEEE80211_FCTL_FROMDS |
					  IEEE80211_FCTL_TODS);
			/* RA TA DA SA */
	 ...
	 * EAPOL frames or vlan tagged EAPOL frames from the local station.
	 */
				...
				u32 ctrl_flags)
{
	...
	if (control_port_protocol_frame(sdata, cpu_to_be16(ethertype), ctrl_flags))
		return false;
				  ...
				  u64 *cookie)
{
	...
	if (sta) {
		...
		if (fast_tx &&
		    ieee80211_xmit_fast(sdata, sta, fast_tx, skb, ctrl_flags))
			goto out;
	}
...
netdev_tx_t ieee80211_subif_start_data_xmit(struct sk_buff *skb,
                                        struct net_device *dev)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	struct sk_buff *next;
			      ...
			      int link_id, u64 *cookie)
{
	...
	u32 flags = 0;
	int err;
	bool is_vlan_tagged_eapol = false;

	if (proto == cpu_to_be16(ETH_P_8021Q)) {
		vh = (struct vlan_hdr *)(buf);
		is_vlan_tagged_eapol = (vh->h_vlan_encapsulated_proto == cpu_to_be16(ETH_P_PAE) ? true : false);
	}
	...
	if (proto != sdata->control_port_protocol &&
	    ...
	    !is_vlan_tagged_eapol)
		return -EINVAL;

	if (proto == sdata->control_port_protocol || is_vlan_tagged_eapol)
