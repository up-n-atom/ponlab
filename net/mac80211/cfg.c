...
static int ieee80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if (sdata->local->ops->driver_free_netdev_allowed) {
                int res = sdata->local->ops->driver_free_netdev_allowed(&sdata->local->hw,
                                                          sdata->dev);
                if (res)
                        return res;
	}
			    ...
			    const u8 *mac_addr, u8 key_idx)
{
	...
	sta = sta_info_get_bss(sdata, mac_addr, INCLUDE_SEARCH_IN_MLDB);
				 ...
				 const u8 *mac, struct station_info *sinfo)
{
	...
	sta = sta_info_get_bss(sdata, mac, INCLUDE_SEARCH_IN_MLDB);
	if (sta) {
		ret = 0;
		sta_set_sinfo(sta, sinfo, true);
	}
			     ...
			     unsigned int link_id)
{
	...
	memset(&sdata->vif.bss_conf.chandef.sb_dfs, 0, sizeof(sdata->vif.bss_conf.chandef.sb_dfs));
				...
				struct station_parameters *params)
{
	...
	if(params->ext_capab_len >= 8)
		sta->sta.opmode_notif = (params->ext_capab[7] &
				WLAN_EXT_CAPA8_OPMODE_NOTIF) >> 6;
	...
	if (params->listen_interval >= 0){
		...
		sta->sta.listen_interval = params->listen_interval;
	}

	sta->sta.rssi = params->rssi;

	if(params->resp){
		sta->sta.resp = params->resp;
		sta->sta.resp_len = params->resp_len;
	}
...
EXPORT_SYMBOL(ieee80211_del_station);
				    ...
				    struct station_parameters *params)
{
	...
	sta = sta_info_get_bss(sdata, mac, IGNORE_SEARCH_IN_MLDB);
	if (!sta) {
		err = -ENOENT;
		goto out_err;
	}
				...
				struct bss_parameters *params)
{
	...
	if (params->ap_isolate >= 0) {
		...
		ieee80211_check_fast_rx_iface(sdata);
		if (sdata->vif.type == NL80211_IFTYPE_AP &&
                    ieee80211_hw_check(&sdata->local->hw, DATA_OFFLOAD)) {
                        sdata->vif.bss_conf.ap_isolate = params->ap_isolate;
                        changed |= BSS_CHANGED_AP_ISOLATE;
                }
	...
	if (ieee80211_is_radar_required(local)){
                ...
                list_for_each_entry(sdata_iter, &local->interfaces, list) {
                        if (sdata_iter->wdev.cac_started) {
                                ...
                                if (wiphy->allow_scan_during_cac) {
                                        /* Pause scan, hostapd will restart it once scan is complete. */
                                        chandef = &sdata_iter->vif.bss_conf.chandef;
                                        cancel_delayed_work_sync(&sdata_iter->dfs_cac_timer_work);
                                        drv_notify_cac_finished(local, sdata_iter, true);
                                        ieee80211_link_release_channel(&sdata_iter->deflink);
                                        cfg80211_cac_event(sdata_iter->dev, chandef,
					   ...
					   u32 cac_time_ms)
{
	...
	err = drv_prepare_cac_start(local, sdata, cac_time_ms);
	if (err)
		goto out_unlock;
			      ...
			      bool cac_canceled)
{
	...
	list_for_each_entry(sdata, &local->interfaces, list) {
		...
		if (sdata->wdev.cac_started) {
			drv_notify_cac_finished(local, sdata, cac_canceled);
			ieee80211_link_release_channel(&sdata->deflink);
...
static int ieee80211_ml_chan_switch_done_notify(struct ieee80211_vif *vif,
                                         uint flag)
{

        struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);

        return cfg80211_ml_chan_switch_done_notify(sdata->dev, flag);
}


void ieee80211_ml_chan_switch_notif_done(struct ieee80211_vif *vif, uint flag)
{
        ieee80211_ml_chan_switch_done_notify(vif, flag);
}
EXPORT_SYMBOL(ieee80211_ml_chan_switch_notif_done);
			   ...
			   struct cfg80211_csa_settings *params)
{
	...
	/* If color change in progress, fail chan switch. Aborting CCA
	 * involves complications in reverting color for the VAPs that
	 * actually completed the countdown and its associated STAs.
	 */
	...
	if (sdata->vif.bss_conf.color_change_active) {
		...
		ieee80211_link_unreserve_chanctx(&sdata->deflink);
		err = -EBUSY;
		sdata_info(sdata,"channel switch failed,"
				"bss color change is in progress\n");
		goto out;
        }
				  ...
				  const u8 *peer, u64 *cookie)
{
	...
	rcu_read_lock();
	sta = sta_info_get_bss(sdata, peer, INCLUDE_SEARCH_IN_MLDB);
	if (!sta) {
		ret = -ENOLINK;
		goto unlock;
	}
					      ...
					      const bool enabled)
{
	...
	if (ieee80211_hw_check(&sdata->local->hw, DATA_OFFLOAD))
                return -EOPNOTSUPP;
				    ...
				    struct cfg80211_tid_config *tid_conf)
{
	...
	sta = sta_info_get_bss(sdata, tid_conf->peer, INCLUDE_SEARCH_IN_MLDB);
	if (!sta) {
                                             ...
                                             struct wiphy_work *work)
{
	...
	sdata_lock(sdata);
	cfg80211_obss_color_collision_notify(sdata->dev, link->color_bitmap,  &link->u.mgd.bssid[0]);
	sdata_unlock(sdata);
}
...
{
        struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
        sdata->color_coll_notify.color_bitmap = color_bitmap;
        memcpy(sdata->color_coll_notify.bssid, bssid, ETH_ALEN);

        ieee80211_queue_work(&sdata->local->hw,
                             &sdata->color_coll_notify.work);
}
...
void ieee80211_color_coll_notify_work(struct work_struct *work)
{
        ...
        ieee80211_obss_color_collision_notify(&sdata->vif,
                sdata->color_coll_notify.color_bitmap,
                GFP_ATOMIC);
unlock:
        sdata_unlock(sdata);
}
		       ...
		       struct cfg80211_color_change_settings *params)
{
	...
	sdata->vif.bss_conf.color_switch_time = params->switch_time;
	sdata->vif.bss_conf.color_change_cntdown = params->count;
	sdata->vif.he_oper_color_offset_beacon = params->he_oper_color_offset_beacon;

	cfg80211_color_change_started_notify(sdata->dev, params->count, params->color);

	if (changed)
		ieee80211_color_change_bss_config_notify(sdata, 0, false, changed);
	else
				...
				struct link_station_parameters *params)
{
	...
	sta = sta_info_get_bss(sdata, params->mld_mac, INCLUDE_SEARCH_IN_MLDB);
	if (!sta)
		return -ENOENT;
...
static int ieee80211_set_mac_acl(struct wiphy *wiphy, struct net_device *dev,
