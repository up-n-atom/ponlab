...
static bool rx_control_port_protocol_frame(__be16 proto, struct ieee80211_rx_data *rx)
{
	...
	if (status)
		ret = (proto == rx->sdata->control_port_protocol || status->flag & RX_FLAG_CTRL_PORT_PROTO);
...
ieee80211_rx_h_decrypt(struct ieee80211_rx_data *rx)
{
	...
	if (rx->sta) {
		...
		if (ieee80211_has_protected(fc)) {
			/* SKIP Decryption if already decrypted */
...
__ieee80211_data_to_8023(struct ieee80211_rx_data *rx, bool *port_control)
{
	...
	*port_control = false;
	...
	if (rx_control_port_protocol_frame(ehdr->h_proto, rx))
		*port_control = true;
...
static bool ieee80211_frame_allowed(struct ieee80211_rx_data *rx, __le16 fc)
{
	 ...
	 */
	if (unlikely(rx_control_port_protocol_frame(ehdr->h_proto, rx)))
						 ...
						 struct ieee80211_rx_data *rx)
{
	...
	if (unlikely(((skb->protocol == sdata->control_port_protocol ||
		     status->flag & RX_FLAG_CTRL_PORT_PROTO) ||
...
ieee80211_deliver_skb(struct ieee80211_rx_data *rx)
{
	...
	if ((sdata->vif.type == NL80211_IFTYPE_AP ||
	    ...
	    !rx_control_port_protocol_frame(ehdr->h_proto, rx) &&
	    (sdata->vif.type != NL80211_IFTYPE_AP_VLAN || !sdata->u.vlan.sta)) {
		       ...
		       struct sk_buff *skb)
{
	...
	skb_set_queue_mapping(skb, ieee80211_select_queue_80211(sdata, skb, hdr))
...
ieee80211_rx_h_action(struct ieee80211_rx_data *rx)
{
	...
	case WLAN_CATEGORY_PUBLIC:
		...
		if (len < offsetof(struct ieee80211_mgmt,
			...
			goto invalid;
		goto queue;
	case WLAN_CATEGORY_PROTECTED_HE_ACTION:
		if (len < IEEE80211_MIN_ACTION_SIZE + 1)
			goto invalid;
		if (sdata->vif.type != NL80211_IFTYPE_STATION)
			break;
		...
		if (!ether_addr_equal(mgmt->bssid, sdata->deflink.u.mgd.bssid))
			break;
		if (mgmt->u.action.u.color_switch.action_code !=
				WLAN_PROTECTED_HE_BSS_COLOR_CHNG)
			break;
		if (len < offsetof(struct ieee80211_mgmt,
				       ...
				       struct sk_buff *skb, bool consume)
{
	...
	if (link_sta) {
		...
		sta = sta_info_get_bss(rx->sdata, hdr->addr2, INCLUDE_SEARCH_IN_MLDB);
		if (status->link_valid)
			link_id = status->link_id;
	}
...
#endif
{
	...
	if (unlikely(ieee80211_is_probe_resp(hdr->frame_control) ||
		     ...
		     (mgmt->u.action.u.public_action.action_code == WLAN_PUB_ACTION_FILS_DISCOVERY)))
