	...
	tcf_exts_for_each_action(i, act, exts) {
		...
		if (is_tcf_gact_ok(act)) {
			...
			case TCA_VLAN_ACT_PUSH:
				...
				entry->vlan.flags = tcf_vlan_push_flags(act);
				break;
			...
			entry->police.peak_bytes_ps =
				tcf_police_peak_bytes_ps(act);
			...
			if (err)
				goto err_out_locked;
		} else if (is_tcf_colmark(act)) {
			entry->id = FLOW_ACTION_COLMARK;
			entry->colmark.blind =  tcf_colmark_mode(act);
			entry->colmark.precedence = tcf_colmark_precedence(act);
			entry->colmark.type = tcf_colmark_mtype(act);
			entry->colmark.flags = tcf_colmark_flags(act);
		} else {
			err = -EOPNOTSUPP;
			goto err_out_locked;
		}
