...
Module:				pacRxc
SOC Revision:		latest
Generated at:       2022-03-29 07:45:58
Purpose:
...
#define	REG_PAC_RXC_RX_TO_ME_AIR_TIME_CTR_WR                   (PAC_RXC_BASE_ADDRESS + 0x138)
#define	REG_PAC_RXC_RX_TO_ME_AIR_TIME_CTR_RD                   (PAC_RXC_BASE_ADDRESS + 0x13C)
...
typedef union
{
	...
	struct
	{
		uint32 minMpduLength : 14; //Min MPDU size, reset value: 0xe, access type: RW
		uint32 reserved0 : 18;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		...
		uint32 rxcPhyStatusSm : 4; //RxC PHY status state machine, reset value: 0x0, access type: RO
		...
		uint32 hdrCtrlSm : 5; //Header control state machine, reset value: 0x0, access type: RO
		uint32 addr2IdxDoneSm : 2; //Addr2index done state machine, reset value: 0x0, access type: RO
...
/*REG_PAC_RXC_RX_TO_ME_AIR_TIME_CTR_WR 0x138 */
typedef union
{
	uint32 val;
	struct
	{
		uint32 rxToMeAirTimeCtrWr : 28; //Rx to me air time counter, write value, reset value: 0x0, access type: RW
		uint32 reserved0 : 4;
	} bitFields;
} RegPacRxcRxToMeAirTimeCtrWr_u;

/*REG_PAC_RXC_RX_TO_ME_AIR_TIME_CTR_RD 0x13C */
typedef union
{
	uint32 val;
	struct
	{
		uint32 rxToMeAirTimeCtrRd : 28; //Rx to me air time counter, reset value: 0x0, access type: RO
		uint32 reserved0 : 4;
	} bitFields;
} RegPacRxcRxToMeAirTimeCtrRd_u;
...
typedef union
{
	...
	struct
	{
		uint32 rssiNotMyStaFifoEn : 1; //Enable A-MPDU with no BA agreement reject filter, reset value: 0x0, access type: RW
		uint32 reserved0 : 31;
	} bitFields;
