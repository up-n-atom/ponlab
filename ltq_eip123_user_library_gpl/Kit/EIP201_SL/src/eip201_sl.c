...
// this implementation supports only the EIP-201 HW1.1
// 0xC9  = 201
// 0x39  = binary inverse of 0xC9
// 0x110 = version: 1.1.0
#define EIP201_EXPECTED_VERSION  0x011036C9
        ...
        Device_Handle_t Device)
{
    ...
    if (Value != EIP201_EXPECTED_VERSION)
        return false;
    ...
    // Set all configured interrupt polarities and read it back
    {
        uint32_t Mask = (0xFFFFFFFFU >> (32 - Value));
        EIP201_Write32(Device, EIP201_REGISTER_OFFSET_POL_CTRL, Mask);
        Value = EIP201_Read32(Device, EIP201_REGISTER_OFFSET_POL_CTRL);
        if ((Value & Mask) != Mask)
            return false;
    }

    // Clear all configured interrupt polarities and read it back
