...
#define CHAN_SOC2DEV_P(ctx) (&((ctx)->ch[DCMODE_CHAN_SOC2DEV]))
#define CHAN_SOC2DEV_RET_P(ctx) (&((ctx)->ch[DCMODE_CHAN_SOC2DEV_RET]))
#define CHAN_DEV2SOC_P(ctx) (&((ctx)->ch[DCMODE_CHAN_DEV2SOC]))
#define CHAN_DEV2SOC_RET_P(ctx) (&((ctx)->ch[DCMODE_CHAN_DEV2SOC_RET]))
#define CHAN_DEV2SOC_EX_P(ctx) (&((ctx)->ch[DCMODE_CHAN_DEV2SOC_EXCEPT]))
...
static struct xrx500_dcmode0_dev_info g_dcmode0_dev[XRX500_DCMODE0_MAX_DEV_NUM];
...
xrx500_setup_pmac_port(int32_t port_id, int32_t dma_cid, uint32_t flags);
static inline int32_t
...
xrx500_setup_ring_resources(struct xrx500_dcmode0_dev_info *dev_ctx, int32_t port_id, struct dc_dp_res *res, uint32_t flags)
{
    ...
    ret = ltq_request_dma(dev_ctx->dma_ch, "dma1tx dc");
    if (ret) {
    ...
    ret = ltq_dma_chan_desc_alloc(dev_ctx->dma_ch, res->rings.dev2soc.size);
    if (ret) {
        DC_DP_ERROR("failed to allocate %d descriptors for DMA1-TX DMA channel 0x%x!!!\n",
                        res->rings.dev2soc.size, dev_ctx->dma_ch);
        goto err_out_free_dma;
    }

    /* Update returned 'resource' structure */
    res->rings.dev2soc.phys_base = (void *)ltq_dma_chan_get_desc_phys_base(dev_ctx->dma_ch);
    ...
    dev_ctx->num_bufpools = num_bufpools;
    dev_ctx->buflist = buflist;
...
xrx500_cleanup_ring_resources(struct xrx500_dcmode0_dev_info *dev_ctx, int32_t port_id, struct dc_dp_res *res, uint32_t flags)
{
    ...
    ltq_dma_chan_reset(dev_ctx->dma_ch);

    /* De-allocate Tx buffer pool */
    xrx500_free_ring_buffers(dev_ctx->num_bufpools, &dev_ctx->buflist);

    /* Free the DMA1-TX DMA channel descriptors */
    if (ltq_dma_chan_desc_free(dev_ctx->dma_ch)) {
        DC_DP_ERROR("failed to free descriptors for DMA1-TX DMA channel 0x%x!!!\n", dev_ctx->dma_ch);
    }

    /* Free DMA1-TX DMA channel */
    if (ltq_free_dma(dev_ctx->dma_ch)) {
...
xrx500_setup_umt_port(struct xrx500_dcmode0_dev_info *dev_ctx, int32_t port_id, struct dc_dp_res *res, uint32_t flags)
{
    ...
    /* Initialize */
    dev_ctx->cbm_pid = cbm_pid;
    dev_ctx->umt_id = umt_id;
    dev_ctx->dma_ctrlid = dma_ctrlid;
    dev_ctx->dma_cid = dma_cid;
    dev_ctx->dma_ch = _DMA_C(dma_ctrlid, DMA1TX_PORT, dma_cid);
    dev_ctx->umt_period = XRX500_DCMODE0_UMT_PERIOD_DEFAULT;
...
static int32_t
xrx500_dcmode0_register_dev(void *ctx,
                            ...
                            struct dc_dp_res *resources, struct dc_dp_dev *devspec, uint32_t flags)
{
    ...
    if (flags & DC_DP_F_DEREGISTER) {
        ...
        DC_DP_DEBUG(DC_DP_DBG_FLAG_DBG, "De-configuring DMA1-Tx channel 0x%x.\n",
                    dev_ctx->dma_ch);
        xrx500_cleanup_ring_resources(dev_ctx, port_id, resources, flags);

        /* Cleanup UMT resources */
        xrx500_cleanup_umt_port(port_id, dev_ctx->umt_id);
    ...
    for (dev_idx = 0; dev_idx < XRX500_DCMODE0_MAX_DEV_NUM; dev_idx++) {
        if (g_dcmode0_dev[dev_idx].status != XRX500_DCMODE0_DEV_STATUS_USED) {
            break;
        }
    }

    if (dev_idx >= XRX500_DCMODE0_MAX_DEV_NUM) {
    ...
    /* Setup UMT resources */
    ret = xrx500_setup_umt_port(dev_ctx, port_id, resources, flags);
    if (ret != DP_SUCCESS) {
    ...
    ret = xrx500_setup_pmac_port(port_id, dev_ctx->dma_cid, flags);
    if (ret != DP_SUCCESS) {
        DC_DP_ERROR("failed to setup UMT resources!!!\n");
    ...
    DC_DP_DEBUG(DC_DP_DBG_FLAG_DBG, "Configuring DMA1-Tx channel 0x%x.\n",
                dev_ctx->dma_ch);
    ret = xrx500_setup_ring_resources(dev_ctx, port_id, resources, flags);
    if (ret) {
    ...
    /* Initialize DC Mode0 device structure */
                              ...
                              const uint8_t *subif_name, struct dp_subif *subif_id, uint32_t flags)
{
    ...
    if (flags & DC_DP_F_DEREGISTER) {
        ...
        /* For the last subif */
        if (dev_ctx->num_subif == 1) {
            /* Disable DMA1-TX DMA channel */
            if (ltq_dma_chan_off(dev_ctx->dma_ch)) {
            ...
            if (ltq_umt_enable(dev_ctx->umt_id, subif_id->port_id, UMT_DISABLE)) {
                DC_DP_ERROR("failed to disable umt_id=%d, port_id=%d!!!\n",
                                dev_ctx->umt_id, subif_id->port_id);
            }
#endif /* #if defined(CONFIG_LTQ_UMT_EXPAND_MODE) */
            /* Delay for ongoing UMT transfer */
            udelay(XRX500_DCMODE0_UMT_PERIOD_DEFAULT * 2);
            ...
            xrx500_flush_cbm_queue(subif_id->port_id);
        }
        ...
        if (dev_ctx->num_subif == 1)
            xrx500_restore_tmu_queuemap(subif_id->port_id);
    ...
    if (dev_ctx->num_subif == 0) {

#if defined(CONFIG_LTQ_UMT_EXPAND_MODE)
        /* Enable UMT hw */
        DC_DP_DEBUG(DC_DP_DBG_FLAG_DBG, "Enabling UMT HW.\n");
        ret = ltq_umt_enable(dev_ctx->umt_id, subif_id->port_id, UMT_ENABLE);
        if (ret) {
        ...
        ret = ltq_dma_chan_on(dev_ctx->dma_ch);
        if (ret) {
...
err_out_disable_dmach:
    ltq_dma_chan_off(dev_ctx->dma_ch);

err_out_disable_umt:
#if defined(CONFIG_LTQ_UMT_EXPAND_MODE)
    ltq_umt_enable(dev_ctx->umt_id, subif_id->port_id, UMT_DISABLE);
#endif /* #if defined(CONFIG_LTQ_UMT_EXPAND_MODE) */
                     ...
                     struct sk_buff *skb, int32_t len)
{
    ...
    if ( !ret ) {
        /* Update Rx mib */
        for (dev_idx = 0; dev_idx < XRX500_DCMODE0_MAX_DEV_NUM; dev_idx++) {
            if ( (g_dcmode0_dev[dev_idx].status != XRX500_DCMODE0_DEV_STATUS_FREE) &&
                 (g_dcmode0_dev[dev_idx].port_id == rx_subif.port_id)) {
                subif_idx = XRX500_DCMODE0_GET_SUBIFIDX(rx_subif.subif);
                g_dcmode0_dev[dev_idx].subif_mib[subif_idx].rx_fn_rxif_pkts ++;
                g_dcmode0_dev[dev_idx].subif_mib[subif_idx].rx_fn_rxif_bytes += len;
                break;
            }
        }
    }
...
xrx500_dcmode0_change_dev_status(void *ctx, int32_t port_id, uint32_t flags)
{
    ...
    if (flags & DC_DP_F_DEQ_ENABLE) {
        ...
        /* Disable DMA1-TX DMA channel */
        ret = ltq_dma_chan_off(dev_ctx->dma_ch);
    }
...
xrx500_dcmode0_dump_proc(void *ctx, struct seq_file *seq)
{
    ...
    seq_printf(seq, "    dma_ch:            %d\n",
           _DMA_CHANNEL(dev_ctx->dma_ch));

    seq_printf(seq, "    num_bufpools:      %02d\n",
           dev_ctx->num_bufpools);
    for (i = 0; i < dev_ctx->num_bufpools; i++) {
        seq_printf(seq, "    buflist %d:\n", (i + 1));
        seq_printf(seq, "      virtual range:    0x%p-0x%p (%d KB)\n",
               dev_ctx->buflist[i].pool,
            ((uint8_t *)dev_ctx->buflist[i].pool + dev_ctx->buflist[i].size),
            (dev_ctx->buflist[i].size >> 10));
        seq_printf(seq, "      physical range:   0x%p-0x%p (%d KB)\n",
               dev_ctx->buflist[i].phys_pool,
            ((uint8_t *)dev_ctx->buflist[i].phys_pool + dev_ctx->buflist[i].size),
            (dev_ctx->buflist[i].size >> 10));
    }
    ...
    for (i = 0; i < DCMODE_CHAN_MAX; i++) {
        ...
        seq_printf(seq, "      Ring Base:           P:0x%p, V:0x%p\n",
               dev_ctx->ch[i].ring.phys, dev_ctx->ch[i].ring.virt);
        seq_printf(seq, "      Ring Size:           %d, each of size %d DW\n",
               dev_ctx->ch[i].ring.size, dev_ctx->ch[i].ring.desc_dwsz);
        seq_printf(seq, "      Ring Track:          rp=%#x wp=%#x\n",
               dev_ctx->ch[i].ring.trck.rp, dev_ctx->ch[i].ring.trck.wp);
        seq_printf(seq, "      Soc To Dev Write Counter:           P:0x%p, V:0x%p (Size %d Bytes)\n",
               dev_ctx->ch[i].write_cntr.phys, dev_ctx->ch[i].write_cntr.virt,
               dev_ctx->ch[i].write_cntr.size);
        seq_printf(seq, "      Soc From Dev Read Counter:           P:0x%p, V:0x%p (Size %d Bytes)\n",
               dev_ctx->ch[i].read_cntr.phys, dev_ctx->ch[i].read_cntr.virt,
               dev_ctx->ch[i].read_cntr.size);
    }
#endif /* #ifdef DEV2SOC_FRAG_EXCEPTION_HANDLING */
...
static struct dc_dp_dcmode_ops xrx500_dcmode0_ops = {
    ...
    .register_dev = xrx500_dcmode0_register_dev,
