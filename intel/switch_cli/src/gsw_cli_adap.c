...
u32 gsw_gswss_reg_read(int fd, GSW_MAC_Cli_t *param)
{
	int ret = 0;
...
int gsw_gswss_reg_write(int fd, GSW_MAC_Cli_t *param)
{
...
int gswss_cli_reg_rd(int fd, GSW_MAC_Cli_t *param, u32 reg)
{
	param->cmdType = GSWSS_REG_WR;
	param->val[0] = reg;
...
int gswss_cli_get_switch_ss_reset(int fd, GSW_MAC_Cli_t *param)
{
...
int gswss_cli_get_macsec_reset(int fd, GSW_MAC_Cli_t *param)
{
	u32 macsec_en, reset;
...
int gswss_cli_get_cfg1_1588(int fd, GSW_MAC_Cli_t *param)
{
	u32 cfg0;
...
int gswss_cli_get_clkmode(int fd, GSW_MAC_Cli_t *param)
{
	u32 clk_mode_cfg;
	int clk_mode;
...
int gswss_cli_dbg_macsec_to_mac(int fd, GSW_MAC_Cli_t *param)
{
...
int gswss_cli_get_nco(int fd, GSW_MAC_Cli_t *param)
{
	int nco = 0;
...
int gswss_cli_get_cfg0_1588(int fd, GSW_MAC_Cli_t *param)
{
	u32 cfg0;
...
int gswss_cli_get_int_en_sts(int fd, GSW_MAC_Cli_t *param)
{
...
int gswss_cli_get_phy2mode(int fd, GSW_MAC_Cli_t *param)
{
	u32 phy_mode;
...
int gswss_cli_get_txtstamp_fifo(int fd, GSW_MAC_Cli_t *param)
{
...
int gswss_cli_get_mtu(int fd, GSW_MAC_Cli_t *param)
{
	u32 mtu = 0;
...
int gswss_cli_get_macif(int fd, GSW_MAC_Cli_t *param)
{
	u32 mac_if_cfg, val;
...
int gswss_cli_get_mac_reset(int fd, GSW_MAC_Cli_t *param)
{
...
int gswss_cli_get_mac_en(int fd, GSW_MAC_Cli_t *param)
{
	...
	printf("GSWSS: MAC %d: %s\n", param->mac_idx, enable ? "ENABLED" : "DISABLED");

	return ret;
}

int gswss_cli_get_corese(int fd, GSW_MAC_Cli_t *param)
{
	u32 core_en;
	int ret;
					...
					if ((!strcmp(gswss_cfg[i].cmdname, "r")) ||
						...
						continue;
							...
							printf("gsw gswss    get %10s \t %s\n",
								gswss_cfg[i].cmdname, "<0/1/2... Mac Idx>");
					}
				} else {
					if ((!strcmp(gswss_cfg[i].cmdname, "r")) ||
					    (!strcmp(gswss_cfg[i].cmdname, "w"))) {
...
int gswss_main(u32 argc, char *argv[], int fd)
{
	u32 start_arg = 0;
	u32 idx;
		...
		for (i = 0; i < num_of_elem; i++) {
			...
			if (gswss_cfg[i].cmdid < GSWSS_MAX_MAC) {
				...
				if (!strcmp(argv[start_arg], gswss_cfg[i].cmdname)) {
					...
					if (!argv[start_arg]) {
						printf("switch_cli gswss get %s <0/1/2..: MacIdx>\n", gswss_cfg[i].cmdname);
						goto end;
					}
