...
/**
   \file lib_tapi_lt_gr909_dxt.c  Linetesting library DxT specific part.
   DuSLIC-xT specific calculation of result values.
*/
...
const IFX_float_t fVoltFsMeasRange = 363.7;
const IFX_float_t fResistanceScale = 16.3;
...
IFX_LT_GR909_CFG_t dxt_cfg =
{
   /* default :  R1 = 1.5 MOhm */
   1500000.0,
   /* not used for DuSLIC-xT */
   0.0,
   /* not used for DuSLIC-xT */
   0.0,
};
...
static IFX_void_t lt_dxt_getres_hpt  (IFX_TAPI_GR909_RESULT_t *pTapiRes,
                                      IFX_LT_GR909_HPT_t *p_hpt);
static IFX_void_t lt_dxt_getres_femf (IFX_TAPI_GR909_RESULT_t *pTapiRes,
                                      IFX_LT_GR909_FEMF_t *p_femf);
static IFX_void_t lt_dxt_getres_rft (IFX_TAPI_GR909_RESULT_t *pTapiRes,
                                     IFX_LT_GR909_RFT_t *p_rft,
                                     IFX_boolean_t bExtended);
static IFX_void_t lt_dxt_getres_roh  (IFX_TAPI_GR909_RESULT_t *pTapiRes,
                                      IFX_LT_GR909_ROH_t *p_roh);
static IFX_void_t lt_dxt_getres_rit  (IFX_TAPI_GR909_RESULT_t *pTapiRes,
                                      IFX_LT_GR909_RIT_t *p_rit);
IFX_void_t DXT_LT_GR909_CalcResults (IFX_LT_GR909_RESULT_t *p_res,
...
{
   return (fVoltFsMeasRange / sqrt(2.0) / 32768.0 * fR1 / fVoltSensRes);
}

/* Formula for HPT DC calculations */
static inline IFX_float_t HPT_FEMF_DC_CALC(IFX_float_t fR1)
{
   return (fVoltFsMeasRange / 32768.0 * fR1 / fVoltSensRes);
}
                                     ...
                                     IFX_LT_GR909_HPT_t *p_hpt)
{
   ...
   fConstFactor = HPT_FEMF_AC_CALC(dxt_cfg.f_R1);
   /* HPT AC RING wire to GND result */
   p_hpt->f_hpt_ac_r2g = fConstFactor * pTapiRes->HPT_AC_R2G;
   /* HPT AC TIP wire to GND result */
   p_hpt->f_hpt_ac_t2g = fConstFactor * pTapiRes->HPT_AC_T2G;
   /* HPT AC TIP wire to RING wire result */
   p_hpt->f_hpt_ac_t2r = fConstFactor * pTapiRes->HPT_AC_T2R;

   fConstFactor = HPT_FEMF_DC_CALC(dxt_cfg.f_R1);
   ...
   /* If R1 equals 1.5 MOhm an addtional offset applies. */
   if ( (dxt_cfg.f_R1 > (fVoltSensRes - 1000)) &&
        (dxt_cfg.f_R1 < (fVoltSensRes + 1000)) )
   {
      p_hpt->f_hpt_dc_r2g -= 0.75 /*[V]*/;
      p_hpt->f_hpt_dc_t2g -= 0.75 /*[V]*/;
   }
                                      ...
                                      IFX_LT_GR909_FEMF_t *p_femf)
{
   ...
   if ( (dxt_cfg.f_R1 > (fVoltSensRes - 1000)) &&
        (dxt_cfg.f_R1 < (fVoltSensRes + 1000)) )
   {
      p_femf->f_femf_dc_r2g -= 0.75 /*[V]*/;
      p_femf->f_femf_dc_t2g -= 0.75 /*[V]*/;
   }
                                     ...
                                     IFX_boolean_t bExtended)
{
   ...
   {
      fConstFactor = RFT_ROH_RIT_CALC(dxt_cfg.f_R1);
      /* RFT RING wire to GND result */
      p_rft->f_rft_r2g = fConstFactor * pTapiRes->RFT_R2G;
      /* RFT TIP wire to GND result */
      p_rft->f_rft_t2g = fConstFactor * pTapiRes->RFT_T2G;
      /* RFT TIP wire to RING wire result */
      p_rft->f_rft_t2r = fConstFactor * pTapiRes->RFT_T2R;
   }
   else
   {
      /* RFT RING wire to GND result */
                                     ...
                                     IFX_LT_GR909_ROH_t *p_roh)
{
   ...
   p_roh->f_roh_t2r_l = fConstFactor * pTapiRes->ROH_T2R_L;
   /* ROH result high value */
   p_roh->f_roh_t2r_h = fConstFactor * pTapiRes->ROH_T2R_H;
                                     ...
                                     IFX_LT_GR909_RIT_t *p_rit)
{
   ...
   fConstFactor = RFT_ROH_RIT_CALC(dxt_cfg.f_R1);
   /* RIT result */
   p_rit->f_rit_res = fConstFactor * pTapiRes->RIT_RES;
