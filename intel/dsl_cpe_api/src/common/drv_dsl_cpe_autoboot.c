...
DSL_Error_t DSL_DRV_AutobootSignalDeviceEvt(
   DSL_Context_t *pContext)
{
   ...
   DSL_DRV_ThreadParams_t *param)
{
   ...
   DSL_int_t nOsRet = 0;
   DSL_Error_t nRet;
   DSL_boolean_t bFwEventActivation, bAutobootThreadShutdown = DSL_FALSE;
   DSL_Error_t nErrCode = DSL_SUCCESS;
#if defined(INCLUDE_DSL_CPE_API_VRX) || \
   ...
   while( bAutobootThreadShutdown == DSL_FALSE && nErrCode == DSL_SUCCESS)
   {
      ...
      {
         DSL_DRV_AutobootHandleDeviceEvt(pContext);
      }
   ...
   DSL_Context_t *pContext)
{
   ...
   if (bAutobootRestart)
   {
      if (!DSL_DRV_BONDING_ENABLED || !bSoftRestart)
      {
...
#else
         if (DSL_DRV_BONDING_ENABLED && DSL_DRV_LINES_PER_DEVICE == 2 && bSoftRestart)
         {
   ...
   DSL_Context_t *pContext)
{
   ...
   if (nErrCode == DSL_SUCCESS)
   {
    ...
    defined(INCLUDE_DSL_CPE_API_VRX)
               ...
               if (DSL_DRV_BONDING_ENABLED &&  nPortMode == DSL_PORT_MODE_DUAL)
               {
               ...
               DSL_DEBUG(DSL_DBG_WRN, (pContext,
                  SYS_DBG_WRN"DSL[%02d]: Reboot on training timeout (%d)!!!"
                  DSL_DRV_CRLF, DSL_DEV_NUM(pContext),
...
#  endif
   ...
   DSL_boolean_t bUseFWOptimizedDownload = DSL_FALSE;
   ...
   if (nPortMode == DSL_PORT_MODE_SINGLE)
   {
   ...
   DSL_CTX_READ_SCALAR(pContext, nErrCode, pDevCtx->bUseFWOptimizedDownload, bUseFWOptimizedDownload);

   while(1)
   {
      if (bUseFWOptimizedDownload)
      {
         /* Try optimized FW download */
         nErrCode = DSL_DRV_FwDownload(pContext,
            (DSL_char_t*) 0, 0,
            (DSL_char_t*) 0, 0,
            &nLoff, &nOff, DSL_TRUE, DSL_FALSE);

         if (nErrCode != DSL_SUCCESS)
         {
            DSL_DEBUG( DSL_DBG_WRN,
                       (pContext, SYS_DBG_WRN"DSL[%02d]: WARNING - FW optimized download failed!"
                        DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
         }
         else
         {
