...
DSL_devCtx_t ifxDevices[DSL_DRV_MAX_SUPPORTED_ENTITIES] = { {DSL_NULL} };
   ...
   DSL_IN_OUT DSL_InstanceStatus_t *pData)
{
   ...
   /* Expected to check flag only from current Context */
   pData->data.bFwEventActivation      = pContext->bFwEventActivation;
...
{
   if (nNum >= DSL_DRV_ENTITIES || pRefOpenContext == DSL_NULL)
   {
...
/*
   ...
   please refer to the description in the header file 'drv_dsl_cpe_intern.h'
*/
DSL_Error_t DSL_DRV_LinkTerminate(
   DSL_IN DSL_Context_t *pContext)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;

   DSL_DEBUG( DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_LinkTerminate"DSL_DRV_CRLF,
      DSL_DEV_NUM(pContext)));

   nErrCode = DSL_DRV_DEV_LinkTerminate(pContext);
   if (nErrCode != DSL_SUCCESS)
   {
      DSL_DEBUG( DSL_DBG_ERR,
         (pContext, SYS_DBG_ERR"DSL[%02d]: Modem couldn't be terminated"DSL_DRV_CRLF,
         DSL_DEV_NUM(pContext)));
   }
   else
   {
      DSL_DEBUG( DSL_DBG_MSG,
         (pContext, SYS_DBG_MSG"DSL[%02d]: Modem was terminated..."DSL_DRV_CRLF,
         DSL_DEV_NUM(pContext)));
   ...
   DSL_DEBUG( DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: OUT - DSL_DRV_LinkTerminate"DSL_DRV_CRLF,
      DSL_DEV_NUM(pContext)));
   ...
   DSL_IN_OUT DSL_Init_t *pData)
{
   ...
   pContext->lineFeatureDataCfg[DSL_MODE_ADSL][DSL_UPSTREAM].bTrellisEnable = DSL_TRUE;
   pContext->lineFeatureDataCfg[DSL_MODE_ADSL][DSL_DOWNSTREAM].bTrellisEnable = DSL_TRUE;

   pContext->lineFeatureDataCfg[DSL_MODE_ADSL][DSL_UPSTREAM].bBitswapEnable = DSL_TRUE;
   pContext->lineFeatureDataCfg[DSL_MODE_ADSL][DSL_DOWNSTREAM].bBitswapEnable = DSL_TRUE;
...
#else
   pContext->lineFeatureDataCfg[DSL_MODE_ADSL][DSL_DOWNSTREAM].b20BitSupport = DSL_FEATURE_NA;
#endif /* INCLUDE_DSL_CPE_API_DANUBE */

#ifdef INCLUDE_DSL_CPE_API_VRX
   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_UPSTREAM].bTrellisEnable = DSL_TRUE;
   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_DOWNSTREAM].bTrellisEnable = DSL_TRUE;

   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_UPSTREAM].bBitswapEnable = DSL_TRUE;
   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_DOWNSTREAM].bBitswapEnable = DSL_TRUE;

   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_DOWNSTREAM].bReTxEnable = DSL_TRUE;
   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_UPSTREAM].bReTxEnable = DSL_TRUE;

   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_UPSTREAM].b20BitSupport = DSL_FEATURE_NA;
   pContext->lineFeatureDataCfg[DSL_MODE_VDSL][DSL_DOWNSTREAM].b20BitSupport = DSL_FEATURE_NA;
   ...
   pContext->RebootCriteriaNeConfigData[DSL_MODE_ADSL] =
      DSL_REBOOT_CRITERIA_ESE | DSL_REBOOT_CRITERIA_LOM | DSL_REBOOT_CRITERIA_LOF |
      DSL_REBOOT_CRITERIA_LOS;
#if defined(INCLUDE_DSL_CPE_API_DANUBE)
   pContext->RebootCriteriaNeConfigData[DSL_MODE_ADSL] |=
                                          DSL_REBOOT_CRITERIA_NEGATIVE_MARGIN;
#endif

#ifdef INCLUDE_DSL_CPE_API_VRX
   pContext->RebootCriteriaNeConfigData[DSL_MODE_VDSL] =
      DSL_REBOOT_CRITERIA_ESE | DSL_REBOOT_CRITERIA_LOM | DSL_REBOOT_CRITERIA_LOF |
      DSL_REBOOT_CRITERIA_LOS;
#endif /* INCLUDE_DSL_CPE_API_VRX*/
...
#ifdef INCLUDE_DSL_CPE_API_VRX
   /* SRA disabled by default for VRX in ADSL mode */
   ...
   /* SRA enabled by default for VRX in VDSL mode */
...
#else
   /* SRA disabled by default for ARX */
   pContext->rateAdaptationMode[DSL_MODE_ADSL][DSL_UPSTREAM] = DSL_G997_RA_MODE_AT_INIT;
   pContext->rateAdaptationMode[DSL_MODE_ADSL][DSL_DOWNSTREAM] = DSL_G997_RA_MODE_AT_INIT;
#endif /* INCLUDE_DSL_CPE_API_VRX*/
   ...
   DSL_IN_OUT DSL_AutobootLoadFirmware_t *pData)
{
   ...
   if (nErrCode == DSL_SUCCESS)
   {
      ...
      nErrCode = DSL_DRV_FwDownload(
                    ...
                    pData->data.bLastChunk, pData->data.bForceDownload);
#else
      /*
      ...
      else
      {
         switch (pData->data.nCommand)
         {
            ...
            case DSL_AUTOBOOT_CTRL_RESTART_FULL:
               ...
               {
                  DSL_CTX_WRITE_SCALAR(pContext, nErrCode, pDevCtx->bUseFWOptimizedDownload, DSL_FALSE);
               }
   ...
   DSL_OUT DSL_AutobootStatus_t *pData)
{
   ...
   DSL_CTX_READ(pContext, nErrCode, nAutobootStatus, pData->data);
   ...
   DSL_IN DSL_FirmwareRequestType_t nFirmwareRequestType)
{
   ...
   {
      DSL_DEBUG( DSL_DBG_WRN,
         (pContext, SYS_DBG_WRN"DSL[%02d]: WARNING - Autoboot thread is not started!"
         DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

      /* Set nFirmwareRequestType in the DSL CPE context*/
      DSL_CTX_WRITE_SCALAR(pContext, nErrCode,
         nAutobootStatus.nFirmwareRequestType, nFirmwareRequestType);
   ...
   DSL_OUT DSL_ResourceUsageStatistics_t *pData)
{
   ...
   */
   ...
   DSL_DEBUG( DSL_DBG_PRN, (pContext,
      SYS_DBG_PRN"DSL:   device context structure     : %10d bytes" DSL_DRV_CRLF, (int)(sizeof(DSL_devCtx_t)*DSL_DRV_ENTITIES)));
    ...
    DSL_IN_OUT DSL_VersionInformation_t *pData)
{
   ...
   if (nErrCode == DSL_SUCCESS)
   {
      nErrCode = DSL_DRV_DEV_ChipSetTypeGet(pContext, DSL_NULL, pData->data.DSL_ChipSetType);
   }
   ...
   if (nErrCode >= DSL_SUCCESS)
   {
      ...
      if (bFirmwareReady != DSL_FALSE)
      {
         ...
         if (nErrCode >= DSL_SUCCESS)
         {
            nErrCode = DSL_DRV_DEV_ChipHardwareVersionGet(pContext, &verNum,
               pData->data.DSL_ChipSetHWVersion);
         }

         if (nErrCode >= DSL_SUCCESS)
         {
            /* update chipset type based on HW_Version (VRX517 eFUSE) */
            DSL_DRV_DEV_ChipSetTypeGet(pContext, &verNum, pData->data.DSL_ChipSetType);
   ...
   DSL_IN_OUT DSL_LineFeature_t *pData)
{
   ...
   DSL_CTX_READ_SCALAR(pContext, nErrCode,
      lineFeatureDataCfg[pData->nDslMode][DSL_DOWNSTREAM].bReTxEnable,
      bReTxEnableDs);
   ...
   if (pData->data.b20BitSupport != DSL_FEATURE_NA)
   {
#if defined(INCLUDE_DSL_CPE_API_DANUBE)
      /* b20BitSupport is supported only on ADSL platforms and only for
         downstream direction */
      if ((pData->nDslMode != DSL_MODE_ADSL) ||
          (pData->nDirection != DSL_DOWNSTREAM))
#endif
      {
         pData->data.b20BitSupport = DSL_FEATURE_NA;
         nRet = DSL_WRN_CONFIG_PARAM_IGNORED;
      }
   }
   ...
   DSL_IN_OUT DSL_LineFeature_t *pData)
{
   ...
   DSL_CTX_READ(pContext, nErrCode,
                lineFeatureDataCfg[pData->nDslMode][pData->nDirection],
                pData->data);
   ...
   DSL_IN_OUT DSL_LoopLengthStatus_t *pData)
{
   ...
   {
      DSL_DEBUG(DSL_DBG_WRN, (pContext, SYS_DBG_WRN"DSL[%02d]: ERROR - "
...
#if defined(INCLUDE_DSL_CPE_API_VRX)
   /* Negative margine not supported for VRX */
   if (pData->data.nRebootCriteria & DSL_REBOOT_CRITERIA_NEGATIVE_MARGIN)
   {
      pData->data.nRebootCriteria &= ~DSL_REBOOT_CRITERIA_NEGATIVE_MARGIN;
      nRet = DSL_WRN_CONFIG_PARAM_IGNORED;
   }
#endif /* defined(INCLUDE_DSL_CPE_API_VRX) */

   DSL_CTX_WRITE(pContext, nErrCode, RebootCriteriaNeConfigData[pData->nDslMode],
   ...
   DSL_IN_OUT DSL_RebootCriteriaConfig_t *pData)
{
   ...
   DSL_CTX_READ(pContext, nErrCode, RebootCriteriaNeConfigData[pData->nDslMode],
                                                   pData->data.nRebootCriteria);
   DSL_DEBUG(DSL_DBG_MSG,
...
*/
#ifndef DSL_DEBUG_DISABLE
DSL_Error_t DSL_DRV_DBG_ModuleDestinationSet(
   DSL_IN DSL_Context_t *pContext,
   DSL_IN DSL_DBG_ModuleDestination_t *pData)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   ...
   DSL_g_dbgDestination = pData->data.nDbgDestination;

   DSL_DEBUG( DSL_DBG_LOCAL,
      (pContext, SYS_DBG_LOCAL"DSL[%02d]: new debug destination: %s" DSL_DRV_CRLF ,
      DSL_DEV_NUM(pContext),
      (pData->data.nDbgDestination == DSL_DBG_DST_CONSOLE)?"CONSOLE":"LOGGER") );
...
*/
#ifndef DSL_DEBUG_DISABLE
DSL_Error_t DSL_DRV_DBG_ModuleDestinationGet(
   DSL_IN DSL_Context_t *pContext,
   DSL_IN_OUT DSL_DBG_ModuleDestination_t *pData)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   ...
   pData->data.nDbgDestination = DSL_g_dbgDestination;
...
DSL_Error_t DSL_DRV_T1413_XTUO_RevisionStatusGet(
   DSL_IN DSL_Context_t *pContext,
   DSL_OUT DSL_T1413RevisionStatus_t *pData)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   DSL_uint8_t XTSE1 = 0;
   DSL_LineStateValue_t nCurrentState = DSL_LINESTATE_UNKNOWN;
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: IN - DSL_DRV_T1413_XTUO_RevisionStatusGet"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   if (!DSL_DRV_VRX_FirmwareXdslModeCheck(pContext, DSL_VRX_FW_ADSL))
   {
      DSL_DEBUG(DSL_DBG_WRN, (pContext, SYS_DBG_WRN
         "DSL[%02d]: ERROR - xTU-O T1.413 Revision are not supported "
               "in current VDSL mode" DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

      return DSL_ERR_NOT_SUPPORTED_IN_CURRENT_VDSL_MODE;
   }

   DSL_CTX_READ(pContext, nErrCode, xtseCfg[0], XTSE1);

   if (XTSE1 & XTSE_1_01_A_T1_413)
   {
      DSL_CTX_READ_SCALAR(pContext, nErrCode, nLineState, nCurrentState);
      ...
      if ((nCurrentState == DSL_LINESTATE_SHOWTIME_TC_SYNC) ||
          (nCurrentState == DSL_LINESTATE_SHOWTIME_NO_SYNC))
      {
         DSL_CTX_READ(pContext, nErrCode, xtseCurr[0], XTSE1);

         if (XTSE1 & XTSE_1_01_A_T1_413)
         {
            nErrCode = DSL_DRV_DEV_T1413_XTUO_RevisionStatusGet(pContext, &(pData->data));
         }
         else
         {
            DSL_DEBUG(DSL_DBG_ERR,
               (pContext, SYS_DBG_ERR"DSL[%02d]: ERROR - T1.413 XTUO Revision Number get "
               "not supported in current ADSL mode or Annex!"
               DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

            pData->data.nT1413RevisionNum = 0;
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: OUT - DSL_DRV_T1413_XTUO_RevisionStatusGet, retCode=%d"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext), nErrCode));

   return nErrCode;
}

DSL_Error_t DSL_DRV_T1413_XTUR_RevisionStatusGet(
   DSL_IN DSL_Context_t *pContext,
   DSL_OUT DSL_T1413RevisionStatus_t *pData)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: IN - DSL_DRV_T1413_XTUR_RevisionStatusGet"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   {
      DSL_DEBUG(DSL_DBG_WRN, (pContext, SYS_DBG_WRN
         "DSL[%02d]: ERROR - xTU-R T1.413 Revision are not supported "
               "in current VDSL mode" DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   if (XTSE1 & XTSE_1_01_A_T1_413)
   {
      nErrCode = DSL_DRV_DEV_T1413_XTUR_RevisionStatusGet(pContext, &(pData->data));
   }
   else
   {
      DSL_DEBUG(DSL_DBG_ERR,
         (pContext, SYS_DBG_ERR"DSL[%02d]: ERROR - T1.413 XTUR Revision Number get "
         "not supported in current ADSL mode or Annex!"
         DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: OUT - DSL_DRV_T1413_XTUR_RevisionStatusGet, retCode=%d"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext), nErrCode));

   return nErrCode;
}

DSL_Error_t DSL_DRV_T1413_XTUO_VendorRevisionStatusGet(
   DSL_IN DSL_Context_t *pContext,
   DSL_OUT DSL_T1413VendorRevisionStatus_t *pData)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   DSL_uint8_t XTSE1 = 0;
   DSL_LineStateValue_t nCurrentState = DSL_LINESTATE_UNKNOWN;
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: IN - DSL_DRV_T1413_XTUO_VendorRevisionStatusGet"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   {
      DSL_DEBUG(DSL_DBG_WRN, (pContext, SYS_DBG_WRN
         "DSL[%02d]: ERROR - xTU-O T1.413 Vendor Revision are not supported "
               "in current VDSL mode" DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   if (XTSE1 & XTSE_1_01_A_T1_413)
   {
          ...
          (nCurrentState == DSL_LINESTATE_SHOWTIME_NO_SYNC))
      {
         ...
         if (XTSE1 & XTSE_1_01_A_T1_413)
         {
            nErrCode = DSL_DRV_DEV_T1413_XTUO_VendorRevisionStatusGet(pContext, &(pData->data));
         }
         else
         {
            DSL_DEBUG(DSL_DBG_ERR,
               (pContext, SYS_DBG_ERR"DSL[%02d]: ERROR - T1.413 XTUO Vendor Revision Number get "
               "not supported in current ADSL mode or Annex!"
               DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

            pData->data.nT1413VendorRevisionNum = 0;
            nErrCode = DSL_ERR_NOT_SUPPORTED_IN_CURRENT_ADSL_MODE_OR_ANNEX;
         }
      }
      else
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: OUT - DSL_DRV_T1413_XTUO_VendorRevisionStatusGet, retCode=%d"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext), nErrCode));

   return nErrCode;
}

DSL_Error_t DSL_DRV_T1413_XTUR_VendorRevisionStatusGet(
   DSL_IN DSL_Context_t *pContext,
   DSL_OUT DSL_T1413VendorRevisionStatus_t *pData)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: IN - DSL_DRV_T1413_XTUR_VendorRevisionStatusGet"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   {
      DSL_DEBUG(DSL_DBG_WRN, (pContext, SYS_DBG_WRN
         "DSL[%02d]: ERROR - xTU-R T1.413 VendorRevision are not supported "
               "in current VDSL mode" DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   if (XTSE1 & XTSE_1_01_A_T1_413)
   {
      nErrCode = DSL_DRV_DEV_T1413_XTUR_VendorRevisionStatusGet(pContext, &(pData->data));
   }
   else
   {
      DSL_DEBUG(DSL_DBG_ERR,
         (pContext, SYS_DBG_ERR"DSL[%02d]: ERROR - T1.413 XTUR Vendor Revision Number get "
         "not supported in current ADSL mode or Annex!"
         DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   DSL_DEBUG( DSL_DBG_MSG, (pContext,
      "DSL[%02d]: OUT - DSL_DRV_T1413_XTUR_VendorRevisionStatusGet, retCode=%d"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext), nErrCode));
   ...
   DSL_IN_OUT DSL_PSDCalibration_t *pData)
{
   ...
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_PSDCalibrationConfigGet"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

   if (pData->nDirection == DSL_UPSTREAM)
   {
      DSL_CTX_READ(pContext, nErrCode, usCalibration, pData->data);
   }
   else
   {
      DSL_CTX_READ(pContext, nErrCode, dsCalibration, pData->data);
   }
   ...
   DSL_IN_OUT DSL_PSDCalibration_t *pData)
{
   ...
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_PSDCalibrationConfigSet"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

   if (pData->nDirection == DSL_UPSTREAM)
   {
      DSL_CTX_WRITE(pContext, nErrCode, usCalibration, pData->data);
   }
   else
   {
      DSL_CTX_WRITE(pContext, nErrCode, dsCalibration, pData->data);
   }
...
DSL_IOCTL_REGISTER(DSL_FIO_G997_ATTAINABLE_NDR_STATUS_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_G997_AttainableNdrStatusGet,
                   sizeof(DSL_G997_AttainableNdrStatus_t)),
#endif /* INCLUDE_DSL_CPE_API_VRX*/
...
DSL_IOCTL_REGISTER(DSL_FIO_G997_RATE_ADAPTATION_STATUS_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_G997_RateAdaptationStatusGet,
                   sizeof(DSL_G997_RateAdaptationStatus_t)),
...
#ifndef DSL_DEBUG_DISABLE
/* DSL_FIO_DBG_MODULE_LEVEL_SET */
DSL_IOCTL_REGISTER(DSL_FIO_DBG_MODULE_DESTINATION_SET, DSL_IOCTL_HELPER_SET,
                   DSL_FALSE, DSL_DRV_DBG_ModuleDestinationSet,
                   sizeof(DSL_DBG_ModuleDestination_t)),
/* DSL_FIO_DBG_MODULE_LEVEL_GET */
DSL_IOCTL_REGISTER(DSL_FIO_DBG_MODULE_DESTINATION_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_DBG_ModuleDestinationGet,
                   sizeof(DSL_DBG_ModuleDestination_t)),
#endif /* DSL_DEBUG_DISABLE*/
#endif /* INCLUDE_DSL_CPE_DEBUG_LOGGER_SUPPORT */
#ifdef INCLUDE_REAL_TIME_TRACE
...
DSL_IOCTL_REGISTER(DSL_FIO_T1413_XTUO_REVISION_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_T1413_XTUO_RevisionStatusGet,
                   sizeof(DSL_T1413RevisionStatus_t)),
/* DSL_FIO_T1413_XTUR_REVISION_GET */
DSL_IOCTL_REGISTER(DSL_FIO_T1413_XTUR_REVISION_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_T1413_XTUR_RevisionStatusGet,
                   sizeof(DSL_T1413RevisionStatus_t)),
/* DSL_FIO_T1413_XTUO_VENDOR_REVISION_GET */
DSL_IOCTL_REGISTER(DSL_FIO_T1413_XTUO_VENDOR_REVISION_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_T1413_XTUO_VendorRevisionStatusGet,
                   sizeof(DSL_T1413VendorRevisionStatus_t)),
/* DSL_FIO_T1413_XTUR_VENDOR_REVISION_GET */
DSL_IOCTL_REGISTER(DSL_FIO_T1413_XTUR_VENDOR_REVISION_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_T1413_XTUR_VendorRevisionStatusGet,
                   sizeof(DSL_T1413VendorRevisionStatus_t)),
/* DSL_FIO_PSD_CALIBRATION_CONFIG_GET */
DSL_IOCTL_REGISTER(DSL_FIO_PSD_CALIBRATION_CONFIG_GET, DSL_IOCTL_HELPER_GET,
                   DSL_FALSE, DSL_DRV_PSDCalibrationConfigGet,
                   sizeof(DSL_PSDCalibration_t)),
/* DSL_FIO_PSD_CALIBRATION_CONFIG_SET */
...
/*  DSL CPE API Ioctl Table */
DSL_IOCTL_Table_t bondingIoctlTable[] = {
/* DSL_FIO_BND_HW_INIT */
...
/*  DSL CPE API Ioctl Table */
DSL_IOCTL_Table_t onChipBondingIoctlTable[] = {
   ...
   DSL_uint32_t nArg)
{
...
#ifndef DSL_DEBUG_DISABLE
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_IoctlHandle, call 0x%08X - "
   ...
   switch (nCommand)
   {
      ...
      default:
         ...
         while (pIoctlRecord->pFunc)
         {
            if (pIoctlRecord->nCommand != nCommand)
            {
               /* Move to the next Table entry*/
               pIoctlRecord++;
               if (!pIoctlRecord->pFunc && DSL_DRV_BONDING_ENABLED && pIoctlTable == ioctlTable)
               {
                   pIoctlTable = bondingIoctlTable;
                   pIoctlRecord = bondingIoctlTable;
               }
               else if (!pIoctlRecord->pFunc && DSL_DRV_BONDING_ENABLED && DSL_CHANNELS_PER_LINE > 1 && pIoctlTable == bondingIoctlTable)
               {
                   pIoctlTable = onChipBondingIoctlTable;
                   pIoctlRecord = onChipBondingIoctlTable;
               }
               continue;
            }

            nErrCode = DSL_DRV_IoctlHandleHelperCall(pOpenCtx, pContext,
               bIsInKernel, pIoctlRecord->bInstanceCall, pIoctlRecord->accessType,
               pIoctlRecord->pFunc, (DSL_void_t*)nArg, pIoctlRecord->nArgSz);
   ...
   DSL_DEBUG(DSL_DBG_MSG, (pContext, SYS_DBG_MSG"DSL[%02d]: OUT - DSL_DRV_IoctlHandle,"
      " return(from 0x%08X - %s) %d"DSL_DRV_CRLF,
   ...
   DSL_boolean_t bInit)
{
   ...
   {
      DSL_DEBUG(DSL_DBG_WRN,
         (pContext, SYS_DBG_WRN"DSL[%02d]: ERROR - This function is applicable "
