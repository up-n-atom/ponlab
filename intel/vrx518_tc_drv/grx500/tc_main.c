 ...
 * @History:
 * Version 1.5.7:
 *   FEATURE_CONF_DESC_LENGTH
 *     518 TC driver supports configuration
 *     number of US descriptors.
 *     Modification: ACA, US FP, US QoS, OutQ blocks.
 * Version 1.5.8:
 *  Support power saving and PPE dynamic frequency setting
 * Version 1.5.9:
 * 	FEATURE_POWER_DOWN
 * */
#define DRV_VERSION	"1.5.12.5"
...
/* 2017-12-15: from version 1.5.5, turn on power saving mode */
static u32 powersaving = 1;
#ifdef FIX_DMA_BYTE_ALIGNMENT
static u32 ena_dma_byte_alignment;
#endif
...
#ifdef FIX_DMA_BYTE_ALIGNMENT
module_param(ena_dma_byte_alignment, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(ena_dma_byte_alignment, "Enable DMA Byte Alignment SG: 0 - Disable/1-Enable");
#endif
...
static int pcie_ep_probe(struct tc_priv *priv)
{
	...
	for (i = 0; i < dev_num; i++) {
        ...
        /* 20180921: Turn on PMU_EMA to prevent BAR hang in DSL FW */
        tc_clkon(&priv->ep_dev[i], PMU_EMA);
	}
...
static void pcie_ep_release(struct tc_priv *priv)
{
	...
	for (i = 0; i < priv->ep_num; i++) {
        tc_clkoff(&priv->ep_dev[i], PMU_EMA);
		dc_ep_dev_info_release(i);
    }
}
...
static inline void init_local_param(struct tc_priv *priv)
{
	...
	#ifdef FIX_DMA_BYTE_ALIGNMENT
	priv->param.ena_dma_tc_sg = ena_dma_byte_alignment;
	#endif
...
void tc_unload(struct tc_priv *priv)
{
	...
	if (priv->tc_mode == TC_PTM_SL_MODE ||
		...
		pr_err("%s-%d: Invalid TC MODE %d\n",
			__func__, __LINE__, priv->tc_mode);
		return;
	}
...
void tc_load(struct tc_priv *priv, u32 id, enum dsl_tc_mode mode)
{
    ...
    if (mode == TC_NONE_MODE) {
	    ...
	    if (priv->dsl_pwr_policy[id]) {
		power_configure(priv, priv->dsl_pwr_policy[id] - 1,
		    id, SW_OFF_VRX518_PLL);
		tc_dbg(priv, MSG_SWITCH, "TC Request OFF on line [%d] with PLL [%d]\n",
		    id, priv->dsl_pwr_policy[id]);
		/* clear the policy */
		priv->dsl_pwr_policy[id] = 0;
	    }
#endif
		priv->tc_stat = NO_TC;
		return;
	...
	tc_dbg(priv, MSG_SWITCH, "TC Load on [%d]: from[%d] to[%d]\n",
		id, priv->tc_mode,
		mode);
