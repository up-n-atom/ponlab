...
static void atm_aca_init(struct atm_priv *priv)
{
	...
	priv->tc_priv->tc_ops.soc_cfg_get(NULL, &priv->tc_priv->cfg, 0, 0);
...
int proc_clear_atm_wanmib(struct atm_priv *priv)
{
	...
	/* FEATURE_CONF_DESC_LENGTH: QOSQ_NUM - 2 */
	for (i = 0; i < 2; i++) {
		tc_memset(priv,
	...
	tc_mem_write(priv, fpi_addr(__QOSQ_MIB_BASE), &qos, (sizeof(qos) * QOSQ_NUM));
...
static void atm_umt_init(struct atm_priv *priv)
{
	...
	tc_dbg(priv->tc_priv, MSG_INIT, "\tUMT period: %d, dst: 0x%x\n",
		period, dst);
	tcpriv->tc_ops.umt_init(0, period, dst, v_dst);
}
...
int mpoa_setup_proc(int vpi, int vci, struct net_device *dev, int mpoa_type, int f_llc)
{
	unsigned int conn;
	struct atm_priv *priv;
	int ep_id;

	priv = (struct atm_priv *)g_atm_tc;
	if (!priv || !dev) {
		pr_err("%s : Invalid ATM private data\n", __func__);
		return -EINVAL;
	}
	tc_dbg(priv->tc_priv, MSG_INIT,
		"VPI[%d]VCI[%d] %s - mpoa_type: %d, f_llc: %d\n",
		vpi, vci, dev->name, mpoa_type, f_llc);
