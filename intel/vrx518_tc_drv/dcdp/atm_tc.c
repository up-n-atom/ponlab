...
static void atm_aca_init(struct atm_priv *priv)
{
	...
	pr_info("txin: bswp: %d, hdsz:%d, pd: dbase(0x%x), dnum(%d), sz_indw(%d), soc_dbase:0x%x, soc_dnum:0x%x, soc cnt addr: 0x%x\n",
	...
	pr_info("txout: bswp: %d, hdsz:%d, pd: dbase(0x%x), dnum(%d), sz_indw(%d), soc_dbase:0x%x, soc_dnum:0x%x, soc cnt addr: 0x%x\n",
	...
	pr_info("rxout: bswp: %d, hdsz:%d, pd: dbase(0x%x), dnum(%d), sz_indw(%d), soc_dbase:0x%x, soc_dnum:0x%x, soc cnt addr: 0x%x\n",
	...
	pr_info("rxin: bswp: %d, hdsz:%d, pd: dbase(0x%x), dnum(%d), sz_indw(%d), soc_dbase:0x%x, soc_dnum:0x%x, soc cnt addr: 0x%x\n",
...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
	...
	prio = skb->priority >= ATM_TX_Q_NUM ?
			ATM_TX_Q_NUM - 1 : skb->priority;
	mpoa_pt = 1;
...
int ppe_send_oam(struct atm_vcc *vcc, void *cell, int flags)
{
	...
	#endif
	if (ATM_PRIO_Q_NUM <= priv->tc_priv->param.oam_prio)
...
#ifdef CONFIG_SOC_TYPE_XWAY
static int xway_atm_get_qid_by_vcc(struct net_device *dev, struct sk_buff *skb,
		void *vcc, uint32_t flags)
{
	struct atm_priv *priv;
	unsigned int conn;
...
int get_mpoa_type(uint32_t dslwan_qid, uint32_t *mpoa_type)
{
    ...
    if (qid >= 0 && qid < txq_num()) {
    	tc_mem_read(priv, &rx, fpi_addr(WRX_Q_CFG(qid)),
    		sizeof(rx));
