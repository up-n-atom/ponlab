...
/**
   Parse layout specific arguments.

   \param argc - number of parameters
   \param argv - array of parameter strings
*/
DSL_CPE_STATIC  DSL_void_t DSL_CPE_ArgParseLayout (
   DSL_int32_t argc,
   DSL_char_t * argv[]
)
{
   ...
   /* extract number of devices, lines and channels */
   while (1)
   {
      DSL_int_t c;
      ...
      {
      case 'V':
            sscanf(optarg, "%hhu", &g_nDevices);
            break;
      case 'L':
            sscanf(optarg, "%hhu", &g_nLines);
          break;
      case 'C':
            sscanf(optarg, "%hhu", &g_nChannels);
          break;
      default:
          break;
      }
      ...
      case 'b':
         {
            if (DSL_CPE_DSL_ENTITIES <= 1 )
            {
               printf( DSL_CPE_PREFIX " backward compatibility setting ignored ");
      ...
      case 'S':
         if( optarg != NULL )
         {
            ...
            if (token != DSL_NULL)
            {
               ...
               /* Automatic detection from XTSE configuration is NOT required and
                  supported anymore. For backward compatibility reason this value
                  is mapped to the new default value DSL_ACT_SEQ_STD */
               if (g_ActivationFsmConfig.nActivationSequence == DSL_ACT_SEQ_AUTO)
               {
                  g_ActivationFsmConfig.nActivationSequence = DSL_ACT_SEQ_STD;
               }
   ...
   DSL_CPE_File_t *out)
{
   ...
   if (pContext->bBackwardCompMode)
   {
      ...
      if (nDeviceNumber >= DSL_CPE_DSL_ENTITIES)
      {
         DSL_CCA_DEBUG(DSL_CCA_DBG_ERR,
            (DSL_CPE_PREFIX "Invalid device number (%d) specified!" DSL_CPE_CRLF,
            nDeviceNumber));
      ...
      if (DSL_CPE_DSL_ENTITIES > 1 )
      {
         if(arg != DSL_NULL)
         {
            ...
            else
            {
               ...
               /* Get device number*/
               sscanf (arg, "%d", &nDevNum);

               /* Check device number*/
               if (nDevNum >= DSL_CPE_DSL_ENTITIES)
               {
                  DSL_CCA_DEBUG(DSL_CCA_DBG_ERR,
                                (DSL_CPE_PREFIX "Invalid device number (%d) specified, valid range 0...%d!" DSL_CPE_CRLF,
                                 nDevNum, (DSL_CPE_DSL_ENTITIES-1)));
   ...
   DSL_char_t *pcFw2)
{
   ...
   /*
      If firmware name is not defined by current call
      choose last defined file (or default if startup
      value was not changed).
   */
   if (nPortMode == DSL_PORT_MODE_DUAL && DSL_CPE_LINES_PER_DEVICE == 2)
   {
      if ( pcFw2 == DSL_NULL )
      {
   ...
   do
   {
      if ( bOptimize == 1 )
      {
         ...
         */
         ...
         {
            DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
               " firmware file not found!" DSL_CPE_CRLF));
            nRet = DSL_ERROR;
            break;
         }
   ...
   DSL_CPE_ScriptBuffer_t *script_buf)
{
   ...
   if (!script_buf->buffer_cmd)
   {
      return DSL_ERROR;
   }

   if (!script_buf->buffer_tmp)
   {
      return DSL_ERROR;
   }
   ...
   while (1)
   {
      memset (str_command, 0, script_buf->nSize);
      ...
      if ((sscanf (buf, nSize, str_command) == 0) || (buf[0] == '#'))
      {
         continue;
      }
#else
   ...
   while (1)
   {
      ...
      if ((sscanf (buf, "%"_MKSTR(MAX_SCRIPT_CMD_LEN)"s", str_command) == 0) || (buf[0] == '#'))
      {
         continue;
      }
      ...
      {
         if (sscanf (buf, "%"_MKSTR(MAX_SCRIPT_CMD_LEN)"s %3s", str_command, echo_val) != 0)
         {
   ...
   DSL_CPE_ScriptSection_t searchSection)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   DSL_CPE_File_t *pFile;
   DSL_CPE_ScriptBuffer_t script_buf = {0};
   ...
   do
   {
      ...
      script_buf.nSize = MAX_SCRIPT_CMD_LEN + 1;
      script_buf.buffer_cmd = DSL_CPE_Malloc(script_buf.nSize);
      script_buf.buffer_tmp = DSL_CPE_Malloc(script_buf.nSize);

      if ((script_buf.buffer_cmd == DSL_NULL) || (script_buf.buffer_tmp == DSL_NULL))
      {
         DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
            "Could not allocate memory %d bytes" DSL_CPE_CRLF, script_buf.nSize));
         nErrCode = DSL_ERROR;
      }
      else
      {
         nErrCode = DSL_CPE_ScriptExecute(pContext, nDevice, pFile, searchSection, &script_buf);
      }

      DSL_CPE_Free(script_buf.buffer_cmd);
      DSL_CPE_Free(script_buf.buffer_tmp);
      script_buf.nSize = 0;
   ...
   DSL_EventStatus_t *pEvent)
{
   ...
   if (g_sRcScript != DSL_NULL)
   {
      ...
      else if ( (nLineState == DSL_LINESTATE_LOOPDIAGNOSTIC_ACTIVE) &&
                (g_nPrevLineState[nDevice] != DSL_LINESTATE_LOOPDIAGNOSTIC_ACTIVE) )
      {
         if (DSL_CPE_SetEnv("DSL_INTERFACE_STATUS", "LDM_ACTIVE") == DSL_SUCCESS)
         {
            bExec = DSL_TRUE;
         }
      }
      else if ( (nLineState == DSL_LINESTATE_LOOPDIAGNOSTIC_COMPLETE) &&
                (g_nPrevLineState[nDevice] != DSL_LINESTATE_LOOPDIAGNOSTIC_COMPLETE) )
      {
         if (DSL_CPE_SetEnv("DSL_INTERFACE_STATUS", "LDM_COMPLETE") == DSL_SUCCESS)
         {
            bExec = DSL_TRUE;
         }
      }
...
#if defined(DSL_VRX_DEVICE_VR10) || defined(DSL_VRX_DEVICE_VR10_320)
      printf(DSL_CPE_PREFIX "Workaround for DSLCPE_SW-995: performing full Firmware "
         "download for device %d." DSL_CPE_CRLF, nDevice);
      pContext->bForceFirmwareDownload[nDevice] = DSL_TRUE;
      DSL_CPE_DownloadFirmware(pContext->fd[nDevice], nDevice,
         DSL_FW_REQUEST_XDSL, DSL_PORT_MODE_NA, DSL_NULL, DSL_NULL);
#endif
      break;
   ...
   DSL_PortMode_t nPortMode)
{
   ...
   else
   {
      ...
      {
         nRet = DSL_CPE_DownloadFirmware(fd, nDevice, nFwReqType, nPortMode, DSL_NULL, DSL_NULL);
...
static DSL_Error_t DSL_CPE_SwVersionFromStringGet(DSL_char_t* pVerString, DSL_SwVersion_t* pVer)
{
   ...
   nFields = sscanf(pVerString,"%d.%d.%d.%d", &pVer->nMajor, &pVer->nMinor, &pVer->nDevelopment,
               &pVer->nMaintenance);
   ...
   {
      sscanf(pPatchString,"-p%c%d",&dummy,&pVer->nPatch);
   }
   ...
   DSL_char_t * argv[])
{
...
#endif /* defined(INCLUDE_DSL_CPE_CLI_SUPPORT) */
...
#ifdef INCLUDE_DSL_CPE_CLI_SUPPORT
      if (DSL_CPE_TcpDebugCliIntfStart(pCtrlCtx, g_bEnableTcpCli) < 0)
      {
         DSL_CCA_DEBUG(DSL_CCA_DBG_ERR, (DSL_CPE_PREFIX
   ...
   if (bInit != 1)
   {
      ...
      for (nDevice = 0; nDevice < DSL_CPE_DSL_ENTITIES; nDevice++)
      {
         ...
         ret = DSL_CPE_DownloadFirmware(pCtrlCtx->fd[nDevice], nDevice,
