   ...
   DSL_CPE_File_t *out)
{
   ...
   sscanf (pCommands, "%u %x", &(pData.data.bEventActivation),
   ...
   DSL_CPE_File_t *out)
{
   ...
   sscanf (pCommands, "%u", &pData.nDirection);
   ...
   DSL_CPE_File_t *out)
{
   ...
   if (token != DSL_NULL)
   {
      for (i = 0; i < nParams; i++)
      {
         ...
         {
            nIdx = i;
            sscanf (token, "%bx", (unsigned char *)&(pData.data.SystemVendorID[nIdx]));
         }
         else if (i < nParamList[1])
         {
            nIdx = (DSL_uint16_t)(i - nParamList[0]);
            sscanf (token, "%bx", (unsigned char *)&(pData.data.VersionNumber[nIdx]));
         }
         else if (i < nParamList[2])
         {
            nIdx = (DSL_uint16_t)(i - nParamList[1]);
            sscanf (token, "%bx", (unsigned char *)&(pData.data.SerialNumber[nIdx]));
         }
#if (INCLUDE_DSL_CPE_API_VDSL_SUPPORT == 1)
         else if (i < nParamList[3])
         {
            nIdx = (DSL_uint16_t)(i - nParamList[2]);
            sscanf (token, "%bx", (unsigned char *)&(pData.data.Auxiliary.pData[nIdx]));
         }
#endif
         else
         {
   ...
   DSL_CPE_File_t *out)
{
   ...
   sscanf (pCommands, "%u %u", &pData->nDirection, &pData->nDeltDataType);
   ...
   DSL_CPE_File_t *out)
{
   ...
   sscanf ( pCommands, "%hu %hu %s %hu %hu %hu %hu",
