...
{
	local num_of_radios=`iw dev | grep -c "phy#"`
	local radios_up=0
	local primary_vap cur_vap

	for primary_vap in wlan0.0 wlan2.0 wlan4.0
	do
		cur_vap=`ifconfig | grep -c $primary_vap`
		radios_up=$((radios_up+cur_vap))
	done

	if [ $radios_up -eq $num_of_radios ]
	then
		echo 0
	else
		echo 1
...
{
	local timeout=0
	while [ `check_all_interfaces` -eq 1 ] && [ $timeout -lt 30 ]
	do
		sleep 1
		timeout=$((timeout+1))
	done
	...
	if [ "$ap_ack_policy" != "" ]; then
		...
		0)
			...
			if [ "$ap_ack_policy_mac" != "" ]; then
				ap_aid=`cat /proc/net/mtlk/${CURRENT_WLAN_NAME}/sta_list | grep "0" | grep "$ap_ack_policy_mac" | awk '{print $3}'`
