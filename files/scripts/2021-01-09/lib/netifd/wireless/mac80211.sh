...
drv_mac80211_init_be_config() {
	...
	config_add_int eht_oper_centr_freq_seg1_idx
}
...
mac80211_append_be_parameters() {
	if [ "$ieee80211be" = "1" ]; then
		...
		[ -n "$eht_oper_centr_freq_seg1_idx" ] && append base_cfg "eht_oper_centr_freq_seg1_idx=$eht_oper_centr_freq_seg1_idx" "$N"
	fi
}
...
mac80211_hostapd_setup_base() {
	...
	if [ "$band" = "6GHz" ]; then
		...
		case "$htmode" in
			...
			EHT320_0)
				...
				acs_eht_mode=0
				case "$channel" in
					1|5|9|13|17|21|25|29|33|37|41|45|49|53|57|61) eht_oper_centr_freq_seg1_idx=31 ;;
					33|37|41|45|49|53|57|61|65|69|73|77|81|85|89|93) eht_oper_centr_freq_seg1_idx=63 ;;
					65|69|73|77|81|85|89|93|97|101|105|109|113|117|121|125) eht_oper_centr_freq_seg1_idx=95 ;;
					97|101|105|109|113|117|121|125|129|133|137|141|145|149|153|157) eht_oper_centr_freq_seg1_idx=127 ;;
					129|133|137|141|145|149|153|157|161|165|169|173|177|181|185|189) eht_oper_centr_freq_seg1_idx=159 ;;
					161|165|169|173|177|181|185|189|193|197|201|205|209|213|217|221) eht_oper_centr_freq_seg1_idx=191 ;;
				esac
				case "$channel" in
					1|5|9|13|17|21|25|29) eht_oper_centr_freq_seg0_idx=15 ;;
					33|37|41|45|49|53|57|61) eht_oper_centr_freq_seg0_idx=47 ;;
					65|69|73|77|81|85|89|93) eht_oper_centr_freq_seg0_idx=79 ;;
					97|101|105|109|113|117|121|125) eht_oper_centr_freq_seg0_idx=111 ;;
					129|133|137|141|145|149|153|157) eht_oper_centr_freq_seg0_idx=143 ;;
					161|165|169|173|177|181|185|189) eht_oper_centr_freq_seg0_idx=175 ;;
					193|197|201|205|209|213|217|221) eht_oper_centr_freq_seg0_idx=207 ;;
				esac
			;;
			EHT320_1)
				#for 6GHz , ieee80211be/EHT need to be enabled for 320MHz
	...
	if [ "$band" = "6GHz" ]; then
		if [ "$ieee80211be" = "1" ]; then
			if [ -n "$eht_oper_chwidth" ]; then
				append base_cfg "eht_oper_chwidth=$eht_oper_chwidth" "$N"
			else
				[ -n "$he_oper_chwidth" ] && append base_cfg "eht_oper_chwidth=$he_oper_chwidth" "$N"
			fi
			...
			if [ -n "$eht_oper_centr_freq_seg1_idx" ]; then
				append base_cfg "eht_oper_centr_freq_seg1_idx=$eht_oper_centr_freq_seg1_idx" "$N"
			else
				[ -n "$he_oper_centr_freq_seg1_idx" ] && append base_cfg "eht_oper_centr_freq_seg1_idx=$he_oper_centr_freq_seg1_idx" "$N"
			fi
		else
			[ -n "$he_oper_chwidth" ] && append base_cfg "he_oper_chwidth=$he_oper_chwidth" "$N"
			[ -n "$he_oper_centr_freq_seg0_idx" ] && append base_cfg "he_oper_centr_freq_seg0_idx=$he_oper_centr_freq_seg0_idx" "$N"
...
drv_mac80211_setup_phy() {
	...
	do
		...
		if [ $is_phy_5g = '0' ]; then
			iw $phy info | grep -q "Device is meant only for DFS"
			is_phy_zwdfs=$?
			...
			for reconf_vap in $reconf_vaps_list; do
				[ -n "$hostapd_ctrl" ] && {
					if [ "$reconf_vap" = "$reconf_radio" ]; then
						ret=$(/usr/sbin/hostapd_cli -i${reconf_radio} reconf)
					else
						ret=$(/usr/sbin/hostapd_cli -i${reconf_radio} reconf ${reconf_vap})
					fi
