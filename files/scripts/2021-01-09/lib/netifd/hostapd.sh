...
hostapd_prepare_device_config() {
	...
	json_get_vars country country_ie beacon_int:100 doth require_mode legacy_rates \
					...
					allow_scan_during_cac background_cac dfs_ch_state_file \
					...
					adv_proto_query_resp_len \
					rssi_reject_assoc_rssi mem_only_cred process_radar_slave_events dfs_unii4 dynamic_wmm dynamic_edca \
...
hostapd_set_bss_options() {
	...
	json_get_vars \
		...
		intra_vap_mcast enable_hairpin \
		mgmt_frame_power_control greylist_logpath greylist_file cmmac transition_disable sRTSmode \
		unsol_bcast_probe_resp_interval fils_discovery_max_interval
