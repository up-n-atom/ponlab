...
if [ "$OS_NAME" = "RDKB" ]; then
	...
	if [ -n "$atom_cpu" ]; then
# Differentiation needed between LGM and OCTOPUS as PCI_SLOT values are different for Wave600-D2 card
# PCI_SLOT value of phy3 is 0006 on OCTOPUS and 0007 on LGM
...
ap_set_rfeature()
{
	...
	while [ "$1" != "" ]; do
		...
		case "$token" in
			...
			CHNUM_BAND)
				...
				ap_cmd="$HOSTAPD_CLI_CMD -i$CURRENT_WLAN_NAME chan_switch 10 $ap_new_freq center_freq1=$ap_centr_freq bandwidth=$ap_band auto-ht"
				ap_tmp=`eval $ap_cmd`
			;;
			RTS_FORCE)
	...
	if [ "$ap_teardownalltwt" = "1" ]; then
		if [ "$ap_rxmac" != "" ]; then
			...
			ap_aid=`cat /proc/net/mtlk/$CURRENT_WLAN_NAME.0/sta_list | grep "0" | grep -iF "$ap_rxmac" | awk '{print $5}'`
			if [ "$ap_aid" = "" ]; then
...
ap_get_parameter()
{
        ...
        while [ "$1" != "" ]; do
                ...
                PARAMETER)
                        ...
                        lower "$1" param
                        if [ "$param" = "pmk" ]; then
                                local vaps=`uci show wireless | grep ifname | grep ${CURRENT_WLAN_NAME/.0} | cut -d"=" -f2 |grep "\." | tr -d "'"`
                                for interface_name in $vaps; do
