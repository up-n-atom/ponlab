...
if [ "$activate_timer" = "1" ]; then
	debug_print "----------------------------------------- Static Plan Timer Started -----------------------------------------"
	static_plan_interface1=$1
	static_plan_interface2=$4
	CONFIG_WLAN_NAME=$5
	counter=0

	debug_print "static_plan_interface1:$static_plan_interface1 static_plan_interface2:$static_plan_interface2 CONFIG_WLAN_NAME:$CONFIG_WLAN_NAME"
	while [ $counter -le "15" ]
	do
		counter=$((counter+1))
		sleep 5
	done
	debug_print "----------------------------------------- Static Plan Timer Expired -----------------------------------------"

	#send plan_off once the timer expired
	handler_func "plan_off" ${static_plan_interface1} &
	handler_func "plan_off" ${static_plan_interface2} &
	sleep 2

	debug_print "Request FW not to sync all MLD STA's active link in the same band"
	iw dev $CONFIG_WLAN_NAME iwlwav sDoSimpleCLI 151 0
