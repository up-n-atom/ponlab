...
function do_debug_assert(){
	wave600_count=`lspci -n | grep -c -E "$LTQ_CODE_WAVE600|$LTQ_CODE_WAVE600D2|$LTQ_CODE_WAVE600D2B|$LTQ_CODE_WAVE700"`

	if [ "$wave600_count" = "0" ]; then
	...
	while [ $card_idx -lt $wave600_count ]; do
		...
		while [ $res -ne 0 ] && [ $count -lt 10 ]; do
			sleep 1
			`head -n 1 ${WAVE600_FW_DUMP_FILE_PREFIX}${card_idx}${WAVE600_FW_DUMP_FILE_SUFFIX} 2>/dev/null | grep $DUMP_HEADER_MAGIC > /dev/null`
			res=$?
			count=$((count+1))
		done
...
function save_logs()
{
	...
	#adding beerock logs
	[ "$optionFlag" != "-cert" ] && [ "$(ls -l ${BEEROCKS_LOG}*)" != "" ] && mkdir -p ${TMP_LOGS_PATH}/beerock_logs && cp -a ${BEEROCKS_LOG}* ${TMP_LOGS_PATH}/beerock_logs
	...
	#Save the logread logs
	[ "$optionFlag" = "-cert" ] && cp ${TMP_PATH}/logread_logs.txt ${TMP_LOGS_PATH}/logread_logs.txt
