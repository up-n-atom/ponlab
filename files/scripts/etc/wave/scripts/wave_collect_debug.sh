...
function do_debug_assert(){
	...
	wave600_count=`lspci -n | grep -c $LTQ_CODE_WAVE600`
...
function save_logs()
{
	...
	[ "$(ls -l ${FW_DUMP_LOC}/fw_dump*)" != "" ] && cp ${FW_DUMP_LOC}/fw_dump* ${TMP_LOGS_PATH}
	...
	if [ -e /proc/net/mtlk/wlan0 ]; then
		...
		cat /proc/net/mtlk/wlan0/Debug/sta_list > ${TMP_LOGS_PATH}/driver_wlan0_sta_list
	fi
	if [ -e /proc/net/mtlk/wlan2 ]; then
		...
		cat /proc/net/mtlk/wlan2/Debug/sta_list > ${TMP_LOGS_PATH}/driver_wlan2_sta_list
	fi
