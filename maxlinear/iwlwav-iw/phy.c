...
static int print_channels_handler(struct nl_msg *msg, void *arg)
{
	...
	struct nlattr *tb_iftype_data[NL80211_BAND_IFTYPE_ATTR_MAX + 1];
	...
	if (tb_msg[NL80211_ATTR_WIPHY_BANDS]) {
		nla_for_each_nested(nl_band, tb_msg[NL80211_ATTR_WIPHY_BANDS], rem_band) {
			...
			/* HE and EHT can be supported without backward compatibility with
			 * HT/VHT, so if present mark all channel bandwidths as supported */
			if (tb_band[NL80211_BAND_ATTR_IFTYPE_DATA]) {
				nla_for_each_nested(nl_iftype_data, tb_band[NL80211_BAND_ATTR_IFTYPE_DATA], rem_band) {
					nla_parse(tb_iftype_data, NL80211_BAND_IFTYPE_ATTR_MAX,
						  nla_data(nl_iftype_data), nla_len(nl_iftype_data), NULL);

					if (tb_iftype_data[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PHY] ||
					    tb_iftype_data[NL80211_BAND_IFTYPE_ATTR_EHT_CAP_PHY]) {
						ctx->width_40 = ctx->width_80 = ctx->width_160 = true;
						break;

						/* TODO: add support for 320 MHz flags
						 * when supported by NL and regulatory */
					}
				}
			}
...
static int handle_wmm(struct nl80211_state *state,
			  struct nl_msg *msg,
			  int argc, char **argv,
			  enum id_input id)
{
	...
	if (!params) {
		printf("err to start msg params\n");
		goto fail;
	}

	if (!strcasecmp(argv[0], "VO"))
		queue = NL80211_TXQ_Q_VO;
	else if (!strcasecmp(argv[0], "VI"))
		queue = NL80211_TXQ_Q_VI;
	else if (!strcasecmp(argv[0], "BK"))
		queue = NL80211_TXQ_Q_BK;
	else if (!strcasecmp(argv[0], "BE"))
		queue = NL80211_TXQ_Q_BE;
	else {
		printf("Invalid tx queue type: %s\n", argv[0]);
		return 2;
	}

	if(nla_put_u8(msg, NL80211_TXQ_ATTR_QUEUE, queue)) {
		printf("Invalid tx queue type: %s\n", argv[0]);
		goto fail;
	}

	txop = strtoul(argv[1], &end, 0);
	if (*end)
		return -1;

	cw_min = strtoul(argv[2], &end, 0);
	if (*end)
		return -1;

	cw_max = strtoul(argv[3], &end, 0);
	if (*end)
		return -1;

	aifs = strtoul(argv[4], &end, 0);
	if (*end)
		return -1;

	if(nla_put_u16(msg, NL80211_TXQ_ATTR_CWMIN, cw_min) ||
		nla_put_u16(msg, NL80211_TXQ_ATTR_CWMAX, cw_max) ||
		nla_put_u8(msg, NL80211_TXQ_ATTR_AIFS, aifs) ||
		nla_put_u16(msg, NL80211_TXQ_ATTR_TXOP, txop)) {
		printf("set params err for queue: %s\n", argv[0]);
		goto fail;
	}
...
COMMAND(set, wmm, "<queue> <txop> <cw_min> <cw_max> <aifs>",
	NL80211_CMD_SET_WIPHY, 0, CIB_NETDEV, handle_wmm,
	"Set wmm params to use for TX\n"
	"The driver may reject antenna configurations it cannot support.");
