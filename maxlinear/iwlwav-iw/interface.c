...
static int print_iface_handler(struct nl_msg *msg, void *arg)
{
	...
	if ((!wiphy && tb_msg[NL80211_ATTR_WIPHY]) && tb_msg[NL80211_ATTR_WIPHY_TXQ_PARAMS]) {
	struct nlattr *nl_txq_params;
	int rem_txq_params = 0;
	struct nlattr *tb[NL80211_TXQ_ATTR_MAX + 1];
	int result = 0;
	int ac =0, txop=0, cwmin = 0, cwmax =0, aifs = 0;
	...
	printf("%s\ttxqueue params\n", indent);
	indent = "\t";

	nla_for_each_nested(nl_txq_params,
		tb_msg[NL80211_ATTR_WIPHY_TXQ_PARAMS],rem_txq_params) {
		result = nla_parse_nested(tb, NL80211_TXQ_ATTR_MAX,
					nl_txq_params,
					txq_params_policy);
		if (result){
			printf("!!err parsing nl_txq_params\n");
			return result;
		}

		ac = nla_get_u8(tb[NL80211_TXQ_ATTR_AC]);
		txop = nla_get_u16(tb[NL80211_TXQ_ATTR_TXOP]);
		cwmin = nla_get_u16(tb[NL80211_TXQ_ATTR_CWMIN]);
		cwmax = nla_get_u16(tb[NL80211_TXQ_ATTR_CWMAX]);
		aifs = nla_get_u8(tb[NL80211_TXQ_ATTR_AIFS]);

                switch (ac) {
                case 0:
                        printf("%s\t-vo, txop %d, cwmin %d, cwmax %d, aifs %d\n",
                        indent, txop, cwmin, cwmax, aifs);
                        break;
                case 1:
                        printf("%s\t-vi, txop %d, cwmin %d, cwmax %d, aifs %d\n",
                        indent, txop, cwmin, cwmax, aifs);
                        break;
                case 2:
                        printf("%s\t-be, txop %d, cwmin %d, cwmax %d, aifs %d\n",
                        indent, txop, cwmin, cwmax, aifs);
                        break;
                case 3:
                        printf("%s\t-bk, txop %d, cwmin %d, cwmax %d, aifs %d\n",
                        indent, txop, cwmin, cwmax, aifs);
                        break;
                default:
                        printf("%s\t-unknown ac type %d\n", indent, ac);
                        break;
                }
...
static int set_acl(struct nl80211_state *state,
		   struct nl_msg *msg,
		   int argc, char **argv,
		   enum id_input id)
{
	int i, ret = 0;
	int acl_policy, num_macs;
	unsigned char mac_addr[ETH_ALEN];
	struct nl_msg *acl = nlmsg_alloc();
	...
	acl_policy = atoi(argv[0]);
	NLA_PUT_U32(msg, NL80211_ATTR_ACL_POLICY, acl_policy);

	for (i = 0; i < num_macs; i++) {
		if (mac_addr_a2n(mac_addr, argv[i + 1])) {
			fprintf(stderr, "Invalid MAC address at index %d\n", i);
			ret = 2;
			goto err;
		}

		NLA_PUT(acl, i + 1, ETH_ALEN, mac_addr);
	}
...
COMMAND(set, acl, "<policy> [MAC1] [MAC2] ...",
	NL80211_CMD_SET_MAC_ACL, 0, CIB_NETDEV, set_acl,
	"Set ACL.");
