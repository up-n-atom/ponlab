...
{
	dp_sputs(s, "Cat Not implemented yet !\n");
}
...
static int dp_proc_release(struct inode *inode, struct file *file)
{
	...
	if (p->data) { /* allocated by caller during dp_proc_init_callback_t */
		kfree(p->data);
		p->data = NULL;
	}
...
#define MEM_ZERO(x, y) dp_memset(&(x), 0, y)
