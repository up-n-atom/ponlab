...
{
	static GSW_CTP_portConfig_t *ctp = NULL;
	static GSW_EXTENDEDVLAN_config_t *vcfg = NULL;
	GSW_return_t ret;
	ltq_bool_t enable;
	u32 block;
	...
	if (!ctp)
		return -EIO;
	dp_memset(&alloc, 0, sizeof(GSW_EXTENDEDVLAN_alloc_t));
	...
	if (enable) {
		...
		if (ret != GSW_statusOk) {
			kfree(ctp);
			pr_err("DPM: Fail:Ingress VLAN operate disable in ctp\n");
			return -EIO;
		}
	...
	vcfg = dp_kzalloc(sizeof(*vcfg), GFP_ATOMIC);
	if (!vcfg) {
		kfree(ctp);
		return -EIO;
	}
			...
			GSW_EXTENDEDVLAN_config_t *vcfg)
{
	...
	dp_memset(bpcfg, 0, sizeof(*bpcfg));
	...
	for (i = 0; i < alloc.nNumberOfEntries; i++) {
		...
		if (ret != GSW_statusOk) {
			pr_err("DPM: Fail updating Extended VLAN entry (%u, %u).\n",
	...
	if (ret != GSW_statusOk) {
		pr_err("DPM: Failed in attaching Extended VLAN to Bridge Port.\n");
		ops->gsw_extvlan_ops.ExtendedVlan_Free(ops, &alloc);
		return -EIO;
	} else {
		return 0;
		       ...
		       int flag)
{
	...
	new_priv = dp_kzalloc(new_priv_size, GFP_ATOMIC);
