...
DTI_STATIC IFX_uint_t DTI_cliControlSearchKey(
                        const DTI_cliConfigKeyword_t *pKeyWords,
                        const IFX_char_t        **ppInCliControlStr,
                        IFX_uint_t              *pRemainLen);

DTI_STATIC IFX_int_t DTI_cliControlHelpPrint(
                        const DTI_cliConfigKeyword_t *pGroupEntry,
                        const IFX_char_t       *pDescript,
                        const DTI_cliConfigKeyword_t *pKeyWords,
...
*/
const DTI_cliConfigKeyword_t DTI_cliControlGroupEntries[] =
{
...
*/
const DTI_cliConfigKeyword_t DTI_cliControlGrConfigEntries[] =
{
...
*/
const DTI_cliConfigKeyword_t DTI_cliControlConfigImageEntries[] =
{
...
*/
const DTI_cliConfigKeyword_t DTI_cliControlGrDeviceEntries[] =
{
...
*/
const DTI_cliConfigKeyword_t DTI_cliControlGrCliEntries[] =
{
                        ...
                        DTI_PacketError_t    *pPacketError)
{
   ...
   *pPacketError = DTI_eNoError;
   ...
   else
   {
      ...
      pAgentCtx->imageCntrl[imageId].useCount = DTI_IMAGE_CTRL_UPD_PENDING;
...
/**
   Check and release the requested image control struct from the given user context.
...
\param
   imageId        - image id of the requested image.
\param
   pImageGntrl    - contains the image control structure to release
...
\return
   ...
   IFX_ERROR:
      NULL pointer is returned.
*/
DTI_STATIC IFX_int_t DTI_imageCntrlReleaseFunction(
                        IFX_void_t           *pUserCtx,
                        DTI_ImageControl_t   *pImageCntrl,
                        DTI_PacketError_t    *pPacketError)
{
   IFX_int_t      retVal = IFX_SUCCESS, imageId;
   DTI_AgentCtx_t *pAgentCtx = (DTI_AgentCtx_t *)pUserCtx;

   if ( (pAgentCtx == IFX_NULL) || (pImageCntrl == IFX_NULL) || (pPacketError == IFX_NULL) )
   {
      DTI_PRN_USR_ERR_NL(DTI_DBG, DTI_PRN_LEVEL_ERR,
         ("ERROR: Callback Image Cntrl Release - missing return ptr."DTI_CRLF));
   ...
   imageId = pImageCntrl->imageId;
   if (imageId >= DTI_MAX_NUM_OF_IMAGES)
   {
      DTI_PRN_USR_ERR_NL(DTI_DBG, DTI_PRN_LEVEL_ERR,
         ("ERROR: Callback Image Cntrl Release - invalid image ID = %d >= max %d."DTI_CRLF,
           imageId, DTI_MAX_NUM_OF_IMAGES));
   ...
   *pPacketError = DTI_eNoError;
   ...
   {
      DTI_PRN_USR_ERR_NL(DTI_DBG, DTI_PRN_LEVEL_ERR,
         ("ERROR: Callback Image Cntrl Release - semaphore."DTI_CRLF));
   ...
   if (pImageCntrl->useCount != DTI_IMAGE_CTRL_UPD_PENDING)
   {
      DTI_PRN_USR_ERR_NL(DTI_DBG, DTI_PRN_LEVEL_ERR,
         ("ERROR: Callback Image Cntrl Release - image not in update state (count = %d)"DTI_CRLF,
         pImageCntrl->useCount));
   ...
   {
      DTI_PRN_USR_DBG_NL(DTI_DBG, DTI_PRN_LEVEL_HIGH,
         ("Callback Image Cntrl Release - release image update (image ID %d)"DTI_CRLF,
           imageId));

      pImageCntrl->useCount = 0;
   }
   ...
   *pPacketError = DTI_eNoError;
   ...
   else
   {
      if (lockOnOff)
      {
         if (ppImageGntrl)
         {
            if (pAgentCtx->imageCntrl[imageId].useCount != DTI_IMAGE_CTRL_UPD_PENDING)
            {
               *ppImageGntrl = &pAgentCtx->imageCntrl[imageId];
               pAgentCtx->imageCntrl[imageId].useCount++;
            }
      ...
      else
      {
         ...
         {
            DTI_PRN_USR_ERR_NL(DTI_DBG, DTI_PRN_LEVEL_ERR,
               ("ERROR: Callback Image Unlock - image blocked for config."DTI_CRLF));
            *pPacketError = DTI_eErrPortOperation;
         }
      }
   }
...
*/
DTI_STATIC IFX_uint_t DTI_cliControlSearchKey(
                        DTI_cliConfigKeyword_t const *pKeyWords,
                        const IFX_char_t        **ppInCliControlStr,
                        IFX_uint_t              *pRemainLen)
{
                        ...
                        IFX_uint32_t            bufferOutSize)
{
   ...
   DTI_cliConfigKeyword_t const *pKeyWords = IFX_NULL;
                        ...
                        IFX_uint_t              dtiBufOutLen)
{
   ...
   {
      pPacketIn       = (DTI_Packet_t *)pWorkerCtx->dtiProtocolServerCtx.packetIn.buffer;
      ...
      case DTI_PacketType_eCliString:
              ...
              && (pPacketIn->header.payloadSize > (IFX_uint32_t)DTI_StrLen(pCliControlKey) ) )
         {
                     ...
                     DTI_StrLen(pCliControlKey)) == 0)
            {
               ...
               {
                  pPacketOut      = (DTI_Packet_t *)pWorkerCtx->dtiProtocolServerCtx.packetOut.buffer;
                        ...
                        DTI_WorkerCtx_t         **ppWorkerCtx)
{
   ...
   if (pWorkerCtx == IFX_NULL)
   {
      ...
      pWorkerCtx = (DTI_WorkerCtx_t *)DTI_Malloc(sizeof(DTI_WorkerCtx_t));
      if (pWorkerCtx == IFX_NULL)
      {
         DTI_PRN_USR_ERR_NL(DTI_DBG, DTI_PRN_LEVEL_ERR,
      ...
      DTI_MemSet(pWorkerCtx, 0x00, sizeof(DTI_WorkerCtx_t));
   }

   /*
      Init DTI protocol part
   */
   pProtocolServerCtx = (DTI_ProtocolServerCtx_t *)&pWorkerCtx->dtiProtocolServerCtx;
   pProtocolServerCtx->packetIn.pPacket  = (DTI_Packet_t *)pProtocolServerCtx->packetIn.buffer;
   pProtocolServerCtx->packetOut.pPacket = (DTI_Packet_t *)pProtocolServerCtx->packetOut.buffer;
   ...
   */
   ...
   pProtocolServerCtx->pCbFctImageCntrlRelease  = DTI_imageCntrlReleaseFunction;
                        ...
                        volatile IFX_boolean_t  *pBShutdown)
{
   ...
   */
   ...
   {
      DTI_WORKER_THR_OP_RUN_INC(pAgentCtx, pWorkerCtx->workerNum);
...
DTI_WORKER_THREAD_EXIT:
   ...
   DTI_PRN_USR_DBG_NL(DTI_DBG, DTI_PRN_LEVEL_HIGH,
      ("worker thr[%d] - leave (ret=%d - %s)." DTI_CRLF,
         (pWorkerCtx != IFX_NULL) ? pWorkerCtx->workerNum : 99, nIfxRet,
          (nIfxRet == DTI_CON_RECV_STATUS_CON_CLOSED) ? "close" : "error?" ));
   ...
   if ( (pAgentCtx != IFX_NULL) && (pWorkerCtx != IFX_NULL) )
   {
      ...
      if (pProtocolServerCtx != IFX_NULL)
      {
         ...
         if (pProtocolServerCtx->pCtxImageCntrl != IFX_NULL)
         {
            DTI_PacketError_t tmp;
            DTI_imageCntrlReleaseFunction(
                        pAgentCtx, pProtocolServerCtx->pCtxImageCntrl, &tmp);
            pProtocolServerCtx->pCtxImageCntrl = IFX_NULL;
         }
      }
   }
                        ...
                        IFXOS_ThreadParams_t    *pThreadParams)
{
   ...
   DTI_WORKER_THR_PID_SET(pAgentCtx, pWorkerCtx->workerNum, DTI_ProcessIdGet());
   /* DTI_WORKER_THR_TID_SET(pAgentCtx, pWorkerCtx->workerNum, DTI_ThreadIdGet()); */
   DTI_WORKER_THR_TID_SET(pAgentCtx, pWorkerCtx->workerNum, pWorkerCtx->thr.tid);
   ...
   while (pThreadParams->bShutDown == IFX_FALSE)
   {
      ...
      {
         /* wait for resume */
         DTI_WORKER_THR_OP_IDLE_INC(pAgentCtx, pWorkerCtx->workerNum);
         DTI_MSecSleep(100);
      }
   }

   DTI_WORKER_THR_PID_SET(pAgentCtx, pWorkerCtx->workerNum, 0);
   DTI_WORKER_THR_TID_SET(pAgentCtx, pWorkerCtx->workerNum, 0);
                        ...
                        IFXOS_ThreadParams_t *pThreadParams)
{
   ...
   DTI_LISTEN_THR_PID_SET(pAgentCtx, DTI_ProcessIdGet());
   /* DTI_LISTEN_THR_TID_SET(pAgentCtx, DTI_ThreadIdGet()); */
   DTI_LISTEN_THR_TID_SET(pAgentCtx, pAgentCtx->listenerThr.tid);
   ...
   */
   while(pAgentCtx->bListenRun == IFX_TRUE && pThreadParams->bShutDown == 0)
   {
      ...
      DTI_LISTEN_THR_OP_RUN_INC(pAgentCtx);
   }     /* thread loop */
...
DTI_LISTENER_THREAD_EXIT:
   ...
   if (pAgentCtx != IFX_NULL)
   {
      ...
      DTI_LISTEN_THR_PID_SET(pAgentCtx, 0);
      DTI_LISTEN_THR_TID_SET(pAgentCtx, 0);
   }
