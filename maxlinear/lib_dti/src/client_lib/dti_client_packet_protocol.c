...
{
   IFX_uint8_t *pPaylSend = IFX_NULL;
   ...
   if ( (pPacketPayload != IFX_NULL) && (payloadSize_byte > 0) )
   {
      ...
      {
         case DTI_e16Bit:
            if (!(((IFX_ulong_t)pDtiPacketSend->payload) & 0x1))
            {
               pPaylSend = pDtiPacketSend->payload;
            }
            break;
         case DTI_e32Bit:
            if (!(((IFX_ulong_t)pDtiPacketSend->payload) & 0x3))
            {
               pPaylSend = pDtiPacketSend->payload;
            }
            break;
      ...
      DTI_MemCpy(pPaylSend, pPacketPayload, (IFX_int_t)payloadSize_byte);
   }
                        ...
                        IFX_uint_t        imageSize_byte)
{
   ...
   pPayloadSend = (DTI_H2D_ImageLoadStart_t *)pDtiPacketSend->payload;
   (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
                        ...
                        const IFX_char_t *fileName)
{
    ...
    pPayloadSend = (DTI_H2D_ImageDownloadStart_t *)pDtiPacketSend->payload;
    (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
                                                          ...
                                                          const IFX_char_t *remoteScript)
{
    ...
    pPayloadSend = (DTI_H2D_ExecuteRemoteShScript_t *)pDtiPacketSend->payload;
    (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
                        ...
                        IFX_uint8_t       *pImage)
{
   ...
   pPayloadSend    = (DTI_H2D_ImageLoadChunk_t *)pDtiPacketSend->payload;
   (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
                        ...
                        IFX_uint_t        offset_byte)
{
    ...
    pPayloadSend = (DTI_H2D_ImageDownloadChunk_t *)pDtiPacketSend->payload;
    (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
                        ...
                        const IFX_char_t  *pDestFilename)
{
   ...
   pPayloadSend    = (DTI_H2D_ImageWriteToFile_t *)pDtiPacketSend->payload;
   (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
                        ...
                        IFX_uint_t        imageNum)
{
   ...
   pPayloadSend    = (DTI_H2D_ImageRelease_t *)pDtiPacketSend->payload;
   (void)DTI_headerPacketTypeSet(
                        pDtiPacketSend,
   ...
   while (retVal == DTI_CON_RECV_STATUS_PENDING || retVal == DTI_CON_RECV_STATUS_PARTIAL_DONE)
   {
      ...
      if( pDtiPacket != IFX_NULL )
      {
         if( pDtiPacket->header.packetType == DTI_PacketType_eCliPartialString ) /* Partial packet */
         {
            ...
            {
               if( (iBytesInBuffer + recvBytes - ((int)sizeof(DTI_PacketHeader_t))) >= recvBufferSize_byte )
               {
                  DTI_PRN_USR_ERR_NL(DTI_CLIENT, DTI_PRN_LEVEL_ERR,
