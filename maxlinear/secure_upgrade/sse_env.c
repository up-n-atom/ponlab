...
#ifndef PLATFORM_LGM
static inline int check_boardtype(void)
{
	return -1;
}
#else /* PLATFORM_LGM */
 ...
 */
static char *getDevFromPartition(char *name, char type)
{
	DIR *FD;
...
int add_env(char *name, char *val)
{
	...
	printf("add_env : setting %s=%s for %d bytes\n", name, val, len);
