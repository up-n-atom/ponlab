...
#define ST_ZWDFS					(0x7)
#define ST_CALIBRATE_AFTER_CAC		(0x8)
#define ST_LAST						(0x9)
...
#define REGD_CODE_FCC_SP     0x10
#define REGD_CODE_FCC        REGD_CODE_FCC_SP //Maintain compatibility with IqLite
#define REGD_CODE_FCC_LPI    0x11
...
#define CHAN_PARAMS_CHAN_INDEX_MAX_G7	(15)

#define CHAN_PARAMS_CHAN_BANDWIDTH_MIN			(0)
#define CHAN_PARAMS_CHAN_BANDWIDTH_MAX			(4)
#define CHAN_PARAMS_CHAN_BANDWIDTH_MAX_G7		(5)
...
#define CHAN_PARAMS_CHAN_SWITCH_TYPE_MAX		(7)
...
#define CHAN_PARAMS_CAC_PENDING_MIN				(0)
#define CHAN_PARAMS_CAC_PENDING_MAX				(1)
...
typedef struct _UM_SET_CHAN_PARAMS
{
	...
	uint8	cac_pending;						// set if CAC is active
	...
	uint8 addExtCsaIe;  // Indicates if ext csa IE needs to be added
	uint8 op_class;     // Operating class to be set in ext CSA IE

	uint8 reserved;
	uint16 eht_dis_subch_bitmap;
	uint8  reserved2[2];
...
typedef union _UMI_MAN
{
	...
	UMI_CREATE_MLD					sCreateMld;
	UMI_ADD_STA_MLD					sAddStaMld;
	UMI_STA_REMOVE					sRemoveStaMld;
	UMI_REQUEST_SID_MLD				sRequestSidMld;
	UMI_DESTROY_MLD					sDestroyMld;
	UMI_MLO_LINK_STATS				sMloLinkStats;
...
typedef union _UMI_IND
{
	...
	UMI_BEACON_SET					sUmiBeaconSet;
	UMI_SET_WMM_PARAMETERS			sUmiSetWmmParameters;
	UMI_BEACON_DYN_WMM_SET			sUmiBeaconDynWmmSet;
	UM_SET_CHAN_PARAMS				sUmSetChanParams;
	PLATFORM_DATA_FIELDS			sUmiPlatformDataField;
	UMI_TRIGGER_FRAME_CONFIG		sTriggerFrameConfig;
	...
	UMI_802_1X_FILTER				sUmiSet802_1xFilter;
