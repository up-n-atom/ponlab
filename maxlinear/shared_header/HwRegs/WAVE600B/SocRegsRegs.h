...
#define	REG_SOC_REGS_EFUSE_INIT_VECTOR               (SOC_REGS_BASE_ADDRESS + 0xB4)
#define	REG_SOC_REGS_PVT_SENSOR_CTRL                 (SOC_REGS_BASE_ADDRESS + 0xD0)
#define	REG_SOC_REGS_PVT_SENSOR_DATA                 (SOC_REGS_BASE_ADDRESS + 0xD4)
...
typedef union
{
	...
	struct
	{
		...
		uint32 swEfuseRstn : 1; //efuse software reset request  :   , 0 - efuse module is in reset state , 1 - efuse module is in active state , , reset value: 0x1, access type: RW
		uint32 reserved0 : 7;
...
typedef union
{
	...
	struct
	{
		uint32 gpioDriveCtrl0 : 20; //gpio drive strength control ds0 enable : , 0 - GPIO[0--19] pads drive strength control ds0 disable , 1 - GPIO[0--19] pads drive strength control ds0 enable, reset value: 0x0, access type: RW
		uint32 reserved0 : 12;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 gpioDriveCtrl1 : 20; //gpio drive strength control ds1 enable : , 0 - GPIO[0--19] pads drive strength control ds1 disable , 1 - GPIO[0--19] pads drive strength control ds1 enable, reset value: 0xfffff, access type: RW
		uint32 reserved0 : 12;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 gpioOpenDrain : 20; //gpio open drain enable : , 0 - GPIO[0--19] pads open drain disable/push pull disable , 1 - GPIO[0--19] pads open drain enable/push pull  enable, reset value: 0x0, access type: RW
		uint32 reserved0 : 12;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 crRegPcsCommonClocksReserved : 1; //Select Common Clock Mode for Receive Data Path : , When both ends of a PCIe link share a common reference clock, the latency through the receiver's elasticity buffer can be shorter, because no frequency offset exists between the two ends of the link., reset value: 0x0, access type: RW
		...
		uint32 crRegPhyRx0EqReserved : 3; //Rx Equalizer Setting : , Sets the fixed value of the Rx equalizer. To enable tuning at the board level for Rx eye compliance, connect this signal to a register. The recommended setting is 0b010. The optimal EQ setting will vary depending on users' system environment., reset value: 0x2, access type: RW
		uint32 crRegPhyTx0TermOffsetReserved : 5; //Transmitter Termination Offset : , Enables adjustment of the transmitter termination value from the default value of 50 Ohm., reset value: 0x0, access type: RW
		uint32 crRegPipePortSelReserved : 1; //PIPE Enable Request : , PIPE enable request. Change only when phy_reset is asserted. , For x1 PHYs: , - 0: Disables PIPE. , - 1: Enables PIPE., reset value: 0x1, access type: RW
...
typedef union
{
	...
	struct
	{
		uint32 pcie0SlvRespErrMap : 6; //Slave Response Error Map: ,  This map signal is designed to allow the application to selectively map the errors  , received from the PCIe completion (for non-posted requests) to the AXI slave response slv_rresp or slv_bresp. For each bit, a "1" maps the completion error to SLVERR and a '0' maps the completion error to DECERR. The recommended setting is SLVERR.  For more details, see the 'Advanced Error Handling For  , Received TLPs' appendix. The individual bits represent the following completion errors:   , - [0]: UR (unsupported request)  , - [1]: CRS (configuration request retry status)  , - [2]: CA (Completer Abort)  , - [3]: Poisoned  , - [4]: Reserved , - [5]: Completion Timeout, reset value: 0x0, access type: RW
		uint32 pcie0MstrRespErrMap : 2; //Master Response Error Map :  , Allows you to map the error response from your AXI application slave to a PCIe  , completion status error (for non-posted requests) as follows:   , - 0: CPL abort  , - 1: Unsupported request  , Recommended setting is '0'. For more details, see the 'Advanced  , Error Handling For Received TLPs' appendix.  , - AXI: Bit [0] corresponds to SLVERR and bit [1] corresponds to  , DECERR., reset value: 0x0, access type: RW
		uint32 reserved0 : 8;
		uint32 pcie1SlvRespErrMap : 6; //Slave 1 Response Error Map: ,  This map signal is designed to allow the application to selectively map the errors  , received from the PCIe completion (for non-posted requests) to the AXI slave response slv_rresp or slv_bresp. For each bit, a "1" maps the completion error to SLVERR and a '0' maps the completion error to DECERR. The recommended setting is SLVERR.  For more details, see the 'Advanced Error Handling For  , Received TLPs' appendix. The individual bits represent the following completion errors:   , - [0]: UR (unsupported request)  , - [1]: CRS (configuration request retry status)  , - [2]: CA (Completer Abort)  , - [3]: Poisoned  , - [4]: Reserved , - [5]: Completion Timeout, reset value: 0x0, access type: RW
		uint32 pcie1MstrRespErrMap : 2; //Master 1 Response Error Map :  , Allows you to map the error response from your AXI application slave to a PCIe  , completion status error (for non-posted requests) as follows:   , - 0: CPL abort  , - 1: Unsupported request  , Recommended setting is '0'. For more details, see the 'Advanced  , Error Handling For Received TLPs' appendix.  , - AXI: Bit [0] corresponds to SLVERR and bit [1] corresponds to  , DECERR., reset value: 0x0, access type: RW
		uint32 reserved1 : 8;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 pcieIntrVec : 8; //pcie interrupt vector status : , 0. Host I/F data Tx out interrupt , 1. Host I/F data Rx out interrupt , 2. MGMT Tx out interrupt , 3. MGMT Rx out interrupt , 4. FW-Driver mailbox interrupt , 5. RAB PHI interrupt , 6. WLAN IP's XBAR error interrupt , 7. WDT interrupt, reset value: 0x0, access type: RO
		uint32 reserved0 : 24;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 venMsiEna : 8; //pcie msi interrupts enable  : , 0. Host I/F data Tx out interrupt ,   0 : disable ,   1 : enable , 1. Host I/F data Rx out interrupt ,   0 : disable ,   1 : enable , 2. MGMT Tx out interrupt ,   0 : disable ,   1 : enable , 3. MGMT Rx out interrupt ,   0 : disable ,   1 : enable , 4. FW-Driver mailbox interrupt ,   0 : disable ,   1 : enable , 5. RAB PHI interrupt ,   0 : disable ,   1 : enable , 6. WLAN IP's XBAR error interrupt ,   0 : disable ,   1 : enable , 7. WDT interrupt ,   0 : disable ,   1 : enable, reset value: 0x0, access type: RW
		uint32 reserved0 : 24;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		...
		uint32 reserved0 : 13;
		uint32 pcie1DiagCtrlBus : 3; // Description: Diagnostic Control Bus :   , - x01: Insert LCRC error by inverting the LSB of LCRC  , - x10: Insert ECRC error by inverting the LSB of ECRC  , - The rising edge of these two signals ([1:0]) enable the core to  , assert an LCRC or ECRC to the packet that it currently being  , transferred. When LCRC or ECRC error packets are transmitted  , by the core, the core asserts diag_status_bus[lcrc_err_asserted]  , or diag_status_bus[ecrc_err_asserted] to report that the  , requested action has been completed. This handshake between  , control and status allows your application to control a specific  , packet being injected with an CRC or ECRC error. The LCRC and  , ECRC errors are generated by simply inverting the last bit of the  , LCRC or ECRC value.  , - 1xx: Select Fast Link Mode. Sets all internal timers to Fast Mode  , for speeding up simulation purposes. Forces the LTSSM training  , (link initialization) to use shorter timeouts and to link up faster.  , The scaling factor is 1024 for all internal timers. Fast Link Mode  , can also be activated by setting the "Fast Link Mode" bit of the  , "Port Link Control Register"., reset value: 0x0, access type: RW
		uint32 reserved1 : 13;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		...
		uint32 pcie1SmlhLinkUp : 1; //PHY Link up/down indicator:   , - 1: Link is up  , - 0: Link is down, reset value: 0x0, access type: RO
...
typedef union
{
	...
	struct
	{
		uint32 efuseAddr : 11; //BIT_ADDRESS[4:0] :	Bit address use only for programming operations. The value is ignored for reads. , WORD_ADDRESS[10:5] : WORD address of the fuse array use for read and program operation. ,  , When efuse bit TEST_EFUSE_DISABLE (bit 7 of byte 0) is fused to 1, the upper 1kb efuse write is disabled. The programming to the upper 1kb will be aborted. Read is not affected., reset value: 0x0, access type: RW
		uint32 reserved0 : 21;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 efuseTpgm : 16; //TIMER_TPGM , This register will define the length of STROBE pulse during read write. , During write the strobe should be 12us. , , reset value: 0x1E0, access type: RW
		uint32 efuseTsuCsStrobe : 16; //TIMER_TSU_CS_STROBE , Setup time beween CSB activation to STROBE activation. SMIC: 100ns , The value is multiplied by 16 by HW internally to achieve "silent replacement", reset value: 0x1, access type: RW
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 efuseTrd : 16; //TIMER_TRD  , Strobe width during read. Minimum size is 120ns. , SMIC: 500 ns , The value is multiplied by 4 by HW internally to achieve "silent replacement", reset value: 0xC, access type: RW
		uint32 efuseTsuPsCsb : 16; //TSU_PS_CSB , Setup time between PS and CSB : 45ns  , SMIC: 150ns , The value is multiplied by 4 by HW internally to achieve "silent replacement", reset value: 0x3, access type: RW
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 efuseInitFsmOvr : 1; //efuse init sm override : , 0 - no override , 1 - efuse init sm override, reset value: 0x0, access type: RW
		uint32 reserved0 : 31;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 chipId : 32; //WAV600-TT ChipID  : , Manufacturer ID = 0x009 , Part number = 0xE7D3 , version ID = 0x1 (via efuse) , , reset value: 0x1E7D3013, access type: RO
	} bitFields;
} RegSocRegsChipId_u;

/*REG_SOC_REGS_EFUSE_INIT_VECTOR 0xB4 */
typedef union
{
	uint32 val;
	struct
	{
		uint32 efuseInitVector : 32; //First WORD of EFUSE , There is a HW invertor implemented for Bit 7 to register. Without fusing, this bit in register is '1'. , reset value: 0x80, access type: RO
	} bitFields;
} RegSocRegsEfuseInitVector_u;

/*REG_SOC_REGS_PVT_SENSOR_CTRL 0xD0 */
typedef union
{
	uint32 val;
	struct
	{
		uint32 pvtEna : 1; //PVT enable ,  0 - PVT disabled ,  1 - PVT enabled, reset value: 0x0, access type: RW
		uint32 pvtVsample : 1; //SMIC VT Sensor TDC Mode Selection ,  0 - Temperature Sensor ,  1 - Voltage Sensor , Vsample must be 0 when MXL_PD is used., reset value: 0x0, access type: RW
		uint32 pvtPsample : 2; //MXL_PD.Ring Select/PD Enable , 00 - MXL_PD Disabled , 01 - LVT , 10 - SVT , 11 - HVT, reset value: 0x0, access type: RW
		uint32 pvtPsampleP : 1; //SMIC VT Sensor VTDC Block Power Up , 0 - SMIC_VT Power down , 1 - SMIC_VT Power up , Psample_P must be 0 when MXL_PD is used., reset value: 0x0, access type: RW
		uint32 pvtPsampleN : 1; //Not used , Psample_N must be 0 when MXL_PD is used., reset value: 0x0, access type: RW
		uint32 reserved0 : 2;
		uint32 pvtTrim : 5; //SIMC VT sensor trim. Only lower 4 bits used., reset value: 0x7, access type: RW
		uint32 reserved1 : 1;
		uint32 pvtWindowMult : 2; //Sensor Process Detector Count Window Multiplier , 00: x1 , 01: x256 , 10: x512 , 11: x1024 , NOTE: This can be set to 10 for now. The final value will be determined after RINGO simulation data is provided., reset value: 0x0, access type: RW
		uint32 pvtWindowSize : 8; //Process Detector Count Window size (0-255) , This must be set based on the following formula: , [pvt_window_size * PVT Window Multiplier (set by pvt_window_mult)] >= [3*f_xo_ref_clk/(error%_max*f_roclk_min)] , NOTE: This can be set to 1 for now. The final value will be determined after RINGO simulation data is provided., reset value: 0x0, access type: RW
		uint32 pvtPrescale : 2; //Process Detector Prescale , 00: x1 , 01: x2 , 10: x4 , 11: x8 , NOTE: This can be set to 00 for now. The final value will be determined after RINGO simulation data is provided., reset value: 0x0, access type: RW
		uint32 reserved2 : 6;
	} bitFields;
} RegSocRegsPvtSensorCtrl_u;

/*REG_SOC_REGS_PVT_SENSOR_DATA 0xD4 */
typedef union
{
	uint32 val;
	struct
	{
		uint32 pvtData : 10; //PVT Data Out , Bit 11:2, reset value: 0x0, access type: RO
		uint32 pvtDataLsb : 2; //PVT Data Out LSB , Bit 1:0, reset value: 0x0, access type: RO
		uint32 pvtDataValid : 1; //PVT Data Valid : ,  0 - Not valid ,  1 - Data valid, reset value: 0x0, access type: RO
		uint32 reserved0 : 18;
		uint32 pvtPadDataSel : 1; //PADS PVT Data Out Select for DFT , 0b0: Select pvt_data[9:0] , 0b1: Select pvt_data[11:2], reset value: 0x0, access type: RW
	} bitFields;
} RegSocRegsPvtSensorData_u;
...
typedef union
{
	...
	struct
	{
		...
		uint32 reserved1 : 3;
		uint32 phyDebugEn : 1; //Enable GPIO12 and GPIO13 for use as PHY debug, reset value: 0x0, access type: RW
		uint32 reserved2 : 23;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 pciePhyCrParaSel : 1; //Reserved.  , This bit can be read and written but there is no HW functionality related to it., reset value: 0x1, access type: RW
		uint32 reserved0 : 31;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 pcie0IatuSel : 3; //PCIe iATU registers access enable. To access iATU registers, this register shall be configured to 3'b110. Otherwise, it shall be configured to 3'b0., reset value: 0x0, access type: RW
		uint32 reserved0 : 29;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 pcie1IatuSel : 3; //PCIe iATU registers access enable. To access iATU registers, this register shall be configured to 3'b110. Otherwise, it shall be configured to 3'b0., reset value: 0x0, access type: RW
		uint32 reserved0 : 29;
	} bitFields;
