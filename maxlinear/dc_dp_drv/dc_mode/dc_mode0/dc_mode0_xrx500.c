...
xrx500_setup_pmac_port(int32_t port_id, int32_t dma_cid, int32_t ref_port_id, uint32_t dev_cap_req, uint32_t flags)
{
    ...
    for (i = 0; i <= 15; i++) {
        for (j = 0; j <= 3; j++) {
            ...
            gswr_ops->gsw_pmac_ops.Pmac_Eg_CfgSet(gswr_ops, &egCfg);
        }
    }
    ...
    gswr_ops->gsw_pmac_ops.Pmac_Ig_CfgSet(gswr_ops, &igCfg);
    ...
    gswr_ops->gsw_common_ops.RegisterGet(gswr_ops, &regCfg);
    ...
    gswr_ops->gsw_common_ops.RegisterSet(gswr_ops, &regCfg);
...
static int32_t
xrx500_dcmode0_mark_pkt_devqos(uint32_t devqos, struct sk_buff *skb)
{
                                ...
                                uint8_t class2prio[], uint8_t prio2devqos[])
{
    ...
    for (i = 0; i < DC_DP_MAX_SOC_CLASS; i++) {
        ...
        for (j = 0; j <= 3; j++) {
            ...
            egcfg.nTrafficClass = i;
            egcfg.nFlowIDMsb    = j;
            gswr_ops->gsw_pmac_ops.Pmac_Eg_CfgGet(gswr_ops, &egcfg);
...
xrx500_dcmode0_get_wol_cfg(void *ctx, int32_t port_id, struct dc_dp_wol_cfg *cfg, uint32_t flags)
{
    ...
    ret = gswr_ops->gsw_wol_ops.WoL_CfgGet(gswr_ops, &wol_cfg);
...
xrx500_dcmode0_set_wol_cfg(void *ctx, int32_t port_id, struct dc_dp_wol_cfg *cfg, uint32_t flags)
{
    ...
    ret = gswr_ops->gsw_wol_ops.WoL_CfgSet(gswr_ops, &wol_cfg);
...
xrx500_dcmode0_set_wol_ctrl(void *ctx, int32_t port_id, uint32_t enable)
{
    ...
    ret = gswr_ops->gsw_wol_ops.WoL_PortCfgSet(gswr_ops, &wol_port_cfg);
...
xrx500_dcmode0_get_wol_ctrl_status(void *ctx, int32_t port_id)
{
    ...
    memset(&wol_port_cfg, 0, sizeof(GSW_WoL_PortCfg_t));
    wol_port_cfg.nPortId = port_id;
    ret = gswr_ops->gsw_wol_ops.WoL_PortCfgGet(gswr_ops, &wol_port_cfg);
    if (ret == 0) {
...
static struct dc_dp_dcmode_ops xrx500_dcmode0_ops = {
    ...
    .mark_pkt_devqos = xrx500_dcmode0_mark_pkt_devqos,
