                                ...
                                struct net_device *dev, int32_t port_id, uint32_t flags)
{
    ...
    if ( (flags & (DC_DP_F_FASTPATH | DC_DP_F_FAST_DSL)) ) {
        ...
        /* Otherwise, allocate a DP port */
        {
            ...
            if (flags & DC_DP_F_FAST_WLAN_EXT)
                dp_alloc_flags |= DP_F_FAST_WLAN_EXT;
            else if (flags & DC_DP_F_FAST_WLAN)
            ...
            if (flags & DC_DP_F_MULTI_PORT) {
                /* Align with the WAV700 requirement */
                if (flags & DC_DP_F_FAST_WLAN_2_4G)
                    dp_dev_port = 0;
                else if (flags & DC_DP_F_FAST_WLAN_5G)
                    dp_dev_port = 1;
                else if (flags & DC_DP_F_FAST_WLAN_6G)
                    dp_dev_port = 2;

                if (flags & DC_DP_F_SHARED_RES) {
                    dp_port_id &= ~0x3; /* port range base 4n */
                    dp_port_id += dp_dev_port;
                }
            ...
            ret = dp_alloc_port_ext(0, owner, dev, dp_dev_port, dp_port_id, NULL, NULL, dp_alloc_flags);
            ...
            ret = dp_alloc_port(owner, dev, dp_dev_port, dp_port_id, NULL, dp_alloc_flags);
...
{
    if (dev_ctx && dev_ctx->dcmode && dev_ctx->dcmode->dcmode_ops && dev_ctx->dcmode->dcmode_ops->set_subif_param)
...
{
    if (dev_ctx && dev_ctx->dcmode && dev_ctx->dcmode->dcmode_ops && dev_ctx->dcmode->dcmode_ops->get_subif_param)
        dev_ctx->dcmode->dcmode_ops->get_subif_param(dev_ctx->priv, dc_subif);
    else
        dc_subif->vap_id = dc_dp_get_subifidx(dc_subif->subif);
}
