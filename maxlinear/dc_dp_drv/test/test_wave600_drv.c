...
#elif IS_ENABLED(CONFIG_DPM_DATAPATH) || \
      IS_ENABLED(CONFIG_INTEL_DATAPATH) || IS_ENABLED(CONFIG_LTQ_DATAPATH)
...
struct test_wlan_priv_data {
	...
	struct dc_dp_dccntr dccntr[DC_DP_MAX_TX_RINGS];
...
static int32_t g_resvd_port = -1;
static int32_t g_wave6x_radio_num = 0;
static struct test_wlan_priv_data *first_port_priv = NULL;
...
static void eth_tx_timeout(struct net_device *, unsigned int);
#else
...
static int32_t dp_fp_get_subif(struct net_device *netif, struct sk_buff *skb,
	void *subif_data, char *mac_addr, struct dp_subif *subif, uint32_t flags);
#ifdef PUMA_DC_MODE1
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 5, 19)
void eth_tx_timeout(struct net_device *dev, unsigned int __unused__)
#else
...
static INLINE void eth_rx_handler(struct sk_buff *skb, int id)
{
	...
	if (priv->dp_subif.port_id > 0) {
		...
		if (!ltq_strcmpi(priv->flag_str, "FAST_WAVE5x") ||
		    ...
		    !ltq_strcmpi(priv->flag_str, "FAST_WAVE7x")) {
#if IS_ENABLED(CONFIG_DIRECTCONNECT_DP_API)
			uint32_t sta_id = 0;
    ...
    desc_1->field.ep = port_id; \
			...
			if (dc_dp_xmit
			    (NULL /*g_test_wlan_dev[id]*/, NULL, &priv->dp_subif, skb, skb->len,
			     0) == DP_SUCCESS) {
				PRINTK("dc_dp_xmit: success\n");
				priv->dp_pkts_to_ppe++;
				return;
			} else {
				PRINTK("dc_dp_xmit: failure\n");
				priv->dp_pkts_to_ppe_fail++;
				return;
			}
#endif
	...
	void *subif_data, char *mac_addr, struct dp_subif *subif, uint32_t flags)
{
	...
	subif->port_id = priv->dp_subif.port_id;
	subif->subif = priv->dp_subif.subif;
	return 0;
}
...
#if IS_ENABLED(CONFIG_LTQ_PPA_GRX500) && IS_ENABLED(CONFIG_LTQ_PPA_API_DIRECTPATH)
...
static int str_to_dc_dp_dev_type(char *str)
{
	if (!ltq_strcmpi(str, "FAST_WAVE6x")) 
		return DC_DP_DEV_WAV_600_CDB;
	if (!ltq_strcmpi(str, "FAST_WAVE7x")) 
		return DC_DP_DEV_WAV_700;
	return -1;
}

static int dc_dp_common_alloc_dev(int i, int32_t port_id, uint32_t alloc_flags)
{
	int j;
	int32_t dp_port_id;
	uint32_t num_dccntr = NUM_DCCNTR;
	struct dc_dp_dev_config dev_config = {0};
	struct test_wlan_priv_data *priv = netdev_priv(g_test_wlan_dev[i]);
	...
	if (g_wave6x_radio_num == 0) {
		...
		first_port_priv = priv;

		/* Only after first port allocation */
		if (dc_dp_get_peripheral_config(str_to_dc_dp_dev_type(priv->flag_str), &dev_config) != 0) {
			PRINTK("failed in dc_dp_get_peripheral_config() for %s\n",
				g_test_wlan_dev[i]->name);
	...
	priv->wave500_cb.get_subif_fn = (dc_dp_get_netif_subinterface_fn_t) dp_fp_get_subif;
#ifdef PUMA_DC_MODE1
	...
	num_dccntr += 1;
#endif
	for (j = 0; j < num_dccntr; j++) {
		if ((g_wave6x_radio_num > 1) && (j == 0)) {
			memcpy(&priv->dccntr[0], &first_port_priv->dccntr[0], sizeof(struct dc_dp_dccntr));
			continue;
		}
#ifdef PUMA_DC_MODE1
		priv->dccntr[j].soc_write_dccntr_mode = (DC_DP_F_DCCNTR_MODE_INCREMENTAL | DC_DP_F_DCCNTR_MODE_BIG_ENDIAN);
#else
		priv->dccntr[j].soc_write_dccntr_mode = (DC_DP_F_DCCNTR_MODE_INCREMENTAL | DC_DP_F_DCCNTR_MODE_LITTLE_ENDIAN);
		if (j == 1)
			priv->dccntr[j].soc_write_dccntr_mode = (DC_DP_F_DCCNTR_MODE_CUMULATIVE |
								 DC_DP_F_DCCNTR_MODE_LITTLE_ENDIAN |
								 DC_DP_F_DCCNTR_MODE_4MSG_MODE);
#endif
		//priv->dccntr[j].dev2soc_ret_enq_base = (void *)dma_alloc_coherent(NULL, 8, &dccntr_phys_base, GFP_DMA);
		//priv->dccntr[j].dev2soc_ret_enq_phys_base = (void *)dccntr_phys_base;
		priv->dccntr[j].dev2soc_ret_enq_base = (void *)kmalloc(16, GFP_DMA);
		priv->dccntr[j].dev2soc_ret_enq_phys_base = (void *)virt_to_phys(priv->dccntr[j].dev2soc_ret_enq_base);
		priv->dccntr[j].dev2soc_ret_enq_dccntr_len = 4;
		priv->dccntr[j].soc2dev_enq_base = (void *)(priv->dccntr[j].dev2soc_ret_enq_base + 4);
		priv->dccntr[j].soc2dev_enq_phys_base = (void *)(priv->dccntr[j].dev2soc_ret_enq_phys_base + 4);
		priv->dccntr[j].soc2dev_enq_dccntr_len = 4;
		priv->dccntr[j].dev2soc_deq_base = (void *)(priv->dccntr[j].dev2soc_ret_enq_base + 8);
		priv->dccntr[j].dev2soc_deq_phys_base = (void *)(priv->dccntr[j].dev2soc_ret_enq_phys_base + 8);
		priv->dccntr[j].dev2soc_deq_dccntr_len = 4;
		priv->dccntr[j].soc2dev_ret_deq_base = (void *)(priv->dccntr[j].dev2soc_ret_enq_base + 12);
		priv->dccntr[j].soc2dev_ret_deq_phys_base = (void *)(priv->dccntr[j].dev2soc_ret_enq_phys_base + 12);
		priv->dccntr[j].soc2dev_ret_deq_dccntr_len = 4;
	}
	if (g_wave6x_radio_num > 1)
		memcpy(&priv->resources, &first_port_priv->resources, sizeof(struct dc_dp_res));
	else {
...
#ifdef MULTI_RING
	priv->resources.num_additional_tx_ring = 1;
	priv->resources.additional_tx_rings[0].soc2dev.size = 64;
	priv->resources.additional_tx_rings[0].soc2dev.desc_dwsz = 4;
	priv->resources.additional_tx_rings[0].soc2dev.flags = DC_DP_F_PKTDESC_ENQ;
	priv->resources.additional_tx_rings[0].soc2dev_ret.size = 64;
	priv->resources.additional_tx_rings[0].soc2dev_ret.desc_dwsz = 4;
	priv->resources.additional_tx_rings[0].soc2dev_ret.flags = DC_DP_F_PKTDESC_RET;
#endif
	priv->resources.num_dccntr = num_dccntr;
	priv->resources.dccntr = &priv->dccntr[0];
	if (dc_dp_register_dev(THIS_MODULE, dp_port_id, g_test_wlan_dev[i],
		...
		//dma_free_coherent(NULL, 8, priv->dccntr[0].dev2soc_ret_enq_base, (dma_addr_t)priv->dccntr[0].dev2soc_ret_enq_phys_base);
		//kfree(priv->dccntr[0].dev2soc_ret_enq_base); // FIXME
		dc_dp_alloc_port(THIS_MODULE, priv->dev_port, g_test_wlan_dev[i],
			      dp_port_id, DC_DP_F_DEREGISTER);
	        if (g_wave6x_radio_num == 1)
	        	g_resvd_port = -1;
...
static int register_dev(int i)
{
	...
	if (!ltq_strcmpi(priv->flag_str, "FAST_WAVE5x")) { // How to know it is allready allocated
		...
		dp_port_id = dc_dp_alloc_port(priv->owner, priv->dev_port, g_test_wlan_dev[i],
					   0, (DC_DP_F_FAST_WLAN | DC_DP_F_FASTPATH));
		if (dp_port_id <= 0) {
			PRINTK
			    ("failed in register fastpath for %s\n",
			     g_test_wlan_dev[i]->name);
		...
		PRINTK("dc_dp_alloc_port get port %d for %s\n",
		       dp_port_id, g_test_wlan_dev[i]->name);
		/* Only after first port allocation */
		if (dc_dp_get_peripheral_config(DC_DP_DEV_WAV_600_5G, &dev_config) != 0) {
			PRINTK("failed in dc_dp_get_peripheral_config() for %s\n",
				g_test_wlan_dev[i]->name);
			dc_dp_alloc_port(THIS_MODULE, priv->dev_port, g_test_wlan_dev[i],
				dp_port_id, DC_DP_F_DEREGISTER);
		...
		PRINTK("dc_dp_get_peripheral_config() success for %s\n",
			       	g_test_wlan_dev[i]->name);
		priv->wave500_cb.stop_fn = (dp_stop_tx_fn_t) dp_fp_stop_tx;
		priv->wave500_cb.restart_fn = (dp_restart_tx_fn_t) dp_fp_restart_tx;
		priv->wave500_cb.rx_fn = (dc_dp_rx_fn_t) dp_fp_rx;
...
#ifdef PUMA_DC_MODE1
			priv->dccntr[i].soc_write_dccntr_mode = (DC_DP_F_DCCNTR_MODE_INCREMENTAL | DC_DP_F_DCCNTR_MODE_BIG_ENDIAN);
#else
			priv->dccntr[i].soc_write_dccntr_mode = (DC_DP_F_DCCNTR_MODE_INCREMENTAL | DC_DP_F_DCCNTR_MODE_LITTLE_ENDIAN);
			if (i == 1)
				priv->dccntr[i].soc_write_dccntr_mode = (DC_DP_F_DCCNTR_MODE_CUMULATIVE |
									 DC_DP_F_DCCNTR_MODE_LITTLE_ENDIAN |
									 DC_DP_F_DCCNTR_MODE_4MSG_MODE);
#endif
			//priv->dccntr[i].dev2soc_ret_enq_base = (void *)dma_alloc_coherent(NULL, 8, &dccntr_phys_base, GFP_DMA);
			//priv->dccntr[i].dev2soc_ret_enq_phys_base = (void *)dccntr_phys_base;
			priv->dccntr[i].dev2soc_ret_enq_base = (void *)kmalloc(16, GFP_DMA);
			priv->dccntr[i].dev2soc_ret_enq_phys_base = (void *)virt_to_phys(priv->dccntr[i].dev2soc_ret_enq_base);
			priv->dccntr[i].dev2soc_ret_enq_dccntr_len = 4;
			priv->dccntr[i].soc2dev_enq_base = (void *)(priv->dccntr[i].dev2soc_ret_enq_base + 4);
			priv->dccntr[i].soc2dev_enq_phys_base = (void *)(priv->dccntr[i].dev2soc_ret_enq_phys_base + 4);
			priv->dccntr[i].soc2dev_enq_dccntr_len = 4;
			priv->dccntr[i].dev2soc_deq_base = (void *)(priv->dccntr[i].dev2soc_ret_enq_base + 8);
			priv->dccntr[i].dev2soc_deq_phys_base = (void *)(priv->dccntr[i].dev2soc_ret_enq_phys_base + 8);
			priv->dccntr[i].dev2soc_deq_dccntr_len = 4;
			priv->dccntr[i].soc2dev_ret_deq_base = (void *)(priv->dccntr[i].dev2soc_ret_enq_base + 12);
			priv->dccntr[i].soc2dev_ret_deq_phys_base = (void *)(priv->dccntr[i].dev2soc_ret_enq_phys_base + 12);
			priv->dccntr[i].soc2dev_ret_deq_dccntr_len = 4;
		}
		...
		/* DC counter ENQ */
		priv->dccntr[0].dev2soc_except_enq_base = (void *)((uint8_t *)priv->resources.rings.dev2soc_except.base + 256);
		priv->dccntr[0].dev2soc_except_enq_phys_base = (void *)((uint8_t *)priv->resources.rings.dev2soc_except.phys_base + 256);
		priv->dccntr[0].dev2soc_except_enq_dccntr_len = 4;

		/* DC counter DEQ */
		priv->dccntr[0].dev2soc_except_deq_base = (void *)((uint8_t *)priv->resources.rings.dev2soc_except.base + 260);
		priv->dccntr[0].dev2soc_except_deq_phys_base = (void *)((uint8_t *)priv->resources.rings.dev2soc_except.phys_base + 260);
		priv->dccntr[0].dev2soc_except_deq_dccntr_len = 4;
		...
		if (dc_dp_register_dev(priv->owner, dp_port_id, g_test_wlan_dev[i],
			...
			PRINTK
			    ("dc_dp_register_dev failed for %s\n and port_id %d",
			     g_test_wlan_dev[i]->name, dp_port_id);
			//dma_free_coherent(NULL, 8, priv->dccntr[0].dev2soc_ret_enq_base, (dma_addr_t)priv->dccntr[0].dev2soc_ret_enq_phys_base);
			kfree(priv->dccntr[0].dev2soc_ret_enq_base);
		...
		if (dc_dp_common_alloc_dev(i, port_id, alloc_flags | DC_DP_F_FASTPATH) != 0) 
			return -1;
#endif
	}
	else if (!ltq_strcmpi(priv->flag_str, "FAST_WAVE7x")) {
#if IS_ENABLED(CONFIG_DIRECTCONNECT_DP_API)
		//dma_addr_t dccntr_phys_base;
		if (g_wave6x_radio_num > 3) {
			PRINTK
			    ("Port reservation failed g_wave6x_radio_num=%d, not allowed to reg dev=%s as FAST_WAVE6x?\n",
			     g_wave6x_radio_num, g_test_wlan_dev[i]->name);
			return -1;
		...
		alloc_flags = DC_DP_F_FAST_WLAN | DC_DP_F_MULTI_PORT | DC_DP_F_FAST_WLAN_EXT;

		// 1st radio
		if (g_wave6x_radio_num == 0) {
			port_id = 0;
			alloc_flags |= DC_DP_F_FAST_WLAN_2_4G;
			priv->vap_id = 16;
		} else if (g_wave6x_radio_num == 1) {
			/* 2nd radio */
			port_id = g_resvd_port;
			alloc_flags |= DC_DP_F_SHARED_RES | DC_DP_F_FAST_WLAN_5G;
			priv->vap_id = 0;
		} else {
			/* 3rd radio */
			port_id = g_resvd_port;
			alloc_flags |= DC_DP_F_SHARED_RES | DC_DP_F_FAST_WLAN_6G;
			priv->vap_id = 32;
		}
			  ...
			  loff_t *ppos)
{
 ...
 help:
	...
	PRINTK("        Note, the maximum index is %lu\n",
				 ...
				 size_t count, loff_t *ppos)
{
			...
			if (!ltq_strcmpi(priv->flag_str, "FAST_WAVE5x")) {
				...
				if (dc_dp_register_subif
					...
					goto exit;
	    		        ...
	    		        (!ltq_strcmpi(priv->flag_str, "FAST_WAVE7x"))) {
#if IS_ENABLED(CONFIG_DIRECTCONNECT_DP_API)
				if(!ltq_strcmpi(priv->flag_str, "FAST_WAVE7x")) {
					struct dc_dp_subif dc_subif = {0};
					dc_subif.port_id = priv->dp_subif.port_id;
					dc_subif.vap_id = priv->vap_id;
					dc_dp_set_subif_param(&dc_subif);
					priv->dp_subif.subif = dc_subif.subif;
				}
				if (dc_dp_register_subif
    ...
    IS_ENABLED(CONFIG_INTEL_DATAPATH) || IS_ENABLED(CONFIG_LTQ_DATAPATH)
			...
			if (param_list_num > 4) {
				vap_id = ltq_atoi(param_list[4]);

				dc_subif.port_id = priv->dp_subif.port_id;
				dc_subif.vap_id = vap_id;
				dc_dp_set_subif_param(&dc_subif);
				priv->dp_subif.subif = dc_subif.subif;
			}
#if CONFIG_WAVE600_USING_DP_API
