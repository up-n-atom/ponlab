...
#if defined(CONFIG_DPM_DATAPATH) || defined(CONFIG_INTEL_DATAPATH) || \
    defined(CONFIG_LTQ_DATAPATH) /* for LGM/PRX/PUMA/GRX */
#include <net/datapath_api.h>
#else
#include <datapath_api_common.h>
#endif /* CONFIG_DPM_DATAPATH || CONFIG_INTEL_DATAPATH || CONFIG_LTQ_DATAPATH */
 ...
 */
#define DC_DP_API_VERSION_CODE        0x050103
...
/**
  \brief Alloc flag as FASTPATH WLAN 2.4G
 */
#define DC_DP_F_FAST_WLAN_2_4G	 0x00001000

/**
  \brief Alloc flag as FASTPATH WLAN 5G
 */
#define DC_DP_F_FAST_WLAN_5G	 0x00002000

/**
  \brief Alloc flag as FASTPATH WLAN 6G
 */
#define DC_DP_F_FAST_WLAN_6G	 0x00004000

/**
  \brief Alloc flag as FASTPATH WLAN EXT
 */
#define DC_DP_F_FAST_WLAN_EXT	 0x00008000
...
/**
  \brief DC Counter mode flags: DC counter in 4 Message mode: 1, 2 message mode: 0
 */
#define DC_DP_F_DCCNTR_MODE_4MSG_MODE	0x10

/**
  \brief Maximum number of ACA RXIN/RXOUT ring
    For GRX500/PRX300, only support 1 ring
    For LGM, maximum up to 2 rings
 */
#define DC_DP_MAX_RX_RINGS			2

/**
  \brief Maximum number of ACA TXIN/TXOUT ring
    For GRX500/PRX300, only support 1 ring
    For LGM, maximum up to 4 rings
  \note For 5G, it needs to support up to 16 ring - for future
 */
#define DC_DP_MAX_TX_RINGS			4
...
struct dc_dp_res {
	...
	uint8_t num_additional_tx_ring; /*!< [in/out] Number of additional TXIN/TXOUT rings. */
	uint8_t num_additional_rx_ring; /*!< [in/out] Number of additional RXIN/RXOUT rings. */
	struct dc_dp_tx_ring_res additional_tx_rings[DC_DP_MAX_TX_RINGS - 1]; /*!< [in/out] Additional TXIN/TXOUT rings. */
	struct dc_dp_rx_ring_res additional_rx_rings[DC_DP_MAX_RX_RINGS - 1]; /*!< [in/out] Additional RXIN/RXOUT rings. */
...
#endif /* CONFIG_DPM_DATAPATH || CONFIG_INTEL_DATAPATH */
...
/** \brief  Register/De-register a DirectConnect interface to MCAST helper module
  ...
  \return 0 if OK / -1 if error / -2 if not supported
