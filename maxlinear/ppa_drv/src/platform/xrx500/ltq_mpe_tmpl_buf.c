...
uint32_t set_qmap_from_ing_grp(PKT_INFO pkt) {
	...
	pkt_mark = pkt.extmark;
#else
	pkt_mark = pkt.mark;
#endif
	ing_grp = ((pkt_mark & SESSION_ING_GROUP) >> ING_GROUP_POS);
	switch(ing_grp) {
...
int32_t ppa_form_session_tmpl(PPA_SESSMETA_INFO *metainfo)
{
	...
	/*get the actual txif */
	p_swaHdr->dst_pmac_port_num = 1;
	p_swaHdr->dst_pmac_port_list[0] = p_item->dest_ifid;

	if(p_swaHdr->dst_pmac_port_list[0] < MAX_PMAC_PORT)
		p_swaHdr->uc_vap_list[0] = p_item->dest_subifid ;
	...
	if ((IsBridgedSession(p_item->flag2) && !isvtep) ||
		...
		if (ppa_is_EgressGreSession(p_item) || isvtep) {
			...
			if (isvtep && ppa_get_underlying_vlan_interface_if_vxlan
				(temp_netif, &base_netif, &is_vxlan) == PPA_SUCCESS) {
					ppa_get_vxlan_hdrlen(base_netif, &hdrlen);
					p_swaHdr->tunnel_type = TUNL_VXLAN;
					p_swaHdr->vni = p_item->pkt.vni;
					p_swaHdr->udp_vxlan_port = p_item->pkt.udp_vxlan_port;
			} else if (ppa_tmpl_get_underlying_vlan_interface_if_eogre
				(temp_netif, &base_netif, &is_eogre) == PPA_SUCCESS) {
					ppa_get_gre_hdrlen(base_netif, &hdrlen);
			}

			p_swaHdr->tunnel_ip_offset_en = 1;
			p_swaHdr->tunnel_ip_offset = tlen - hdrlen;
			...
			if (!isvtep) {
				tlen += ETH_HLEN;
				p_swaHdr->in_eth_iphdr_offset_en = 1;
				p_swaHdr->in_eth_iphdr_offset = tlen;
			} else {
				p_swaHdr->tunnel_udp_offset_en = 1;
				p_swaHdr->tunnel_udp_offset = ((p_swaHdr->tunnel_ip_offset +
						(isIPv4VxLAN ? IPV4_HDR_LEN : IPV6_HDR_LEN)));
				p_swaHdr->in_eth_iphdr_offset_en = 1;
				p_swaHdr->in_eth_iphdr_offset = p_swaHdr->tunnel_udp_offset +
			...
			if ( ppa_is_EgressPppoeSession(p_item) ) { /* upstream tunnel header */
				p_swaHdr->pppoe_offset_en = 1;
				p_swaHdr->pppoe_offset = p_swaHdr->tunnel_ip_offset - PPPOE_HLEN;
			}
		} else if (ppa_is_IngressGreSession(p_item)) {
			p_swaHdr->tunnel_rm_en = 1;
			p_swaHdr->tunnel_type = TUNL_EOGRE;
		...
		if( IsLanSession(p_item->flags) ) {
			if( ppa_is_MapESession(p_item)) {
				p_swaHdr->tunnel_type = TUNL_DSLITE;
				p_swaHdr->tunnel_ip_offset_en = 1;
				p_swaHdr->tunnel_ip_offset = tlen - IPV6_HDR_LEN;
				p_swaHdr->new_src_ip_en = 1;
				p_swaHdr->new_src_ip.ip4.ip = p_item->pkt.nat_ip.ip;
				if (IsValidNatPort(p_item->flags))
					p_swaHdr->new_src_port = p_item->pkt.nat_port;
				proto_type = ETH_P_IPV6;
			} else if ( IsDsliteSession(p_item->flags) ) {
				...
				p_swaHdr->tunnel_type = TUNL_6RD;
				p_swaHdr->tunnel_ip_offset_en = 1;
				p_swaHdr->tunnel_ip_offset	= tlen - IPV4_HDR_LEN;
				p_swaHdr->in_eth_iphdr_offset_en = 1;
				p_swaHdr->in_eth_iphdr_offset = tlen;
				proto_type = ETH_P_IP;
				...
				p_swaHdr->tunnel_type = TUNL_L2TP;
				p_swaHdr->tunnel_ip_offset_en = 1;
				p_swaHdr->tunnel_ip_offset	= tlen - L2TP_HLEN;
				p_swaHdr->in_eth_iphdr_offset_en = 1;
				p_swaHdr->in_eth_iphdr_offset = tlen;
				p_swaHdr->tunnel_udp_offset_en = 1;
				p_swaHdr->tunnel_udp_offset = ((p_swaHdr->tunnel_ip_offset + IPV4_HDR_LEN));
				proto_type = ETH_P_IP;
				...
				if (IsPppoeSession( p_item->flags))
					...
					mtu += 40;
				p_swaHdr->tunnel_id = p_item->tunnel_idx;

			} else if( IsIpsecSession(p_item->flag2)) {

				p_swaHdr->tunnel_type = TUNL_ESP;
				p_swaHdr->tunnel_id = p_item->tunnel_idx;
				p_swaHdr->tunnel_ip_offset_en = 0;
				p_swaHdr->tunnel_ip_offset = tlen - IPSEC_HLEN;
				p_swaHdr->esp_ip_offset_en = 1;
				p_swaHdr->esp_ip_offset = tlen - IPSEC_HLEN;
			...
			if( IsPppoeSession(p_item->flags) ) {
				p_swaHdr->pppoe_offset_en = 1;
				if (p_swaHdr->tunnel_ip_offset >= PPPOE_HLEN)
					p_swaHdr->pppoe_offset = p_swaHdr->tunnel_ip_offset - PPPOE_HLEN;
				else
					p_swaHdr->pppoe_offset = tlen - PPPOE_HLEN;
			}
		} else {
			...
			if (Is6rdSession(p_item->flags)) {
				...
				p_swaHdr->tunnel_type = TUNL_DSLITE;
				p_swaHdr->new_dst_ip_en = 1;
#if IS_ENABLED(CONFIG_PPA_NAT_LOOP_BACK)
				p_swaHdr->new_dst_ip.ip4.ip = p_item->pkt.nat_dst_ip.ip;
#endif
				if (IsValidNatPort(p_item->flags))
					p_swaHdr->new_dst_port = p_item->pkt.nat_port;
			} else if (IsDsliteSession(p_item->flags)) {
				p_swaHdr->tunnel_type = TUNL_DSLITE;
			} else if (IsL2TPSession(p_item->flags)) {
				p_swaHdr->tunnel_type = TUNL_L2TP;
				...
				p_swaHdr->vni = p_item->pkt.vni;
				p_swaHdr->udp_vxlan_port = p_item->pkt.udp_vxlan_port;
				p_swaHdr->in_eth_iphdr_offset_en = 0;
				p_swaHdr->in_eth_iphdr_offset = 0;
				p_swaHdr->outer_dscp_mode = 1;
			}
		}
	} else {
	...
	if (need_nat) {
		...
		if( IsLanSession(p_item->flags) ) {
			...
			if( isIPv6 ) {
				...
				p_swaHdr->new_dst_ip.ip6.ip[3] = p_item->pkt.dst_ip.ip6[0];
				p_swaHdr->new_dst_ip.ip6.ip[2] = p_item->pkt.dst_ip.ip6[1];
				p_swaHdr->new_dst_ip.ip6.ip[1] = p_item->pkt.dst_ip.ip6[2];
				p_swaHdr->new_dst_ip.ip6.ip[0] = p_item->pkt.dst_ip.ip6[3];

				p_swaHdr->new_src_ip.ip6.ip[3] = p_item->pkt.src_ip.ip6[0];
				p_swaHdr->new_src_ip.ip6.ip[2] = p_item->pkt.src_ip.ip6[1];
				p_swaHdr->new_src_ip.ip6.ip[1] = p_item->pkt.src_ip.ip6[2];
				p_swaHdr->new_src_ip.ip6.ip[0] = p_item->pkt.src_ip.ip6[3];

			} else {
				if (IsValidNatIP(p_item->flags))
					p_swaHdr->new_src_ip.ip4.ip = p_item->pkt.nat_ip.ip;
				else
					p_swaHdr->new_src_ip.ip4.ip = p_item->pkt.src_ip.ip;
				p_swaHdr->new_dst_ip.ip4.ip = p_item->pkt.dst_ip.ip;
			...
			if( isIPv6 ) {
				...
				p_swaHdr->new_src_ip.ip4.ip = p_item->pkt.src_ip.ip;
				if (IsValidNatIP(p_item->flags))
					p_swaHdr->new_dst_ip.ip4.ip = p_item->pkt.nat_ip.ip;
				else
					p_swaHdr->new_dst_ip.ip4.ip = p_item->pkt.dst_ip.ip;
				...
				if (IsDsQosSession(p_item)) {
					if (!(pkt_mark & SESSION_PCE_CLASSIFICATION)) {
						/* Classication done in SW */
						p_swaHdr->ig_traffic_class_en = (pkt_mark & SESSION_ING_TC_SET) >> ING_TC_SET_POS;

						p_swaHdr->new_traffic_class_en = (pkt_mark & SESSION_EGR_TC_SET) >> EGR_TC_SET_POS;

						if (p_swaHdr->ig_traffic_class_en) {
							p_swaHdr->qmap = set_qmap_from_ing_grp(p_item->pkt);
							p_swaHdr->pkt_loop = 1;
						}

						if (!(p_swaHdr->new_traffic_class_en) && (pkt_mark & SESSION_FLAG_TC_REMARK)) {
							p_swaHdr->new_traffic_class_en = 1;
						}
					} else {
						/* Classification done in HW */
						p_swaHdr->pkt_loop = 0;
						p_swaHdr->ig_traffic_class_en = 0;
						p_swaHdr->new_traffic_class_en = 0;
					}

					p_swaHdr->ig_traffic_class = ((pkt_mark & SESSION_INGRESS_TC) >> INGRESS_TC_BIT_POS) - 1;
					p_swaHdr->traffic_class = ((pkt_mark & TC_MASK) >> TC_START_BIT_POS) - 1;
				}
#endif /* CONFIG_INTEL_IPQOS_MPE_DS_ACCEL */
			}
		}
		...
		if (IsValidNatLoopback(p_item->flag2)) {
			p_swaHdr->new_src_ip.ip4.ip = p_item->pkt.nat_src_ip.ip;
			p_swaHdr->new_dst_ip.ip4.ip = p_item->pkt.nat_dst_ip.ip;
		}
#endif
	...
	p_swaHdr->entry_vld = 1;
	p_swaHdr->routing_flag = 1;
	p_swaHdr->templ_len = tlen;
	p_swaHdr->mtu = mtu;
	p_swaHdr->pkt_len_delta = delta;

	if (pkt_mark & FLAG_TC_REMARK) {
		p_swaHdr->new_traffic_class_en = 1;
		p_swaHdr->traffic_class = (p_item->pkt.priority >= 15) ? 15 : p_item->pkt.priority;
	}

	if (p_item->flags & SESSION_VALID_NEW_DSCP) {
		p_swaHdr->new_inner_dscp = p_item->pkt.new_dscp;
		p_swaHdr->new_inner_dscp_en = 1;
	}

        if (p_swaHdr->tunnel_type != TUNL_NULL)  {
		if (p_item->flags & SESSION_VALID_NEW_DSCP)
			p_swaHdr->outer_dscp_mode = 2;
		else
			p_swaHdr->outer_dscp_mode = 1;
	}

	if( p_swaHdr->templ_len ) {
		p_swaHdr->templ_buf = ppa_alloc_dma(p_swaHdr->templ_len);
		if (!p_swaHdr->templ_buf) {
			ppa_free(p_swaHdr);
			return PPA_ENOMEM;
		}
		ppa_memset(p_swaHdr->templ_buf, 0, p_swaHdr->templ_len);
	}
	...
	if(!(p_item->flags & SESSION_TX_ITF_IPOA_PPPOA_MASK)) { /* put ethernet header */ 
		...
		if( IsLanSession(p_item->flags) && IsPppoeSession(p_item->flags) ) {
			...
			if (p_swaHdr->tunnel_type == TRAN_ESP)
				ppa_htons((uint16_t*)(hdr+4), 0x16);
			else
				ppa_htons((uint16_t*)(hdr+4),
					(p_swaHdr->templ_len - (p_swaHdr->pppoe_offset + PPPOE_LEN)));
	...
	if( IsBridgedSession(p_item->flag2) || isEgGreTap || isvtep) {
		if (ppa_is_EgressGreSession(p_item) || (isvtep && IsLanSession(p_item->flags))) {
			...
			if (ppa_tmpl_get_underlying_vlan_interface_if_eogre(temp_netif, &base_netif, &is_eogre) == PPA_SUCCESS) {
				...
				ppa_update_grehdr_inherited(metainfo->skb, base_netif, isIPv4Gre, hdr, p_swaHdr);
...
void ppa_remove_session_mc_tmplbuf(void *sess_act)
{
	...
	if(s_act) {
		ppa_free(s_act->templ_buf); /* free templ_buf */
		ppa_free(s_act->mc_act); /* free templ_buf */
		ppa_free(s_act); /* free complete session_action */
	}
}
...
struct session_action *ppa_form_mc_tmplbuf(PPA_MC_INFO *mc, uint32_t dest_list)
{
	...
	sessionAction->mc_act = ppa_alloc_dma(sizeof(struct mc_session_action));
	if (!sessionAction->mc_act) {
		dbg_print("MC sessionAction is NULL\n");
		ppa_free(sessionAction);
		return NULL;
	}
	ppa_memset(sessionAction->mc_act, 0, sizeof(struct mc_session_action));
	...
	if (IsBridgedSession(p_item->flag2)) {
		sessionAction->mc_ing_vlan = 1;
		sessionAction->pkt_len_delta -= VLAN_HLEN;
	} else {
		sessionAction->routing_flag = 1;
	...
	/* TODO: tunnel handling part needs to added*/
