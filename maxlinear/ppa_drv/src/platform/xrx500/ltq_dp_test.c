...
extern int32_t add_routing_entry(PPA_ROUTING_INFO *route);
extern int32_t add_vlan_entry(PPA_OUT_VLAN_INFO *vlan_entry);
extern int32_t add_wan_mc_entry(PPA_MC_INFO *mc_route);
			...
			uint32_t portmap, uint16_t groupid, uint16_t subifid, uint8_t numvap)
{
	...
	pitem.bridging_flag = 0;
	pitem.grp.ip_mc_group.f_ipv6 = f_ipv6;
	 
	if(!f_ipv6) {
	 	pitem.grp.source_ip.ip.ip = src_IP->ip;
 		pitem.grp.ip_mc_group.ip.ip = dst_IP->ip;
	} else {
		memcpy(&pitem.grp.source_ip.ip.ip6,src_IP->ip6, sizeof(IP_ADDR));
		memcpy(&pitem.grp.ip_mc_group.ip.ip6,dst_IP->ip6, sizeof(IP_ADDR));
	}
 
	route.dest_list = portmap;
	pitem.dest_subifid = subifid;
	pitem.grp.group_id = groupid;
	pitem.num_vap =	numvap;	
	 
	memcpy(pitem.s_mac,srcMAC,sizeof(srcMAC));
			...
			uint16_t tunnel_index, uint32_t tunnel_type,IP_ADDR *t_srcip, IP_ADDR *t_dstip)
{
	...
	if(tunnel_index) {
		...
		if (!t_entry) {
			printk(KERN_INFO "tunnel_entry malloc Failure\n");
			return PPA_FAILURE;
		}
	...
	printk(KERN_INFO "add_routing_entry Success sip=%pI6 dip=%pI6 sp=%u dp=%u tun-src=%pI4 tun-dst=%pI4 index=%d\n",
		&(Src_IP->ip6), &(Dst_IP->ip6), SrcPort, DstPort, &(g_tunnel_table[tunnel_index]->tunnel_info.ip4_hdr.saddr),
		&(g_tunnel_table[tunnel_index]->tunnel_info.ip4_hdr.daddr), tunnel_index);
...
void pce_rule_read(int index)
{
	...
	if((ret=gsw_api_kioctl(gswr, GSW_PCE_RULE_READ, &rule)) < GSW_statusOk) {
...
void routing_entry_read(int index)
{
	...
	if((ret=gsw_api_kioctl(gswr, GSW_ROUTE_ENTRY_READ, &rt_entry)) < GSW_statusOk) {
...
int ipv4_routing_vlan(void)
{
	...
	memset(&vid, 0, sizeof(PPA_OUT_VLAN_INFO));
	vid.vlan_id = 10;
	vid.port_id = 15;
	vid.subif_id = 1;
	vid.ctag_ins = 1; 
...
int pppoe_routing(void)
{
	...
	u8 lanpcMAC[6]	= {0x00, 0x00, 0x00, 0x00, 0x00, 0x20};
	u8 br0MAC[6]	= {0x00, 0x00, 0x00, 0x00, 0x00, 0x02};
	u8 eth1MAC[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0xee}; 
	u8 gwMAC[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x16};
...
/* since no where used following function, commented to fix warning*/
#if 0
