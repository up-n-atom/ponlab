 ...
 * - asm_tc_sync(): Gets called from tx-module while polling the HW TC-SYNC flag (src/atm/ima_tx_module.c)
 */
...
init_device_asm_metadata(struct bond_drv_cfg_params *cfg)
{
	...
	PRIV->fsm.rxlinkstatus[0]
		= PRIV->fsm.rxlinkstatus[1]
			= RX_UNUSABLE;
	spin_lock_init(&PRIV->fsm.spinlock);
	PRIV->fsm.showtime[0]
		= PRIV->fsm.showtime[1]
			= false;
	PRIV->fsm.tcsync[0]
...
asm_showtime_enter(u32 linkid)
{
	...
	/* Get tcsync-status of this link */
	spin_lock_bh(&PRIV->fsm.spinlock);
	tcsync_state = PRIV->fsm.tcsync[linkid];
	spin_unlock_bh(&PRIV->fsm.spinlock);

	if (tcsync_state) {

		// Inject LINK_UP event
		if (!asm_fsm_push_event(LINK_UP, linkid)) {
			// TODO: Update stats on too many events
			pr_crit( "bonding driver: %s: ASM Event Q full. Could not inject LINK_UP event for link-%u\n", __func__, linkid );
		}
	}

	/* Remember showtime-status of this link */
	spin_lock_bh(&PRIV->fsm.spinlock);
	PRIV->fsm.showtime[linkid] = true;
	spin_unlock_bh(&PRIV->fsm.spinlock);
}
...
asm_showtime_exit(u32 linkid)
{
	...
	// Inject LINK_DOWN event
	if (!asm_fsm_push_event(LINK_DOWN, linkid)) {
		// TODO: Update stats on too many events
		pr_crit( "bonding driver: %s: ASM Event Q full. Could not inject LINK_DOWN event for link-%u\n", __func__, linkid );
	}
...
void
asm_tc_sync( u32 linkid, bool up)
{
	bool showtime_state;

	/* Get showtime-status of this link */
	spin_lock_bh(&PRIV->fsm.spinlock);
	showtime_state = PRIV->fsm.showtime[linkid];
	spin_unlock_bh(&PRIV->fsm.spinlock);
	...
	/* Remember tcsync-status of this link */
	spin_lock_bh(&PRIV->fsm.spinlock);
	PRIV->fsm.tcsync[linkid] = up;
	spin_unlock_bh(&PRIV->fsm.spinlock);
}
...
asm_fsm( ASMFSMEvent_e event, u32 linkid, void *data )
{
	...
	switch (PRIV->fsm.state) {
		...
		case LEARNING:
			switch (event) {
				case LINK_UP: PRIV->fsm.state = STEADY;
					PRIV->fsm.txlinkstatus[0]
						= PRIV->fsm.txlinkstatus[1]
							= TX_ACTIVE;
					PRIV->fsm.rxlinkstatus[0]
				...
				case LINK_DOWN: PRIV->fsm.state = LEARNING;
					deactivate_tx_link(linkid);
		...
		case STEADY:
			switch (event) {
				...
				case LINK_DOWN: PRIV->fsm.state = STEADY;
					deactivate_tx_link(linkid);
					deactivate_rx_link(linkid);
					asm_fsm_disable_txtimer( linkid);
					break;
...
stringify_tx_link_event( u32 event )
{
	...
	switch((ASMTxLinkStatusFSMEvent_e)event) {
		case ev_TX_LINK_UP: strcpy( retval, "LINK_UP" ); break;
		case ev_TX_LINK_DOWN: strcpy( retval, "LINK_DOWN" ); break;
...
stringify_rx_link_event( u32 event )
{
	...
	switch((ASMRxLinkStatusFSMEvent_e)event) {
		case ev_RX_LINK_UP: strcpy( retval, "LINK_UP"); break;
		case ev_RX_LINK_DOWN: strcpy( retval, "LINK_DOWN"); break;
...
asm_tx_link_fsm( ASMTxLinkStatusFSMEvent_e event, u32 linkid )
{
	...
	switch (PRIV->fsm.txlinkstatus[linkid]) {
		...
		case TX_SHOULDNT_BE_USED:
			...
			switch (event) {
				case ev_TX_LINK_UP: PRIV->fsm.txlinkstatus[linkid] = TX_ACCEPTABLE;
					break;
		...
		case TX_ACCEPTABLE:
			switch (event) {
				case ev_TX_LINK_UP: PRIV->fsm.txlinkstatus[linkid] = TX_ACCEPTABLE;
					break;
				case ev_TX_LINK_DOWN: PRIV->fsm.txlinkstatus[linkid] = TX_SHOULDNT_BE_USED;
					break;
...
asm_rx_link_fsm( ASMRxLinkStatusFSMEvent_e event, u32 linkid )
{
	...
	switch (PRIV->fsm.rxlinkstatus[linkid]) {
		...
		case RX_UNUSABLE:
			...
			switch (event) {
				case ev_RX_LINK_UP: PRIV->fsm.rxlinkstatus[linkid] = RX_SHOULDNT_BE_USED;
					activate_rx_link(linkid);
					break;
				case ev_RX_LINK_DOWN: PRIV->fsm.rxlinkstatus[linkid] = RX_UNUSABLE;
					deactivate_rx_link(linkid);
					break;
	...
	case RX_ACCEPTABLE:
			switch (event) {
				case ev_RX_LINK_UP: PRIV->fsm.rxlinkstatus[linkid] = RX_ACCEPTABLE;
					activate_rx_link(linkid);
					break;
				case ev_RX_LINK_DOWN: PRIV->fsm.rxlinkstatus[linkid] = RX_UNUSABLE;
					deactivate_rx_link(linkid);
		...
		case RX_SELECTED:
			switch (event) {
				case ev_RX_LINK_UP: PRIV->fsm.rxlinkstatus[linkid] = RX_SELECTED;
					activate_rx_link(linkid);
					break;
				case ev_RX_LINK_DOWN: PRIV->fsm.rxlinkstatus[linkid] = RX_UNUSABLE;
...
asm_fsm( ASMFSMEvent_e event, u32 linkid, void *data )
{
	...
	switch (PRIV->fsm.state) {
		...
		case LEARNING:
			switch (event) {
				case LINK_UP: PRIV->fsm.state = LEARNING;
					asm_tx_link_fsm( ev_TX_LINK_UP, linkid );
					asm_rx_link_fsm( ev_RX_LINK_UP, linkid );
					asm_fsm_enable_txtimer( linkid);
					break;

				case LINK_DOWN: PRIV->fsm.state = LEARNING;
					asm_tx_link_fsm( ev_TX_LINK_DOWN, linkid );
					asm_rx_link_fsm( ev_RX_LINK_DOWN, linkid );
					asm_fsm_disable_txtimer( linkid);
					break;
