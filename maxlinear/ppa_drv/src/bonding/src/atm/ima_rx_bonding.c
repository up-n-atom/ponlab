...
{
	cleanup_rx_reassembler();
}

int32_t
init_device_rx_bonding(void *cfg)
{
	int32_t ret;
	...
	ret = init_rx_reassembler(cfg, 35);
	if (ret)
		goto out;
...
void
cleanup_device_rx_bonding(void)
{
	cleanup_rx_reassembler();
}
...
u8 *get_buffer(u32 lineid, u8 *buffer, u32 bufferlen)
{
	...
	buf_phys = dma_map_single(NULL, (void *)buf, 56, DMA_FROM_DEVICE);
