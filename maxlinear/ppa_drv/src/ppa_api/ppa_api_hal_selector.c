...
static inline bool is_ipsec_session(struct uc_session_node * p_item)
{
	bool ret = false;
...
static inline void ppa_bridge_macdb_add(struct uc_session_node * p_item)
{
	if (!(p_item->flags & SESSION_TX_ITF_IPOA_PPPOA_MASK) &&
	    !(p_item->flags & SESSION_VALID_NEW_SRC_MAC)) {
		p_item->flags |= SESSION_VALID_NEW_SRC_MAC;
		if (p_item->br_rx_if) {
			ppa_bridge_entry_add(p_item->s_mac, NULL,
					     p_item->br_rx_if,
					     PPA_F_STATIC_ENTRY);
		} else if (p_item->rx_if && !(is_ipsec_session(p_item))) {
			ppa_bridge_entry_add(p_item->s_mac, NULL,
					     p_item->rx_if,
					     PPA_F_STATIC_ENTRY);
		}
		/* Destination mac learning in macdb.
		 * Needed for uni directional UDP traffic in lab scenario.
		 * If it is enabled it may cause some mal behavior specially
		 * when destination client is not directly connected
		 * with the CPE
		 */
		if (g_dst_mac_learning && p_item->tx_if) {
			ppa_bridge_entry_add(p_item->pkt.dst_mac, NULL,
			     p_item->tx_if,
			     PPA_F_STATIC_ENTRY);
		}
	}
...
static inline void ppa_bridge_macdb_del(struct uc_session_node * p_item)
{
	if ((p_item->flags & SESSION_VALID_NEW_SRC_MAC) &&
	    !is_zero_mac(p_item->s_mac)) {
		ppa_bridge_entry_delete(p_item->s_mac, NULL,
					PPA_F_STATIC_ENTRY);
		/* Delete the dst mac learned.
		 * Only for STC (lab scenario workaround)
		 */
		if (g_dst_mac_learning)
			ppa_bridge_entry_delete(p_item->pkt.dst_mac, NULL,
						PPA_F_STATIC_ENTRY);
	}
}
...
static int32_t ppa_hsel_add_supporting_session(struct uc_session_node *p_item,
					       PPA_ROUTING_INFO *route,
					       PPA_HAL_ID hal_id)
{
	int32_t ret;

	ret = ppa_hsel_add_routing_entry(route, 0, hal_id);
	if (ret == PPA_SUCCESS) {
...
static void ppa_hsel_del_supporting_session(struct uc_session_node *p_item,
					    PPA_ROUTING_INFO *route)
{
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
	ppa_hsel_get_routing_entry_bytes(route, 1, PPV4_HAL);
	ppa_hsel_del_routing_entry(route, 0, PPV4_HAL);
#elif IS_ENABLED(CONFIG_SOC_GRX500) && !IS_ENABLED(CONFIG_SOC_PRX)
...
int32_t ppa_hsel_add_routing_session(struct uc_session_node * p_item, PPA_BUF *skb,
		void *session_meta, uint32_t flags)
{
	...
	route.p_item = p_item;
	route.session_meta = session_meta;
	...
	for (i = 0; i < num_caps;) {
		...
		switch (p_item->caps_list[i].cap) {
			...
			case TUNNEL_VXLAN:
				...
				/* add source mac to macdb */
				ppa_bridge_macdb_add(p_item);

				if (pkt_mark & SESSION_FLAG_TC_REMARK)
				...
				route.tc_remark = ((pkt_mark & TC_MASK) >> TC_START_BIT_POS) - 1;
				...
				if ((p_item->caps_list[i].hal_id == SWAC_HAL) && hw_litepath_available
					...
					pret = ppa_hsel_add_supporting_session(p_item, &route,
							PAE_HAL);
					if (pret == PPA_SUCCESS) {
...
SKIP_ADD_SESS_META:
					if ((ret = ppa_hsel_add_routing_entry(&route, 0,
...
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
						/* configure supportive PP bridged session */
						if ((p_item->flags & SESSION_ADDED_IN_SW) &&
						    (p_item->caps_list[i].hal_id == SWAC_HAL))
							ppa_hsel_add_supporting_session(p_item, &route,
								PPV4_HAL);
#endif
#if IS_ENABLED(CONFIG_SOC_GRX500)
						if ((p_item->flags & SESSION_ADDED_IN_SW) && (p_item->caps_list[i].hal_id == SWAC_HAL) &&
							...
							/* Try PAE Engine explicity for lp hw accel */
							pret = ppa_hsel_add_supporting_session(p_item,
									&route, PAE_HAL);
							if (pret == PPA_SUCCESS) {
								p_item->sess_hash = route.entry;
							} else {
...
void ppa_hsel_del_routing_session(struct uc_session_node *p_item)
{
	...
	if ((p_item->flags & SESSION_ADDED_IN_HW) && (p_item->flags & SESSION_ADDED_IN_SW)) {
		ppa_hsel_del_supporting_session(p_item, &route);
	} else {
...
int32_t ppa_hsel_add_wan_mc_group(struct mc_session_node *p_item, PPA_C_OP *cop)
{
	...
	for (i = 0; i < num_caps;) {
		...
		switch (p_item->caps_list[i].cap) {
			case SESS_MC_DS:
				if (p_item->caps_list[i].hal_id == PPV4_HAL) {
					...
					if ((p_item->grp.sess_info->ref_count > 1) ||
					    (p_item->flag2 & SESSION_FLAG2_BRIDGED_SESSION)) {
						ret = PPA_FAILURE;
						break;
					}
...
SESSION_SWAP_CONTINUE:
	...
	ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
			...
			dest_subifid=%0x\n group_id=%u\n p_item->sess_hash=%u\n num_caps=%d \nlast_hit_time=%d \n", __FILE__,
			__FUNCTION__, __LINE__, p_item->grp.sess_info->source_ip.ip.ip, p_item->grp.sess_info->ip_mc_group.ip.ip,
			p_item->grp.sess_info->src_port, p_item->grp.sess_info->dst_port, p_item->grp.src_netif->name, p_item->dest_ifid,
...
void ppa_hsel_del_wan_mc_group(struct mc_session_node *p_item)
{
	...
	/*when init, these entry values are ~0, the max the number which can be detected by these functions*/
	ppa_debug(DBG_ENABLE_MASK_DUMP_MC_GROUP,
		  "%s: remove gid:%d from HW mc_entry:%d\n", __func__,
		  p_item->grp.group_id, p_item->mc_entry);
	for (i = 0; i < p_item->num_caps;) {
		...
		uint32_t flags)
{
	...
	if ((p_item->flags & SESSION_ADDED_IN_HW)) {
		...
		ppa_debug(DBG_ENABLE_MASK_DUMP_MC_GROUP,
			  "%s: update gid:%d to HW mc_entry:%d\n", __func__,
			  p_item->grp.group_id, p_item->mc_entry);
		for (i = 0; i < p_item->num_caps;) {
			j = ppa_group_hals_in_capslist(i, p_item->num_caps, p_item->caps_list);
...
int32_t ppa_lro_entry_criteria(struct uc_session_node *p_item, PPA_BUF *ppa_buf, uint32_t flags)
{
	/* 1. Session gets added to lro only after 64k bytes are transmitted (TCP segment size)
	   2. TODO: URX: TCP acks non-lro ppv4 accelerated.
	   3. GRX: Session gets added to lro only if packet size is large (to avoid tcp ack) */

#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
	/* if hw_litepath disabled, allow LRO session based on the
	 * respective interface feature (NETIF_F_GRO_HW | NETIF_F_LRO)
	 */
	if ((is_lpdev(NULL) == PPA_FAILURE) && p_item->rx_if &&
	    !(p_item->rx_if->features & (NETIF_F_GRO_HW | NETIF_F_LRO))) {
		p_item->flags |= SESSION_NOT_ACCELABLE;
		return PPA_FAILURE;
	}
#endif

	if (p_item->host_bytes > 64000) {
#if IS_ENABLED(CONFIG_SOC_GRX500)
		if (ppa_skb_len(ppa_buf) > (p_item->mtu - 128) ) {
			return PPA_SUCCESS;
		} else
			return PPA_FAILURE;
#endif /* CONFIG_SOC_GRX500 */
