...
void session_mgmt_stats(struct uc_session_node *p_item, enum ppa_oper_type oper)
{
	int mark;
	...
	if(p_item->flags & SESSION_ADDED_IN_HW) {
		if (p_item->num_caps == 0 || p_item->num_caps != 1) {
			/*if number of selected hals is not set to 1 then its complementery prcoessing*/
			return;
		}
...
void update_session_mgmt_stats(struct uc_session_node *p_item, enum ppa_oper_type oper)
{
...
/* 	To arrive at correct data rate for TCP session,we wait for few seconds to calculate data rate.
	wait_time.tv_sec = nf_conntrack_sessionmgmt_wait_time : this is period in seconds we want to wait
	before calculating data rate (will be kept as 5 sesonds by default).This can be modified using proc entry on the fly
...
*/
uint32_t ppa_session_pass_criteria(PPA_BUF *ppa_buf, struct uc_session_node *p_item,uint32_t flags)
{
	uint64_t time_msec;
...
void add_pitem_node(struct uc_session_node *p_item, enum ppa_engine_type engine)
{
	if(engine == PAE || engine == MPE) {
		if (ppa_hsel_add_routing_session(p_item, NULL,
			...
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ppa_hsel_add_session(p_item) fail\n");
		}
#if IS_ENABLED(CONFIG_PPA_API_SW_FASTPATH)
	} else if (engine == SWAC) {
#else
...
void delete_pitem_node(struct uc_session_node *p_item, enum ppa_engine_type engine)
{
	if(engine == PAE || engine == MPE) {
...
uint32_t ppa_reset_ewma_stats( struct uc_session_node *p_item,uint32_t flags)
{
...
uint32_t ppa_update_ewma(PPA_BUF *ppa_buf,struct uc_session_node *p_item,uint32_t flags)
{
	if ( ppa_get_session_limit_enable(0) ) {
...
uint32_t ppa_session_store_ewma( PPA_BUF *ppa_buf, struct uc_session_node *p_item,uint32_t flags)
{
...
uint32_t ppa_decide_collision(struct uc_session_node *p_item) {
...
uint32_t ppa_session_limit_update_flags(struct uc_session_node *p_item,uint32_t flags)
{
...
uint32_t	ppa_session_record_time(PPA_BUF *ppa_buf,struct uc_session_node *p_item,uint32_t flags)
{
	PPA_TIMESPEC before;
	if ( ppa_get_session_limit_enable(0) ) {
