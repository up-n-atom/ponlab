...
#define CGU_LJPLL4_CFG1		0x104
#define CGU_LJPLL4_CFG2		0x108
...
static const struct lgm_pll_clk_data lgm_pll_clks[] = {
	...
	LGM_PLL(LGM_CLK_PLL1, "pll1", pll_p, CLK_IGNORE_UNUSED,
...
/* Procedure for changing VCO:
 * - Disable PLL
 * - Change REFDIV, FBDIV accordingly
 * - Enable PLL
 * - Poll for PLL lock
 * - Change POSTDIV1, POSTDIV2 accordingly
 */
static void lgm_clk_change_vco(struct regmap *map)
{
	unsigned int cfg0;

	/* Change LJPLL4 VCO to 9.6GHz for best PCIe clock output.
	 * REFDIV = 1, FBDIV = 240 (FVCO = 9.6GHz) DSMEM = 0 (INT mode)
	 * POSTDIV1 = 3'b101 (/6), POSTDIV2 = 3'b011 (/4)
	 *
	 * Note: DSMEN has set to be 1 to support SSC.
	 */
	regmap_write(map, CGU_LJPLL4_CFG0, 0x50000000);
	regmap_write(map, CGU_LJPLL4_CFG2, 0x100400f0);
	regmap_write(map, CGU_LJPLL4_CFG0, 0xd0000001);

	if (regmap_read_poll_timeout_atomic(map, CGU_LJPLL4_CFG0, cfg0,
					    cfg0 & BIT(1), 1000, 10 * 1000))
		pr_err("LJPLL4 not locked\n");

	regmap_write(map, CGU_LJPLL4_CFG1, 0x0100001d);
}

/* Adjust PHY clk speed to 15ppm faster than 2.5Ghz crystal clock speed */
static void lgm_pll_pre_init(struct regmap *map, struct device_node *np)
{
	regmap_write(map, CGU_LJPLL3_CFG0, 0xd007ae19);
	lgm_clk_change_vco(map);
	pr_debug("lgm pll pre init done!\n");
}
...
static int lgm_cgu_probe(struct platform_device *pdev)
{
	...
	lgm_pll_pre_init(ctx->membase, np);
	ctx->np = np;
	ctx->dev = dev;
...
static int __init cgu_drv_register(void)
{
	return platform_driver_register(&lgm_cgu_driver);
}
subsys_initcall(cgu_drv_register);
