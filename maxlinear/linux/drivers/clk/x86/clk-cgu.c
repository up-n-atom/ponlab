...
static void
lgm_clk_set_ssc(struct lgm_clk_provider *ctx,
		u32 reg, u32 spread, u32 dir, u32 div, u32 en)
{
	lgm_set_clk_val(ctx->membase, reg, 2, 5, spread);
	lgm_set_clk_val(ctx->membase, reg, 12, 1, dir);
	lgm_set_clk_val(ctx->membase, reg, 8, 4, div);
	lgm_set_clk_val(ctx->membase, reg, 0, 1, en);
}
...
void lgm_clk_ssc_probe(struct lgm_clk_provider *ctx)
{
	...
	np_ssc = of_find_node_by_name(ctx->np, "cgussc");
	if (!np_ssc)
		return;
	...
	for_each_child_of_node(np_ssc, nc) {
		...
		lgm_clk_set_ssc(ctx, reg, spread, dir, div, en);
	}
