...
/**
 * eqbr_grp_vals: mux group register offset and value pair
 * @map:	base address of register
 * @reg:	offset of the register from base address
 * @val:	register value
 * @mask:	mask
 */
struct eqbr_grp_vals {
	struct regmap	*map;
	unsigned int	reg;
	unsigned int	val;
	unsigned int	mask;
...
/**
 * eqbr_pmx_mux: pinmux configure for a mux group
 * @pinmux:	pinmux array
 * @mux_nr:	number of pinmux array
 * @vals:	topmux setting for the group
 * @nvals:	number of the topmux setting
 */
struct eqbr_pmx_mux {
	unsigned int		*pinmux;
	unsigned int		mux_nr;
