...
u64 service_log_inga_base_addr;
u64 service_log_ingb_base_addr;
u64 service_log_qos_base_addr;
...
{
	if (srvs_log >= SERVICE_INGA && srvs_log < SERVICE_LOG_NUM)
		return true;
	pr_err("invalid PP service_log=%d param\n", srvs_log);
	return false;
}
...
s32 infra_service_log_fault_en_set(enum service_log srvs_log, bool en)
{
	...
	PP_REG_WR32(SERVICE_LOG_IS_EN_ADDR(srvs_log), (u32)en);
	return 0;
}

s32 infra_service_log_fault_en_get(enum service_log srvs_log, bool *en)
{
	if (ptr_is_null(en) || !is_service_log_valid(srvs_log))
		return -EINVAL;

	*en = !!PP_REG_RD32(SERVICE_LOG_IS_EN_ADDR(srvs_log));
	return 0;
}
...
s32 infra_service_log_err_clear(enum service_log srvs_log)
{
	...
	PP_REG_WR32(SERVICE_LOG_ERR_CLR_ADDR(srvs_log), (u32)1);
	return 0;
}

/**
 * @brief Get err->is_err register value for specific service log
 */
static void service_log_is_err(enum service_log srvs_log,
			      struct srvs_log_err *err)
{
	err->is_err = !!PP_REG_RD32(SERVICE_LOG_IS_ERR_ADDR(srvs_log));
}

/**
 * @brief Read err from register in case is err is on
 */
static void get_err_fields(enum service_log srvs_log, struct srvs_log_err *err)
{
	ulong err_log_val;

	/* in case there is no error we put zero's in err */
	service_log_is_err(srvs_log, err);
	if (!err->is_err) {
		err->seq_id = 0;
		err->subrange = 0;
		err->init_flow = INIT_FLOW_NONE;
		err->target_flow = TARGET_FLOW_NONE;
		return;
	}

	err_log_val = PP_REG_RD32(SERVICE_LOG_ERR_LOG_ADDR(srvs_log));

	err->seq_id = PP_FIELD_GET(SRVICE_LOG_SEQ_ID_MSK, err_log_val);
	err->subrange = PP_FIELD_GET(SRVICE_LOG_SUBRANGE_MSK, err_log_val);
	err->init_flow = PP_FIELD_GET(SRVICE_LOG_INIT_FLOW_MSK, err_log_val);
	err->target_flow =
		PP_FIELD_GET(SRVICE_LOG_TARGET_FLOW_MSK, err_log_val);
}

s32 infra_service_log_get_err(enum service_log srvs_log,
			      struct srvs_log_err *err)
{
	if (ptr_is_null(err) || !is_service_log_valid(srvs_log))
		return -EINVAL;

	get_err_fields(srvs_log, err);
	return 0;
}
...
s32 infra_init(struct infra_init_param *init_param)
{
	...
	if (!init_param->valid || !init_param->is_srvs_log_valid)
		return -EINVAL;

	bootcfg_base_addr  = init_param->bootcfg_base;
	clk_ctrl_base_addr = init_param->clk_ctrl_base;
	service_log_inga_base_addr = init_param->service_log_inga_base;
	service_log_ingb_base_addr = init_param->service_log_ingb_base;
	service_log_qos_base_addr = init_param->service_log_qos_base;
