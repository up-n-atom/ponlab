...
static void cls_dbg_hash_calc_set(char *cmd_buf, void *data)
{
	...
	while ((tok = strsep(&args, " \t\n")) != NULL) {
		...
		if (kstrtou32(tok, 0, &fv[cnt])) {
			pr_err("kstrtou32 err\n");
			return;
		}
		if (cnt >= ARRAY_SIZE(fv) - 1)
			break;
		cnt++;
	}

	if (cls_hash_calc(fv, cnt << 2, &hash)) {
...
void cls_dbg_gre_v4_v4_ds_sess_create(struct seq_file *f)
{
	...
	si.word[0]  = MOD_RCP_IPV4_OFF3_DECP;
	/* dpu start 0x30, sce start 0x40, fv size 128, bce start 0x50,
	 * bce size 48
	 */
	si.word[1]  = 0x9F530000;
...
/**
 * @brief Dump classifier stats
 */
void __cls_global_stats_show(struct seq_file *f)
{
	struct cls_stats stats;

	if (cls_stats_get(&stats))
		return;

	seq_puts(f, " Classifier global stats:\n");
	seq_puts(f, " =======================\n");
	seq_printf(f, "Rx         = %u\n",
		   stats.pkts_rcvd_from_parser + stats.pkts_rcvd_from_uc);
	seq_printf(f, "Rx(Parser) = %u\n", stats.pkts_rcvd_from_parser);
	seq_printf(f, "Rx(uC)     = %u\n", stats.pkts_rcvd_from_uc);
	seq_printf(f, "Search     = %u\n", stats.total_lu_requests);
	seq_printf(f, "Match      = %u\n", stats.total_lu_matches);
}

PP_DEFINE_DEBUGFS(cls_global_stats, __cls_global_stats_show, NULL);
...
static struct debugfs_file cls_root_files[] = {
	...
	{ "global", &PP_DEBUGFS_FOPS(cls_global_stats) },
};
