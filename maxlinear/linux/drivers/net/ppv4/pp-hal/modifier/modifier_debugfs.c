...
/**
 * @brief Dump modifier stats
 */
void mod_dbg_global_stats_show(struct seq_file *f)
{
	struct mod_stats stats;

	if (mod_stats_get(&stats))
		return;

	seq_puts(f, " Modifier global stats:\n");
	seq_puts(f, " ======================\n");
	seq_printf(f, "Rx          = %u\n",
		   stats.pkts_rcvd_from_chk + stats.pkts_rcvd_from_uc);
	seq_printf(f, "Rx(Checker) = %u\n", stats.pkts_rcvd_from_chk);
	seq_printf(f, "Rx(uC)      = %u\n", stats.pkts_rcvd_from_uc);
	seq_printf(f, "Modified    = %u\n", stats.pkts_modified);
}

PP_DEFINE_DEBUGFS(mod_global_stats, mod_dbg_global_stats_show, NULL);
...
static struct debugfs_file dbg_files[] = {
	...
	{ "global", &PP_DEBUGFS_FOPS(mod_global_stats) },
};
