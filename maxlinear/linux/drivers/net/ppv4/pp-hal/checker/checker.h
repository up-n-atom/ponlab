 ...
 *
 * Description: Packet Processor Checker H file
 */
...
struct chk_dbg_stats {
	u32 sync;
	u32 sync_first;
	u32 to_uc; /* tx to UC */
	u32 uc_drop; /* dropped due to UC timeout */
	u32 drop;
	u32 dynamic;
	u32 ip_opt;
	u32 ttl_exp;
...
/**
 * @brief Get number of exception session pkts
 * @param pkts
 * @return s32 0 on success, error code otherwise
 */
 ...
 * @param eg_info egress port configuration info
 * @return s32 0 on success, error code otherwise
 */
...
/**
 * @brief Get Checker tbm timer prescaler physical address
 * @return dma_addr_t
 */
dma_addr_t chk_tbm_prescaler_phys_addr_get(void);

/**
 * @brief Get Checker crawler timer prescaler physical address
 * @return dma_addr_t
 */
