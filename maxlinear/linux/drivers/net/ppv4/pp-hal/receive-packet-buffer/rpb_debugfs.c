...
void __rpb_stats_show(struct seq_file *f)
{
	...
	struct rpb_stats global_stats;
	u32 port, tc, reg, free_clids, free_cc;
	s32 ret = 0;
	bool port_back_pres;

	if (rpb_hw_stats_get(&stats) || rpb_stats_get(&global_stats))
		return;
	...
	seq_printf(f, "|  %-25s = %-10u  |\n", "Global Rx Packets",
		   (u32)global_stats.pkts_rcvd_from_port_dist);
	seq_printf(f, "|  %-25s = %-10u  |\n", "Global Drop Packets",
		   (u32)global_stats.total_packets_dropped);
	seq_printf(f, "|  %-25s = %-10u  |\n", "Global Bytes Used",
