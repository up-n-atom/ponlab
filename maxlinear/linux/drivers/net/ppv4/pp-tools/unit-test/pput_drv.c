...
struct thread_args {
	u32 component;
	u32 level;
	char single_test_name[MAX_TEST_NAME_LENGTH];
};
...
static int pput_run(u32 component, u32 level, char *single_test_name)
{
	...
	args->level = level;
	if (single_test_name) {
		if (strlen(single_test_name) < MAX_TEST_NAME_LENGTH) {
			strncpy(args->single_test_name ,single_test_name, strlen(single_test_name));
		}
		else {
			pr_err("Test Name name too long %zu , maxSupported %d", strlen(single_test_name), MAX_TEST_NAME_LENGTH);
			return 0;
		}
	}
