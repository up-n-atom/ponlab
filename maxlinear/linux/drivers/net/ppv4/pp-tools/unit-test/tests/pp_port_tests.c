...
static u16 pp_test_port = PP_MAX_PORT;

/**
 * @brief update pp_test_port global variable to free port.
 * @return 0 on success or -1 if didn't find free port.
 */
static s32 update_free_port(void)
{
	int i;

	if (PP_IS_PORT_VALID(pp_test_port))
		return 0;

	/* using top-down loop to find unactive port that also more likely to
	   be unused node (for port_manager_hostif_api_test) */
	for (i = PP_MAX_PORT - 1; i >= 0; i--) {
		if (!pmgr_port_is_active(i)) {
			pp_test_port = (u16)i;
			return 0;
		}
	}
	pr_err("pp_port_add failed: not find free port\n");
	return -1;
}
...
static int port_manager_port_add_test(void *unused)
{
	...
	if (unlikely(pmgr_port_add(pp_test_port, &port))) {
...
static int port_manager_port_del_test(void *unused)
{
	if (unlikely(pmgr_port_del(pp_test_port))) {
...
revert:
	...
	err = pmgr_hostif_dflt_set(&dflt_dp);
	if (unlikely(err)) {
