...
/**
 * @brief Defines Cache replacement policies for the PP session and counters
 *        caches
 */
enum cache_replacment_policy {
 ...
 */
#define num_is_odd(num)                ((num) & (typeof(num))BIT_ULL(0))
...
/**
 * @define Packet Processor Descriptor extension name to register
 * to mxl_skb_ext module
 */
#define PP_DESC_EXT_NAME     "pp-desc-ext"
 ...
 */
#define arr_entry_idx(arr, entry) ((u32)((entry) - (typeof(entry))arr))
...
/**
 * @brief Get current PP clock rate
 *        The returned clock rate is taken from the HW
 * @return u32 the clock rate in MHz
 */
u32 pp_dev_hw_clk_get(void);

/**
 * @brief Get a reference to device's private data
 * @return struct pp_dev_priv*, MUST check for NULL pointer
 */
struct pp_dev_priv * __must_check pp_priv_get(void);
...
static inline s64 buf_fld_get(struct buf_fld_info *fld, const u32 *buf)
{
	...
	while (word_idx <= fld->last_word_idx) {
		if (word_idx < fld->last_word_idx)
			mask = GENMASK_ULL(BITS_PER_U32 - 1, lsb % BITS_PER_U32);
		else
			mask = GENMASK_ULL((msb % BITS_PER_U32),
				       lsb % BITS_PER_U32);
...
static inline s32 buf_fld_set(struct buf_fld_info *fld, u32 *buf, s64 val)
{
	...
	while (word_idx <= fld->last_word_idx) {
		...
		tmp           = PP_FIELD_GET(GENMASK_ULL(hweight - 1, 0), hw_val);
...
{
	ulong mask = GENMASK_ULL(fld->msb - fld->lsb, 0);
...
/**
 * @brief pp_stats_bytes_show version for seq_file
 */
#define pp_stats_show_bytes_seq(f, ssz, num, get, show, data)		\
	pp_stats_show(ssz, num, get, show, data, (f)->buf, (f)->size,	\
			&(f)->count)
 ...
 * @brief pp_pps_show version for seq_file
 */
#define pp_bps_show_seq(f, ssz, num, get, diff, show, data)				\
	pp_pps_show(ssz, num, get, diff, show, data, (f)->buf, (f)->size,	\
		&(f)->count)
