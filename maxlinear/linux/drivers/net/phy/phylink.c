...
static int phylink_sfp_connect_phy(void *upstream, struct phy_device *phy)
{
	...
	/*
	 * If pl->sfp_may_have_phy is defined, the supported field detected from SFP will only be
	 * used in phylink_sfp_module_start(), instead of phylink_sfp_module_insert(). However,
	 * in sfp state machine, sfp_module_start(), i.e phylink_sfp_module_start(), is only called
	 * after sfp_sm_probe_for_phy(), i.e. sfp_sm_probe_phy->sfp_add_phy->phylink_sfp_connect_phy().
	 * This results in mac_validation failure.
	*/
	if (pl->sfp_may_have_phy && phylink_is_empty_linkmode(phy->supported)) {
