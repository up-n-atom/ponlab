...
static u16 cal_hash(ethsw_api_dev_t *gswdev, u32 src_ip_mode,
		    MCAST_HASHTBL_PTN *pattern, u32 type);
		    ...
		    MCAST_HASHTBL_PTN *pattern, u32 type)
{
	...
	if (VRSN_CMP(gswdev->gipver, LTQ_GSWIP_3_2B) >= 0) {
		b[0] = pattern->tci >> 10;
		b[1] = (pattern->tci >> 2) & 0xFF;
		b[2] = ((pattern->tci << 6) | (pattern->fid & 0x3F)) & 0xFF;
		len = 3;
	} else {
		b[0] = pattern->fid;
		len = 1;
	...
	if (type == GSW_IP_SELECT_IPV4) {
		for (i = 0; i < 4; i++, len++)
			b[len] = (pattern->dstip.nIPv4 >> ((3 - i) * 8)) & 0xFF;

		if (src_ip_mode == GSW_IGMP_MEMBER_INCLUDE) {
			xdata = pattern->srcip.nIPv4;
			for (i = 0; i < 4; i++, len++)
				b[len] = (xdata >> ((3 - i) * 8)) & 0xFF;
		} else {
			len += 4;
		}
	} else {
		for (xdata = 0, i = 0; i < 8; i += 2) {
			xdata ^= (pattern->dstip.nIPv6[i] << 16)
				 | (pattern->dstip.nIPv6[i + 1]);
		}
		...
		if (src_ip_mode == GSW_IGMP_MEMBER_INCLUDE) {
			for (xdata = 0, i = 0; i < 8; i += 2) {
				xdata ^= (pattern->srcip.nIPv6[i] << 16)
					 | (pattern->srcip.nIPv6[i + 1]);
			}
	...
	crc &= gswdev->mctblsize - 1;	/* To make sure it is within 512 */
...
int gsw_init_hash_table(void *cdev)
{
	...
	if (MCAST_HASHTBL_SIZE * gswdev->mctblsize > sizeof(phtable)) {
		pr_err("%s:%s:%d: gswdev->mctblsize (%u) > phtable (%lu)\n",
		       __FILE__, __func__, __LINE__, (u32)gswdev->mctblsize,
		       sizeof(phtable));
		return GSW_statusErr;
	}
...
int gsw_get_swmcast_entry(void *cdev, GSW_multicastTableRead_t *parm, u32 loc)
{
	...
	if (pcetable.valid == 1) {
		...
		if (parm->eIPVersion == GSW_IP_SELECT_IPV4) {
			parm->uIP_Gda.nIPv4 = ((u32)pcetable.key[4] << 16) |
					       pcetable.key[3];
			parm->uIP_Gsa.nIPv4 = ((u32)pcetable.key[12] << 16) |
					       pcetable.key[11];
		} else {
			for (i = 0; i < 8; i++)
				parm->uIP_Gda.nIPv6[i] = pcetable.key[10 - i];

			for (i = 0; i < 8; i++)
				parm->uIP_Gsa.nIPv6[i] = pcetable.key[18 - i];
		}
		...
		if (gswdev->gipver == LTQ_GSWIP_3_2B)
			parm->nTci = pcetable.key[19];

		parm->nPortId |= GSW_PORTMAP_FLAG_SET(GSW_multicastTableRead_t);
#ifdef __KERNEL__
		pr_debug("Got Loc        %d\n", loc);
		pr_debug("Valid	         %d\n", pcetable.valid);
...
static int set_pce_hash_table(void *cdev, MCAST_HASHTBL *phtable, u32 loc)
{
	...
	if (phtable->first_idx == 0xFFFF &&
	    VRSN_CMP(gswdev->gipver, LTQ_GSWIP_3_2) <= 0)
		pcetable.key[0] = 0;
	else
	...
	if (gswdev->gipver == LTQ_GSWIP_3_2B)
		pcetable.key[19] = phtable->key.tci;

	if (phtable->ip_type == GSW_IP_SELECT_IPV4) {
		pcetable.key[3] = phtable->key.dstip.nIPv4 & 0xFFFF;
		pcetable.key[4] = (phtable->key.dstip.nIPv4 >> 16) & 0xFFFF;
		pcetable.key[11] = phtable->key.srcip.nIPv4 & 0xFFFF;
		pcetable.key[12] = (phtable->key.srcip.nIPv4 >> 16) & 0xFFFF;
...
int gsw_insert_hashtable_entry(void *cdev, GSW_multicastTable_t *parm)
{
	...
	pattern.tci = parm->nTci;

	hashidx = cal_hash(gswdev, GSW_IGMP_MEMBER_EXCLUDE, &pattern,
			   parm->eIPVersion);
	...
	if (new_hash_slot == TBL_FULL) {
		...
		return TBL_FULL;
	} else if (new_hash_slot >= gswdev->mctblsize) {
	...
	new_table_ptr->key.tci = pattern.tci;
