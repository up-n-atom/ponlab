...
#define sw_w32(x, y)	writel((x), (addr_gswl + (y)))
#define sw_r32(x)	readl(addr_gswl + (x))

#define gsw1_w32(x, y)	writel((x), ((y)))
#define gsw1_r32(x)	readl((x))
	...
	for (i = 0; i < num_of_types; i++) {
		...
		case GSW_PBB_MAGIC:
			if (IS_VRSN_32(gswdev->gipver)) {
				ops_content.pNext = NULL;
				//printk("address of gsw_pbb_ops =%x....\n",(unsigned int)&ops->gsw_pbb_ops);
				ops_content.pFkts = (LTQ_ll_fkt *)&ops->gsw_pbb_ops;
				ops_content.nType = GSW_PBB_MAGIC;
				//printk("size of pdata_t.ops.gsw_pbb_ops = %x\n",(sizeof(ops->gsw_pbb_ops)/sizeof(ops_content.pFkts[0])));
				ops_content.nNumFkts = (sizeof(ops->gsw_pbb_ops) / sizeof(ops_content.pFkts[0]));
				insert_ioctl_type(ops_content, node);
			}
		...
		case GSW_GPID_MAGIC:
			if (IS_VRSN_32(gswdev->gipver)) {
				ops_content.pNext = NULL;
				//printk("address of gsw_gpid_ops =%x....\n",(unsigned int)&ops->gsw_gpid_ops);
				ops_content.pFkts = (LTQ_ll_fkt *)&ops->gsw_gpid_ops;
				ops_content.nType = GSW_GPID_MAGIC;
				//printk("size of pdata_t.ops.gsw_gpid_ops = %x\n",(sizeof(ops->gsw_gpid_ops)/sizeof(ops_content.pFkts[0])));
				ops_content.nNumFkts = (sizeof(ops->gsw_gpid_ops) / sizeof(ops_content.pFkts[0]));
				insert_ioctl_type(ops_content, node);
			}

			break;

		case GSW_VLANCOUNTER_MAGIC:
			if (VRSN_CMP(gswdev->gipver, LTQ_GSWIP_3_2B) >= 0) {
				ops_content.pNext = NULL;
				ops_content.pFkts = (LTQ_ll_fkt *)&ops->gsw_vlan_counter_ops;
				ops_content.nType = GSW_VLANCOUNTER_MAGIC;
				ops_content.nNumFkts = (sizeof(ops->gsw_vlan_counter_ops) / sizeof(ops_content.pFkts[0]));
				insert_ioctl_type(ops_content, node);
			}
...
int ltq_gsw_api_register(struct platform_device *pdev)
{
	...
	ethsw_core_init_t core_init;
	struct gswss *gswss = dev_get_drvdata(pdev->dev.parent);
	u32 device_id;
	...
	device_id = gswdev->devid;
	if (device_id >= LTQ_FLOW_DEV_MAX)
		return -EINVAL;

	/*Initialize global array*/
	gswss->core_dev[device_id] = pdev;
	/** Clear core_init */
	memset(&core_init, 0, sizeof(ethsw_core_init_t));
	...
	result = gsw_cdev_interface(GSW_API_MAJOR_NUMBER, device_id, gswss);
	if (result != 0) {
	...
	if (device_id == LTQ_FLOW_DEV_INT_R && (!gswdev->ext_switch)) {
		/* Init FLOW Switch Core Layer */
	...
	if (gswdev->ext_switch) {
		...
		if (mdio_node)
			gswdev->master_mii_bus = of_mdio_find_bus(mdio_node);
		if (!gswdev->master_mii_bus) {
			dev_err(&pdev->dev, "\ncannot get Master MDIO bus\n");
			return -EPROBE_DEFER;
		}
		get_device(&gswdev->master_mii_bus->dev);
		gswdev->master_mii_node = mdio_node;
		/* if external switch , try to get misc info */
		get_gsw_ext_handle(gswdev, node);
	}
	...
	if (device_id == LTQ_FLOW_DEV_EXT_AX3000_F24S) {
		...
		gswdev->gsw_dev = LTQ_FLOW_DEV_EXT_AX3000_F24S;
		gswdev->parent_devid = LTQ_FLOW_DEV_INT_R;
...
{
	int pidx;
	gswl_addr = (void __iomem *)(0xbc000000);
	gswr_addr = (void __iomem *)(0xba000000);
