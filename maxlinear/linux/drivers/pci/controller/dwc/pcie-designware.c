					...
					u64 phy_addr, u64 target_addr, size_t size)
{
	...
	dw_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_LOWER_BASE, lower_32_bits(phy_addr));
	dw_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_UPPER_BASE, upper_32_bits(phy_addr));

	dw_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_LOWER_LIMIT,
				 lower_32_bits(phy_addr + size - 1));
	dw_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_UPPER_LIMIT,
				 upper_32_bits(phy_addr + size - 1));

	dw_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_LOWER_TARGET, lower_32_bits(target_addr));
	dw_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_UPPER_TARGET, upper_32_bits(target_addr));

	/* Only MEM type is supported */
	...
	/*
	 * Make sure ATU enable takes effect before any subsequent mem accesses.
	 * NB: some platforms even read successfully, issues still observed
	 * in this case, some delay is needed.
	 */
	for (retries = 0; retries < LINK_WAIT_MAX_IATU_RETRIES; retries++) {
		val = dw_pcie_readl_ib_unroll(pci, index,
					      PCIE_ATU_UNR_REGION_CTRL2);
...
EXPORT_SYMBOL_GPL(dwc_pcie_rc_prog_inbound_atu_unroll);
...
EXPORT_SYMBOL_GPL(dw_pcie_disable_atu);
...
EXPORT_SYMBOL_GPL(dw_pcie_link_width_resize);
...
EXPORT_SYMBOL_GPL(dw_pcie_link_speed_change);
