...
/*! \enum FrameWorkErrCode
    \brief Enum containing the type of error code which can be returned from the framework 
     modules like servd, csd, and cal. Buffer error code -199 to -240
*/
typedef enum {
	ERR_NO_FRAMEWORK = -199, /*!< Framework not present */
...
typedef enum {
	...
	ERR_REJECT_BRIDGED_WAN_ROUTED_WAN_EXISTS_ON_SAME_LINK = -268, /*!< Routed WAN connection already present on link; Please delete all routed WAN on link then create Bridged WAN */
	ERR_REJECT_ROUTED_WAN_BRIDGED_WAN_EXISTS_ON_SAME_LINK = -269 /*!< Bridged WAN connection already present on link; Please delete all bridged WAN on link then create Routed WAN */
} SLErrCode;
