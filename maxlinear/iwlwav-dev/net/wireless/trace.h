...
DEFINE_EVENT(wiphy_netdev_evt, rdev_end_cac,
	     TP_PROTO(struct wiphy *wiphy, struct net_device *netdev, bool cac_canceled),
	     TP_ARGS(wiphy, netdev, cac_canceled)
);
#endif
...
TRACE_EVENT(rdev_end_cac,
        ...
        TP_printk(WIPHY_PR_FMT ", " NETDEV_PR_FMT ", canceled=%u",
                  WIPHY_PR_ARG, NETDEV_PR_ARG, __entry->cac_canceled)
);
...
TRACE_EVENT(cfg80211_rx_mgmt,
	...
	TP_fast_assign(
		...
		__entry->snr_db =  info->snr_db;
	),
	TP_printk(WDEV_PR_FMT ", freq: "KHZ_F", sig dbm: %d, snr_db: %d",
		  WDEV_PR_ARG, PR_KHZ(__entry->freq), __entry->sig_dbm,  __entry->snr_db)
);
