...
static unsigned long wiphy_counter = 0;
				   ...
				   const char *newname)
{
	...
	int wiphy_idx = WIPHY_IDX_INVALID, taken = -1, digits;
			   ...
			   const char *requested_name)
{
	...
	/* Pre-processing: Give it the correct wiphy_idx from the phy name.
	   If the phy number is specified in the name, then use this number, otherwise take the first free one */
	if (requested_name && requested_name[0]) {
		unsigned long wiphy_idx = 0;
		int res;
		if (strncmp(requested_name, PHY_NAME, sizeof(PHY_NAME)-1)) {
			/* Not a "phy" device */
			requested_name = NULL;
			goto use_default_index;
		}

		res = kstrtoul(requested_name + sizeof(PHY_NAME)-1, 10, &wiphy_idx);
		if (res || wiphy_idx >= sizeof(wiphy_counter) * BITS_PER_BYTE) {
			/* phy number is not specified or is too large */
			requested_name = NULL;
			goto use_default_index;
		}

		if (test_bit(wiphy_idx, &wiphy_counter)) {
			/* phy number already is in use */
			kfree(rdev);
			rtnl_unlock();
			return NULL;
		}
		rdev->wiphy_idx = wiphy_idx;
	} else {
use_default_index:
		if (unlikely(wiphy_counter == ~0UL)) {
			/* no more free */
			kfree(rdev);
			rtnl_unlock();
			return NULL;
		}
		rdev->wiphy_idx = ffz(wiphy_counter);
	}

	set_bit(rdev->wiphy_idx, &wiphy_counter);
...
int wiphy_register(struct wiphy *wiphy)
{
	...
	if (WARN_ON(wiphy->max_acl_mac_addrs && !rdev->ops->set_mac_acl))
		return -EINVAL;
	...
	for (band = 0; band < NUM_NL80211_BANDS; band++) {
		...
		for (i = 0; i < sband->n_iftype_data; i++) {
			 ...
			 * reject if so.
			 */
			if (iftd->eht_cap.has_eht && has_ap && has_non_ap) {
                                pr_warn("AP and non-AP capabilities for EHT\n");
			}
		}
...
#if 0
	/* Part of the dc_dp workaround IWLWAV TODO*/
...
{
	rtnl_lock();
	clear_bit(get_wiphy_idx(wiphy), &wiphy_counter);
	rtnl_unlock();
