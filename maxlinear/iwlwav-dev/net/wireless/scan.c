				      ...
				      gfp_t gfp)
{
	...
	if (ieee80211_is_action(mgmt->frame_control) &&
			((mgmt->u.action.category == WLAN_CATEGORY_PUBLIC) && (mgmt->u.action.u.public_action.action_code == WLAN_PUB_ACTION_FILS_DISCOVERY))) {

		fils_ielen = (len - (offsetof(struct ieee80211_mgmt, u.action.u.public_action.variable)));
		fils_ielen += (FILS_VENDOR_IE_OFFSET + MXL_OUI_LENGTH);
		ies = kzalloc(sizeof(*ies) + fils_ielen, gfp);
		if (!ies)
			return NULL;
		ies->len = fils_ielen;
		memcpy(&tsf, (mgmt->u.action.u.public_action.variable + FILS_TIMESTAMP_OFFSET), sizeof(tsf));
		ies->tsf = le64_to_cpu(tsf);
		ies->from_beacon = ieee80211_is_beacon(mgmt->frame_control);
		memcpy(tmp.pub.bssid, mgmt->bssid, ETH_ALEN);
		tmp.pub.channel = data->chan;
		tmp.pub.scan_width = data->scan_width;
		tmp.pub.signal = data->signal;
		memcpy(&beacon_interval, (mgmt->u.action.u.public_action.variable + FILS_BEACON_INTERVAL_OFFSET), sizeof(beacon_interval));
		tmp.pub.beacon_interval = le16_to_cpu(beacon_interval);
		tmp.ts_boottime = data->boottime_ns;
		tmp.parent_tsf = data->parent_tsf;
		tmp.pub.chains = data->chains;
		memcpy(tmp.pub.chain_signal, data->chain_signal, IEEE80211_MAX_CHAINS);
		tmp.pub.noise = data->noise;
		ether_addr_copy(tmp.parent_bssid, data->parent_bssid);
		signal_valid = true;
		ies->data[0] = WLAN_EID_VENDOR_SPECIFIC;
		ies->data[1] = (fils_ielen - FILS_VENDOR_IE_OFFSET);
		ies->data[2] = 0xAC;
		ies->data[3] = 0x9A;
		ies->data[4] = 0x96;
		memcpy(&ies->data[5], mgmt->u.action.u.public_action.variable, fils_ielen);
		rcu_assign_pointer(tmp.pub.ies, ies);
