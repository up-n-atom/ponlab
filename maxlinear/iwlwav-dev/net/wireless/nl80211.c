...
static const struct nla_policy nl80211_policy[NUM_NL80211_ATTR] = {
	...
	[NL80211_ATTR_HEOPER_COLOR_OFFS_BEACON] = { .type = NLA_BINARY },
	...
	[NL80211_ATTR_DFS_STATE] = { .type = NLA_U32 },
	[NL80211_ATTR_DFS_TIME] = { .type = NLA_U32 },
	[NL80211_ATTR_DFS_STATE_FORCE] = { .type = NLA_FLAG },
	...
	[NL80211_ATTR_ML_STA_INFO] = NLA_POLICY_EXACT_LEN(sizeof(struct ieee80211_ml_sta_info)),
	...
	[NL80211_ATTR_COLOR_CHANGE_SWITCH_TIME] = { .type = NLA_U32 },
	[NL80211_ATTR_COLOR_CHANGE_ELEMS] = NLA_POLICY_NESTED(nl80211_policy),
	[NL80211_ATTR_RADAR_BACKGROUND] = { .type = NLA_FLAG },
	[NL80211_ATTR_SB_DFS_BW] = { .type = NLA_U8 },
	[NL80211_ATTR_EHT_CAPABILITY] = { .type = NLA_BINARY,
					.len = NL80211_EHT_MAX_CAPABILITY_LEN },
	[NL80211_ATTR_EHT_OPERATION] = { .type = NLA_BINARY,
					.len = NL80211_EHT_MAX_OPERATION_LEN },
...
static int nl80211_msg_put_cac_time(struct sk_buff *msg, struct wiphy *wiphy,
                                    struct ieee80211_channel *chan)
{
        bool offchannel_cac = !!(wiphy->flags & WIPHY_FLAG_USE_OFFCHANNEL_CAC);
        enum nl80211_dfs_regions dfs_region = reg_get_dfs_region(wiphy);
        unsigned int dfs_cac_ms;

        if (offchannel_cac && dfs_region == NL80211_DFS_ETSI)
                dfs_cac_ms = chan->off_channel_dfs_cac_ms;
        else
                dfs_cac_ms = chan->dfs_cac_ms;

        return nla_put_u32(msg, NL80211_FREQUENCY_ATTR_DFS_CAC_TIME,
                           dfs_cac_ms);
}
	...
	if (chan->flags & IEEE80211_CHAN_RADAR) {
		...
		if (large) {
			...
			if (nla_put_u32(msg, NL80211_FREQUENCY_ATTR_DFS_TIME,
					time))
				goto nla_put_failure;
			if (nl80211_msg_put_cac_time(msg, wiphy, chan))
                                goto nla_put_failure;
		}
	}
...
static int nl80211_parse_key(struct genl_info *info, struct key_parse *k)
{
	...
	if (k->idx != -1) {
		if (k->defmgmt) {
			...
			if ((k->idx < 0 || k->idx > 3) && (k->idx < 6 || k->idx > 7)) {
				GENL_SET_ERR_MSG(info, "def key idx not 0-3 or 6-7");
				return -EINVAL;
			}
		} else {
			if (k->idx < 0 || k->idx > 7) {
			 ...
			 const struct ieee80211_sband_iftype_data *iftdata)
{
	...
	const struct ieee80211_he_6ghz_capa *he_6ghz_cap = &iftdata->he_6ghz_capa;
	...
	if (he_cap->has_he) {
		if (nla_put(msg, NL80211_BAND_IFTYPE_ATTR_HE_CAP_MAC,
			...
			nla_put(msg,NL80211_BAND_IFTYPE_ATTR_HE_6GHZ_CAPA,
				sizeof(he_6ghz_cap->capa),&he_6ghz_cap->capa))
		return -ENOBUFS;
	}
				      ...
				      bool large)
{
	...
	/* EML (Multilink) info */
	if (sband->eml_cap.eml_support &&
		(nla_put_u8(msg, NL80211_BAND_ATTR_MULTILINK_CAPA, sband->eml_cap.eml_support)))
		return -ENOBUFS;
			      ...
			      int flags, struct nl80211_dump_wiphy_state *state)
{
	...
	case 0:
		...
		if ((rdev->wiphy.flags & WIPHY_FLAG_TDLS_EXTERNAL_SETUP) &&
			...
			goto nla_put_failure;
		if ((rdev->wiphy.flags & WIPHY_FLAG_USE_OFFCHANNEL_CAC) &&
                    nla_put_flag(msg, NL80211_ATTR_WIPHY_DFS_ANTENNA))
                        goto nla_put_failure;
		state->split_start++;
		if (state->split)
			break;
			  ...
			  struct cfg80211_chan_def *chandef)
{
	...
	if (info->attrs[NL80211_ATTR_SB_DFS_BW])
                chandef->sb_dfs.sub_width = nla_get_u8(info->attrs[NL80211_ATTR_SB_DFS_BW]);
...
static int nl80211_new_interface(struct sk_buff *skb, struct genl_info *info)
{
	...
	wiphy_lock(&rdev->wiphy);
	if (!cfg80211_rdev_by_wiphy_idx(get_wiphy_idx(&rdev->wiphy))) {
                wiphy_unlock(&rdev->wiphy);
                return -ENODEV;
	}
...
static int nl80211_set_beacon(struct sk_buff *skb, struct genl_info *info)
{
	...
	/*SSID Update is required here, incase of Hidden SSID tests with SSID name change via reconf*/
	if(err == 0){
		if (info->attrs[NL80211_ATTR_SSID]) {
			wdev->u.ap.ssid_len = nla_len(info->attrs[NL80211_ATTR_SSID]);
			memcpy(wdev->u.ap.ssid, nla_data(info->attrs[NL80211_ATTR_SSID]), wdev->u.ap.ssid_len);
		}
	}
	wdev_unlock(wdev);
...
#define MXL_STA_RSSI_DEFAULT (-60)
...
static int nl80211_set_station(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_STA_RSSI])
		...
		params.rssi = MXL_STA_RSSI_DEFAULT;
					 ...
					 struct genl_info *info)
{
	...
	if (wiphy->cac_time_debug >= 0 && wiphy->cac_time_debug <= IEEE80211_DFS_MAX_CAC_TIME_MS)
                cac_time_ms = wiphy->cac_time_debug;
...
static int nl80211_set_dfs_state(struct sk_buff *skb,
                                          struct genl_info *info)
{
	struct cfg80211_registered_device *rdev = info->user_ptr[0];
	struct net_device *dev = info->user_ptr[1];
	...
	if (dfs_region == NL80211_DFS_UNSET) {
		GENL_SET_ERR_MSG(info,
			"DFS Region is not set. Can't change dfs state");
		return -EINVAL;
	}

	if (!attrs[NL80211_ATTR_WIPHY_FREQ] || !attrs[NL80211_ATTR_DFS_STATE] || !attrs[NL80211_ATTR_DFS_TIME])
		return -EINVAL;

	dfs_state = nla_get_u32(attrs[NL80211_ATTR_DFS_STATE]);
	control_freq = nla_get_u32(attrs[NL80211_ATTR_WIPHY_FREQ]);
	dfs_time = nla_get_u32(attrs[NL80211_ATTR_DFS_TIME]);
	force = nla_get_flag(attrs[NL80211_ATTR_DFS_STATE_FORCE]);

	if (!force && (dfs_state == NL80211_DFS_AVAILABLE) && (dfs_region != NL80211_DFS_ETSI)) {
		GENL_SET_ERR_MSG(info,
				"DFS Region is not ETSI. Can't change dfs state to available");
		return -EPERM;
	}
	...
	/* If the chandef is in that state no need to update */
	if (chandef.chan->dfs_state == dfs_state)
		return 0;

	/* Not allowed */
	if (!force &&
	   (chandef.chan->dfs_state == NL80211_DFS_UNAVAILABLE && dfs_state == NL80211_DFS_AVAILABLE)) {
		GENL_SET_ERR_MSG(info,
				"Radar detected. Can't change dfs state to available");
		return -EPERM;
	}

	chandef.chan->dfs_state = dfs_state;
	chandef.chan->dfs_state_entered = jiffies - msecs_to_jiffies(dfs_time * 60 * 1000);
...
static void nl80211_handle_sub_band_dfs(struct wireless_dev *wdev, struct cfg80211_csa_settings *params)
{
	ASSERT_WDEV_LOCK(wdev);

	if ((params->chandef.sb_dfs.sub_width == NL80211_SB_DFS_BW_20) ||
	    (params->chandef.sb_dfs.sub_width == NL80211_SB_DFS_BW_40) ||
	    (params->chandef.sb_dfs.sub_width == NL80211_SB_DFS_BW_80)) {

		if (!wdev->u.ibss.chandef.sb_dfs.sub_width) {
			params->chandef.sb_dfs.width = wdev->u.ibss.chandef.width;
			params->chandef.sb_dfs.center_freq = wdev->u.ibss.chandef.center_freq1;
		}
		else {
			params->chandef.sb_dfs.width = wdev->u.ibss.chandef.sb_dfs.width;
			params->chandef.sb_dfs.center_freq = wdev->u.ibss.chandef.sb_dfs.center_freq;
		}
	}
}
...
skip_beacons:
	...
	wdev_lock(wdev);
	nl80211_handle_sub_band_dfs(wdev, &params);
			    ...
			    struct cfg80211_internal_bss *intbss)
{
	...
	if (nla_put_u8(msg, NL80211_BSS_NOISE, res->noise))
                goto nla_put_failure;
...
static int nl80211_color_change(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[NL80211_ATTR_COLOR_CHANGE_COUNT] ||
	    ...
	    !info->attrs[NL80211_ATTR_COLOR_CHANGE_SWITCH_TIME] ||
	    !info->attrs[NL80211_ATTR_COLOR_CHANGE_ELEMS])
		return -EINVAL;

	params.count = nla_get_u8(info->attrs[NL80211_ATTR_COLOR_CHANGE_COUNT]);
	params.color = nla_get_u8(info->attrs[NL80211_ATTR_COLOR_CHANGE_COLOR]);
	params.switch_time = nla_get_u32(info->attrs[NL80211_ATTR_COLOR_CHANGE_SWITCH_TIME]);
	...
	if (nla_len(tb[NL80211_ATTR_HEOPER_COLOR_OFFS_BEACON]) != sizeof(u16)) {
		err = -EINVAL;
		goto out;
	}

	offset = nla_get_u16(tb[NL80211_ATTR_HEOPER_COLOR_OFFS_BEACON]);
	if (offset >= params.beacon_color_change.tail_len + params.beacon_color_change.head_len) {
		err = -EINVAL;
		goto out;
	}

	params.he_oper_color_offset_beacon = offset;
...
#define INTERNAL_FLAG_SELECTORS(__sel)			\
	...
	SELECTOR(__sel, WIPHY_CLEAR,			\
		 NL80211_FLAG_NEED_WIPHY |		\
		 NL80211_FLAG_CLEAR_SKB)                \
	SELECTOR(__sel, NETDEV_SET_KEY,                 \
		 NL80211_FLAG_NEED_NETDEV_UP |          \
		 NL80211_FLAG_NEED_RTNL | 		\
		 NL80211_FLAG_CLEAR_SKB)       		\
	SELECTOR(__sel, NETDEV_DEL_KEY, 		\
		 NL80211_FLAG_NEED_NETDEV_UP | 		\
		 NL80211_FLAG_NEED_RTNL)      		\
	SELECTOR(__sel, NETDEV_VENDOR_DUMP,   		\
		 NL80211_FLAG_NEED_WIPHY |    		\
		 NL80211_FLAG_NEED_RTNL |   		\
		 NL80211_FLAG_CLEAR_SKB)
...
static const struct genl_small_ops nl80211_small_ops[] = {
	...
	{
                .cmd = NL80211_CMD_SET_DFS_STATE,
                .doit = nl80211_set_dfs_state,
                .flags = GENL_UNS_ADMIN_PERM,
		      ...
		      struct cfg80211_rx_info *info, gfp_t gfp)
{
	...
	if (nla_put_u32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx) ||
	    ...
	    (info->snr_db &&
	     nla_put_u32(msg, NL80211_ATTR_RX_SNR_DB, info->snr_db)) ||
				     ...
				     gfp_t gfp)
{
	...
	while (proto == ETH_P_8021Q) {
                struct vlan_hdr *vh;

                if ((len + VLAN_HLEN) > skb->len) {
                        nlmsg_free(msg);
                        return -EINVAL;
                }

                vh = (struct vlan_hdr *)(skb->data + len);
                proto = be16_to_cpu(vh->h_vlan_encapsulated_proto);
                vlan_id = be16_to_cpu(vh->h_vlan_TCI) & VLAN_VID_MASK;
                len += VLAN_HLEN;
        }
	...
	if (nla_put_u32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx) ||
	    ...
	    (vlan_id && nla_put_u16(msg, NL80211_ATTR_VLAN_ID, vlan_id)) ||
	    (link_id >= 0 &&
	     nla_put_u8(msg, NL80211_ATTR_MLO_LINK_ID, link_id)) ||
	...
	frame = nla_reserve(msg, NL80211_ATTR_FRAME, (skb->len - len));
	if (!frame)
		goto nla_put_failure;
			       ...
			       unsigned int link_id, u16 punct_bitmap)
{
	...
	nl80211_ch_switch_notify(rdev, dev, link_id, chandef, GFP_ATOMIC,
...
int cfg80211_bss_color_notify(struct net_device *dev, gfp_t gfp,
			      ...
			      u64 color_bitmap, u8 *bssid, u8 cca_color)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct wiphy *wiphy = wdev->wiphy;
	struct cfg80211_registered_device *rdev = wiphy_to_rdev(wiphy);
...
int cfg80211_ml_channel_switch_done_notify(struct net_device *dev, gfp_t gfp,
                                           enum nl80211_commands cmd, u8 flag)
{
        struct wireless_dev *wdev = dev->ieee80211_ptr;
        struct wiphy *wiphy = wdev->wiphy;
        struct sk_buff *msg;
        ...
        if (cmd == NL80211_CMD_ML_CHAN_SWITCH_DONE_NOTIFY &&
            (nla_put_u8(msg, NL80211_ATTR_ML_CHAN_SWITCH_DONE_TYPE, flag)))
                goto nla_put_failure;

        genlmsg_end(msg, hdr);

        return genlmsg_multicast_netns(&nl80211_fam, wiphy_net(wiphy),
...
EXPORT_SYMBOL(cfg80211_ml_channel_switch_done_notify);
...
/*
 * Filter only needed beacons to be sent to upper layer.
 * @returns true for allowed beacon, else false.
 */
static bool
filter_report_beacon(int freq, const u8 *frame, size_t len)
{
        ...
        /* 6GHz Beacon. No applications so far with 6GHz beacons */
        if (freq >= 5935 && freq <= 7115) {
                return false;
        }

        cap = cfg80211_find_ie(WLAN_EID_HT_CAPABILITY, mgmt->u.beacon.variable, len - baselen);
        /* Allow No HT cap beacon. Used for HT protection enable */
        if (!cap)
                return true;
        if ((cap[1] >= sizeof(*ht_cap))) {
                ht_cap = (struct ieee80211_ht_cap *)(cap + 2);
                /* Allow HT40 Intolerant beacon for BW switch */
                if (le16_to_cpu(ht_cap->cap_info) & IEEE80211_HT_CAP_40MHZ_INTOLERANT)
                        return true;
        }

        cap = cfg80211_find_ie(WLAN_EID_ERP_INFO, mgmt->u.beacon.variable, len - baselen);
        /* Allow NON ERP set beacon */
        if (cap && (cap[1] >= 1) && (cap[2] & WLAN_ERP_NON_ERP_PRESENT))
                return true;
				     ...
				     size_t len, int freq, int sig_dbm)
{
	...
	if (filter_report_beacon(KHZ_TO_MHZ(freq), frame, len) == false)
                return;
