...
void cfg80211_send_layer2_update(struct net_device *dev, const u8 *addr)
{
	...
	/* msg size padding - workaround for Intel HW (CBM) issue on GRX platforms.
	 * packets with len < 64 bytes (including FCS) break packet learning,
	 * which results in connectivity loss for all wired stations connected to the bridge.
	 */
	skb = dev_alloc_skb(sizeof(*msg) > ETH_ZLEN ? sizeof(*msg) : ETH_ZLEN);
	if (!skb)
		return;
	msg = skb_put(skb, sizeof(*msg) > ETH_ZLEN ? sizeof(*msg) : ETH_ZLEN);
