...
static void ieee80211_do_stop(struct ieee80211_sub_if_data *sdata, bool going_down)
{
	...
	cancel_work_sync(&sdata->color_coll_notify.work);
	...
	switch (sdata->vif.type) {
/* Multi-AP extension: it is allowed to create NL80211_IFTYPE_AP_VLAN interface by WAVE driver
	case NL80211_IFTYPE_AP_VLAN:
		break;
*/
	case NL80211_IFTYPE_MONITOR:
...
static const struct net_device_ops ieee80211_data_offload_dataif_ops = {
	.ndo_open		= ieee80211_open,
	.ndo_stop		= ieee80211_stop,
	.ndo_uninit		= ieee80211_uninit,
	.ndo_start_xmit		= ieee80211_subif_start_data_xmit,
	.ndo_set_rx_mode	= ieee80211_set_multicast_list,
	.ndo_set_mac_address	= ieee80211_change_mac,
	.ndo_get_stats64	= ieee80211_get_stats64,
	.ndo_setup_tc		= ieee80211_netdev_setup_tc,
					      ...
					      struct net_device_path *path)
{
	...
	case NL80211_IFTYPE_AP_VLAN:
		...
		if (is_multicast_ether_addr(ctx->daddr))
			goto out;
		sta = sta_info_get_bss(sdata, ctx->daddr, INCLUDE_SEARCH_IN_MLDB);
		break;
					...
					struct sk_buff *skb)
{
	...
	if (ieee80211_is_action(mgmt->frame_control) &&
		...
		sta = sta_info_get_bss(sdata, mgmt->sa, INCLUDE_SEARCH_IN_MLDB);
		if (sta) {
				  ...
				  enum nl80211_iftype type)
{
	...
	if (sdata->dev) {
		if (!ieee80211_hw_check(&sdata->local->hw, DATA_OFFLOAD))
			...
			sdata->dev->netdev_ops = &ieee80211_data_offload_dataif_ops;
	...
	INIT_WORK(&sdata->color_coll_notify.work, ieee80211_color_coll_notify_work);
                     ...
                     struct vif_params *params)
{
        ...
        int size = ALIGN(sizeof(struct ieee80211_sub_if_data) + local->hw.vif_data_size,
                                sizeof(void *));
        int txq_size = 0;
...
EXPORT_SYMBOL(ieee80211_if_alloc);
		     ...
		     struct vif_params *params)
{
	...
	if (type == NL80211_IFTYPE_P2P_DEVICE || type == NL80211_IFTYPE_NAN) {
		...
		if (local->ops->driver_alloc_netdev) {
			ndev = local->ops->driver_alloc_netdev(&local->hw);
			if (ndev) strcpy(ndev->name, name);
		}
		if (!ndev || !local->ops->driver_alloc_netdev)
			ndev = ieee80211_if_alloc(&local->hw, name, name_assign_type, type, params);
		...
		if (local->ops->wake_tx_queue &&
			type != NL80211_IFTYPE_AP_VLAN &&
			type != NL80211_IFTYPE_MONITOR) {
			txqi = netdev_priv(ndev) + ALIGN(sizeof(struct ieee80211_sub_if_data) + local->hw.vif_data_size,
				sizeof(void *));
...
void ieee80211_remove_interfaces(struct ieee80211_local *local)
{
	...
	list_for_each_entry_safe(sdata, tmp, &local->interfaces, list) {
		...
		} else if ((!local->ops->driver_free_netdev_allowed) ||
			   (!(local->ops->driver_free_netdev_allowed(&local->hw, sdata->dev)))) {
