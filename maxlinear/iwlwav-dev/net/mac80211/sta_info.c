			      ...
			      const u8 *addr)
{
	...
	/* MultiLink STA */
	if (!sta) {
		struct ieee80211_sta *mac80211_sta = drv_pre_get_sta_by_mld_addr(sdata, addr);
		if (mac80211_sta) {
			sta = container_of(mac80211_sta, struct sta_info, sta);
			if (sta->sdata == sdata) {
				rcu_read_unlock();
				return sta;
			}
...
struct sta_info *sta_info_get_bss(struct ieee80211_sub_if_data *sdata,
				  const u8 *addr,
				  enum sta_info_search_type search_type)
{
	struct ieee80211_local *local = sdata->local;
	struct rhlist_head *tmp;
	struct sta_info *sta=NULL;
...
static int sta_info_insert_finish(struct sta_info *sta) __acquires(RCU)
{
	...
	if (sta_info_get_bss(sdata, sta->sta.addr, INCLUDE_SEARCH_IN_MLDB)) {
		err = -EEXIST;
		goto out_err;
	}
			      ...
			      const u8 *addr)
{
	...
	mutex_lock(&sdata->local->sta_mtx);
	sta = sta_info_get_bss(sdata, addr ,IGNORE_SEARCH_IN_MLDB);
	ret = __sta_info_destroy(sta);
	mutex_unlock(&sdata->local->sta_mtx);
					 ...
					 const u8 *addr)
{
	...
	sta = sta_info_get_bss(vif_to_sdata(vif), addr, INCLUDE_SEARCH_IN_MLDB);
	if (!sta)
		return NULL;
		   ...
		   bool tidstats)
{
	...
	if (sdata->vif.type == NL80211_IFTYPE_STATION) {
        ...
        if (!(sinfo->filled & BIT(NL80211_STA_INFO_INACTIVE_TIME))) {
                sinfo->inactive_time =
	...
	if (!(sinfo->filled & (BIT_ULL(NL80211_STA_INFO_RX_BYTES64) |
		...
		sta->deflink.rx_stats.bytes = sinfo->rx_bytes;
	}
	...
	if (!(sinfo->filled & BIT_ULL(NL80211_STA_INFO_RX_PACKETS))) {
		...
		sta->deflink.rx_stats.bytes = sinfo->rx_packets;
	}
	...
	if (!(sinfo->filled & BIT(NL80211_STA_INFO_RX_DROP_MISC))) {
		...
		sta->deflink.rx_stats.dropped = sinfo->rx_dropped_misc;
	}
