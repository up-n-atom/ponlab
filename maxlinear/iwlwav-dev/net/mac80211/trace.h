...
TRACE_EVENT(drv_handle_radar_debug,
        TP_PROTO(struct ieee80211_local *local,
                 struct cfg80211_chan_def *chandef,
                 u8 radar_bitmap),

        TP_ARGS(local, chandef, radar_bitmap),

        TP_STRUCT__entry(
                LOCAL_ENTRY
                CHANDEF_ENTRY
                __field(u8, radar_bitmap)
        ),

        TP_fast_assign(
                LOCAL_ASSIGN;
                CHANDEF_ASSIGN(chandef);
                __entry->radar_bitmap = radar_bitmap;
        ),

        TP_printk(
                LOCAL_PR_FMT " driver radar (bitmap %u) debug for " CHANDEF_PR_FMT,
                LOCAL_PR_ARG, __entry->radar_bitmap, CHANDEF_PR_ARG
        )
);
...
TRACE_EVENT(drv_get_key_seq,
	...
	TP_printk(
		VIF_PR_FMT KEY_PR_FMT,
		VIF_PR_ARG, KEY_PR_ARG
	)
);
...
TRACE_EVENT(drv_prepare_cac_start,
        TP_PROTO(struct ieee80211_local *local,
                 struct ieee80211_sub_if_data *sdata,
                 u32 cac_time_ms),

        TP_ARGS(local, sdata, cac_time_ms),
        ...
        TP_printk(
                LOCAL_PR_FMT VIF_PR_FMT " cac starting, time:%d",
                LOCAL_PR_ARG, VIF_PR_ARG, __entry->cac_time_ms
        )
);

TRACE_EVENT(drv_notify_cac_finished,
        TP_PROTO(struct ieee80211_local *local,
                 struct ieee80211_sub_if_data *sdata,
                 bool cac_canceled),

        TP_ARGS(local, sdata, cac_canceled),

        TP_STRUCT__entry(
                LOCAL_ENTRY
                VIF_ENTRY
                __field(bool, cac_canceled)
        ),

        TP_fast_assign(
                LOCAL_ASSIGN;
                VIF_ASSIGN;
                __entry->cac_canceled = cac_canceled;
        ),

        TP_printk(
                LOCAL_PR_FMT VIF_PR_FMT " cac finished canceled:%d",
                LOCAL_PR_ARG, VIF_PR_ARG, __entry->cac_canceled
        )
);
...
TRACE_EVENT(api_color_switch_done,
        TP_PROTO(struct ieee80211_sub_if_data *sdata, bool success),
...
TRACE_EVENT(drv_set_default_multicast_key,
        TP_PROTO(struct ieee80211_local *local,
                 struct ieee80211_sub_if_data *sdata,
                 int key_idx),
...
TRACE_EVENT(api_radar_detected_rbm,
        TP_PROTO(struct ieee80211_local *local, u8 radar_bit_map),

        TP_ARGS(local, radar_bit_map),

        TP_STRUCT__entry(
                LOCAL_ENTRY
                __field(u8, radar_bit_map)
        ),

        TP_fast_assign(
                LOCAL_ASSIGN;
                __entry->radar_bit_map = radar_bit_map;
        ),

        TP_printk(
                LOCAL_PR_FMT " radar detected with radar bitmap: %hhu",
                LOCAL_PR_ARG, __entry->radar_bit_map
        )
);
...
TRACE_EVENT(drv_set_mac_acl,
        ...
        TP_STRUCT__entry(
                LOCAL_ENTRY
                VIF_ENTRY
                __field(int, acl_policy)
                __field(int, n_acl_entries)
        ),
        ...
        TP_fast_assign(
                ...
                __entry->n_acl_entries = params->n_acl_entries;
        ),

        TP_printk(
                LOCAL_PR_FMT VIF_PR_FMT " acl_policy:%d, n_acl_entries:%d",
                LOCAL_PR_ARG, VIF_PR_ARG, __entry->acl_policy, __entry->n_acl_entries
        )
);
