				...
				ieee80211_conn_flags_t conn_flags)
{
	...
	pos = ieee80211_ie_build_he_cap(pos, he_cap, pos + he_cap_size);
...
static int ieee80211_send_assoc(struct ieee80211_sub_if_data *sdata)
{
	...
	*pos++ = assoc_data->ssid_len;
	...
	if (assoc_data->vendor_vht ) {/* Vefiy */
		for (link_id = 0; link_id < IEEE80211_MLD_MAX_NUM_LINKS; link_id++) {
			struct cfg80211_bss *cbss = assoc_data->link[link_id].bss;
			struct ieee80211_supported_band *sband;
...
static void ieee80211_color_switch_work(struct work_struct *work)
{
	struct ieee80211_sub_if_data *sdata =
		container_of(work, struct ieee80211_sub_if_data, u.mgd.color_switch_work);
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
	u32 changed = 0;
	...
	sdata->vif.bss_conf.he_bss_color.color = sdata->vif.bss_conf.color_change_color;
...
void ieee80211_color_switch_done(struct ieee80211_vif *vif, bool success)
{
	...
	trace_api_color_switch_done(sdata, success);
	if (!success) {
		sdata_info(sdata, "driver color switch failed\n");
		vif->bss_conf.color_change_active = false;
		if (sdata->vif.type == NL80211_IFTYPE_AP) {
			kfree(sdata->deflink.u.ap.next_beacon);
			sdata->deflink.u.ap.next_beacon = NULL;
			vif->bss_conf.he_bss_color.enabled = true;
			sdata_info(sdata, "notify CCA abort\n");
			cfg80211_color_change_aborted_notify(sdata->dev);
		}
	} else {
		ieee80211_queue_work(&sdata->local->hw, &ifmgd->color_switch_work);
	}
}
EXPORT_SYMBOL(ieee80211_color_switch_done);
...
static int
ieee80211_handle_bss_color_change (struct ieee80211_sub_if_data *sdata,
				   struct ieee802_11_elems *elems)
{
	u8 bss_color = elems->cca_ie->color;
	u8 count = elems->cca_ie->count;
	if (sdata->vif.bss_conf.he_bss_color.color != bss_color) {
		sdata->vif.bss_conf.color_change_color = bss_color;
		sdata->vif.bss_conf.color_change_cntdown = count;
		sdata->vif.bss_conf.color_switch_time = (count * sdata->vif.bss_conf.beacon_int);
		sdata->vif.bss_conf.color_change_active = true;
		cfg80211_color_change_started_notify(sdata->dev, count, bss_color);
		return BSS_CHANGED_HE_BSS_COLOR;
	}
	return 0;
}

static int ieee80211_sta_process_color_switch(struct ieee80211_sub_if_data *sdata,
					      struct ieee802_11_elems *elems)
{
	if (sdata->vif.bss_conf.color_change_active || !elems->cca_ie) {
		return 0;
	}
	return ieee80211_handle_bss_color_change(sdata, elems);
}
				 ...
				 bool beacon)
{
	...
	if (local->scanning) {
		/* Cancel the scan so on next mgmt frame which
		 * includes the CSA IE we can follow it.
		 * We cannot directly follow after cancelling scan
		 * as cancelling scan is not blocked call */
		rtnl_lock();
	...
	link->csa_block_tx = ch_switch.block_tx;
...
void ieee80211_dfs_cac_timer_work(struct work_struct *work)
{
	...
	if (sdata->wdev.cac_started) {
		drv_notify_cac_finished(sdata->local, sdata, false);
...
ieee80211_verify_sta_he_mcs_support(struct ieee80211_sub_if_data *sdata,
				    ...
				    const struct ieee80211_he_operation *he_op)
{
	const struct ieee80211_sta_he_cap *sta_he_cap = NULL;
	u16 ap_min_req_set;
	int i;

	if (!sdata || !he_op || !sband)
		return false;

	if(!(sta_he_cap = ieee80211_get_he_iftype_cap(sband,
				ieee80211_vif_type_p2p(&sdata->vif))))
		return false;
				    ...
				    const u8 *elem_start, unsigned int elem_len)
{
	...
	if (ifmgd->vendor_wds == IEEE80211_VENDOR_WDS_DYNAMIC) {
		if (elems->multi_ap &&
		    (*(elems->multi_ap + 4) == WLAN_MULTI_AP_IE_SUBELEMENT) &&
		    (*(elems->multi_ap + 6) & WLAN_MULTI_AP_BACKHAUL_BSS)) {
			sta->sta.vendor_wds = IEEE80211_VENDOR_WDS_4ADDR;
		}
		else {
			sta->sta.vendor_wds = IEEE80211_VENDOR_WDS_3ADDR;
		}
	} else {
		/* Static 3/4 addr configuration */
					 ...
					 struct sk_buff *skb)
{
	...
	elems = ieee802_11_parse_elems_crc(mgmt->u.probe_resp.variable, len - baselen,
			false, 0, 0, NULL);
	if (!elems)
		return;
				     ...
				     struct ieee80211_rx_status *rx_status)
{
	...
	elems = ieee802_11_parse_elems_crc(mgmt->u.beacon.variable, len - baselen,
                        false, 0, 0, NULL);
	if (!elems)
		return;
	...
	if (bss_conf->he_support && sdata->vif.bss_conf.color_change_active == false) {
		u8 color, enabled;
		/* CCA IE look up and color change */
		changed |= ieee80211_sta_process_color_switch(sdata,elems);
		/*
		 * If no CCA IE found, look for any color params mismatch
		 * in HE oper IE and update to it.
		 */
		if (!(changed & BSS_CHANGED_HE_BSS_COLOR) &&
			elems->he_operation) {
			color = le32_get_bits(elems->he_operation->he_oper_params,
					      IEEE80211_HE_OPERATION_BSS_COLOR_MASK);
			enabled = !le32_get_bits(elems->he_operation->he_oper_params,
					         IEEE80211_HE_OPERATION_BSS_COLOR_DISABLED);
			if ((bss_conf->he_bss_color.color != color) ||
				(bss_conf->he_bss_color.enabled != enabled)) {
				sdata->vif.bss_conf.he_bss_color.color = color;
				sdata->vif.bss_conf.he_bss_color.enabled = enabled;
				changed |= BSS_CHANGED_HE_BSS_COLOR;
			}
		}
	}
				  ...
				  struct sk_buff *skb)
{
	...
	case IEEE80211_STYPE_ACTION:
		...
		if (mgmt->u.action.category == WLAN_CATEGORY_SPECTRUM_MGMT) {
			...
			kfree(elems);
		} else if (mgmt->u.action.category == WLAN_CATEGORY_PROTECTED_HE_ACTION) {
			struct ieee802_11_elems *elems;
			u32 changed = 0;
			ies_len = skb->len -
				  offsetof(struct ieee80211_mgmt,
					u.action.u.color_switch.variable);
			if (ies_len < 0)
				break;

			/* Only handling BSS color change under PROTECTED_HE_ACTION frame */
			if (mgmt->u.action.u.color_switch.action_code != WLAN_PROTECTED_HE_BSS_COLOR_CHNG)
				break;

			elems = ieee802_11_parse_elems(mgmt->u.action.u.color_switch.variable,
					ies_len, true, NULL);
			if (!elems)
				break;
...
void ieee80211_sta_setup_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	INIT_WORK(&ifmgd->color_switch_work, ieee80211_color_switch_work);
