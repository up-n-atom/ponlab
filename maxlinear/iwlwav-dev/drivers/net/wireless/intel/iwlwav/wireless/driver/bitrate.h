...
typedef struct {
  ...
  uint8              nss;
  int8               pw_offs;
  uint8              band; /* enum mtlk_hw_band_e */
...
#ifdef MTLK_WAVE_700
#define PHY_MODE_11BE          8
#define PHY_MODE_BF_11BE       9
#define PHY_MODE_11BE_MU_TRIG  10
#define PHY_MODE_11BE_MU_DL    11
#define PHY_MODE_RESERVED12    12
#define PHY_MODE_RESERVED13    13
#define PHY_MODE_RESERVED14    14
#define PHY_MODE_RESERVED15    15
#define PHY_MODE_INVALID       255
#endif
/* TODO: To review these and align between FW and HOST        *
 * Currently the below PHY modes seem to be used only by HOST */
#define PHY_MODE_11AX_BF_DL    16
#define PHY_MODE_11AC_BF_DL    17
...
#ifdef MTLK_WAVE_700
/* Currently 11BE supports till mcs 13 */
#define NUM_RATE_MCS_BE    14   /* 0 .. 13 */
#endif
...
#ifdef MTLK_WAVE_700
#define PHY_STAT_RATE_CBW_MAX   4 /* CW_320 */
#else
#define PHY_STAT_RATE_CBW_MAX   3 /* CW_160 */
#endif
#define PHY_STAT_RATE_CBW_NUM_AX   4
...
#define MTLK_BITRATE_PARAMS_CBW   MTLK_BFIELD_INFO(20,  4) /*  3 bits [ 000 - 20MHz     001 - 40MHz    010 - 80MHz
011 - 160MHz 100 - 320MHz] */
...
int __MTLK_IFUNC mtlk_bitrate_get_value (int index, int sm, int scp, BOOL is_eht);
he_cp_types_e __MTLK_IFUNC wave_get_tx_he_gi_from_cp(int cp);
he_cp_types_e __MTLK_IFUNC wave_get_rx_he_gi_from_cp(int cp);
BOOL __MTLK_IFUNC wave_short_gi_from_tx_data_rate_info(mtlk_wssa_drv_peer_rate_info1_t *ri);
BOOL __MTLK_IFUNC wave_short_gi_from_rx_data_rate_info(mtlk_wssa_drv_peer_rate_info1_t *ri);
...
static __INLINE uint8
mtlk_bitrate_params_get_psdu_mode_g7 (mtlk_bitrate_info16_t rate_info)
{
  return MTLK_BFIELD_GET(rate_info, MTLK_BITRATE_STAT_PSDU_PHYMODE_G7);
}

static __INLINE uint8
mtlk_bitrate_params_get_psdu_cbw_g7 (mtlk_bitrate_info16_t rate_info)
{
  return MTLK_BFIELD_GET(rate_info, MTLK_BITRATE_PSDU_BW_G7);
}

static __INLINE uint8
mtlk_bitrate_params_get_psdu_scp_g7 (mtlk_bitrate_info16_t rate_info)
{
  return MTLK_BFIELD_GET(rate_info, MTLK_BITRATE_PSDU_SCP_G7) |
         (MTLK_BFIELD_GET(rate_info, MTLK_BITRATE_PSDU_SCP_HE_G7) << 1);
}

/* scp:    0 = long,  1 = short;            FW enum Scp_e
 * scp_he: 0 = short, 1 = medium, 2 = long; FW enum HeCp_e */
static __INLINE uint8
...
static __INLINE void
mtlk_bitrate_params_get_psdu_mcs_and_nss (uint8 mode, mtlk_bitrate_info16_t rate_info, uint8 *mcs, uint8 *nss)
{
  *nss = HAL_LEGACY_NSS;
  *mcs = 0;
...
#ifdef MTLK_WAVE_700
    case PHY_MODE_11BE:
    case PHY_MODE_11BE_MU_TRIG:
    case PHY_MODE_11BE_MU_DL:
      *mcs = MTLK_BFIELD_GET(rate_info, MTLK_BITRATE_PSDU_HE_MCS);
      *nss = MTLK_BFIELD_GET(rate_info, MTLK_BITRATE_PSDU_HE_NSS) + 1;
    break;
#endif
