...
_ta_crit_coc (_ta_crit_entry_t *crit)
{
  ...
  uint64    sta_tp, all_tp;
  uint64    val_rx, val_tx;
  uint32    sta_rate;
  ...
  ILOG1_DDD("CID-%04x: COC Criterion calc for %u antennas, interval %u ticks",
          mtlk_vap_get_oid(wave_coc_get_vap(HANDLE_T_PTR(mtlk_coc_t, crit->clb_ctx))),
          coc_ants, ta->coc_cfg.interval);
  ...
  mtlk_dlist_foreach(&ta->sta_wss_list, entry, head)
  {
    ...
    if ((val_rx == 0) && (val_tx ==0)) {
      ILOG2_Y("STA %Y rx/tx values are zero -> skip it", &sta_wss->sta_addr);
      mtlk_sta_decref(sta);
      continue;
    }
    ...
    ILOG2_DYDDDDDDD("CID-%04x: WSS STA %Y ants %u, rssi %i, rate %u, rx %d, tx %d -> STA_TP %u, ALL_TP %u",
                    mtlk_vap_get_oid(sta_wss->vap_handle),
  ...
  /* 64 bit division using the operator (/) is not possible in 32 bit platforms CGR.
     In order to resolve it for all platforms carrying out do_div() function.
  */
  do_div(all_tp,MTLK_BPS_PER_MBPS);
...
mtlk_ta_crit_stop (mtlk_handle_t crit_handle)
{
  ...
  ILOG2_D("Crit STOP 0x%08X", (uint32)crit_handle);
