...
static int _mtlk_df_init(mtlk_df_t *df, mtlk_vap_handle_t vap_handle, const char *name, struct net_device *ndev, BOOL is_secondary)
{
  MTLK_INIT_TRY(df, MTLK_OBJ_PTR(df))
    df->vap_handle  = vap_handle;
    MTLK_INIT_STEP_EX(df, USER, MTLK_OBJ_PTR(df),
                      mtlk_df_user_create, (df, name, ndev, is_secondary), df->df_user,
...
mtlk_df_t* mtlk_df_create(mtlk_vap_handle_t vap_handle, const char *name, struct net_device *ndev, BOOL is_secondary)
{
  ...
  if(MTLK_ERR_OK != _mtlk_df_init(df, vap_handle, name, ndev, is_secondary))
  {
                                ...
                                mtlk_handle_t ctx)
{
  ...
  invocation_context->pclpb = mtlk_clpb_create_req(req_id);
                          ...
                          size_t data_size)
{
  ...
  *ppclpb = mtlk_clpb_create_req(req_id);
  if (NULL == *ppclpb) {
    ELOG_V("Cannot allocate clipboard object");
    res = MTLK_ERR_NO_MEM;
    goto err_clpb_create;
