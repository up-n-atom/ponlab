...
#define MTLK_MEM_PCI_BAR1_INDEX  0
...
/* The index of element is a radio configuration in order 2.4GHz - 5.2GHz - 6GHz - zwdfs - 5GHz-high - 6GHz-high,
   the value of an element is the phy device index */
int phy_index[PHY_INDEX_CONFIG_LIMIT]                 = {0, 1, 2, 3, 4, 5};
int phy_index_count = 0;

/* The index of element is a phy device index,
   the value of an element is the net device index */
int interface_index[PHY_INDEX_CONFIG_LIMIT]           = {0, 2, 4, 6, -1, -1};
...
#ifdef MTLK_WAVE_700
int logger_fifo_cfg = LOGGER_HWFIFO_MUX_B2_B0;
#endif
...
module_param_array(phy_index, int, &phy_index_count, 0);
...
#endif
#ifdef MTLK_WAVE_700
module_param(logger_fifo_cfg, int, 0);
#endif
...
#ifdef MTLK_WAVE_700
MODULE_PARM_DESC(logger_fifo_cfg, "HW FIFO Mux config for data logger in WAVE700");
#endif
            ...
            unsigned device_id, wave_uint card_idx)
{
  ...
  if (MTLK_CARD_PCIEG6_A0_B0 == hw_type || MTLK_CARD_PCIEG6_D2_A0_B0 == hw_type) {
    uint32 chip_id;
...
pci_irq_alloc(mtlk_mmb_drv_t *obj)
{
  ...
  uint32 hw_type = hw_mmb_get_hw_type(obj->hw_api->hw);
...
uint32 __MTLK_IFUNC
mtlk_mmb_drv_get_disable_zwdfs(void)
{
  return disable_zwdfs;
}
...
MTLK_INIT_STEPS_LIST_BEGIN(mmb_drv)
  ...
  MTLK_INIT_STEPS_LIST_ENTRY(mmb_drv, MMB_HW_BSS_TX_HD_MAP_LOCK_INIT)
#endif
#ifdef CPTCFG_IWLWAV_BUS_PCI_PCIE
...
_mmb_drv_cleanup(mtlk_mmb_drv_t *obj, BOOL is_rollback)
{
  ...
  MTLK_CLEANUP_BEGIN(mmb_drv, MTLK_OBJ_PTR(obj))
     ...
     */
#if WAVE_USE_BSS_TX_MONITOR
    MTLK_CLEANUP_STEP(mmb_drv, MMB_HW_BSS_TX_HD_MAP_LOCK_INIT, MTLK_OBJ_PTR(obj),
                      wave_hw_bss_tx_hd_map_lock_cleanup, (obj->hw_api->hw));
#endif
    ...
    MTLK_CLEANUP_STEP(mmb_drv, MMB_RCVRY_INIT, MTLK_OBJ_PTR(obj),
                      wave_rcvry_card_uninit, (obj->hw_api->hw)); /* fully cleaned up at _pci_remove() */
...
static mtlk_error_t
_mmb_drv_phy_index_init (void)
{
  unsigned idx = PHY_INDEX_CONFIG_LIMIT;
  uint8 uniq[PHY_INDEX_CONFIG_LIMIT] = { 0 };

  /* default configuration */
  if (phy_index_count == 0) {
    return MTLK_ERR_OK;
  }

  /* set trailing default values */
  while (idx > phy_index_count) {
    phy_index[--idx] = -1;
  }

  /* check uniqueness */
  while (idx-- > 0) {
    if ((phy_index[idx] < -1) || (phy_index[idx] >= PHY_INDEX_CONFIG_LIMIT)) {
      ELOG_V("phy_index configuration is incorrect, index out of bounds");
      return MTLK_ERR_PARAMS;
    }

    uniq[phy_index[idx]]++;
    if ((phy_index[idx] != -1) && (uniq[phy_index[idx]] > 1)) {
      ELOG_DDDDDD("phy_index configuration is incorrect: %d %d %d %d %d %d", phy_index[0],
                  phy_index[1], phy_index[2], phy_index[3], phy_index[4], phy_index[5]);
      return MTLK_ERR_PARAMS;
    }
  }

  ILOG0_DDDDDD("phy_index configuration: %d %d %d %d %d %d", phy_index[0], phy_index[1],
               phy_index[2], phy_index[3], phy_index[4], phy_index[5]);
...
_pci_probe_error:
  ...
  if (!obj->g6_pci_aux && wave_rcvry_pci_probe_error_get(obj->bar1)) {
    ...
    if (wave_rcvry_enabled_get(obj->bar1))
      wave_rcvry_process_msg_unrecoverable_error(obj->bar1);
...
static struct pci_device_id mmb_pci_tbl[] = {
  ...
  { PCI_VENDOR_ID_INTEL,       PCI_VENDOR_ID_INTEL_WAVE600_D2,   PCI_ANY_ID, PCI_ANY_ID, 0, 0, MTLK_CARD_PCIEG6_D2_A0_B0 },
  { PCI_VENDOR_ID_MAXLINEAR,   PCI_VENDOR_ID_INTEL_WAVE600_D2,   PCI_ANY_ID, PCI_ANY_ID, 0, 0, MTLK_CARD_PCIEG6_D2_A0_B0 },
  { PCI_VENDOR_ID_MXL_WAVE700, PCI_DEVICE_ID_MXL_WAVE700,        PCI_ANY_ID, PCI_ANY_ID, 0, 0, MTLK_CARD_PCIEG7 },
  { 0,}
};
...
MTLK_INIT_STEPS_LIST_BEGIN(bus_drv_mod)
  ...
  MTLK_INIT_STEPS_LIST_ENTRY(bus_drv_mod, DRV_CLPBT_INIT)
#endif /* CPTCFG_IWLWAV_ENABLE_CLPB_TRACKING */
  MTLK_INIT_STEPS_LIST_ENTRY(bus_drv_mod, DRV_DFG_INIT)
  MTLK_INIT_STEPS_LIST_ENTRY(bus_drv_mod, DRV_PHY_INDEX_INIT)
...
__exit_module (void)
{
  MTLK_CLEANUP_BEGIN(bus_drv_mod, MTLK_OBJ_PTR(&drv_state))
    ...
    MTLK_CLEANUP_STEP(bus_drv_mod, DRV_RCVRY_INIT, MTLK_OBJ_PTR(&drv_state),
                      wave_rcvry_cleanup, ());
    MTLK_CLEANUP_STEP(bus_drv_mod, DRV_PHY_INDEX_INIT, MTLK_OBJ_PTR(&drv_state),
                      MTLK_NOACTION, ())
    MTLK_CLEANUP_STEP(bus_drv_mod, DRV_DFG_INIT, MTLK_OBJ_PTR(&drv_state),
                      mtlk_dfg_cleanup, ());
#ifdef CPTCFG_IWLWAV_ENABLE_CLPB_TRACKING
    MTLK_CLEANUP_STEP(bus_drv_mod, DRV_CLPBT_INIT, MTLK_OBJ_PTR(&drv_state),
                      mtlk_clpb_tracking_cleanup, ());
#endif /* CPTCFG_IWLWAV_ENABLE_CLPB_TRACKING */
...
#ifdef CPTCFG_IWLWAV_ENABLE_CLPB_TRACKING
    MTLK_INIT_STEP_VOID(bus_drv_mod, DRV_CLPBT_INIT, MTLK_OBJ_PTR(&drv_state),
                        mtlk_clpb_tracking_init, ());
#endif /* CPTCFG_IWLWAV_ENABLE_CLPB_TRACKING */
    MTLK_INIT_STEP(bus_drv_mod, DRV_DFG_INIT, MTLK_OBJ_PTR(&drv_state),
                   mtlk_dfg_init, ());
    MTLK_INIT_STEP(bus_drv_mod, DRV_PHY_INDEX_INIT, MTLK_OBJ_PTR(&drv_state),
                   _mmb_drv_phy_index_init, ());
