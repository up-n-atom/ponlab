...
#define RSSI_IGNORE_PROBE_REQ_LOWER -90
#define RSSI_IGNORE_PROBE_REQ_UPPER -60
...
typedef enum
{
  ...
  MTLK_CORE_CNT_PD_BROADCAST_PACKETS_SENT,
  MTLK_CORE_CNT_PD_BROADCAST_BYTES_SENT_64,
  MTLK_CORE_CNT_PD_UNICAST_PACKETS_SENT,
  MTLK_CORE_CNT_PD_UNICAST_BYTES_SENT_64,
  MTLK_CORE_CNT_PD_MULTICAST_PACKETS_SENT,
  MTLK_CORE_CNT_PD_MULTICAST_BYTES_SENT_64,
...
#define MTLK_CORE_CNT_PD_BROADCAST_PACKETS_SENT_ALLOWED                             MTLK_WWSS_WLAN_STAT_ID_PD_BROADCAST_PACKETS_SENT_ALLOWED
#define MTLK_CORE_CNT_PD_BROADCAST_BYTES_SENT_64_ALLOWED                            MTLK_WWSS_WLAN_STAT_ID_PD_BROADCAST_BYTES_SENT_64_ALLOWED
#define MTLK_CORE_CNT_PD_UNICAST_PACKETS_SENT_ALLOWED                               MTLK_WWSS_WLAN_STAT_ID_PD_UNICAST_PACKETS_SENT_ALLOWED
#define MTLK_CORE_CNT_PD_UNICAST_BYTES_SENT_64_ALLOWED                              MTLK_WWSS_WLAN_STAT_ID_PD_UNICAST_BYTES_SENT_64_ALLOWED
#define MTLK_CORE_CNT_PD_MULTICAST_PACKETS_SENT_ALLOWED                             MTLK_WWSS_WLAN_STAT_ID_PD_MULTICAST_PACKETS_SENT_ALLOWED
#define MTLK_CORE_CNT_PD_MULTICAST_BYTES_SENT_64_ALLOWED                            MTLK_WWSS_WLAN_STAT_ID_PD_MULTICAST_BYTES_SENT_64_ALLOWED
...
typedef struct _mtlk_core_general_stats_t {
  ...
  uint64  rx_drops;         /*!< total bytes dropped */
  uint64  rx_errors;         /*!< total bytes makred with errors */
...
const char *switchtype2string(unsigned int type)
{
  const char *printable_types[] = {
    ...
    "NORMAL_AFTER_RSSI",
    "CALIBRATE","DPP", "ZWDFS", "CALIBRATE_AFTER_CAC",
    "NONE", "ERROR" };
...
struct nic_slow_ctx {
  ...
  /* used in order to get peer ap and when we are in
   * MULTI_AP_BACKHAUL (then a single station is connected)
   * to get that station
   */
  ...
  uint8 default_bcn_key;    /* default bcn key index */
  bool  wep_enabled;
  bool  rsn_enabled;
  uint8 group_cipher;
  uint8 bcn_prot_cipher;
  struct arc4_ctx  wep_tx; /* crypto engine */
  struct arc4_ctx  wep_rx; /* crypto engine */
  ...
  /* This timer will be set when a FLUSH_STATIONS request is sent
   * to hostapd and canceled when a SYNC_DONE is received from it.
   * The request FLUSH_STATIONS will be resend if timer has expired.
   */
  mtlk_osal_timer_t sync_hostapd_timer;
...
{
  BOOL                 wildcard;
  enum wave_acl_policy policy;
  ieee_addr_list_t     mac_list;
}wave_core_acl_t;
...
struct nic {
  ...
  /* CSI - Channel State Information Packet Aggregation */
  uint32                    csi_data_len;
  uint32                    csi_more_frag;
  uint16                    csi_sid;
  uint16                    csi_vap_id;
  uint16                    csi_data_len_recv;
  uint8                     *csi_stats;
  uint8                     color_change_active;
  uint8                     color_change_color;
  MTLK_DECLARE_INIT_STATUS;
...
/* No multi-AP (similar to MULTI_AP_FRONTHAUL, but multi AP IE's are ignored) */
#define MULTI_AP_DISABLED 0
/* vap for 4addr stations only, will create virtual netdevs */
#define MULTI_AP_BACKHAUL BIT(0)
/* regular mode, 3addr stations only or legacy WDS */
#define MULTI_AP_FRONTHAUL BIT(1)
/* vap for both 3addr and 4addr stations, creates virtual netdevs for 4addr stations */
#define MULTI_AP_EXT_HYBRID (MULTI_AP_BACKHAUL | MULTI_AP_FRONTHAUL)
#define MULTI_AP_SUPPORTED_MODES MULTI_AP_EXT_HYBRID

typedef int multi_ap_mode_t;

typedef enum _wave_offload_mode_t {
  PROBEREQ_OFFLOAD_ON,
  PROBEREQ_OFFLOAD_WILDCARD_SSID_OFF,
  PROBEREQ_OFFLOAD_OFF,
  PROBEREQ_OFFLOAD_MODE_LAST
} wave_offload_mode_t;

typedef enum _duplicate_beacon_bw_mode_t {
...
mtlk_core_get_master (mtlk_vap_handle_t vap_handle)
{
  MTLK_ASSERT(vap_handle != NULL);

  return mtlk_vap_manager_get_master_core(mtlk_vap_get_manager(vap_handle));
}

static __INLINE struct wireless_dev *
mtlk_core_get_wdev (mtlk_vap_handle_t vap_handle)
{
  MTLK_ASSERT(vap_handle != NULL);

  return mtlk_df_user_get_wdev(mtlk_df_get_user(mtlk_vap_get_df(vap_handle)));
}
...
static __INLINE BOOL
mtlk_core_bg_scan_is_running(struct nic *nic)
{
  return wave_radio_bg_scan_is_running(wave_vap_radio_get(nic->vap_handle));
}
...
/* In WAV700 we have two DFS bands in the same LMAC. Inband DFS and ZWDFS */
enum dfsband_e
{
  INBAND_DFS_BAND = 0,
  ZWDFS_BAND = 1
...
void __MTLK_IFUNC wave_clean_all_sta_on_disconnect (struct nic *nic);
...
BOOL __MTLK_IFUNC mtlk_core_cca_is_above_configured (mtlk_core_t *core, iwpriv_cca_th_t *cca_th_params);

void __MTLK_IFUNC mtlk_core_set_umi_key_au8TxSeqNum (UMI_SET_KEY *umi_key, mtlk_core_t *core);
...
void __MTLK_IFUNC mtlk_core_bswap_bcl_request(UMI_BCL_REQUEST *req, BOOL hdr_only);
mtlk_error_t __MTLK_IFUNC mtlk_core_bcl_mac_data_get (mtlk_handle_t hcore, const void* data, uint32 data_size);
mtlk_error_t __MTLK_IFUNC mtlk_core_bcl_mac_data_set (mtlk_handle_t hcore, const void* data, uint32 data_size);
mtlk_error_t __MTLK_IFUNC mtlk_core_simple_cli (mtlk_handle_t hcore, const void* data, uint32 data_size);
...
wave_core_send_bf_mode (mtlk_core_t *core, BeamformingMode_e bf_mode);
...
void wave_reset_ssid_in_probe_response_buffer(uint8 *probe_resp_templ_loc, size_t len);
mtlk_error_t wave_core_add_bss_color_data_in_presp (mtlk_core_t *core, uint8 *buf, size_t *len, size_t max_buflen);
...
static __INLINE struct mtlk_chan_def* __wave_core_zwdfs_chandef_get(struct nic *nic)
{
