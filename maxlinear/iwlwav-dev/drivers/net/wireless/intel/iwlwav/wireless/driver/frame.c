    ...
    if (band == MTLK_HW_BAND_2_4_GHZ) {
        if (oper_rateset & BASIC_RATE_SET_2DOT4_OPERATIONAL_RATE) {
            ...
            if (oper_rateset & ~BASIC_RATE_SET_11B_RATES) {
                if(is_eht)
                    mode = MTLK_NETWORK_11BGNAXBE_MIXED;
                else if(is_he)
            ...
            if(is_eht)
                mode = MTLK_NETWORK_11GNAXBE_MIXED;
            else if(is_he)
        ...
        if (is_eht)
            mode = MTLK_NETWORK_11ANACAXBE_MIXED;
        else if (is_he)
        ...
        if (is_eht)
            mode = MTLK_NETWORK_11AXBE_MIXED;
        else
            mode = MTLK_NETWORK_11AX_6G_ONLY;
    } else {
...
ie_process (mtlk_handle_t context, uint8 *fbuf, int32 flen, bss_data_t *bss_data)
{
  ...
  while (flen > (int32)sizeof(ie_t))
  {
    ...
    {
        ILOG1_V("Not enough room to store all information elements!!!");
        flen = 0;
        break;
    }
    ...
    if (ie->length) {
        ...
        {
          int32 copy_length, i;
          uint8 *essid = GET_IE_DATA_PTR(ie);
          ...
          /* Check if zero content SSID */
          bss_data->is_hidden_ssid = 1;
        ...
        case IE_VHT_CAPABILITIES:
          if (ie->length != IE_VHT_CAPABILITIES_LEN)
            goto err_params;
          bss_data->is_vht = 1;
          bss_data->vht_cap_info = GET_IE_DATA_PTR(ie);
          bss_data->vht_cap_info_len = ie->length;
          mtlk_dump(4, ie, ie->length + sizeof(ie_t),
...
#ifdef MTLK_WAVE_700
	  else if (WLAN_EID_MULTILINK_EXT == GET_IE_DATA_PTR(ie)[0]) {
            bss_data->ml_ie_info = GET_IE_DATA_PTR(ie);
            bss_data->ml_ie_info_len = ie->length;
            mtlk_dump(4, ie, ie->length + sizeof(ie_t),
                  "Multilink information element");
            if (*(bss_data->ml_ie_info + 1) & 0x1)
              bss_data->has_probe_ml_ie = 1;
          }
          else if (WLAN_EID_EHT_CAP_EXT == GET_IE_DATA_PTR(ie)[0]) {
            if (ie->length < IE_EHT_CAPABILITIES_MIN_LEN)
              goto err_params;
            bss_data->is_eht = 1;
            bss_data->eht_cap_info = GET_IE_DATA_PTR(ie);
            bss_data->eht_cap_info_len = ie->length;
            mtlk_dump(4, ie, ie->length + sizeof(ie_t),
                  "EHT Capabilities information element");
          }
#endif
          break;
        case IE_TIM:
      ...
      case IE_SSID:
        bss_data->is_hidden_ssid = 1;
        ILOG4_V("Hidden SSID (zero length)");
        break;
      default:
        break;
      }
...
{
  const u8 *he_cap_ie = bss_data->he_cap_info + WLAN_ELEM_ID_EXTENSION_LEN;
  ...
  u8 he_total_size;
#ifdef MTLK_WAVE_700 
  const u8 *eht_cap_ie = bss_data->eht_cap_info + WLAN_ELEM_ID_EXTENSION_LEN;
  struct ieee80211_sta_eht_cap *eht_cap = &beacon_data->eht_cap;
  struct ieee80211_eht_cap_elem *eht_cap_ie_elem = (void *)eht_cap_ie;
  u8 eht_ppe_size = 0;
  u8 eht_mcs_nss_size;
  u8 eht_total_size;
  u16 eht_ppe_thresh_hdr;
  u8 eht_ppe_thresh_offset_low;
  u8 eht_ppe_thresh_offset_high;
#endif
...
#ifdef MTLK_WAVE_700
  beacon_data->is_eht             = bss_data->is_eht;
#endif
...
#ifdef MTLK_WAVE_700  
  /* EHT */
  if ((bss_data->is_he) && (bss_data->is_eht) && (bss_data->eht_cap_info_len)) {
    eht_mcs_nss_size = ieee80211_eht_mcs_nss_size(he_cap_ie_elem, &(eht_cap_ie_elem->fixed), EHT_CAP_FROM_AP);
    if ((eht_cap_ie_elem->fixed.phy_cap_info[5] &
         IEEE80211_EHT_PHY_CAP5_PPE_THRESHOLD_PRESENT) != 0) {
         eht_ppe_thresh_offset_low = sizeof(eht_cap->eht_cap_elem) + eht_mcs_nss_size;
         eht_ppe_thresh_offset_high = eht_ppe_thresh_offset_low + 1;
         eht_ppe_thresh_hdr = (eht_cap_ie[eht_ppe_thresh_offset_low] |
                              (eht_cap_ie[eht_ppe_thresh_offset_high] << 8)) &
                               0x1FF;
         eht_ppe_size = ieee80211_eht_ppe_size(eht_ppe_thresh_hdr,
                                          eht_cap_ie_elem->fixed.phy_cap_info);
    }
    eht_total_size = sizeof(eht_cap->eht_cap_elem) + eht_mcs_nss_size + eht_ppe_size;

    if (bss_data->eht_cap_info_len < eht_total_size) {
      ILOG0_V("EHT Cap info len < EHT total size!");
      return;
    }
    wave_memcpy(&eht_cap->eht_cap_elem, sizeof(eht_cap->eht_cap_elem),
                eht_cap_ie, sizeof(eht_cap->eht_cap_elem));
    /* EHT Tx/Rx HE MCS NSS Support Field */
    wave_memcpy(&eht_cap->eht_mcs_nss_supp, sizeof(eht_cap->eht_mcs_nss_supp),
                &eht_cap_ie[sizeof(eht_cap->eht_cap_elem)], eht_mcs_nss_size);
    /* Check if there are (optional) PPE Thresholds */
    if (eht_ppe_size) {
      wave_memcpy(eht_cap->eht_ppe_thres, sizeof(eht_cap->eht_ppe_thres),
                  &eht_cap_ie[sizeof(eht_cap->eht_cap_elem) + eht_mcs_nss_size],
                  eht_ppe_size);
    }
  }
                       ...
                       mtlk_mgmt_frame_data_t *fd)
{
  ...
  case MAN_TYPE_PROBE_RES:
    ...
    if (is_scan_running(scan_support))
    {
      ...
      {
        /* If Hidden SSID Beacon, we might restart scan as active for STA to be able to connect */
        if ((MAN_TYPE_BEACON == subtype) && bss_data.is_hidden_ssid) {
          ILOG2_YD("Hidden SSID (%Y) detected during scan at channel %u", bss_data.bssid.au8Addr, bss_data.channel);
          wave_trigger_scan_as_active_if_needed(master_core);
        }
      }
    }
  ...
  case MAN_TYPE_PROBE_REQ:
    ...
    if (ML_PROBE_IE_FOUND(bss_data)) {
      fd->probe_req_mlo_ie = TRUE;
    }
    break;
  case MAN_TYPE_ACTION:
    CPU_STAT_SPECIFY_TRACK(CPU_STAT_ID_RX_MGMT_ACTION);
    {
      ...
      case ACTION_FRAME_CATEGORY_BLOCK_ACK:
        /*
          An action frame with illegal category value of Block Ack could reach the driver due to 2 reasons:
           1) An Illegal frame was transmitted over the air and due to the fact that it is a multicast management(action)
           it was forwarded by the firmware to the driver (regarless of the action code) - expected behavior.
           2) A corruption of the frame occured along the way in the Rx-path - unexpected behavior, internal error.
        */
        WLOG_V("Action frame with invalid category value of Block Ack received");
        /* Ignore and drop Block ACK category action frames */
...
BOOL mgmt_frame_filter_allows (mtlk_core_t *nic, mgmt_frame_filter_t *frame_filter, uint8 *frame, unsigned len, BOOL *is_broadcast)
{
  ...
  if (len <= sizeof(frame_head_t))
    mtlk_dump(2, frame, MIN(len, 256),"Wrong mgmt packet length");
  ...
  /* add filter for fils to return from here */
  if (subtype == MAN_TYPE_ACTION) {
    if (len >= sizeof(frame_head_t) + 2) {
      fbuf += sizeof(frame_head_t);
      if ((fbuf[0] == WLAN_CATEGORY_PUBLIC) && (fbuf[1] == WLAN_PUB_ACTION_FILS_DISCOVERY)) {
        if (wave_radio_get_fils_beacon_flag(wave_vap_radio_get(nic->vap_handle))) {
          return TRUE;
        } else {
          return FALSE;
        }
