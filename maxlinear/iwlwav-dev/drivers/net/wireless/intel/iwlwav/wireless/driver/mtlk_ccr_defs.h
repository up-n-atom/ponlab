...
{
  int res;
  BOOL is_hw_gen7 = FALSE;
#ifdef MTLK_WAVE_700
  if (MTLK_CARD_PCIEG7 == ccr->hw_type)
    is_hw_gen7 = TRUE;
#endif
  res = ccr->card_api->init(&ccr->mem, hw, bar0, bar1, bar1_phy, dev_ctx, shram, is_hw_gen7);
  return res;
}
...
static __INLINE uint32
mtlk_ccr_get_progmodel_version_modified (mtlk_ccr_t *ccr)
{
  return ccr->card_api->get_progmodel_version_modified(&ccr->mem);
}
...
{
  uint32 hds_remain = mtlk_ccr_ring_get_hds_to_process(ccr, regs);

  if (__UNLIKELY(hds_processed > hds_remain)) {
    ELOG_DD("HDs processed > remain (%u > %u), clear all", hds_processed, hds_remain);
    hds_processed = hds_remain;
#if 0  /* Avoid assertion to be able to get FW dumps */
    MTLK_ASSERT(FALSE);
#endif
  }
...
static __INLINE void
mtlk_ccr_ring_initiate_bulk_doorbell_interrupt (mtlk_ccr_t *ccr, mtlk_ring_regs *regs, uint32 hds_ready)
{
  return ccr->card_api->ring_initiate_bulk_doorbell_interrupt(&ccr->mem, regs, hds_ready);
}
...
static __INLINE uint32
mtlk_ccr_read_chip_type (mtlk_ccr_t *ccr)
{
  return ccr->card_api->read_chip_type(&ccr->mem);
}

static __INLINE void
wave_ccr_write_phy_td_tb_page_num (mtlk_ccr_t *ccr, uint8 radio_id, uint8 page_num)
{
  return ccr->card_api->write_phy_td_tb_page_num(&ccr->mem, radio_id, page_num);
}
#undef LOG_LOCAL_GID
#undef LOG_LOCAL_FID
