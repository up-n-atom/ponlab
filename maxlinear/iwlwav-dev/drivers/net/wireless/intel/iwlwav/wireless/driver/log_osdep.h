...
#define CERROR(fname, line_no, fmt, ...)                        \
  log_osdep_do(fname, line_no, "E", KERN_ERR, fmt, ## __VA_ARGS__)
#define CWARNING(fname, line_no, fmt, ...)                      \
  log_osdep_do(fname, line_no, "W", KERN_WARNING, fmt, ## __VA_ARGS__)
...
/* Logger timestamp in microseconds */
#ifdef CPTCFG_IWLWAV_TSF_TIMER_ACCESS_ENABLED /* For sync with FW TSF */
#define mtlk_log_get_timestamp()    ((uint32)mtlk_osal_hr_timestamp_us())
#else
#define mtlk_log_get_timestamp()    mtlk_osal_timestamp_us()
