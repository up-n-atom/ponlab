...
#define WAVE_GET_PERCENTAGE(x, y)       (((y) != 0) ? (div64_u64(((x)*100), (y))) : 0)
...
typedef enum
{
  ...
  WAVE_HW_RADIO_BAND_CFG_DB_4x1  = 4,    /* Gen6: 2 Bands: (16 + 16) VAPs, (4 + 1) Antennas */
  WAVE_HW_RADIO_BAND_CFG_SCAN    = 5,    /* Gen6: 2 Bands: 32 VAPs + 0 VAP, Radar Scan or */
                                         /* Gen6: 2 Bands: 31 VAPs + 1 VAP, Radar & WiFi Scan */
#ifdef MTLK_WAVE_700
  WAVE_HW_RADIO_BAND_CFG_CTB     = 6,    /* Gen7: 3 Bands: 64 VAPs, (16 + 16 + 32) VAPs*/
  WAVE_HW_RADIO_BAND_CFG_DB_4x4  = 7,    /* Gen7: 2 Bands: (16 + 16) VAPs, (4 + 4) Antennas */
#endif
...
mtlk_vap_manager_t * _hw_vap_manager_get(mtlk_hw_t *hw, unsigned radio_idx);
#ifdef MTLK_WAVE_700
BOOL __MTLK_IFUNC hw_get_eht_support(mtlk_handle_t hw_handle);
void __MTLK_IFUNC mtlk_hw_store_logger_fifo_mux_cfg(mtlk_vap_handle_t vap_handle, uint8 mux_mode);
...
{
  mtlk_osal_spinlock_t  lock;
  mtlk_osal_spinlock_t  stats_scan_lock;
  void                 *stats_data;
  void                 *stats_copy;
  void                 *stats_snapshot;
  ...
  uint32 mu_he_eht_la_dl_info;
  uint32 mu_he_eht_la_ul_info;
  wave_mu_he_eht_la_stats32_t mu_he_eht_la_stats32[MAX_NUM_OF_HE_GROUPS];
  wave_mu_he_eht_la_stats_t   mu_he_eht_la_stats[MAX_NUM_OF_HE_GROUPS];
} hw_statistics_t;

/**
  Mimicing rtlog_pck_hdr structure for CSI, as CSI stats uses RTlogger packets to retreive stats.
  CSI is enabled for release Images while RTLOG is not.
 */
#define WAVE_CSI_RTLOG_PCK_HDR_LEN_IN_WORDS  11 /* packet header length */
typedef struct wave_csi_rtlog_pck_hdr {
    union {
        struct {
            struct ethhdr   eth;
            struct iphdr    iph;
        ...
        uint32  words[WAVE_CSI_RTLOG_PCK_HDR_LEN_IN_WORDS];
    };
    uint32 data[0];
} wave_csi_rtlog_pck_hdr_t;
...
uint32 hw_mmb_get_chip_type (mtlk_hw_t *hw);
uint32 hw_mmb_get_hw_type   (mtlk_hw_t *hw);
uint32 hw_mmb_get_hw_vers   (mtlk_hw_t *hw);
...
void __MTLK_IFUNC wave_hw_bss_tx_map_deinit(mtlk_hw_t *hw);
#endif

void __MTLK_IFUNC
wave_hw_cca_stats_get (wave_radio_t *radio, int channel, wave_cca_stats_t *cca_stats);
void __MTLK_IFUNC
wave_hw_reset_chan_statistics (mtlk_hw_t *hw);
void __MTLK_IFUNC
wave_radio_reset_chan_statistics (mtlk_hw_t *hw, int ch_num, uint8 radio_id, BOOL full_reset);
void __MTLK_IFUNC
...
void __MTLK_IFUNC wave_hw_set_whm_fw_warn(mtlk_hw_t *hw, u8 whm_fw_warn);
void __MTLK_IFUNC wave_hw_set_whm_phy_warn(mtlk_hw_t *hw, u8 whm_phy_warn);
