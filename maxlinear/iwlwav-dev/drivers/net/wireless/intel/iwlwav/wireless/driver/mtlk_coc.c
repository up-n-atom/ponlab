...
#ifdef MTLK_WAVE_700
  uint32                 tx_320_max_tp;     /* max tp for 320 Mhz bw in WAVE700 */
#endif
...
static uint8 _coc_get_max_state_by_ant_num_table[MAX_NUM_ANTENNAS + 2] = {
...
wave_coc_get_vap (mtlk_coc_t *coc_obj)
{
  MTLK_ASSERT(NULL != coc_obj);
  return coc_obj->vap_handle;
}
...
_wave_coc_get_ant_mask_cfg (mtlk_coc_t *coc_obj, unsigned nof_ants)
{
  ...
  /* In case of ZWDFS antenna, num of tx antennas is 0 */
  if (wave_radio_get_is_zwdfs_radio(radio) && nof_ants == 0) {
    return 0;
  }
...
_mtlk_coc_switch_mode (mtlk_coc_t *coc_obj, uint8 num_tx_antennas, uint8 num_rx_antennas)
{
  ...
  {
    uint32 chip_id = wave_core_get_chip_id(coc_obj->vap_handle);
    ...
    if (WAVE_WAVE700_B0_CHIP_ID == chip_id) {
/* mirror_mask macro is to mirror the input antmask which is used in case of 5GHz and 6GHz.


    Reason : The antenna placement for 5GHz and 6GHz is of reverse when compared to 2GHz radio.
             Instead of ANT0 being the first antenna, ANT3 is the first antenna for 5GHz and 6GHz.
             The mask for ANT0 is 1 and ANT3 is 8. So, in order to enable/disable ANT3, the mask 0001(1)
             should be mirrored as 1000 (8). Similarly for other NSS configurations too.

    Functionality : For 1x1 configuration, in 5GHz and 6GHz, ANT3 is enabled. (1000)
                    For 2x2 configuration, in 5GHz and 6GHz, ANT3 and ANT2 is enabled. (1100)
                    Similarly for 3x3 and 4x4 ANT1 and ANT0 are enabled correspondingly.
...
*/
#define MIRROR_MASK(a) ((((a&1) << 3) | ((a&2) << 1) | ((a&4)>>1) | ((a&8)>>3)))
        if ((MTLK_HW_BAND_2_4_GHZ != core_cfg_get_freq_band_cur(core))) {
            mac_msg->TxAntsMask = MIRROR_MASK(mac_msg->TxAntsMask);
            mac_msg->RxAntsMask = MIRROR_MASK(mac_msg->RxAntsMask);
            cur_rx_mask = mac_msg->RxAntsMask;
        }
    }
    ...
    if (wave_hw_mmb_get_pcie_auto_gen_transition(hw) && num_tx_antennas != 1) {
    ...
    /* Switch to Gen1 if PCIe auto_gen_transition is enabled */
...
_mtlk_coc_get_interval_by_state (mtlk_coc_t *coc_obj, const _mtlk_coc_power_state_t power_state)
{
  ...
  switch (power_state) {
    ...
    case COC_POWER_4x5_STATE:
      coc_interval = coc_obj->auto_antenna_cfg.interval_4x4; break;
...
mtlk_coc_update_antenna_cfg (mtlk_coc_t *coc_obj, uint8 and_mask, BOOL update_coc_masks, BOOL *update_hw)
{
  ...
  uint32 chip_id = 0;
  uint8 tx_ant_count = 0;
  uint8 tx_ant_mask = _wave_coc_get_ant_mask_cfg(coc_obj, coc_obj->current_antenna_cfg.num_tx_antennas);
  ...
  chip_id = wave_core_get_chip_id(coc_obj->vap_handle);
  mtlk_osal_lock_acquire(&coc_obj->lock);

  if (_chipid_is_gen6_d2(chip_id) && cur_ant_count == MAX_NUM_RX_ANTENNAS_665) {
    tx_ant_count = MAX_NUM_TX_ANTENNAS;
  } else {
    tx_ant_count = cur_ant_count;
  }

  if ((coc_obj->current_antenna_cfg.num_tx_antennas != tx_ant_count) ||
  ...
  if (wave_radio_get_is_zwdfs_radio(wave_vap_radio_get(coc_obj->vap_handle))) {
    coc_obj->current_antenna_cfg.num_tx_antennas = 0;
  } else if (mtlk_coc_is_auto_mode(__wave_core_coc_mgmt_get(mtlk_vap_get_core(coc_obj->vap_handle)))) {
    coc_obj->current_antenna_cfg.num_tx_antennas = MAX_NUM_TX_ANTENNAS;
  }

  if (_chipid_is_gen6_d2(chip_id)) {
    coc_obj->hw_tx_antenna_mask = tx_ant_mask;
    coc_obj->hw_antenna_cfg.num_tx_antennas = coc_obj->current_antenna_cfg.num_tx_antennas;
    coc_obj->hw_antenna_cfg.num_rx_antennas = coc_obj->current_antenna_cfg.num_rx_antennas;
    coc_obj->manual_antenna_cfg.num_tx_antennas = coc_obj->current_antenna_cfg.num_tx_antennas;
  } else {
...
void __MTLK_IFUNC
_mtlk_coc_set_restore_bf_mode (mtlk_core_t *core, uint8 bf, BOOL restore_bf)
{
  mtlk_coc_t *coc_obj = __wave_core_coc_mgmt_get(core);
  MTLK_ASSERT(NULL != coc_obj);

  mtlk_osal_lock_acquire(&coc_obj->lock);
  if (bf != coc_obj->restore_bf_mode) {
    coc_obj->restore_bf_mode = bf;
    coc_obj->restore_bf = restore_bf;
  }
  mtlk_osal_lock_release(&coc_obj->lock);
}

static int
_mtlk_coc_set_bf_mode (mtlk_coc_t *coc_obj, const uint8 num_tx_antennas)
{
  mtlk_core_t *core;
  uint8 bf;
  ...
  if ((num_tx_antennas == 1) && (bf != BF_STATE_NON_BF)) {
    if (MTLK_ERR_OK == wave_core_send_bf_mode(core, BF_STATE_NON_BF)) {
      _mtlk_coc_set_restore_bf_mode(core, bf, TRUE);
    } else {
      ELOG_V("Failed to disable BF mode");
      return MTLK_ERR_PARAMS;
    }
  } else if ((num_tx_antennas > 1) && (coc_obj->restore_bf) && (bf != coc_obj->restore_bf_mode)) {
      if (MTLK_ERR_OK != wave_core_send_bf_mode(core, coc_obj->restore_bf_mode)) {
        ELOG_V("Failed to restore BF mode");
        return MTLK_ERR_PARAMS;
      }
      _mtlk_coc_set_restore_bf_mode(core, bf, FALSE);
  }
...
_mtlk_coc_set_antennas (mtlk_coc_t *coc_obj, const mtlk_coc_antenna_cfg_t *antenna_cfg)
{
  ...
  mtlk_osal_lock_release(&coc_obj->lock);
  if (antenna_cfg->num_tx_antennas == 1) {
    res = _mtlk_coc_set_bf_mode(coc_obj, antenna_cfg->num_tx_antennas);
    if (MTLK_ERR_OK != res)
       return res;
  }
...
_mtlk_coc_auto_mode_process (mtlk_coc_t *coc_obj, mtlk_coc_crit_t *coc_crit)
{
  ...
  unsigned max_antennas, crit_max_ants, max_rx_antennas;
  ...
  mtlk_core_t *core = mtlk_vap_get_core(coc_obj->vap_handle);
  wdev = mtlk_core_get_wdev(core->vap_handle);
  ...
  max_rx_antennas = coc_obj->hw_antenna_cfg.num_rx_antennas;
  ...
  /* Input validation to COC.We should not switch unnecessarily.
   * Sometimes TA reports wrong RSSI especially during switch(1x1->4x4)
   * better to check it and avoid taking decision based on it.
   * */
  if ((current_state == COC_POWER_1x1_STATE) && (max_tp == 0) && ( coc_crit->rssi == 127)) {
    ILOG1_V("No Traffic and NO Station connected so COC state remains in old state");
    goto FINISH;
  }
  /* Only next/previous states are available */
  ILOG2_DD("COC: Entry point curent_state = %d, new_state = %d", current_state, new_state);
  switch (current_state) {
    case COC_POWER_START_STATE:
      nof_antennas = max_antennas;
      if (max_rx_antennas == 5)
      {
        nof_antennas = max_rx_antennas;
      }
      ILOG1_D("Start state -> %u Antennas", nof_antennas);
      goto set_state;
    case COC_POWER_1x1_STATE:
      nof_ants_tp = 1;
    ...
    case COC_POWER_TRANSIENT_STATE:
      coc_obj->trans_flag = 0;
      ILOG1_D("COC_POWER_TRANSIENT_STATE %d", coc_obj->pending_state);
      if ((coc_obj->pending_state != max_antennas) && (coc_obj->pending_state != max_rx_antennas)) {
  ...
  switch (current_state) {
    ...
    case COC_POWER_4x5_STATE:
      ...
      else if ((coc_obj->auto_antenna_cfg.interval_2x2) && (rssi > (coc_obj->auto_antenna_cfg.low_rssi_4x4 - coc_obj->auto_antenna_cfg.rssi_delta_down))) {
        nof_ants_rssi = 2;
      }
  ...
  ILOG1_DDDDDDDDD("CID-%04x: State %d: TP %u, RSSI %d -> Antennas ((TP %u, STA %u), (RSSI %u, HW %u)) -> %u",
      mtlk_vap_get_oid(coc_obj->vap_handle),
      current_state, max_tp, rssi, nof_ants_tp, crit_max_ants, nof_ants_rssi, max_antennas, nof_antennas);
set_state:
  switch (nof_antennas) {
    ...
    case 5: new_state = COC_POWER_4x5_STATE;  break;
    default: MTLK_ASSERT(FALSE);
  }
  ...
  if ((current_state != new_state) ) {
   if (current_state != COC_POWER_TRANSIENT_STATE) {
    ...
    if ((coc_obj->trans_flag == 1) && (new_state == COC_POWER_1x1_STATE)) {
      ...
      if ((coc_obj->auto_antenna_cfg.bandwidth_flag == COC_BW_SWITCH_DISABLED)) {
        bw_params.coc_BW = COC_BW_SWITCH_UP;
      } else {
        bw_params.coc_BW = COC_BW_SWITCH_DOWN;
        ILOG1_DD("COC nl event send to Drop BW to 20Mhz %d and send OMN IE  %d", bw_params.coc_BW, bw_params.coc_omn_IE);
      }
      ...
      /* when BW is not changed, hostapd doesn't intimate
       * power change to the driver and TA is not restarted.
       * So stop TA only when BW switch is enabled */
      if ((coc_obj->auto_antenna_cfg.bandwidth_flag == COC_BW_SWITCH_ENABLED)) {
        ILOG1_D("COC Stop TA and Move coc_state %d", coc_obj->current_state);
        mtlk_ta_crit_stop(coc_obj->ta_crit_handle);
      }
      res = _mtlk_coc_set_interval(coc_obj, COC_POWER_TRANSIENT_STATE);
      if (MTLK_ERR_OK != res) {
        goto FINISH;
      }
      ...
      ILOG1_DD("COC nl event send to Restore BW %d and send OMN IE %d", bw_params.coc_BW, bw_params.coc_omn_IE);
      res = wv_ieee80211_vendor_event_coc(LTQ_NL80211_VENDOR_EVENT_COC_BEACON_UPDATE, wdev, &bw_params, sizeof(bw_params));
      if (MTLK_ERR_OK != res) {
        ELOG_V("COC nl event send failed");
      }
      coc_obj->pending_state = new_state;
      /* BW_SWITCH_UP only happens when the antenna config changes from 1x1 to higher antenna config (2x2, 3x3, 4x4),
       * rest modes it stays with the default BW though BW switch is enabled.
       * So stop TA only when antenna moves from 1x1 else TA will not be restarted as hostapd doesn't intimate
       * changes to driver as no BW_SWITCH_UP happens in other modes (2x2, 3x3, 4x4). */
      if ((coc_obj->auto_antenna_cfg.bandwidth_flag == COC_BW_SWITCH_ENABLED) &&
          (current_state == COC_POWER_1x1_STATE) && (bw_params.coc_BW != 0)) {
        ILOG1_D("COC Stop TA and Move coc_state %d", coc_obj->current_state);
        mtlk_ta_crit_stop(coc_obj->ta_crit_handle);
      }
      res = _mtlk_coc_set_interval(coc_obj, COC_POWER_TRANSIENT_STATE);
...
_mtlk_erp_auto_mode_process (mtlk_erp_t *erp_obj, ta_crit_erp_result_t *erp_result)
{
  ...
  case CW_320:
    if (wave_radio_is_320mhz_supported(radio)) {
      tx_threshold = erp_obj->tx_320_max_tp;
      break;
    }
    fallthrough;
#endif
...
{
  ta_crit_coc_cfg_t ta_crit_cfg = {0};
  uint8  num_ants, ant_mask;
  mtlk_error_t res;
  uint32 chip_id = wave_core_get_chip_id(cfg->vap_handle);
  ...
  /*Auto CoC should be enabled by default only for LGM platform */
#ifdef MTLK_LGM_PLATFORM
  /* For ZWDFS do not enable Auto CoC by default */
  /* As of now we keep the Auto CoC disabled at bootup*/
  if ((_chipid_is_gen6_d2(chip_id)) && (!wave_radio_get_is_zwdfs_radio(wave_vap_radio_get(coc_obj->vap_handle)))) {
    coc_obj->auto_mode_cfg = TRUE;
  }
#else
    coc_obj->auto_mode_cfg = FALSE;
#endif
  coc_obj->hw_max_antenna_mask = cfg->hw_rx_ant_mask;
  if (_chipid_is_gen6_d2(chip_id))
  {
    ant_mask = cfg->hw_rx_ant_mask;
    num_ants = cfg->hw_antenna_cfg.num_rx_antennas;
  } else {
    ant_mask = cfg->hw_tx_ant_mask;
  ...
  if (_chipid_is_gen6_d2(chip_id))
  {
    coc_obj->current_state = _mtlk_coc_get_max_state_by_ant_num(coc_obj->current_antenna_cfg.num_rx_antennas);
  } else {
  ...
  ILOG1_DDDDD("COC : is_auto_mode = %d, TX_ant_num %d, mask 0x%X, initial_state %d, interval %d",
      coc_obj->auto_mode_cfg, coc_obj->current_antenna_cfg.num_tx_antennas, ant_mask, coc_obj->current_state, ta_crit_cfg.interval);
...
#ifdef MTLK_WAVE_700
  erp_obj->tx_320_max_tp = cfg->tx_320_max_tp;
#endif
...
mtlk_coc_set_power_mode (mtlk_coc_t *coc_obj, const BOOL is_auto_mode)
{
  ...
  wave_radio_t *zwdfs_radio = NULL;
  struct intel_vendor_event_coc coc_params;
  struct wireless_dev *wdev;

  MTLK_ASSERT(NULL != coc_obj);
  wdev = mtlk_core_get_wdev(coc_obj->vap_handle);
  radio = wave_vap_radio_get(coc_obj->vap_handle);
  if (!radio) {
  ...
  if (is_auto_mode) {
    if (TRUE) { /* always update */
      if (coc_obj->current_state == COC_POWER_TRANSIENT_STATE) {
        ILOG1_D("Power mode changed because of channel switch continue COC pending state %d \n",coc_obj->pending_state);
        /* set the transient timer to start here */
        res = _mtlk_coc_set_interval(coc_obj, coc_obj->current_state);
        if (MTLK_ERR_OK != res)
          goto FINISH;
        mtlk_ta_crit_start(coc_obj->ta_crit_handle);
      } else {
        /* restore previous configuration */
        ILOG1_DD("Power mode changed because of COC state change %d %d \n", coc_obj->hw_antenna_cfg.num_tx_antennas,
          coc_obj->hw_antenna_cfg.num_rx_antennas);
        /* No need to set omn_ie as we switch to 4x4 */
        memset(&coc_params, 0, sizeof(coc_params));
        if (coc_obj->auto_antenna_cfg.bandwidth_flag == COC_BW_SWITCH_ENABLED)
          coc_params.coc_BW = COC_BW_SWITCH_UP;
        else
          coc_params.coc_BW = coc_obj->bw_params.coc_BW;
        res = wv_ieee80211_vendor_event_coc(LTQ_NL80211_VENDOR_EVENT_COC_BEACON_UPDATE, wdev, &coc_params, sizeof(coc_params));
        if (MTLK_ERR_OK != res) {
          ELOG_V("COC nl event send failed");
          goto FINISH;
        }
        ...
        res = _mtlk_coc_set_interval(coc_obj, COC_POWER_START_STATE);
        if (MTLK_ERR_OK != res)
          goto FINISH;
    ...
    /* restore previous configuration */
    ILOG1_DD("Power mode changed on normal mode %d %d \n", coc_obj->manual_antenna_cfg.num_tx_antennas,
      coc_obj->manual_antenna_cfg.num_rx_antennas);
    if (coc_obj->auto_antenna_cfg.bandwidth_flag == COC_BW_SWITCH_ENABLED) {
       memset(&coc_params, 0, sizeof(coc_params));
       coc_params.coc_BW = COC_BW_SWITCH_UP;
       coc_params.coc_omn_IE = 0;
       ILOG1_DD("COC nl event send to Restore BW %d and send OMN IE %d if CoC BW Switch enabled", coc_params.coc_BW, coc_params.coc_omn_IE);
    ...
    if ((wave_radio_get_is_zwdfs_operational_radio(radio)) &&
       (coc_obj->manual_antenna_cfg.num_rx_antennas == MAX_NUM_RX_ANTENNAS_665)) {

      zwdfs_radio = wave_radio_descr_get_zwdfs_radio(wave_card_radio_descr_get(mtlk_vap_get_hw(coc_obj->vap_handle)));
      if (!zwdfs_radio) {
        res = MTLK_ERR_UNKNOWN;
        goto FINISH;
      }
      if (wave_radio_get_zwdfs_ant_enabled(zwdfs_radio)) {
        ILOG0_DD("rx antennas changed %d to %d because of zwdfs enabled", coc_obj->manual_antenna_cfg.num_rx_antennas,
          coc_obj->hw_antenna_cfg.num_rx_antennas);
        coc_obj->manual_antenna_cfg.num_rx_antennas =  coc_obj->hw_antenna_cfg.num_rx_antennas;
      }
    }
...
#ifdef MTLK_WAVE_700
  erp_obj->tx_320_max_tp     = erp_cfg->tx_320_max_tp;
#endif
...
#ifdef MTLK_WAVE_700
  erp_cfg->tx_320_max_tp     = erp_obj->tx_320_max_tp;
#endif
  mtlk_osal_lock_release(&erp_obj->lock);
...
_wave_coc_check_antenna_params (mtlk_coc_t *coc_obj, const mtlk_coc_antenna_cfg_t *antenna_params)
{
  int res = MTLK_ERR_OK;
  uint32 chip_id = 0;
  int rx_ant_num = antenna_params->num_rx_antennas;

  MTLK_ASSERT(NULL != coc_obj);

  chip_id = wave_core_get_chip_id(coc_obj->vap_handle);
  if (_chipid_is_gen6_d2(chip_id) && rx_ant_num == MAX_NUM_RX_ANTENNAS_665 &&
      antenna_params->num_tx_antennas == MAX_NUM_TX_ANTENNAS) {
    ILOG0_DDD("CID-%04x: Support CoC power mode TX%dxRX%d",
              mtlk_vap_get_oid(coc_obj->vap_handle),
              antenna_params->num_tx_antennas, rx_ant_num);
    return res;
  }
  ...
  if ((antenna_params->num_rx_antennas > coc_obj->hw_antenna_cfg.num_rx_antennas) ||
      ...
      (rx_ant_num != antenna_params->num_tx_antennas)) {
    ELOG_DDD("CID-%04x: Wrong CoC power mode TX%dxRX%d",
             mtlk_vap_get_oid(coc_obj->vap_handle),
...
mtlk_coc_set_auto_params (mtlk_coc_t *coc_obj, const mtlk_coc_auto_cfg_t *in_auto_params)
{
  ...
  /* RSSI validation */
    ...
    if (coc_obj->is_auto_mode) {
      ...
      if (auto_params->bandwidth_flag == COC_BW_SWITCH_DISABLED) {
        wdev = mtlk_core_get_wdev(core->vap_handle);
        memset(&bw_params, 0, sizeof(bw_params));
        bw_params.coc_BW = COC_BW_SWITCH_UP;
