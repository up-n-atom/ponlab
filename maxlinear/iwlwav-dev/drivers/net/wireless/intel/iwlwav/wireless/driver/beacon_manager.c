...
/* FIXME COLOR: Remove and include via wave_80211ax.h after resolving redefinitions errors. */
...
#define IE_EHT_MCS_NSS_OFFSET    14
...
#define IE_HDR_SIZE                           (sizeof(ie_t))

#define ML_IE_CONTROL_LEN                     2
#define ML_IE_COMMON_INFO_LEN_OFFSET          (IE_HDR_SIZE + WLAN_ELEM_ID_EXTENSION_LEN + ML_IE_CONTROL_LEN)
...
void static _wave_check_and_free_mbssid_buffer (mtlk_hw_t *hw, uint32 *p_mbssid_dma_addr, uint8  **p_mbssid_critical_update, int *p_len)
{
  ...
  if (*p_mbssid_dma_addr) {
    mtlk_osal_unmap_phys_addr(mtlk_ccr_get_dev_ctx(mtlk_hw_mmb_get_ccr(hw)),
                              *p_mbssid_dma_addr, *p_len, MTLK_DATA_TO_DEVICE);
    *p_mbssid_dma_addr = 0;
    *p_len = 0;
  }

  if (*p_mbssid_critical_update) {
    mtlk_osal_mem_free(*p_mbssid_critical_update);
    *p_mbssid_critical_update = NULL;
  }
}
#endif
...
#ifdef MTLK_WAVE_700
  _wave_check_and_free_mbssid_buffer(hw, &bmgr_priv->tmpl.mbssid_dma_addr, &bmgr_priv->tmpl.mbssid_ptr, &bmgr_priv->tmpl.mbssid_alloc_size);
#endif
}
...
#ifdef MTLK_WAVE_700
    _wave_check_and_free_mbssid_buffer(mtlk_vap_get_hw(vap_handle), &tmpl->mbssid_dma_addr, &tmpl->mbssid_ptr, &tmpl->mbssid_alloc_size);
#endif
  }
}
...
/* Parses the Multilink IE and creates part7 partition
 * if required.
 *
 * Channel switch Announcement phase:
 * =============================================
 * The Host adds Max channel switch time IE in per STA
 * profile. Part6 includes data upto the ML IE Per STA
 * profile capabilities info field. FW to add CSA/CSW
 * IEs just after the capabilities info field.
 * Part7 starts from Max Channel switch time IE and
 * rest of the IEs.
 *
 * Channel transition phase on the affected link:
 * =============================================
 * The Host adds per STA profile upto the capabilities
 * info field and fw to add max channel switch time IE.
 * part6 will hold all the IEs including ML IE per STA
 * profile. part7 will have remaining vendor IEs. fw to add
 * max channel switch time IE between part6 and part7
 *
 * Note: per STA profile length is updated with expected IE
 * additions in fw already. This API updates ML IE length
 * based on the per STA profile length
 */
static int*
_wave_parse_update_multilink_ie_length(uint8 *ie, int ie_len, int *part8_len,
                                       int *part9_len)
{
  int sta_info_len_offset = 0;
  int sta_info_len = 0;
  int per_sta_prof_len = 0;
  int ml_ie_part8_len = 0;
  int ml_ie_tag_len = (ie_len - IE_HDR_SIZE);
  int ml_ie_common_info_len = ie[ML_IE_COMMON_INFO_LEN_OFFSET];
  int ml_ie_tag_len_no_link_info = (WLAN_ELEM_ID_EXTENSION_LEN +
                                    ML_IE_CONTROL_LEN + ml_ie_common_info_len);

  if (ml_ie_tag_len > ml_ie_tag_len_no_link_info) { /* Per STA profile exists */

    sta_info_len_offset = IE_HDR_SIZE + ml_ie_tag_len_no_link_info +
                      ML_PER_STA_PROF_HDR_LEN + ML_IE_STA_CONTROL_LEN;
     ...
     * that would be added by the FW on the fly
     */
    ml_ie_tag_len = ml_ie_tag_len_no_link_info +
                    ML_PER_STA_PROF_HDR_LEN + per_sta_prof_len;
    ie[IE_LENGTH_OFFSET] = ml_ie_tag_len;

    ml_ie_part8_len = (IE_HDR_SIZE + ml_ie_tag_len_no_link_info +
                      ML_PER_STA_PROF_HDR_LEN + ML_IE_STA_CONTROL_LEN +
                      sta_info_len + ML_IE_STA_PROF_CAPAB_INFO_LEN);
...
static BOOL
_wave_get_beacon_data_length(beacon_template_t *tmpl, int *part1_len, int *part2_len, int *part3_len, int *part4_len, int *part5_len, int *part6_len, int *part7_len, int *part8_len, int *part9_len)
{
  BOOL result = TRUE;
  int ie_id;
  int ie_len;
  ...
  *part9_len = 0;
  ...
  while (pos < last_pos) {
    ...
    case IE_EXTENSION:
      if ((ie[WLAN_EID_EXTENSION_ID_OFFSET] == WLAN_EID_HE_CAP_EXT) ||
          ...
          (ie[WLAN_EID_EXTENSION_ID_OFFSET] == WLAN_EID_MAX_CHAN_SW_TIME_EXT)) {
        *part7_len += ie_len;
        if (ie[WLAN_EID_EXTENSION_ID_OFFSET] == WLAN_EID_MAX_CHAN_SW_TIME_EXT) {
          ILOG1_D("CSA debug: received extension WLAN_EID_MAX_CHAN_SW_TIME_EXT %d", ie[WLAN_EID_EXTENSION_ID_OFFSET]);
        }
      /* Check if ML IE is present and if PER STA Profile exists inside the ML IE */
      } else if (ie[WLAN_EID_EXTENSION_ID_OFFSET] == WLAN_EID_EXT_EHT_MULTI_LINK) {
        last_part_len = _wave_parse_update_multilink_ie_length(ie, ie_len, part8_len, part9_len);
      } else {
        *last_part_len += ie_len;
      }
      break;
    default:
      *last_part_len += ie_len;
  ...
  if (result == TRUE) {
    ...
    if ((tmpl->head_len + tmpl->tail_len) !=
        (*part1_len + *part2_len + *part3_len + *part4_len + *part5_len + *part6_len + *part7_len + *part8_len + *part9_len)) {
      ELOG_DDDDDDDDDD("Beacon template lengths mismatch: (%d + %d) is not equal (%d + %d + %d + %d + %d + %d + %d + %d)",
                   tmpl->head_len, tmpl->tail_len, *part1_len, *part2_len, *part3_len, *part4_len, *part5_len, *part6_len, *part7_len,
                   (*part8_len) + (*part9_len));
      result = FALSE;
    }
  }
...
_wave_reset_mbssid_non_tx_critical_update(uint8 *mbssid_critical_update, uint8 *pos, int buf_len, BOOL *p_send_mbssid_buf)
{
  ...
  int ie_id, ie_len;
  int tag_id, tag_len;
  uint8 *last_pos = pos + buf_len;
  uint8 *ie, *capab, *non_tx_sub;

  wave_memcpy(mbssid_critical_update, buf_len, pos, buf_len);

  while (pos < last_pos) {
    ie = pos;
    ie_id = *pos;
    ie_len = *(pos+1) + IE_HDR_SIZE;
    pos += ie_len;
    ...
    if (ie_id == IE_MULTIPLE_BSSID) {

      non_tx_sub = ie + MBSSID_NON_TX_SUB_ELEM_OFFSET;

      while (non_tx_sub < pos) {

        tag_id = *non_tx_sub;

        if (tag_id) {
          ELOG_D("Wrong Non-Tx BSS ID: %d", tag_id);
          result = MTLK_ERR_UNKNOWN;
          break;
        }

        tag_len = *(non_tx_sub + 1) + IE_HDR_SIZE;
...
static int _wave_beacon_man_template_push(mtlk_core_t *core, beacon_template_t *tmpl)
{
  ...
  if (FALSE == _wave_get_beacon_data_length(tmpl, &part1_len, &part2_len, &part3_len, &part4_len, &part5_len, &part6_len, &part7_len, &part8_len, &part9_len)) {
  ...
  if (mtlk_hw_type_is_gen7(hw)) {
    ...
    if(WAVE_RADIO_PDB_GET_INT(wave_vap_radio_get(core->vap_handle), PARAM_DB_RADIO_VW_TEST_MODE)) {
      wave_set_ie_test_mode_mpdu_density(core, tmpl->ptr + tmpl->head_len, tmpl->tail_len, TRUE);
    }
  }
#endif
  ...
  if (NULL == man_entry) {
    ...
    tmpl->dma_addr = 0;
#ifdef MTLK_WAVE_700
    if (mtlk_hw_type_is_gen7(hw) && mbssid_dma_addr && p_mbssid_critical_update) {
      _wave_check_and_free_mbssid_buffer(hw, &mbssid_dma_addr, &p_mbssid_critical_update, &mbssid_buf_len);
    }
#endif
  ...
  if (hw_get_btwt_support(HANDLE_T(hw))) {
    psUmiBeacon->u16part7Len  = HOST_TO_MAC16(part7_len);
    psUmiBeacon->u16part8Len  = HOST_TO_MAC16(part8_len);
  } else {
    psUmiBeacon->u16part7Len  = HOST_TO_MAC16(part7_len + part8_len);
    psUmiBeacon->u16part8Len  = 0;
  }
  ...
  ILOG1_DD("Set Beacon: u16part6Len = %u u16part7Len = %u",
          MAC_TO_HOST16(psUmiBeacon->u16part6Len), MAC_TO_HOST16(psUmiBeacon->u16part7Len));
  ...
  if (mtlk_hw_type_is_gen7(hw) && mbssid_dma_addr && p_mbssid_critical_update) {
    if (err == MTLK_ERR_OK) {
      if (!tmpl->mbssid_dma_addr && !tmpl->mbssid_ptr) {
        tmpl->mbssid_alloc_size = mbssid_buf_len;
        tmpl->mbssid_dma_addr   = mbssid_dma_addr;
        tmpl->mbssid_ptr        = p_mbssid_critical_update;
      } else {
        ELOG_D("CID-%04x: mbssid buffer not released!", oid);
...
int
wave_beacon_template_get_beacon_data(mtlk_core_t *core, struct sk_buff *beacon, struct sk_buff *probe_resp,
    struct cfg80211_beacon_data *beacon_data, struct ieee80211_mutable_offsets *offs)
{
  ...
  if (!core || !beacon || !probe_resp || !beacon_data || !offs)
    return MTLK_ERR_PARAMS;
  ...
  if (core->slow_ctx->bcn_prot_cipher) {
    /* Remove MME IE added by mac80211 from beacon template. HW will add MME IE for every beacon */
    mme_ie = (ie_t *) cfg80211_find_ie(IE_MME, beacon_data->tail, beacon_data->tail_len);
    if (mme_ie)
      beacon_data->tail_len -= (mme_ie->length + MME_IE_EID_LEN_SIZE);
  }
...
#ifdef MTLK_WAVE_700
static void
_wave_beacon_check_eht_oper_consistency(struct mxl_vendor_eht_oper eht_operation,
    struct mxl_vendor_eht_oper eht_operation_db)
{
  if ((eht_operation.eht_op_params & IEEE80211_EHT_OPERATION_DISABLED_SUBCHANNEL_BITMAP_PRESENT) !=
      (eht_operation_db.eht_op_params & IEEE80211_EHT_OPERATION_DISABLED_SUBCHANNEL_BITMAP_PRESENT)) {
    ELOG_V("Error: Disabled subchannel bitmap flag in beacon template is not in sync with Param DB");
  }
  if (eht_operation.eht_op_params & IEEE80211_EHT_OPERATION_DISABLED_SUBCHANNEL_BITMAP_PRESENT) {
    if ((eht_operation.eht_disable_subch_bitmap[0] != eht_operation_db.eht_disable_subch_bitmap[0]) ||
        (eht_operation.eht_disable_subch_bitmap[1] != eht_operation_db.eht_disable_subch_bitmap[1])) {
      ELOG_V("Error: Disabled subchannel bitmap in beacon template is not in sync with Param DB");
    }
  }
}

mtlk_error_t wave_set_ie_test_mode_mpdu_density(mtlk_core_t *core, u8 *ies_data, size_t ies_len, BOOL enable)
{
  struct ieee80211_ht_cap  *ht_capa;
  struct ieee80211_he_6ghz_capa *he_6g_cap;
  u8 mpdu_den;
  u8 *ie;

  if (enable)
    mpdu_den = IEEE80211_HT_MPDU_DENSITY_8; /* 8us minimum MPDU start spacing time */
  else
    mpdu_den = IEEE80211_HT_MPDU_DENSITY_4; /* 4us minimum MPDU start spacing time */

  ie = (u8 *)cfg80211_find_ie(WLAN_EID_HT_CAPABILITY, ies_data, ies_len);
  if (ie) {
    /* Update MPDU density in HT capabilities */
    ht_capa = (struct ieee80211_ht_cap  *)(ie + 2);
    ht_capa->ampdu_params_info &= ~IEEE80211_HT_AMPDU_PARM_DENSITY;
    ht_capa->ampdu_params_info |= (mpdu_den << IEEE80211_HT_AMPDU_PARM_DENSITY_SHIFT) & IEEE80211_HT_AMPDU_PARM_DENSITY;
  }

  ie = (u8 *)cfg80211_find_ext_ie(WLAN_EID_EXT_HE_6GHZ_CAPA, ies_data, ies_len);
  if (ie) {
    /* Update MPDU density in HE 6G band capabilities */
    he_6g_cap = (struct ieee80211_he_6ghz_capa  *)(ie + 3);
    he_6g_cap->capa &= ~IEEE80211_HE_6GHZ_CAP_MIN_MPDU_START;
    he_6g_cap->capa |= mpdu_den & IEEE80211_HE_6GHZ_CAP_MIN_MPDU_START;
  }
...
mtlk_error_t wave_vw_test_update_all_beacon(mtlk_core_t *core, BOOL enable)
{
  ...
  beacon_template_t       *tmpl, *rcvry_tmpl;
  beacon_manager_t        *bmgr = &(core->slow_ctx->beacon_man_data);
  beacon_manager_priv_t   *bmgr_priv;
  mtlk_vap_handle_t       vap_handle;
  uint32                  vap_idx, max_vaps_count;
  unsigned                new_len;
  int                     res;

  max_vaps_count = mtlk_vap_manager_get_max_vaps_count(vap_mgr);

  for( vap_idx = 0; vap_idx < max_vaps_count; vap_idx++) {
    ...
    if (mtlk_vap_is_master_ap(vap_handle) || !core_cfg_is_connected(core))
      continue;

    core = mtlk_vap_get_core(vap_handle);
    bmgr = &(core->slow_ctx->beacon_man_data);
    ILOG0_D("VapID %u to push template from", vap_idx);

    /* if both buffers belong to fw in current VAP */
    if (bmgr->switch_pending) {
      ILOG0_V("switch pending");
      return MTLK_ERR_NO_MEM;
    }

    rcvry_tmpl = &bmgr->tmpl[MTLK_BEACON_MAN_RCVRY_TMPL_IDX];
    if (rcvry_tmpl->ptr == NULL)
      continue;

    /* driver owns free buffer */
    tmpl = &bmgr->tmpl[bmgr->drv_idx];

    new_len = rcvry_tmpl->head_len + rcvry_tmpl->tail_len;
    if (new_len > tmpl->alloced_size) { /* need to reallocate */
      if (tmpl->ptr)
        mtlk_osal_mem_free(tmpl->ptr);
    ...
    /* Get a copy from recovery template */
    wave_memcpy(tmpl->ptr, new_len, rcvry_tmpl->ptr, new_len);
    tmpl->head_len = rcvry_tmpl->head_len;
    tmpl->tail_len = rcvry_tmpl->tail_len;
    ...
    /* If fw is already busy in handling previous request then queue it and return */
...
_wave_beacon_template_parsing_ies(mtlk_core_t *core, mtlk_beacon_data_t *beacon_data)
{
  ...
  u8 set_bss_flags = 0;
  const u8 *vht_cap_ie;
  BOOL is_he_160mhz_supported = FALSE;
...
#ifdef MTLK_WAVE_700
  /* EHT Capabilities and EHT Operation IEs */
  if (wave_radio_is_gen7(radio)) {
    /* Find EHT Capabilities and Operation extension elements */
    const u8 *eht_cap_ie = (u8 *)cfg80211_find_ext_elem(WLAN_EID_EHT_CAP_EXT, beacon_data->tail, beacon_data->tail_len);
    const u8 *eht_oper_ie = (u8 *)cfg80211_find_ext_elem(WLAN_EID_EHT_OPER_EXT, beacon_data->tail, beacon_data->tail_len);

    mtlk_pdb_size_t eht_mcs_nss_len = 0;
    ...
    if (eht_cap_ie) {
        ...
        if ((band == MTLK_HW_BAND_6_GHZ) && 
          ...
          eht_mcs_nss_len = EHT_MCS_NSS_80MHZ_LEN;
        }
    ...
    if (eht_oper_ie) {
      if (WLAN_EID_EHT_OPER_EXT == eht_oper_ie[WLAN_EID_EXTENSION_ID_OFFSET]) {
        struct mxl_vendor_eht_oper eht_operation = {0};
        struct mxl_vendor_eht_oper eht_operation_db = {0};
        mtlk_pdb_size_t eht_operation_len = sizeof(eht_operation);
        mtlk_pdb_size_t eht_operation_db_len = sizeof(eht_operation);
        MTLK_CORE_PDB_GET_BINARY(core, PARAM_DB_CORE_EHT_OPERATION, &eht_operation_db, &eht_operation_db_len);
        wave_memcpy((u8 *)&eht_operation, eht_operation_len,
                    (u8 *)&eht_oper_ie[WLAN_EID_EXTENSION_ID_OFFSET+1], eht_operation_len);
        _wave_beacon_check_eht_oper_consistency(eht_operation, eht_operation_db);
        wave_pdb_set_binary(param_db_core, PARAM_DB_CORE_EHT_OPERATION, &eht_operation, eht_operation_len);
        mtlk_dump(1, &eht_oper_ie[WLAN_EID_EXTENSION_ID_OFFSET+1], eht_operation_len, "dump of EHT OPERATION:");
      } else {
        ELOG_DD("Incorrect eht_oper_ie_id %d. Expected eht_oper_ie_id %d",
                eht_oper_ie[WLAN_EID_EXTENSION_ID_OFFSET], WLAN_EID_EHT_OPER_EXT);
      }
    } else {
      ILOG1_V("WAVE700: No EHT Operation element found in the Beacon!!!");
    }
  }
#endif
  ...
  wave_update_psd_paramdb(core, set_bss_flags);
  ...
  /* Update network mode by supported rates IEs */
  {
      ...
      mtlk_core_update_network_mode(core,
          mtlk_frame_oper_rateset_to_net_mode(bss_data.operational_rate_set, band,
              ...
              MTLK_BFIELD_GET(set_bss_flags, VAP_ADD_FLAGS_EHT)));
  }
                                                   ...
                                                   uint8 frame_type, uint8 time_stamp_offset, bool enable)
{
  ...
  if (err != MTLK_ERR_OK) {
    ...
    ILOG2_DD("CID-%04x: UM_MAN_SET_UNSOLICITED_REQ failed, Status=%u", oid, psUmiUnsolicitedFrame->Status);
    err = MTLK_ERR_MAC;
  } else {
    ILOG1_D("CID-%04x: UM_MAN_SET_UNSOLICITED_REQ send OK", oid);
  }
...
mtlk_error_t _wave_unsolicited_frame_template_push_wrap( mtlk_core_t *core)
{
  ...
  if (mode == WAVE_UNSOLICITED_FRAME_CFG_MODE_PROBE_RESP) {
    ...
    if (len)
      buf = mtlk_osal_mem_alloc(len, MTLK_MEM_TAG_BEACON_DATA);

    if (!buf || !len) {
      WLOG_PD("Unsolicited Probe Resp template: buf: %p len: %d", buf, len);
      send_umi_msg = false;
    } else {
      wave_pdb_get_binary(radio_param_db,
                          PARAM_DB_RADIO_UNSOLICIT_PROBE_RESP_TEMPL,
                          (void *)buf, &len);
       mtlk_dump(2, buf, len, "PARAM_DB_RADIO_UNSOLICIT_PROBE_RESP_TEMPL before UM_MAN_SET_UNSOLICITED_REQ");
    }
  } else if (mode == WAVE_UNSOLICITED_FRAME_CFG_MODE_FILS_DISC) {
    len = wave_pdb_get_int(radio_param_db, PARAM_DB_RADIO_FILS_DISCOVERY_FRAME_LEN);

    if (len)
      buf = mtlk_osal_mem_alloc(len, MTLK_MEM_TAG_BEACON_DATA);

    if (!buf || !len || (len < FILS_DISC_FRAME_LEN)) {
      WLOG_PD("FILS frame template: buf: %p len: %u", buf, len);
      send_umi_msg = false;
    } else {
      wave_pdb_get_binary(radio_param_db,
      ...
      mtlk_dump(2, buf, len, "PARAM_DB_RADIO_FILS_DISCOVERY_FRAME before UM_MAN_SET_UNSOLICITED_REQ");
    }
  ...
  if (send_umi_msg) {
    ...
    ILOG2_V("UM_MAN_SET_UNSOLICITED_REQ not sent to FW");
  }
...
int wave_beacon_man_set_unsolicited_frame_tx(mtlk_handle_t hcore, const void *data, uint32 data_size)
{
  ...
  MTLK_CLPB_TRY(unsolicited_frame_cfg, unsolicited_frame_cfg_size)
    ...
    if (MTLK_HW_BAND_6_GHZ == band) {
      ...
      ILOG1_DD("UM_MAN_SET_UNSOLICITED_REQ is not supported for band = %d, VAP mode = %d",
...
mtlk_error_t
wave_core_add_bss_color_data_in_presp (mtlk_core_t *core, uint8 *buf, size_t *len, size_t max_buflen)
{
  uint8 *probe_resp_templ_loc, *ies_data;
  size_t ies_len, ies_data_offs;
  uint8 *tmp_ie;
  ie_t *ie;
  ...
  /* Set BSS color State disabled */
  tmp_ie = (uint8*) cfg80211_find_ext_ie(WLAN_EID_EXT_HE_OPERATION, ies_data, ies_len);
  if (!tmp_ie) {
    ELOG_D("CID-%04x: HE operation IE not found", mtlk_vap_get_oid(core->vap_handle));
    return MTLK_ERR_UNKNOWN;
  }
  tmp_ie[IE_HE_OPER_BSS_COLOR_OFFSET] |= HE_OPERATION_BSS_COLOR_DISABLED_FLAG;

#define TOTAL_EXT_CCA_IE_SIZE 5
  if (*len + TOTAL_EXT_CCA_IE_SIZE > max_buflen) {
    ELOG_D("CID-%04x: Not enough space for CCA IE", mtlk_vap_get_oid(core->vap_handle));
    return MTLK_ERR_UNKNOWN;
  }
  /* Move to HE oper IE found */
  ies_len -= tmp_ie - ies_data;
  ies_data += tmp_ie - ies_data;

  /* Below processing is for put CCA IE in extension IE ID sequence */
  /* Try find out EXT IE greater than WLAN_EID_EXT_BSS_COLOR_CHG_ANN or Non EXT IE.*/
  while (ies_len > sizeof(ie_t))
  {
    size_t ie_len;
    ie = (ie_t *)ies_data;
    ...
    if (ie->id != WLAN_EID_EXTENSION ||
        ies_data[WLAN_EID_EXTENSION_ID_OFFSET] >= WLAN_EID_EXT_BSS_COLOR_CHG_ANN) {
      break;
    }
  ...
  /* push all data starts from EXT IE greater than CCA IE or
   * non EXT IE by 5 bytes to make room for CCA IE */
  if (ies_data[0] != WLAN_EID_EXTENSION ||
      ies_data[WLAN_EID_EXTENSION_ID_OFFSET] != WLAN_EID_EXT_BSS_COLOR_CHG_ANN) {
    if (ies_len > 0)
      memmove(ies_data + TOTAL_EXT_CCA_IE_SIZE, ies_data, ies_len);
  }

  /* Add CCA IE */
  *ies_data++ = WLAN_EID_EXTENSION;
  *ies_data++ = 3;
  *ies_data++ = WLAN_EID_EXT_BSS_COLOR_CHG_ANN;
  *ies_data++ = 0;
  *ies_data++ = core->color_change_color;
  *len += TOTAL_EXT_CCA_IE_SIZE;

  return MTLK_ERR_OK;
}

void
wave_reset_ssid_in_probe_response_buffer(uint8 *probe_resp_templ_loc, size_t len)
{
  u8 *ies_data;
  ie_t *ie;
  size_t ies_len, ies_data_offs;
...
_wave_reset_ssid_in_probe_response_templ(uint8 *buf, size_t len)
{
  ...
  wave_reset_ssid_in_probe_response_buffer(probe_resp_templ_loc, len);
...
#ifdef MTLK_WAVE_700
    _wave_check_and_free_mbssid_buffer(mtlk_vap_get_hw(core->vap_handle), &tmpl->mbssid_dma_addr, &tmpl->mbssid_ptr, &tmpl->mbssid_alloc_size);
#endif
  }
