...
typedef struct {
  /** Handle to the reading routine
    ...
    \param[in]      wlan_index wlan index of the card
  ...
  */
  mtlk_error_t (*read)(mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 *buff, uint32 *full_size);
  ...
  /** Handle to the writing routine
    ...
    \return
      error code on failure, MTLK_ERR_OK on success
  */
  mtlk_error_t (*write)(mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 const *buff, uint32 *full_size);
...
__mtlk_eeprom_gen7_get_offset (uint32 offset, uint8 wlan_index, dutNvMemoryType_e storage_type)
{
  if (storage_type == DUT_NV_MEMORY_EEPROM) {
    switch (wlan_index) {
      case 0:
        offset = offset + MTLK_WAV700_EEPROM_2_4G_OFFSET;
        break;
      case 2:
        offset = offset + MTLK_WAV700_EEPROM_5G_OFFSET;
        break;
      case 4:
        offset = offset + MTLK_WAV700_EEPROM_6G_OFFSET;
        break;
      default:
...
_mtlk_eeprom_irbd_eeprom_read (mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 *buf, uint32 *full_size)
{
  ...
  *full_size = 0;
  ...
  ret = eeprom_gpio_init(ccr, &eeprom_size, is_gen7, storage_type);
  if (MTLK_ERR_OK != ret)
  {
  ...
  *full_size = eeprom_size;
  ...
  else
  {
    ...
    /* Offset is different for each cal file on 8K EEPROM */
    if (is_gen7) {
      offset = __mtlk_eeprom_gen7_get_offset(offset, wlan_index, storage_type);
    }

    ret = eeprom_gpio_read(ccr, offset, buf, *size, is_gen7);
    if (MTLK_ERR_OK != ret)
    {
...
_mtlk_eeprom_irbd_eeprom_write (mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 const *buf, uint32 *full_size)
{
  ...
  *full_size = eeprom_size;
  ...
  else
  {
    ...
    ret = eeprom_gpio_write(ccr, offset, buf, *size, is_gen7);
    if (MTLK_ERR_OK != ret)
    {
...
*/
static mtlk_error_t
_mtlk_eeprom_irbd_flash_open (mtlk_eeprom_irbd_t *obj, dutFileType_e file_type, uint8 wlan_index)
{
  mtlk_error_t ret;
  _nvm_flash_handle_t *handle = &obj->flash_handle[file_type];
  mtlk_df_fw_file_buf_t fb;
  ...
  ILOG2_DDD("CID-%04x: DUT DRV load EEPROM data of file type %d and wlan_index %d from flash",
             oid, wlan_index, file_type);
  if (mtlk_hw_type_is_gen7((mtlk_vap_get_hwapi(obj->vap))->hw)) {
    mtlk_snprintf(fname, sizeof(fname), "cal_wlan%d.bin", wlan_index);
  } else {
    (void)mtlk_hw_get_prop(mtlk_vap_get_hwapi(obj->vap), prop_id, fname, sizeof(fname));
  }
...
*/
static mtlk_error_t
_mtlk_eeprom_irbd_flash_read (mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 *buf, uint32 *full_size)
{
  mtlk_error_t ret;
  _nvm_flash_handle_t *handle = &obj->flash_handle[file_type];
  ...
  *full_size = 0;
  ...
  ret = _mtlk_eeprom_irbd_flash_open(obj, file_type, wlan_index);
  if (MTLK_ERR_OK != ret)
    return ret;
...
*/
static mtlk_error_t
_mtlk_eeprom_irbd_flash_write (mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 const *buf, uint32 *full_size)
{
  mtlk_error_t ret;
  _nvm_flash_handle_t *handle = &obj->flash_handle[file_type];
...
*/
static mtlk_error_t
_mtlk_eeprom_irbd_efuse_read (mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 *buf, uint32 *full_size)
{
  MTLK_UNREFERENCED_PARAM(file_type);

  ILOG0_DDD("offset: %d, size: %d, wlan_index: %d", offset, *size, wlan_index);
...
*/
static mtlk_error_t
_mtlk_eeprom_irbd_efuse_write (mtlk_eeprom_irbd_t *obj, uint32 offset, dutFileType_e file_type, uint8 wlan_index, uint32 *size, uint8 const *buf, uint32 *full_size)
{
  MTLK_UNREFERENCED_PARAM(file_type);
                 ...
                 uint32             *size)
{
  ...
  wlan_index = msg->wlanIndex;
  ILOG1_DDDD("CID-%04x: DUT DRV request received: msgId %u msgLength %u, wlan_index : %u",
             oid, msgId, msgLength, wlan_index);
  ...
  switch(msgId)
  {
    case DUT_DGM_READ_NV_MEMORY_REQ:
    {
      ...
      err = ops->read(obj, address, fileType, wlan_index, &length, param->access.data, &eepromSize);
    ...
    case DUT_DGM_WRITE_NV_MEMORY_REQ:
    {
      ...
      err = ops->write(obj, address, fileType, wlan_index, &length, param->access.data, &eepromSize);
