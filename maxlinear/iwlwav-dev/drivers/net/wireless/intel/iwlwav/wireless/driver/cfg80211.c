  ...
  void *data, int data_len)
{
  ...
  {
    /* Under OneWifi need to send event via first non-master vap (due to master vap is not fully initialized) */
    BOOL found = FALSE;
    mtlk_vap_handle_t s_handle;
    unsigned vaps_count, vap_id;
    mtlk_vap_manager_t *vap_mgr = mtlk_vap_get_manager(vap_handle);
    vaps_count = mtlk_vap_manager_get_max_vaps_count(vap_mgr);

    for (vap_id = 0; vap_id < vaps_count; vap_id++) {
        if (mtlk_vap_manager_get_vap_handle(vap_mgr, vap_id, &s_handle) == MTLK_ERR_OK) {
          if (mtlk_vap_is_master(s_handle))
            continue;
          found = TRUE;
          break;
        }
    ...
    if (!found) {
      ELOG_D("CID-%04x: cannot find first non-master AP", mtlk_vap_get_oid(vap_handle));
  ...
  df = mtlk_vap_get_df(vap_handle);
  df_user = mtlk_df_get_user(df);
  wdev = mtlk_df_user_get_wdev(df_user);

  MTLK_ASSERT(NULL != wdev);

  ILOG0_D("CID-%04x: Sending EVENT_FLUSH_STATIONS", mtlk_vap_get_oid(vap_handle));
...
int __MTLK_IFUNC
wv_cfg80211_send_vendor_radar_event (struct wireless_dev *wdev, void *data, int data_len)
{
  mtlk_nbuf_t *evt_nbuf;
  uint8 *cp;
  ...
  evt_nbuf = wv_cfg80211_vendor_event_alloc(wdev, data_len, LTQ_NL80211_VENDOR_EVENT_RADAR_DETECTED);
  if (!evt_nbuf)
  {
    ELOG_D("Malloc event fail. data_len = %d", data_len);
    return MTLK_ERR_NO_MEM;
  ...
  wave_memcpy(cp, data_len, data, data_len);
  wv_cfg80211_vendor_event(evt_nbuf);
  ILOG1_S("%s: Sending Radar vendor event", wdev->netdev->name);
...
void __MTLK_IFUNC
wv_cfg80211_set_dfs_state_bit_map (struct wiphy *wiphy,
                                   ...
                                   enum nl80211_dfs_state state)
{
  ...
  cfg80211_set_dfs_state_bit_map(wiphy, &chandef, rbm, state);

  if (NL80211_DFS_UNAVAILABLE == state) {
...
int __MTLK_IFUNC
wv_cfg80211_get_prop_phy_cap (struct wiphy *wiphy, struct net_device *ndev)
{
  ...
  res = _mtlk_df_user_invoke_core(mtlk_df_user_get_master_df(df_user), WAVE_RADIO_REQ_GET_PROP_PHY_CAP, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval(res, clpb, WAVE_RADIO_REQ_GET_PROP_PHY_CAP, FALSE);
  if (res != MTLK_ERR_OK)
    goto end;

  cap = mtlk_clpb_enum_get_next(clpb, &cap_size);

  MTLK_CLPB_TRY(cap, cap_size)
    res = wv_cfg80211_vendor_cmd_alloc_and_reply(wiphy, cap, cap_size);
