...
/* Don't remove the next line, it is needed to create a LOG function for use inside _mtlk_aux_print_hex_ex() */
/* RLOG_S("%s", ""); */     /* create __RLOG_S_8() */
...
_mtlk_aux_print_hex_ex (const void *buf, unsigned int l, const char *fname, uint8 oid, uint8 gid, uint8 fid, uint16 lid, int level)
{
  ...
  if (!_mtlk_check_snprintf(res, sizeof(lbuf)))
    return;
  __RLOG_S_8(fname, level, oid, gid, fid, lid, "%s", lbuf);
...
void __mtlk_dump(const void *buf, uint32 len, char *str, const char *fname, uint8 oid, uint8 gid, uint8 fid, uint16 lid, int level)
{
  ...
  if(flags)
  {
    __RLOG_S_8(fname, level, oid, gid, fid, lid, "%s", str);
...
void __MTLK_IFUNC get_eht_mcs_nss_20mhz_only(const u8 *ap_eht_mcs_nss,
                    u8 *sta_eht_mcs_nss, u8 *neg_eht_mcs_nss, u8 len)
{
  u8 j, nss_rx, nss_tx;
  struct ieee80211_eht_mcs_nss_supp_20mhz_only tmp;

  if (len != sizeof(tmp.rx_tx_max_nss))
    return;

  for (j = 0; j < len; j++) {
    nss_rx = MIN((ap_eht_mcs_nss[j] & 0x0F), (sta_eht_mcs_nss[j] & 0x0F));
    nss_tx = MIN((ap_eht_mcs_nss[j] >> 4) & 0x0F, (sta_eht_mcs_nss[j] >> 4) & 0x0F);
    neg_eht_mcs_nss[j] = (nss_rx | (u8)(nss_tx << 4));
  }
}

void __MTLK_IFUNC get_eht_mcs_nss_any_bw(const u8 *ap_eht_mcs_nss,
                    u8 *sta_eht_mcs_nss,  u8 *neg_eht_mcs_nss, u8 len)
{
  u8 j, nss_rx, nss_tx;
  struct ieee80211_eht_mcs_nss_supp_bw tmp;
