...
#define GEN7_PROGMODEL_COUNT     4
/* make sure this is maximum progmodels count */
#define PROGMODEL_COUNT_MAX      GEN7_PROGMODEL_COUNT
...
_wave_progmodel_eeprom_get(mtlk_hw_api_t *hwapi)
{
  ...
  (void)mtlk_hw_get_prop(hwapi, MTLK_HW_PROP_EEPROM_DATA, &ee_data, sizeof(typeof(ee_data)));
  return ee_data;
}
...
_wave_progmodel_init (mtlk_txmm_t *txmm, wave_progmodel_t *fw, wave_radio_t *radio, int freq)
{
  ...
  if (MTLK_ERR_OK != mtlk_hw_get_prop(hwapi, MTLK_HW_PROP_CHIP_NAME,
                         &fw->prgmdl_params.chip_name, sizeof(typeof(fw->prgmdl_params.chip_name))))
  {
    goto ERROR;
  }
  ...
  CARD_SELECTOR_START(card_type);
    IF_CARD_G6( fw->prgmdl_params.count = GEN6_PROGMODEL_COUNT; );
    IF_CARD_G7( fw->prgmdl_params.count = GEN7_PROGMODEL_COUNT; );
  CARD_SELECTOR_END();
...
static int
_wave_progmodel_prepare_string (const wave_progmodel_t *fw)
{
  wave_progmodel_data_t pm_data;
  int                   res;

  pm_data.txmm = fw->txmm;  /* txmm is required for loading per radio */
  pm_data.ff = (mtlk_core_firmware_file_t *)&fw->ff[MTLK_FILE_TYPE_HW];
  res = mtlk_hw_set_prop(fw->txmm->hw_api, MTLK_HW_PROGMODEL_STR, &pm_data, 0);
  if (res != MTLK_ERR_OK) {
    ELOG_SD("ERROR: Progmodel (%s) string preparation failed. Err#%d", fw->ff[MTLK_FILE_TYPE_HW].fname, res);
    goto FINISH;
  }

  ILOG0_PD("Saving in radio at %p last_pm_freq %d",
            fw->radio, fw->prgmdl_params.freq);
            wave_radio_last_pm_freq_set(fw->radio, fw->prgmdl_params.freq);
FINISH:
  return res;
...
int __MTLK_IFUNC
wave_progmodel_load(mtlk_txmm_t *txmm, wave_radio_t *radio, int freq, BOOL is_gen7)
{
  wave_progmodel_t *fw;
  int res;
  ...
  if (is_gen7) {
    _wave_progmodel_get_fname(fw);
    res = _wave_progmodel_prepare_loading_to_hw(fw);
    if (res != MTLK_ERR_OK) {
      ELOG_D("ERROR: Progmodel load prepare loading to hw failed for g7. Err#%d", res);
      goto FINISH;
    }
    _wave_progmodel_prepare_string(fw);
  } else {
    /* Check if we need to load progmodels */
...
_wave_progmodel_get_fname (wave_progmodel_t *fw)
{
  ...
  IF_CARD_G7(
    /* file 0: ProgModel_gen7_Common.bin */
    _wave_progmodel_compose_ff_data(fw, 0, MTLK_FILE_TYPE_HW, "Common", fpga_suffix);
    /* file 1: ProgModel_gen7_Seg.bin */
    _wave_progmodel_compose_ff_data(fw, 1, MTLK_FILE_TYPE_SEG, "Seg", fpga_suffix);
    /* file 2: ProgModel_gen7_Td_Ant.bin */
    _wave_progmodel_compose_ff_data(fw, 2, MTLK_FILE_TYPE_TD_ANT, "Td_Ant", fpga_suffix);
    /* file 3: ProgModel_gen7_DFS.bin */
    _wave_progmodel_compose_ff_data(fw, 3, MTLK_FILE_TYPE_DFS, "DFS", fpga_suffix);
  );
  CARD_SELECTOR_END();
}
