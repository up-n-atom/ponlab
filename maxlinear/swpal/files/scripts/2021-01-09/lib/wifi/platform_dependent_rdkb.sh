...
export RADIO_MAP_FILE_1="/nvram/etc/radio_map_file"
...
export DUMP_FILES_PATH="/nvram/etc"
export WAVE_PATH="/nvram/etc/wave"
# generic writable directory for scripts which need to store state
export PERSISTENT_STORAGE="/nvram/etc"
...
export BEEROCKS_LOG=/rdklogs/logs/beerocks
...
function get_is_zwdfs_phy(){
        get_is_zwdfs_from_phy "$@"
}
...
prepare_vars(){
        ...
        rm $RADIO_MAP_FILE_1 2> /dev/null
        rm /nvram/etc/config/radio_map_file 2> /dev/null
...
clean_nvram(){
        rm -f /nvram/etc/config/network /nvram/etc/config/prplmesh /nvram/etc/config/prplmesh_db /nvram/etc/config/wireless /nvram/etc/fw_dump* /nvram/beerocks*.conf
}
