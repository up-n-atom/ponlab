...
function check_meta_factory() {
	if [ ! -f $UCI_DB_PATH/meta-factory ]; then
		...
		local fastpath=`$UCI -c $DEFAULT_DB_PATH/ get meta-factory.driver.fastpath`
		$UCI set meta-factory.driver.fastpath=$fastpath
...
update_mac_address()
{
	...
	board_mac1 board_mac23 board_mac46 vap_mac4 vap_mac5 vap_mac6 phy phy_idx is_zwdfs
	...
	[ "$radio_name" = "wlan2" ] && phy_offset=32
	[ "$radio_name" = "wlan4" ] && phy_offset=48
	...
	phy_idx=`iw $interface_name info | awk '$1=="wiphy" {print $2}'`
	if [ ! -z "$phy_idx" ] ; then
		phy="phy$phy_idx"
		is_zwdfs=`get_is_zwdfs_phy $phy`
	fi

	rdk_error=0
	if [ "$OS_NAME" = "RDKB" ]; then
		# prefer to use itstore_mac_db, otherwise fall back to scapiutil
		if command -v itstore_mac_db >/dev/null; then
			...
			if echo "$interface_name" | grep -q '^wlan4\.'; then
				/usr/sbin/scapiutil get_MbssidNextMacaddr $interface_name public || rdk_error=1
			else
				/usr/sbin/scapiutil get_nextMac $interface_name public || rdk_error=1
			fi
		fi
	fi
	...
	if [ "$OS_NAME" = "UGW" -o "$rdk_error" = "1" ]; then
		...
		board_mac46="$(($board_mac46 & 0xFFFFF0))" # suppress last 4 bits for mbssid calculation
...
function get_index_from_map_file(){
	local phy_name=$1
	local line=$(echo "$radio_map_file_content" | grep $phy_name)
	get_index_from_map_file_return_value=$(echo ${line//[a-z]/} | awk '{print $2}')
}

function validate_radio_map_file(){
	local radio_map_file_name=''

	# copy radio_map_file
	if [ -f "/usr/bin/OneWifi" ]; then
		radio_map_file_name=$DEFAULT_DB_PATH/radio_map_file
	else
		# don't copy radio_map_file if WAVE700 card presents (the default config is used)
		local is_wave700=$(lspci -n | grep -c -E "$LTQ_CODE_WAVE700")
		if [ $is_wave700 == 0 ]; then
			radio_map_file_name=$DEFAULT_DB_PATH/radio_map_file
		fi
	fi

	if [ -f "$radio_map_file_name" ]; then
		cp $radio_map_file_name $RADIO_MAP_FILE_1
		RADIO_MAP_FILE=$RADIO_MAP_FILE_1
	elif [ -f "$RADIO_MAP_FILE_1" ]; then
		rm $RADIO_MAP_FILE_1
	fi
	...
	for phy in $phys; do
		...
		if [ -z "$radio_index" ]; then
			if [ `get_is_zwdfs_phy $phy` = "0" ]; then
				print_logs "RADIO_MAP_FILE is invalid, $phy not provided"
				radio_map_file_content=
				return
			fi
		fi
...
function full_reset_radio(){
	...
	local phy_idx=`iw $iface_name info | awk '$1=="wiphy" {print $2}'`
...
function reset_radio(){
	...
	local greylist_file=`uci show wireless | grep greylist_file \
	| grep $radio | cut -d"=" -f2`
	local greylist_logpath=`uci show wireless | grep greylist_logpath \
	| grep $radio | cut -d"=" -f2`
...
get_iface_idx_on_device(){
	...
	if [ `get_is_zwdfs_phy $1` = "1" ]; then
		iface_idx=6
	fi
}

get_band_on_device(){
	if iw $1 info | grep -Eq "\* 6...(\.[0-9])? MHz"
	then
		echo 6
	elif iw $1 info | grep -Eq "\* 5...(\.[0-9])? MHz"
	then
		echo 5
	elif iw $1 info | grep -Eq "\* 24..(\.[0-9])? MHz"
	then
		echo 2
	fi
}
...
function get_is_zwdfs_from_phy(){
	`iw ${1} info | grep "Device is meant only for DFS" > /dev/null`
	if [ $? -eq 0 ]; then
		echo "1"
	else
		echo "0"
