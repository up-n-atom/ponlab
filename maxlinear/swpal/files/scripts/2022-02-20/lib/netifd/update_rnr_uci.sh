...
if [ "$1" == "unsolicited_frame" ]; then
	...
	do
		radio_6GHz=`echo $i | cut -d"." -f2`
		radio_6GHz_array=$radio_6GHz_array" "$radio_6GHz
		case "$2" in
		disable)
			ubus_call_set "$radio_6GHz" "unsol_bcast_probe_resp_interval" "0"
			ubus_call_set "$radio_6GHz" "fils_discovery_max_interval" "0"
			;;
		probe)
			ubus_call_set "$radio_6GHz" "unsol_bcast_probe_resp_interval" "20"
			ubus_call_set "$radio_6GHz" "fils_discovery_max_interval" "0"
			;;
		fils)
			ubus_call_set "$radio_6GHz" "fils_discovery_max_interval" "20"
			ubus_call_set "$radio_6GHz" "unsol_bcast_probe_resp_interval" "0"
			;;
		esac
	done
	ubus_call_commit
	# On disable, force wifi reconf and wait for it's completion
	[ "$2" = "disable" ] && wifi reconf && sleep 10
	exit
fi
	...
	if [ $1 = "enable" ]; then
		...
		*)
			...
			;;
				...
				hidden_mode=`uci show $radio | grep ignore_broadcast_ssid | cut -d"=" -f2 | tr -d "'"`
                [ -z "$hidden_mode" ] && hidden_mode=0
		...
		max_tx_power="$(iw $phy_6g info | grep -F "[$channel]"  | grep -o '([0-9]*' | tr -d '(' )"
		...
		do
			...
			ubus_call_add_list "wifi-device" "$j" "colocated_6g_vap_info" "0 $bssid $ssid $multibss_enable
			$is_transmitted_bssid $set_20_tu_probe_response $max_tx_power $hidden_mode"
		done
		for j in $default_radio_5g
		do
			ubus_call_set "$j" "colocated_6g_radio_info" "0 $op_class $channel"
