...
process_num=`ps | grep $0 | grep -v "grep" | wc -l`
if [ $process_num -gt 2 ]; then
    #echo "$0 : process is already running process num:$process_num" > /dev/console
    return
fi

#This is to provide a delay if the sigma agent tries to run before the bridge-lan interface is up
is_ifconfig_up()
{
	if_config_status=`ifconfig br-lan | grep "inet addr"`
}
while [ "$if_config_status" = "" ]; do
	is_ifconfig_up
	if [ "$if_config_status" = "" ]; then
		echo "Bridge Interface is not up"
		sleep 5
	fi
done
