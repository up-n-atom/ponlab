...
MT_UBYTE ReadFileParmterToReadfrom_WL()
{
    ...
    while ((read = fscanf(fp,"%08x %08x %08x",&chip_mode,&Add_val,&len)) == 3)
    {
...
MT_UINT32 TFTP_Get(MT_BYTE* source_path, MT_BYTE* dest_path)
{
    ...
    if (dest_path[0] != '/') {
        if(snprintf(destPath, sizeof(destPath), "%s/%s", __FTP_PATH__, dest_path) >= sizeof(destPath)) {
            strcpy_s((char*) source_path, MAX_PATH_SIZE, "Error: dst path too long.");
            return MT_RET_FAIL; 
        }
    } else {
        if (snprintf(destPath, sizeof(destPath), "%s", dest_path) >= sizeof(destPath)) {
            strcpy_s((char*) source_path, MAX_PATH_SIZE, "Error: dst path too long.");
            return MT_RET_FAIL;
        }
    }

    if (sprintf_s(cmd, sizeof(cmd), "tftp -g -r \"%s\" -l \"%s\" \"%s\" ", source_path, destPath, safe_ip) <= 0) {
        strcpy_s((char*) source_path, MAX_PATH_SIZE, "Error: src or dst paths too long.");
        return MT_RET_FAIL; 
    }
...
MT_UINT32 TFTP_Put(MT_BYTE* source_path, MT_BYTE* dest_path)
{
    ...
    /* Use the /tmp path unless a full path is given */
    if (source_path[0] != '/') {
        if (snprintf(srcPath, sizeof(srcPath), "%s/%s", __FTP_PATH__, source_path) >= sizeof(srcPath)) {
            strcpy_s((char*) source_path, MAX_PATH_SIZE, "Error: src path too long.");
            return MT_RET_FAIL;
        }
    } else {
        if (sprintf_s(srcPath, sizeof(srcPath), "%s", source_path) <= 0) {
            strcpy_s((char*) source_path, MAX_PATH_SIZE, "Error: src path too long.");
            return MT_RET_FAIL;
        }
    }

    if (sprintf_s(cmd, sizeof(cmd), "tftp -p -l \"%s\" -r \"%s\" \"%s\" ", srcPath, dest_path, safe_ip) <= 0) {
        strcpy_s((char*) source_path, MAX_PATH_SIZE, "Error: src or dst paths too long.");
        return MT_RET_FAIL;
    }
