...
MT_UINTPTR MT_BCL_arg[MAX_PARAM_COUNT] = {0};										/*Contains the arguments for the BCL function to execute. */
    ...
    *IL_error = 0;
    ...
    while(i < MAX_PARAM_COUNT)
    {
        if(bsILPtrArgs & 1) /*If the bit is lit... */
        {
            ...
            MT_BCL_arg[i] = (MT_UINTPTR) & MT_BCL_item_list[MT_BCL_arg[i]];
        }
        i++;
...
int executeBCLCommand(char *cmd_line, int length)
{
    ...
    for(i = 0; i < num_cmds; i++)
    {
        ...
        {
            MT_UINTPTR result;    /*Will hold the result returned from the WrapperFunction. */
            int IL_error;    
