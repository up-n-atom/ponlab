...
save_whm_dumps:
  ...
  if ((event->layer_id == WHM_DRIVER_TRIGGER) && (event->id == MAC_FATAL_EVENT_ID)) {
    sprintf_res = sprintf_s(tar_file_name, sizeof (tar_file_name),
                          "whm_dump_%d_%02d_%02d_%02d_%02d_%02d_%lu_%s.tar.gz",
                          tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
                          tm->tm_hour, tm->tm_min, tm->tm_sec, time_stamp_ms, "MAC_FATAL");
  } else {
    sprintf_res = sprintf_s(tar_file_name, sizeof (tar_file_name),
                          "whm_dump_%d_%02d_%02d_%02d_%02d_%02d_%lu_%s_W%d.tar.gz",
                          tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
...
_print_help (const char *app_name)
{
  ...
  sprintf_res = sprintf_s(version, sizeof(version), "%s: %s\n%s %u %s",
                          "WHM dump evacuation application version",
                          WHM_HANDLER_VERSION,
                          "Debug: echo",
                          LOG_LOCAL_OID,
...
int whm_handler_dwpald_term_cond_get(void)
{
  return g_wh_config.terminate;
}

static void whm_handler_dwpald_term_cond_set(BOOL val)
{
  g_wh_config.terminate = val;
}
...
static int driver_whm_dump_event_handler(char *ifname, int drv_event_id, void *data, size_t len)
{
  ...
  if (MTLK_ERR_FILEOP == res) {
    whm_handler_dwpald_term_cond_set(TRUE);
  }
...
again:
  ...
  ret = dwpald_start_blocked_listen(whm_handler_dwpald_term_cond_get);
