...
typedef enum
{
  ...
  MISC_DBSTATUS = 0x80000000,/*!< Get DB status bit */
}Misc;
...
typedef enum
{
   ...
   OPT_THREAD_ENVL = 0x20, /*!< consider thread handling in ubus communication */
   OBJOPT_NO_ACTION = 0x40, /*!< No action on incoming object, ignore the object from any get,set operation.
				when this flag set on any object, csd will ignore DB operation on the node,
				currently the intent of this flag is to return positive response back to Management Entity, 
				The Positive Response will indicate the Operation is successful, but it has an accompanying Message 
				that needs to be displayed (in case of WebUI)*/
   OBJOPT_SKIP_COMMIT = 0x80, /*!< Skip UCI commit but the DB should be updated (return objlist to servd) */
   OBJOPT_DO_COMMIT = 0x100 /*!< Do UCI commit but don't update the DB (do not return objlist to servd) */
}ObjOper;
...
typedef enum
{
   ...
   MOPT_MESSAGE = 0x4000,/*!< Message transfer from external daemons */
   MOPT_PREWATCH = 0x8000,/*!< GET or SET operations to Pre-Watch list */
   MOPT_POSTWATCH = 0x10000,/*!< GET or SET operations to Post-watch list */
   MOPT_COMTREE = 0x20000,/*!< complete tree GET request */
}MainOper;
