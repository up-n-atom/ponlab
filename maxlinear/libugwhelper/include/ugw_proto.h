...
#ifndef memset_ss
#define memset_ss(buf,size,token) memset_s(buf,size,token,size)
#endif
...
/*!  \brief API to send msg to server, which is registered with UBUSD
  ...
  \param[in] nTimeout ubus timeout period
...
/*!
	\brief API to get total object and parameter count
	\param[in] head msg head
	\param[out] pxDataModStat object and parameter count
	\return 
*/
int help_blobToDataModStat(struct blob_attr *head, DataModelStat *pxDataModStat);

/*!
	\brief API to get time in millisecods
	\return 
*/
int64_t help_getTimeMsec(void);

/*!
	\brief API to return notification id to string
	\param[in] nErrCode to return the string format id
	\return
*/
char *getNIDString(int nErrCode);

/*!
	\brief API to retrive data from the file
	\param[in] pcPath path to fetch the data
	\param[out] pxMsg msg header to fill the data from the file
	\return UGW_SUCCESS/UGW_FAILURE
*/
int help_retrieveRespData(IN const char *pcPath, OUT MsgHeader *pxMsg);

/*!
	\brief API to store the data in file
	\param[in] pcPath path to store the data
	\param[in] pxMsg msg header to store in file
	\return UGW_SUCCESS/UGW_FAILURE
*/
