...
int32_t scapi_startCrond(void)
{
	...
	if (scapi_fileExists(CRON_ROOT_FILE) == UGW_FAILURE) {
		/*CRON_ROOT_FILE is not available create one */
		if ((fCron = scapi_getFilePtr(CRON_ROOT_FILE, "w")) != NULL) {
			fclose(fCron);
		} else {
	...
	/* Mandatory argumetns */
	if (sprintf_s (xProcdObj.sServiceName, MAX_LEN_PARAM_VALUE_D, "crond") <= 0) {
                LOGF_LOG_ERROR( "Error : sprintf_s failed %s\n", strerror(errno));
		nRet=-1;
		goto end;
	}
	if (sprintf_s(xProcdObj.sCommand, MAX_LEN_VALID_VALUE_D, "crond -f -c %s", CRON_DIR) <= 0) {
                LOGF_LOG_ERROR( "Error : sprintf_s failed %s\n", strerror(errno));
		nRet=-1;
		goto end;
	}
	...
	/* Updating conf file */
	if (sprintf_s(xProcdObj.sConfFile, sizeof(xProcdObj.sConfFile), "%s", CRON_ROOT_FILE) <=0) {
                LOGF_LOG_ERROR( "Error : sprintf_s failed %s\n", strerror(errno));
		nRet=-1;
		goto end;
	}
...
int32_t scapi_setCronjob(IN char *pcMin, IN char *pcHour, IN char *pcDayOfMon, IN char *pcMonth, IN char *pcDayOfWeek, IN char *pcCommand, IN int32_t nAction)
{
	...
	if (sprintf_s(sCmd, MAX_LEN_VALID_VALUE_D, "%s %s %s %s %s %s", pcMin, pcHour, pcDayOfMon, pcMonth, pcDayOfWeek, pcCommand) <= 0) {
                LOGF_LOG_ERROR( "Error : sprintf_s failed %s\n", strerror(errno));
		nRet = -EXIT_FAILURE;
		goto end;
	}
	/* Open CRON_ROOT_FILE for reading */
	fCron = scapi_getFilePtr(CRON_ROOT_FILE, "r");
	if (!fCron) {
	...
	fCronTemp = scapi_getFilePtr(CRON_ROOT_FILE_TEMP, "a+");
	if (!fCronTemp) {
	...
	if (nAction == ADD_ENTRY) {
		while ((fgets(sLine, MAX_LEN_VALID_VALUE_D, fCron)) != NULL) {
			if ((bEntryFound != true) && (strncmp(sLine, sCmd, strnlen_s(sCmd,MAX_LEN_VALID_VALUE_D)) == 0)) {
		...
		while ((fgets(sLine, MAX_LEN_VALID_VALUE_D, fCron)) != NULL) {
			if (strncmp(sLine, sCmd, strnlen_s(sCmd,MAX_LEN_VALID_VALUE_D)) != 0) {
				fprintf(fCronTemp, "%s", sLine);
			}
		}
	}
