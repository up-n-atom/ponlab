...
int8 UGW_TLIB_TimersInit(uint16 unNumOfTimers, uchar8 ucTimerNotifyType)
{
	...
	{
		if (scapi_fileExists("/dev/voip_timer_driver") == UGW_SUCCESS){         
			scapi_spawn("mknod /dev/voip_timer_driver c 229 0", SCAPI_BLOCK, &nChildRet);
		}
		...
		void *pCallBackFnParm)
{
	...
	if(vucTimerNotifyType == UGW_TLIB_NOTIFY_VIA_SIGNAL){	
retry_lck:
		if (pthread_mutex_lock(&global_timer_mutex) != 0 ) { 
			if (nCnt < MAX_TRIAL) {
				return UGW_TLIB_FAIL;
			} else {
				nCnt++;
	...
	if(UGW_TLIB_GetTimerNode(&pxTempNode) != UGW_TLIB_FAIL)
	{
		*punTimerId = pxTempNode->unTimerIndex;
		...
		if(vucTimerNotifyType == UGW_TLIB_NOTIFY_VIA_SIGNAL){	
retry_unlk:
			if (pthread_mutex_unlock(&global_timer_mutex) != 0 ) {
				if (nCnt < MAX_TRIAL) {
					return UGW_TLIB_FAIL;
				} else {
					nCnt++;
