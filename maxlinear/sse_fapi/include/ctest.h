...
void assert_true_cmp(long int exp, long int real, const char* caller, int line);
#define ASSERT_TRUE_CMP(exp, real) assert_true_cmp(exp, real, __FILE__, __LINE__)
...
void assert_true_cmp(long int exp, long int real, const char* caller, int line) {
    if (exp > real) {
        CTEST_ERR("%s:%d  should be true exp %ld greater then real %ld", caller, line, exp, real);
    }
}
