...
#define MAX_NUM_RRM_BEACON_RPTS 100
...
#if 0
static DwpalService dwpalService[] = { { DWPAL_CONN_TYPE_HOSTAP, "wlan0", "TWO_WAY", -1, -1 },  /* Will send commands and get events on the same socket */
#else
static DwpalService dwpalService[] = { { DWPAL_CONN_TYPE_HOSTAP, "wlan0", "ONE_WAY", -1, -1 },  /* Will send commands and get events on a different socket  */
#endif
                                       { DWPAL_CONN_TYPE_HOSTAP, "wlan1", "ONE_WAY", -1, -1 },
                                       { DWPAL_CONN_TYPE_HOSTAP, "wlan2", "ONE_WAY", -1, -1 },  /* Will send commands and get events on a different socket */
                                       { DWPAL_CONN_TYPE_HOSTAP, "wlan3", "ONE_WAY", -1, -1 },
                                       { DWPAL_CONN_TYPE_HOSTAP, "wlan4", "ONE_WAY", -1, -1 },
                                       { DWPAL_CONN_TYPE_HOSTAP, "wlan5", "ONE_WAY", -1, -1 },
                                       { DWPAL_CONN_TYPE_DRIVER, "ALL",   "TWO_WAY", -1, -1 } };
...
/* Internal implementation of strndup_s(). Seems, safelib doesn't implement such function */
static char* _strndup_s(const char *s, size_t maxlen)
{
	if (NULL == s)
		return NULL;
	{
		size_t size = strnlen_s(s, maxlen) + 1;
		char* dup = (char*)malloc(size * sizeof(char));
		if (dup) {
			strncpy_s(dup, size, s, maxlen);
		}
		return dup;
	}
}
...
DWPAL_Ret nlCliOneShotCalFileReadyCallback(char *ifname, size_t len, unsigned char *data)
{
	...
	{
		console_printf("%s; can't write '%zu' bytes to temporary file '%s' [%d]", __FUNCTION__, len, fname_template, written);
		ret = DWPAL_FAILURE;
	}
	else
	{
...
DWPAL_Ret nlCliOneShotEventCallback(char *ifname, int event, int subevent, size_t len, unsigned char *data)
{
	...
	if (event == NL80211_CMD_VENDOR)
	{
		switch (subevent)
		{
			...
			case LTQ_NL80211_VENDOR_EVENT_UNRECOVERABLE_ERROR:
				if (!strncmp("UNRECOVERABLE_ERROR", opCodeForListener[0], sizeof("UNRECOVERABLE_ERROR") - 1))
				{
					console_printf_err("%s; LTQ_NL80211_VENDOR_EVENT_UNRECOVERABLE_ERROR received from %s\n", __FUNCTION__, ifname);
					exit(0); /* Terminate the process */
				}
				break;
			...
			default:
				console_printf("%s; unsupported vendor sub event received; ifname= '%s', event= %d, subevent= %d (len= %zu)\n",
...
DWPAL_Ret nlCliEventCallback(char *ifname, int event, int subevent, size_t len, unsigned char *data)
{
	...
	console_printf("%s Entry; ifname= '%s', event= %d, subevent= %d (len= %zu), data= %p, lenToPrint= %zu\n", __FUNCTION__, ifname, event, subevent, len, (void *)data, lenToPrint);
...
static char *dupstr(const char *s)
{
	...
	{
		console_printf_err("Error: Out of memory. Exiting\n");
		sigterm_handler();
	}
	else
	{
...
static int interfaceIndexGet(DwpalConnectionType connectionType, const char *radioName)
{
	int    i;
	size_t numOfInterfaces = ARRAY_SIZE(dwpalService);
...
static DWPAL_Ret dwpal_acs_report_handle(void *localContext, char *VAPName, bool isDwpalExtenderMode)
{
	...
	else
	{
		...
		if (resultsPrint(fieldsToParse, ARRAY_SIZE(acs_report), sizeof(DWPAL_acs_report_get)) == false)
		{
			console_printf("%s; resultsPrint ERROR ==> Abort!\n", __FUNCTION__);
		}
...
static DWPAL_Ret dwpal_beacon_report_handle(char *ifName, unsigned int reportIx, char *output, int output_size)
{
	...
	if(dwpal_ext_interfaceIndexGet(DWPAL_CONN_TYPE_HOSTAP, ifName, &ifidx) != DWPAL_SUCCESS)
	{
...
static void _dfs_radar_detected_store_single(DWPAL_dfs_radar_detected_parsed *dfs_radar_event)
{
	...
	*saved_dfs_radar_event = *dfs_radar_event;
	...
	console_printf("%s; DFS-RADAR-DETECTED: ifName=%s timestamp=%ld freq=%d chan_width=%d cf1=%d\n",
			__FUNCTION__, ifName,
...
static void _dfs_radar_detected_store(DWPAL_dfs_radar_detected_event *dfs_radar_detected_event)
{
	...
	dfs_radar_event->timestamp = dwpal_get_uptime();
...
static DWPAL_Ret dwpal_radar_event_handle(char *ifName, unsigned int reportIx, char *output, int output_size)
{
	...
	if(!strncmp(ifName, dfs_radar_detected_event->VAPName, sizeof(dfs_radar_detected_event->VAPName))) {
		ret = sprintf_s(output, output_size, "ifName=%s reportIx=%u, primary_channel=%u bandwidth=%u autobandwidth=0 has_autobandwidth=0"
			" center=%d has_center=%d, timestamp=%ld\n",
...
static DWPAL_Ret dwpal_sta_table_update(DWPAL_sta_connected_event *sta_connected_event, int oper)
{
	...
	int i = 0, oldest_sta = 0, found = 0;

	uptime = dwpal_get_uptime();
...
static DWPAL_Ret dwpal_ap_sta_connected_event_parse(char *msg, size_t msgLen)
{
	...
	sta_connected_event = (DWPAL_sta_connected_event *)malloc(sizeof(DWPAL_sta_connected_event));
...
static bool boolFromStringGet(const char *boolString)
{
	if (boolString == NULL)
	{
	...
	else if ( (!strncmp(boolString, "false", sizeof("false") - 1)) || (!strncmp(boolString, "0", 1)) )
	{
		return false;
	}
	else
...
static void scanParametersGet(char *strOfParams, ScanParams *scanParams)
{
	...
	dmaxLen = strnlen_s(strOfParams, DWPAL_CLI_LINE_STRING_LENGTH);

	console_printf("%s; dmaxLen= %zu; strOfParams= '%s'\n", __FUNCTION__, dmaxLen, strOfParams);
...
static int dwpal_get_radio_noise(char *ifName, bool isDwpalExtenderMode, char *output, int output_size)
{
	int idx, ret;
	DWPAL_radio_info_get radio_info;
	if ((idx = interfaceIndexGet(DWPAL_CONN_TYPE_HOSTAP, ifName)) == -1)
	{
...
static int dwpalOneShotEventCallback(char *radioName, char *opCode, char *msg, size_t msgStringLen)
{
	...
	console_printf("%s; radioName= '%s', opCode= '%s', msgStringLen= %zu (numOfListeningEvents= %d)\n",
		...
		if (!strncmp(opCode, opCodeForListener[i], DWPAL_OPCODE_STRING_LENGTH))
		{
			if (msg != NULL)
			{
				...
				{  /* Handle regular events */
					char *localMsg = _strndup_s(msg, msgStringLen);

					if (localMsg == NULL)
					{
						console_printf("%s; _strndup_s failed ==> Exit!!!\n", __FUNCTION__);
						return (int)DWPAL_FAILURE;
					}
...
static int dwpalExtEventCallback(char *radioName, char *opCode, char *msg, size_t msgStringLen)
{
	...
	console_printf("%s; radioName= '%s', opCode= '%s', msgStringLen= %zu, msg= '%s'\n", __FUNCTION__, radioName, opCode, msgStringLen, msg);
	return 0;
}
#endif /* DISABLE_DWPAL_HOSTAP_SUPPORT */
...
{
#ifndef DISABLE_DWPAL_HOSTAP_SUPPORT
	if (DWPAL_CONN_TYPE_HOSTAP == dwpalServiceLocal->connectionType)
	{
...
#endif /* DISABLE_DWPAL_HOSTAP_SUPPORT */
	if (DWPAL_CONN_TYPE_DRIVER == dwpalServiceLocal->connectionType)
	{
...
static DWPAL_Ret interfaceSet(DwpalService *dwpalServiceLocal, int idx)
{
	...
	else if (DWPAL_CONN_TYPE_DRIVER == dwpalServiceLocal->connectionType)
	{
...
{
	int     i, highestValFD, ret, numOfServices = ARRAY_SIZE(dwpalService);
#ifndef DISABLE_DWPAL_HOSTAP_SUPPORT
	char    *msg;
		...
		for (i=0; i < numOfServices; i++)
		{
			...
			if (DWPAL_CONN_TYPE_HOSTAP == dwpalService[i].connectionType)
			{
				if (dwpalService[i].fd > 0)
				{
					...
					{
						/*console_printf("%s; event received; connectionType= '%s', radioName= '%s', serviceName= '%s'\n",
						       __FUNCTION__, connectionTypeToStr(dwpalService[i].connectionType), dwpalService[i].radioName, dwpalService[i].serviceName);*/
						...
						{
							console_printf("%s; dwpal_hostap_event_get ERROR; radioName= '%s', serviceName= '%s', msgLen= %zu\n",
			...
			if (DWPAL_CONN_TYPE_DRIVER == dwpalService[i].connectionType)
			{
				...
				{
					/*console_printf("%s; [Driver] event received; connectionType= '%s', radioName= '%s', serviceName= '%s', dwpalService[%d].fd= %d\n",
						   __FUNCTION__, connectionTypeToStr(dwpalService[i].connectionType), dwpalService[i].radioName, dwpalService[i].serviceName, i, dwpalService[i].fd);*/
				...
				{
					/*console_printf("%s; [Driver] event received; connectionType= '%s', radioName= '%s', serviceName= '%s', dwpalService[%d].fdCmdGet= %d\n",
						   __FUNCTION__, connectionTypeToStr(dwpalService[i].connectionType), dwpalService[i].radioName, dwpalService[i].serviceName, i, dwpalService[i].fdCmdGet);*/
...
{
	int i, numOfServices = ARRAY_SIZE(dwpalService);
...
{
	int                             i;
	char                            *p2str, *opCode, *VAPName = NULL, *field;
	rsize_t                         dmaxLen;
	int idx;
	...
	else if (!strncmp(opCode, "BEACON_REP_CLEAR", sizeof("BEACON_REP_CLEAR") - 1)) {
		VAPName = strtok_s(NULL, &dmaxLen, " ", &p2str);
		if (VAPName == NULL) {
			...
			if(dwpal_ext_interfaceIndexGet(DWPAL_CONN_TYPE_HOSTAP, VAPName, &ifidx) != DWPAL_SUCCESS)
			{
				console_printf("%s; dwpal_ext_interfaceIndexGet return ERROR for ifname=%s\n", __FUNCTION__, VAPName);
				return;
			}
		...
		if (VAPName != NULL)
		{
			...
			if (!strncmp(opCode, "DWPAL_HOSTAP_CMD_SEND", sizeof("DWPAL_HOSTAP_CMD_SEND") - 1))
			{
				...
				*/
				...
				if ((idx = interfaceIndexGet(DWPAL_CONN_TYPE_HOSTAP, VAPName)) == -1)
				{
					console_printf("%s; interfaceIndexGet (radioName= '%s', serviceName= 'Both') returned ERROR ==> Abort!\n", __FUNCTION__, VAPName);
					return;
				}
...
#endif /* DISABLE_DWPAL_HOSTAP_SUPPORT */
			if ( (!strncmp(opCode, "DWPAL_DRIVER_NL_CMD_SEND", sizeof("DWPAL_DRIVER_NL_CMD_SEND") - 1)) ||
			          (!strncmp(opCode, "DWPAL_DRIVER_NL_GET", sizeof("DWPAL_DRIVER_NL_GET") - 1)) )
			{
				...
				*/
				...
				else
				{
					...
					if (vendorDataSize > ARRAY_SIZE(vendorData))
					{
						console_printf("%s; vendorDataSize (%zu) bigger than sizeof(vendorData) (%zu) ==> Abort!\n", __FUNCTION__, vendorDataSize, sizeof(vendorData));
						return;
					}
				...
				{
					if ((idx = interfaceIndexGet(DWPAL_CONN_TYPE_DRIVER, "ALL")) == -1)
					{
...
static void dwpal_cli_readline_callback(char *strLine)
{
	...
	if (idx != (-1))
	{
		...
		if (entry)
		{
			free ((void *)(unsigned long)entry->line);
			free (entry);
		}
	}
...
{
	int numOfServices = ARRAY_SIZE(dwpalService);
	int i;
...
int main(int argc, char *argv[])
{
	/* Usage:
	   ...
	   Start NL socket listener up until "UNRECOVERABLE_ERROR" will be received for any "wlanX":
	   dwpal_cli -iDriver -l"UNRECOVERABLE_ERROR"
	...
	*/
	...
	char          oneShotTimeoutString[4] = "\0", vendorDataString[512] = "\0", *vendorDataTempString, *p2str;
	unsigned char vendorData[128] = "\0";
	size_t        stringLength, vendorDataSize = 0;
	rsize_t       dmaxLen;
	extern char   *optarg;
	ScanParams    scanParams, *scanParamsPtr = NULL;
	...
	{  /* one-shot mode */
		...
		while ( ((option = getopt(argc, argv, "i:m:v:c:l:a:t:d:p:s:")) != -1) && (optarg != NULL) )
		{
			...
			switch (option)
			{
				...
				default:
					console_printf_err("Invalid option (%d) ==> Abort!\n", option);
					return -1;
					break;
			}
		}
			...
			if (!strncmp(interfaceType, "hostap", sizeof("hostap") - 1))
			{
				if (oneShotInterfaceName[0][0] != '\0')
				{
					...
					if (oneShotCommand[0] != '\0')
					{
						...
						{
							console_printf("%s; dwpal_ext_hostap_cmd_send (oneShotInterfaceName[0]= '%s', oneShotCommand= '%s') returned ERROR (reply= '%s') ==> Abort!\n",
