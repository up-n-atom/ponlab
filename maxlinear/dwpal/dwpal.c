...
#ifndef DISABLE_DWPAL_HOSTAP_SUPPORT
static pthread_mutex_t hostap_context = PTHREAD_MUTEX_INITIALIZER;
#endif /* DISABLE_DWPAL_HOSTAP_SUPPORT */
...
static DWPAL_Ret scanParamHandle(struct nl_msg *msg, ScanParams *scanParams)
{
	...
	if (scanParams->meshid[0] != '\0')
	{
		...
		console_printf("%s; meshid_len= %zu\n", __FUNCTION__, meshid_len);
...
static bool mandatoryFieldValueGet(char *buf, size_t *bufLen, char **p2str, size_t totalSizeOfArg, char fieldValue[] /*OUT*/)
{
	...
	if (fieldValue != NULL)
	{
		if (strcpy_s(fieldValue, totalSizeOfArg, param)) {
			console_printf("%s; param ('%s') size (%zu) is higher than allocated size (%zu) ==> Abort!\n", __FUNCTION__,
				param, strnlen_s(param, HOSTAPD_TO_DWPAL_VALUE_STRING_LENGTH) + 1, totalSizeOfArg);
			return false;
		}
	}
...
static bool arrayValuesGet(char *stringOfValues, size_t totalSizeOfArg, ParamParsingType paramParsingType, size_t *numOfValidArgs /*OUT*/, void *array /*OUT*/)
{
	...
	do
	{
		...
		if (idx < totalSizeOfArg)
		{
			...
			{
				((int *)array)[idx] = (int)strtol(param, NULL, 16);
			}
			else if (paramParsingType == DWPAL_INT_ARRAY_PARAM)
			{
				((int *)array)[idx] = (int)strtol(param, NULL, 0);
			}
	...
	if (idx >= totalSizeOfArg)
	{
		console_printf("%s; actual number of arguments (%zu) is bigger/equal then totalSizeOfArg (%zu) ==> Abort!\n", __FUNCTION__, idx, totalSizeOfArg);
		return false;
	}
...
{
	char    *localMsg = _strndup_s(msg, msgLen), *lineMsg, *p2str;
...
DWPAL_Ret dwpal_nl80211_cmd_send(void *context, struct nl_msg *msg, int *cmd_res /*OUT*/, DWPAL_nl80211Callback nlCallback, void *cb_arg)
{
	...
	if (cmd_res == NULL)
	{
		console_printf("%s; cmd_res is NULL ==> Abort!\n", __FUNCTION__);
		return DWPAL_FAILURE;
	}
...
DWPAL_Ret dwpal_driver_nl_scan_dump_sync(void *context, char *ifname, int *cmd_res /*OUT*/, DWPAL_nl80211Callback nlCallback, void *cb_arg)
{
	...
	if (ifname == NULL)
	{
		console_printf("%s; ifname is NULL ==> Abort!\n", __FUNCTION__);
		return DWPAL_FAILURE;
	}
...
DWPAL_Ret dwpal_driver_nl_attach(void **context /*OUT*/)
{
	...
	*context = malloc(sizeof(DWPAL_Context));
	...
	for (i = 0; i < ARRAY_SIZE(gNlServices); i++)
	{
...
DWPAL_Ret dwpal_string_to_struct_parse(char *msg, size_t msgLen, FieldsToParse fieldsToParse[], size_t userBufLen)
{
	...
	{
		console_printf("%s; msgStringLen (%zu) is bigger than msgLen (%zu) ==> Abort!\n", __FUNCTION__, msgStringLen, msgLen);
		free((void *)stringOfValues);
		return DWPAL_FAILURE;
	}
	...
	while (fieldsToParse[i].parsingType != DWPAL_NUM_OF_PARSING_TYPES)
	{
		...
		if (fieldsToParse[i].field != NULL)
		{
			switch (fieldsToParse[i].parsingType)
			{
				...
				case DWPAL_INT64_HEX_PARAM:
					sizeOfStruct += sizeof(long long int);
					//console_printf("%s; DWPAL_LONG_LONG_INT_PARAM/DWPAL_INT64_HEX_PARAM; sizeOfStruct= %d\n", __FUNCTION__, sizeOfStruct);
					break;
	...
	if (numOfNameArrayArgs > 0)
	{
		...
		endFieldName = (char **)malloc(sizeof(typeof(*endFieldName)) * numOfNameArrayArgs);
		if (endFieldName == NULL)
		{
		...
		{
			memset((void *)endFieldName, 0, sizeof(typeof(*endFieldName)) * numOfNameArrayArgs);
		}
			...
			switch (fieldsToParse[i].parsingType)
			{
				case DWPAL_STR_PARAM:
					...
					{  /* Handle mandatory parameters WITHOUT any string-prefix */
						if (localMsg != NULL)
						{
							localMsgDup = _strndup_s(localMsg, msgStringLen); // The maximal length is not bigger then message length
							if (localMsgDup == NULL)
							{
								console_printf("%s; localMsgDup is NULL, Failed _strndup_s ==> Abort!\n", __FUNCTION__);
								ret = DWPAL_FAILURE;
								break;
							}
						}
					...
					else
					{
						...
						else
						{
							...
							if (fieldValuesGet(lineMsg, msgLen, fieldsToParse[i].parsingType, fieldsToParse[i].stringToSearch, endFieldName, stringOfValues) == true)
							{
								...
								{
									console_printf("%s; string length (%zu) is bigger the allocated string size (%zu)\n",
				...
				case DWPAL_INT64_HEX_PARAM:
					if (isEndFieldNameAllocated == false)
					{
						console_printf("%s; DWPAL_INT64_HEX_PARAM; isEndFieldNameAllocated=false ==> Abort!\n", __FUNCTION__);
						ret = DWPAL_FAILURE;
						break;
					}
					...
					if (field == NULL)
					{
						console_printf("%s; DWPAL_INT64_HEX_PARAM; fieldsToParse[%d].field=NULL ==> cont...\n", __FUNCTION__, i);
					}
					else
					{
						memset(stringOfValues, 0, HOSTAPD_TO_DWPAL_VALUE_STRING_LENGTH);  /* reset the string value array */
						if (fieldValuesGet(lineMsg, msgLen, fieldsToParse[i].parsingType, fieldsToParse[i].stringToSearch, endFieldName, stringOfValues) == true)
						{
							...
							*((u64 *)field) = strtoull(stringOfValues, NULL, 16);
						}
						else
						{
							isMissingParam = true;
				...
				case DWPAL_BOOL_PARAM:
					...
					else
					{
						...
						if (fieldValuesGet(lineMsg, msgLen, fieldsToParse[i].parsingType, fieldsToParse[i].stringToSearch, endFieldName, stringOfValues) == true)
						{
							...
							*((bool *)field) = (bool)atoi(stringOfValues);
						}
						else
						{
							isMissingParam = true;
...
{
	int     ret;
	DWPAL_Ret res = DWPAL_SUCCESS;
	char    *localOpCode;
	rsize_t dmaxLen;
	char    *localMsg, *p2str;
	struct  wpa_ctrl *wpaCtrlPtr = NULL;

	MUTEX_LOCK(&hostap_context);
	...
	if (wpa_ctrl_recv(wpaCtrlPtr, msg, msgLen) == 0)
	{
		...
		else
		{
			...
			localMsg = _strndup_s(msg, *msgLen);
			if (NULL == localMsg) {
				console_printf("%s; localMsg is NULL, Failed _strndup_s ==> Abort!\n", __FUNCTION__);
				return DWPAL_FAILURE;
			}
...
DWPAL_Ret dwpal_hostap_interface_attach(void **context /*OUT*/, const char *VAPName, DWPAL_wpaCtrlEventCallback wpaCtrlEventCallback)
{
	...
	*context = NULL;
	...
	localContext = malloc(sizeof(DWPAL_Context));
	if (localContext == NULL)
	{
		console_printf("%s; malloc for context failed ==> Abort!\n", __FUNCTION__);
		return DWPAL_FAILURE;
	...
	if (access(wpaCtrlName, F_OK) == 0)
	{
		...
		strcpy_s(localContext->interface.hostapd.operationMode, sizeof(localContext->interface.hostapd.operationMode), "AP");
		strcpy_s(localContext->interface.hostapd.wpaCtrlName, sizeof(localContext->interface.hostapd.wpaCtrlName), wpaCtrlName);
	}
	else
	{
		...
		if (access(wpaCtrlName, F_OK) == 0)
		{
			...
			strcpy_s(localContext->interface.hostapd.operationMode, sizeof(localContext->interface.hostapd.operationMode), "STA");
			strcpy_s(localContext->interface.hostapd.wpaCtrlName, sizeof(localContext->interface.hostapd.wpaCtrlName), wpaCtrlName);
		}
		else
		{
...
/**************************************************************************/
/*! \fn const char* dwpal_ret_to_string(DWPAL_Ret val)
 **************************************************************************
 *  \brief returns string representation of DWPAL codes
 *  \param[in] DWPAL_Ret val - dwpal error code
 *  \return const char* - string representation of DWPAL codes
 ***************************************************************************/
const char* dwpal_ret_to_string(DWPAL_Ret val)
{
	switch (val) {
		case DWPAL_SOCKET_FAILURE:
			return "DWPAL_SOCKET_FAILURE";
		case DWPAL_FAILURE:
			return "DWPAL_FAILURE";
		case DWPAL_SUCCESS:
			return "DWPAL_SUCCESS";
		case DWPAL_NO_PENDING_MESSAGES:
			return "DWPAL_NO_PENDING_MESSAGES";
		case DWPAL_MISSING_PARAM:
			return "DWPAL_MISSING_PARAM";
