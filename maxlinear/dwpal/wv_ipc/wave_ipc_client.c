...
typedef struct _cmd_response {
	uint8_t seq_num;
	volatile cmd_response_status status;
	wv_ipc_msg *response;
} cmd_response;
...
struct _wv_ipclient {
	...
	/* Internal pipe to send "interrupt" to function select() */
	int pipe_fds[2];
...
static wv_ipc_ret wave_ipcc_socket_deinit(wv_ipclient *ipclient)
{
	...
	// close pipe
	if (ipclient->pipe_fds[0] != -1) {
		close(ipclient->pipe_fds[0]);
		ipclient->pipe_fds[0] = -1;
	}
	if (ipclient->pipe_fds[1] != -1) {
		close(ipclient->pipe_fds[1]);
		ipclient->pipe_fds[1] = -1;
	}
...
static wv_ipc_ret wave_ipcc_socket_init(wv_ipclient *ipclient)
{
	...
	// - Create internal "self-pipe"
	if (-1 == pipe(ipclient->pipe_fds)) {
		ELOG("error creating pipe, errno = %d", errno);
		goto failure;
	}
	if (-1 == fcntl(ipclient->pipe_fds[0], F_SETFL, O_NONBLOCK)) {
		ELOG("error set pipe mode, errno = %d", errno);
		goto failure;
	}
	...
	/* To avoid leaking FD to other processes, like call to system() */
	if (-1 == fcntl(ipclient->socket, F_SETFD, FD_CLOEXEC)) {
		ELOG("error setting FD_CLOEXEC, errno = %d", errno);
		goto failure;
	}
...
failure:
	if (ipclient->socket != -1) close(ipclient->socket);
	if (ipclient->pipe_fds[0] != -1) close(ipclient->pipe_fds[0]);
	if (ipclient->pipe_fds[1] != -1) close(ipclient->pipe_fds[1]);
	ipclient->socket = ipclient->pipe_fds[0] = ipclient->pipe_fds[1] = -1;
	return WAVE_IPC_ERROR;
}
					...
					wv_ipc_msg **msg)
{
	...
	while ((!ipclient->listener && !term_cond(clb_arg)) ||
		...
		FD_SET(ipclient->pipe_fds[0], &rfds);

		tv.tv_sec = 1;
		tv.tv_usec = 0;

		res = select(MAX(ipclient->socket, ipclient->pipe_fds[0]) + 1, &rfds, NULL, NULL, &tv);
		if (res == -1 && errno == EINTR) {
			...
			if (ret != WAVE_IPC_SUCCESS) {
				...
				return WAVE_IPC_SUCCESS;
		} else if (FD_ISSET(ipclient->pipe_fds[0], &rfds)) {
			// self-pipe: request for terminate
			char signal;
			if (read(ipclient->pipe_fds[0], &signal, sizeof(signal)) < 0) {
				ELOG("Pipe read error!");
			}
			if (ipclient->listener && !ipclient->listener->terminate) {
				BUG("Unexpected pipe data");
				continue;
			}
			break;
		}
...
wv_ipc_ret wave_ipcc_emulate_event(wv_ipclient *handle, wv_ipc_sync_mode mode, wv_ipc_msg *msg)
{
	listener_thread_data *listener;
	...
	if (listener->events_serializer) {
		...
		if (mode == WV_IPC_ASYNC)
			res = serializer_exec_work_async(listener->events_serializer, 0, msg, listener);
		else
			res = serializer_exec_work(listener->events_serializer, 0, msg, listener, NULL, 15000);

		if (res) {
			BUG("failed push emulated %s event work to serializer", (mode == WV_IPC_ASYNC ? "ASYNC" : "SYNC"));
			wave_ipc_msg_put(msg);
			return WAVE_IPC_ERROR;
		}
	}
		...
		if (WV_IPC_SYNC != mode) {
			BUG("cannot run event in ASYNC mode w/o event thread");
			wave_ipc_msg_put(msg);
			return WAVE_IPC_ERROR;
		}
...
static void _ipcc_terminate(wv_ipclient *handle)
{
	static const char signal ='T';
	handle->listener->terminate = 1;
	if (write(handle->pipe_fds[1], &signal, sizeof(typeof(signal))) < 0) {
		ELOG("Pipe write error!");
	}
}
...
static wv_ipc_ret wave_ipcc_send_cmd_listener(wv_ipclient *ipclient, uint8_t seq_num,
					      wv_ipc_msg *cmd, wv_ipc_msg **reply, int timeout)
{
	wv_ipc_ret ret;
	int cw_res = 0;
	struct timespec ts;
	...
	ts.tv_sec += (timeout < 0) ? WAVE_IPC_CMD_TIMEOUT_SECS : (unsigned)timeout;
	while (resp_handle->status == RESP_NONE) {
...
again:
	...
	FD_ZERO(&rfds);
	FD_SET(socket, &rfds);
	tv.tv_sec = (timeout < 0) ? WAVE_IPC_CMD_TIMEOUT_SECS : (unsigned)timeout;
	tv.tv_usec = 0;
...
wv_ipc_ret wave_ipcc_send_cmd_ex(wv_ipclient *handle,
			      wv_ipc_msg *cmd, wv_ipc_msg **reply, int timeout)
{
	...
	/* Cleanup reply to return NULL in case of error */
	*reply = NULL;
	...
	if (handle->listener && pthread_self() != handle->listener->thread_id)
		ret = wave_ipcc_send_cmd_listener(handle, seq_num, cmd, reply, timeout);
	else
		ret = wave_ipcc_send_cmd_no_listener(handle, seq_num, cmd, reply, timeout);
...
{
	return wave_ipcc_send_cmd_ex(handle, cmd, reply, -1);
}
...
bool wv_ipcc_is_event_thread(wv_ipclient *handle)
{
	if (handle == NULL || handle->listener == NULL ||
		handle->listener->events_serializer == NULL)
		return false;

	return serializer_in_context(handle->listener->events_serializer);
}
