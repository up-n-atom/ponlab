...
/* User MUST specify these params before calling any GPHY APIs */
#define GPHY_ADDR 0x04

struct gpy2xx_mdio mdio;

int gpy211_mdio_read(void *busdata, u16 phyaddr, u32 regnum)
{
	pr_err("User MUST complete this API\n");
}
...
/* Populate GPHY struct input and init it */
int pop_gphy_init(struct gpy211_device *gphy)
{
	int ret=0;

	/* Optional call back funs */
	gphy->lock = NULL;
	gphy->unlock = NULL;
	gphy->lock_data = NULL;
	/* Madatory call back funs and params. The user must specify */
	gphy->mdiobus_read = gpy211_mdio_read;
	gphy->mdiobus_write = gpy211_mdio_write;
	gphy->mdiobus_data = &mdio;
	gphy->phy_addr = GPHY_ADDR;
	/* Optional params */
	gphy->smdio_addr = 0;
	gphy->priv_data = NULL;
	gphy->shared_data =NULL;
	gphy->wol_supported =0x60;
	gphy->ptp_clock = 0x1836E210;
	gphy->macsec_supported = 0;
	gphy->nr_of_sas = 32;

	ret = gpy2xx_init(gphy);
	if (ret < 0) {
...
static int gpy2xx_driver_probe(struct platform_device *pdev)
{
	...
	ret = pop_gphy_init(&driver_data->gdev);
