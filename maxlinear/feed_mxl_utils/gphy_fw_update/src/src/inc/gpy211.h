...
/** \brief Macros used by \b gpc_sel in \ref gpy211_synce and \ref gpy2xx_pps_ctrl */
...
struct gpy2xx_pps_ctrl {
	...
	u32 pps0_width;
	/** \brief GPC pin to be used as in \ref gpy211_gpc_sel */
...
/** \brief Indicating which slice is the current master
	Write access: current master slice
	Change: when the current master slices needs to go to low power mode,
	it can grant the master ship to another slice.
	The Selector drives the selection mux to EXINT0 and EXINT1.
	0	SLICE0 Slice 0
	1	SLICE1 Slice 1
	2	SLICE2 Slice 2
	3	SLICE3 Slice 3
*/
enum master_slice_num {
	/** \brief Write access: current master slice */
	MASTER_SLICE_0,
	/** \brief Write access: current master slice */
	MASTER_SLICE_1,
	/** \brief Write access: current master slice */
...
struct gpy211_ulp_ctrl {
	...
	u8 ulp_sta_block;
	/** \brief Indicating which slice is the current master. */
	enum master_slice_num master_slice;
};
/**@}*/ /* GPY211_MISC */
...
static inline int gpy2xx_read(struct gpy211_device *phy, u32 regaddr)
{
	...
	if (regaddr <= 31)
		...
		LOG_WARN("WARN: Invalid input reg address\n");
		return -EINVAL;
	}
}
				  ...
				  u32 regaddr)
{
	...
	if ((devtype == MDIO_MMD_PMAPMD) || (devtype == MDIO_MMD_PCS) ||
	    (devtype == MDIO_MMD_AN) || (devtype == MDIO_MMD_VEND1) ||
	    (devtype == MDIO_MMD_VEND2))
		return phy->mdiobus_read(phy->mdiobus_data, phy->phy_addr,
					 (MII_ADDR_C45 | (devtype << 16) | (regaddr & 0xffff)));
	else {
		LOG_WARN("WARN: Invalid input MMD dev type\n");
		return -EINVAL;
	}
}
				   ...
				   u32 regaddr, u16 data)
{
	...
	if ((devtype == MDIO_MMD_PMAPMD) || (devtype == MDIO_MMD_PCS) ||
	    ...
	    (devtype == MDIO_MMD_VEND2))
		return phy->mdiobus_write(phy->mdiobus_data, phy->phy_addr,
					  (MII_ADDR_C45 | (devtype << 16) | (regaddr & 0xffff)), data);
	else {
		LOG_WARN("WARN: Invalid input MMD dev type\n");
		return -EINVAL;
	}
