...
static int __is_rw_done(struct gpy211_device *phy, u32 reg, u32 bit)
{
	...
	do {
		...
		if ((!value && !busyRetry)) {
			LOG_CRIT("\nERROR: Target register RW busy timeout\n");
			retCode = -1;
		}
	} while (0);
...
static int _gpy2xx_ptp_set_ssecinc(struct gpy211_device *phy)
{
	...
	if (0 != phy->ptp_clock) {
		...
		_rem_nsec = (u64)(_rem_nsec * 1000);
		_rem_nsec = ((u64)(_rem_nsec / phy->ptp_clock)) + 1; //+1 for 3 decimals increment
	} else {
		LOG_CRIT("ERROR: PTP clock is not set\n");
		return -1;
	}
	 ...
	 */
	_sub_nsec = ((u64)(_rem_nsec * 256)) / 1000;
...
static int _gpy2xx_ptp_set_addend(struct gpy211_device *phy, u32 addend)
{
	...
	if (ret < 0) {
		LOG_CRIT("ERROR: TSADDREG_POS set fails\n");
		return ret;
	}
		      ...
		      struct gpy211_ptp_ctrl *ptp_cfg)
{
	...
	_update_freq_hz = phy->ptp_clock / 2; //203.125MHz
	/* Calc as (2^32 * PTP_UPDATE_FREQ) / PTP ref clock */
	temp = (u64)(0x100000000ull * _update_freq_hz);
	//printf("Dividend 0x%I64x\n", temp);
	_def_addend = div_u64_rem(temp, phy->ptp_clock, &rem);
	//printf("Def addend bef 0x%I64x, rem 0x%x\n", _def_addend, rem);
...
{
	int ret = -EINVAL;
	u32 _num_of_units, _gpio_get_val;
	u16 _pps_ctrl_low;
	u64 _ulong_multiply = 0, _ulong_divide = 0;
	u16 _nsec_tt_low, _nsec_tt_hi, _sec_tt_low, _sec_tt_hi;
	u16 _units_int_low, _units_int_hi, _units_wid_low, _units_wid_hi;
	u16 _gpc_min = SYNCE_GPC1, _gpc_pin_num = GPIO_PIN10_GPC0_FUN;
	u32 _gmacf_base_addr, _gpc_sel, _gpio_dir, _alt0_sel, _alt1_sel, _gpcx_base;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* Validate input params */
	/* Validate GPC pins */
	if (!(pps_ctrl->gpc_sel >= OUTTIMER && pps_ctrl->gpc_sel <= SYNCE_GPC2)) {
		LOG_WARN("WARN: Invalid input (gpc_sel) - GPC pin\n");
		return -EINVAL;
	}
	...
	/* Validate GPC select */
	if (!(pps_ctrl->gpc_sel >= _gpc_min &&
	      pps_ctrl->gpc_sel <= SYNCE_GPC2)) {
	...
	if (ret < 0) {
		LOG_CRIT("ERROR: api _is_rw_done fails\n");
		goto END;
	}
	...
	_nsec_tt_low = pps_ctrl->nsec_tgttime & GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_L_TTSL0_MASK;
	...
	_nsec_tt_hi = pps_ctrl->nsec_tgttime >> 16;
	 ...
	 * interval.
	 */
	_ulong_multiply = (u64)((u64)pps_ctrl->pps0_interal * (u64)phy->ptp_clock);
	_ulong_divide = (u64)(_ulong_multiply / SEC_TO_NANOSEC(1));

	if (_ulong_divide == 0) {
		LOG_CRIT("ERROR: Wrong PPS interval\n");
		goto END;
	}

	_num_of_units = _ulong_divide - 1;
	LOG_INFO("The muliplied %llx, divided %llx\n", _ulong_multiply, _ulong_divide);
	...
	_ulong_multiply = (u64)((u64)pps_ctrl->pps0_width * (u64)phy->ptp_clock);
	_ulong_divide = (u64)(_ulong_multiply / SEC_TO_NANOSEC(1));

	if (_ulong_divide == 0) {
		LOG_CRIT("ERROR: Wrong PPS width\n");
		goto END;
	}
	...
	_units_wid_low = _num_of_units & GMACF_MAC_PPS0_WIDTH_L_PPSWIDTH0_MASK;
	...
	if (phy->id.family == ID_P34X) {
		_gpcx_base = CGU_PDI_CGU_PDI_GPC0_OUT_CONF;
	} else {
		_gpcx_base = CGU_PDI_CGU_PDI_GPC0_CONF;
	}

	ret = PHY_MBOX_HWRD(phy, _gpcx_base + ((pps_ctrl->gpc_sel - 1) * 4), &_gpc_sel);

	if (ret < 0) {
		LOG_CRIT("ERROR: GPCx_OUT_CONF read fails\n");
		goto END;
	}

	if (phy->id.family == ID_P34X) {
		FIELD_REPLACE(_gpc_sel, (CONST_CGU_PDI_GPC0_OUT_CONF_SEL_SYNCE0 + (phy->phy_addr & 0x3)), CGU_PDI_GPC0_OUT_CONF_SEL);
	} else {
		FIELD_REPLACE(_gpc_sel, CONST_CGU_PDI_GPC0_CONF_SEL_OUTTIMER, CGU_PDI_GPC0_CONF_SEL);
	}

	ret = PHY_MBOX_HWWR(phy, _gpcx_base + ((pps_ctrl->gpc_sel - 1) * 4), _gpc_sel);

	if (ret < 0) {
		LOG_CRIT("ERROR: GPCx_OUT_CONF write fails\n");
		goto END;
	}

	/* Read GPIO DIRECTION */
	ret = PHY_MBOX_HWRD(phy, GPIO_PDI_REGISTERS_GPIO_PDI_REGISTERS_GPIO_DIR, &_gpio_dir);
	...
	/* GPCx used as output */
	_gpio_dir |= ((u16)0x0001u << (_gpc_pin_num + pps_ctrl->gpc_sel - 1));

	/* Write GPIO DIRECTION */
	ret = PHY_MBOX_HWWR(phy, GPIO_PDI_REGISTERS_GPIO_PDI_REGISTERS_GPIO_DIR, _gpio_dir);

	if (ret < 0) {
		LOG_CRIT("ERROR: GPIO_DIR write fails\n");
		goto END;
	}

	ret = PHY_MBOX_HWRD(phy, GPIO_PDI_REGISTERS_GPIO_PDI_REGISTERS_GPIO_DIR, &_gpio_get_val);
	...
	if (_gpio_dir != _gpio_get_val) {
		LOG_CRIT("ERROR: GPIO_DIR set %x and get %x vals are not matching\n", _gpio_dir, _gpio_get_val);
		goto END;
	}
	...
	if (phy->id.family == ID_P34X) {
		_alt0_sel |= ((u16)0x0001u << (_gpc_pin_num + pps_ctrl->gpc_sel - 1));
	} else {
		_alt0_sel &= ~((u16)0x0001u << (_gpc_pin_num + pps_ctrl->gpc_sel - 1));
	}
	...
	ret = PHY_MBOX_HWRD(phy, GPIO_PDI_REGISTERS_MODULE_BASE + GPIO_PDI_REGISTERS_GPIO_ALTSEL0, &_gpio_get_val);
	...
	if (_alt0_sel != _gpio_get_val) {
		LOG_CRIT("ERROR: GPIO_ALTSEL0 set %x and get %x vals are not matching\n", _alt0_sel, _gpio_get_val);
		goto END;
	}
	...
	_alt1_sel |= ((u16)0x0001u << (_gpc_pin_num + pps_ctrl->gpc_sel - 1));
	...
	ret = PHY_MBOX_HWRD(phy, GPIO_PDI_REGISTERS_MODULE_BASE + GPIO_PDI_REGISTERS_GPIO_ALTSEL1, &_gpio_get_val);
	...
	if (_alt1_sel != _gpio_get_val) {
		LOG_CRIT("ERROR: GPIO_ALTSEL1 set %x and get %x vals are not matching\n", _alt1_sel, _gpio_get_val);
		goto END;
	}
...
int gpy2xx_ptp_get_ppsctrl(struct gpy211_device *phy,
			   struct gpy2xx_pps_ctrl *pps_ctrl)
{
	int ret = -EINVAL;
	u32 _num_of_units;
	...
	/* If the System Time Nanoseconds register has an accuracy of 1 ns */
	pps_ctrl->nsec_tgttime = (_nsec_tt_hi << 16) | _nsec_tt_low;;
	...
	pps_ctrl->pps0_interal = (((u64)((_num_of_units + 1) * SEC_TO_NANOSEC(1))) / phy->ptp_clock);
	...
	pps_ctrl->pps0_width = (((u64)((_num_of_units + 1) * SEC_TO_NANOSEC(1))) / phy->ptp_clock);
...
int gpy2xx_ptp_adjfreq(struct gpy211_device *phy, s32 ppb)
{
	...
	_update_freq_hz = phy->ptp_clock / 2; //203.125MHz
	/* Calc as (2^32 * PTP_UPDATE_FREQ) / PTP ref clock */
	temp = (u64)(0xFFFFFFFFull * _update_freq_hz);
	//printf("Dividend 0x%I64x\n", temp);
		      ...
		      struct gpy211_ptp_cfg *ptp_cfg)
{
	...
	ret = gpy2xx_ptp_get_ppsctrl(phy, &(ptp_cfg->pps_ctrl));
