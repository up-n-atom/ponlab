...
export STAGING_DIR=/local/gmudepa/ugw_sw/openwrt/staging_dir
...
EN_API_FLAG="-DEN_MSEC_DEPRECATED_API=0 -DSUPPORT_MACSEC_VLAN=0 -DEN_GMAC_DEBUG_ACCESS=0 -DEN_MSEC_DEBUG_ACCESS=1 -DEN_REPROGM_MISSING_4B=0 -DEN_USER_CHOICE=0 -DTEST_ALL_ZERO_IMAGE=0 -DEN_EXEC_TIME_PER_MDIO_FREQ=0 -DEN_ERASE_FLASH_ONLY_TEST=0 -DEN_TEST_REG_READ_WRITE=0"
OPTINAL_TFLAGS="${LOG_CFLAGS} ${EN_API_FLAG} -DP31G_OPT_SPEED=0 -DEN_SMDIO_RW=0 -DP31G_B0=1 -DSAFE_C_LIB=0 -DREG_RD_PRINT=0 -DREG_WR_PRINT=0 -DEN_BACKWARD_COMP_API=0 -Wno-format -Wno-format"
...
${GCC} ${UART_FLAG} ${OPTINAL_TFLAGS} -Wall -fstack-protector-all ${ALL_INC} -c -o gpy2xx_mdio_static.o "gpy2xx_mdio.c"
${GCC} ${UART_FLAG} ${OPTINAL_TFLAGS} -Wall -pie -fstack-protector-all -I${DDKAES_INC} ${ALL_INC} -c -o aes_static.o "${DDKAES_SRC}/aes.c"
${AR} rcs libgpy2xx_static.a gpy211_phy_static.o gpy211_chip_static.o gpy211_macsec_static.o gpy2xx_mdio_static.o aes_static.o
...
if [[ "lib" != "$1" ]]; then
	${GCC} ${UART_FLAG} ${OPTINAL_TFLAGS} -Wall -fstack-protector-strong ${ALL_INC} gpy2xx_cli.c -L. -lgpy2xx_static -o gpy2xx_cli
fi
