...
void mem_help(void);
int neededBytes(int range_lc);
int map(char *fname, unsigned int startaddress,unsigned int len);
void unmap(int mapped_size);
void mem_write(void);
void mem_dump(void);
...
int map(char *fname, unsigned int startaddress,unsigned int len)
{
	...
	g_address = mmap(NULL, mapped_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offset);
