...
{
	int i;
	char* ptr=NULL, *pcSavePtr=NULL;
	ptr = strtok_s(str, &n, delim, &pcSavePtr);
	if (ptr != NULL) {
		if(n==0)
			return ptr;
		for(i=0;i<n;i++){
			ptr = strtok_s(NULL, &n, delim, &pcSavePtr);
		}
	}
	return ptr;
}
...
int compare_version(char *v_old, char *v_new)
{
	...
	int new_v_num=0;
	for(i=0;i<32;i++){
		if(strncpy_s(tmp_str, 32, v_old, 32) != EOK ) {
		...
		//printf("old_v_num=%d\n",old_v_num);
		if (strncpy_s(tmp_str, 32, v_new, 32) != EOK) {
			printf("strncpy failed\n");
			return 0;	
		}
		p = get_string_value(tmp_str,".",i);
...
int do_upgrade( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]) {
	...
	do {
		...
		if(upgrade_img(srcHeader, curParSize, name, dir, 0)) {
			...
			sprintf_s(strimg_crc, sizeof(strimg_crc), "f_%s_crc", name);
			sprintf_s(buf, sizeof(buf), "%lX", ntohl(pimg_header->ih_dcrc));
...
int image_check_hcrc (const image_header_t *hdr)
{
	...
	if (memmove_s(&header, len, (const char *)hdr, len)  != EOK) {
	...
	do {
...
#ifdef CONFIG_FEATURE_LQ_NEW_UPGRADE
		/*srcHeader = (ulong)(!srcHeader ? xHeader : (char *)srcHeader + curParSize);
		  pimg_header = (image_header_t *)srcHeader;
		  if (!image_check_hcrc (pimg_header) || !image_check_dcrc (pimg_header)) {
		...
		switch(xImgHeader.ih_type) {
			...
			case IH_TYPE_FILESYSTEM:
				...
				break;
			case IH_TYPE_KERNEL:
				if (strncmp((char *)xImgHeader.ih_name, "MIPS 4Kec Bootcore", sizeof(xImgHeader.ih_name)) == 0)
					sprintf_s(name,sizeof(name), "bootcore");
				else
					sprintf_s(name, sizeof(name), "kernel");
				dir = 1;
...
#if !defined (CONFIG_TARGET_UBI_MTD_SUPPORT)
			sprintf_s(strimg_crc, sizeof(name), "f_%s_crc", name);
			sprintf_s(buf, sizeof(name), "%lX", ntohl(xImgHeader.ih_dcrc));
			setenv(strimg_crc, buf);
			saveenv();
#ifdef UBOOT_ENV_COPY
			saveenv_copy();
