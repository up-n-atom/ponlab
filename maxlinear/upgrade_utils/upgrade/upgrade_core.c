...
{
	ifx_debug_printf("program_img : the srcAddr is 0x%08lx, length is %d while destAddr is 0x%08lx\n",srcAddr,srcLen,destAddr);
	...
	ifx_debug_printf("Erase Flash from 0x%08lx to 0x%08lx\n", destAddr, destAddr + srcLen-1);
...
int FindPartBoundary(ulong img_addr,ulong *curpart_begin,ulong *nextpart_begin)
{
	...
	for(i = 0; i < nPartNo; i++){
		...
		sprintf_s(strPartName,sizeof(strPartName), "part%d_begin",i);
...
int FindNPImgLoc(ulong img_addr,ulong *nextStartAddr,ulong *preEndAddr)
{
	...
	ifx_debug_printf("For the image address 0x%08lx, partition boundary is found as [0x%08lx,0x%08lx]\n",img_addr,curpart_begin,nextpart_begin);
	*nextStartAddr = nextpart_begin;
	*preEndAddr = curpart_begin;

	for(i = 0; i < (int)nDBNo; i++){
		memset(strDBName[i],0x00,sizeof(strDBName[i]));
		memset(strTemp,0x00,sizeof(strTemp));
		sprintf_s(strTemp, sizeof(strTemp), "data_block%d",i);
		if (strncpy_s(strDBName[i], 32, (char *)getenv(strTemp), 32) != EOK) {
	...
	for(i = 0; i < (int)nDBNo; i++){
		memset(strTemp,0x00,sizeof(strTemp));
		strT = NULL;
		sprintf_s(strTemp, sizeof(strTemp), "f_%s_addr",strDBName[i]);
		...
		memset(strTemp,0x00,sizeof(strTemp));
		strT = NULL;
		sprintf_s(strTemp, sizeof(strTemp), "f_%s_size",strDBName[i]);
		strT = (char *)getenv(strTemp);
		if(strT == NULL){
			printf("Variable %s is not set\n",strTemp);
			return 1;
...
int upgrade_img(ulong srcAddr, ulong srcLen, char *imgName, enum ExpandDir dir, int bSaveEnv)
{
  ...
  memset(strimg_vol_id,0x00,sizeof(strimg_vol_id));
	sprintf_s(strimg_vol, sizeof(strimg_vol), "%s_vol",imgName);
	sprintf_s(strimg_vol_id,sizeof(strimg_vol_id), "%s_id",getenv(strimg_vol));
#endif
	...
	if (ntohl(pimg_header->ih_magic) == IH_MAGIC) {
		...
		if(pimg_header->ih_type != IH_TYPE_KERNEL) {
			ifx_debug_printf("This is not kernel image and so removing header\n");
	...
	sprintf_s(strimg_dataout, sizeof(strimg_dataout), "/usr/sbin/vol_mgmt upgrade_dualvol_stdin %s %u %s", imgName, (size_t)srcLen, pimg_header->ih_name);
  #else
	sprintf_s(strimg_dataout, sizeof(strimg_dataout), "/usr/sbin/vol_mgmt upgrade_vol_stdin %s %u %s", imgName, (size_t)srcLen, pimg_header->ih_name);
  #endif
...
int upgrade_img(ulong srcAddr, ulong srcLen, char *imgName, enum ExpandDir dir, int bSaveEnv)
{
	...
	sprintf_s(strimg_addr, sizeof(strimg_addr), "f_%s_addr",imgName);
	sprintf_s(strimg_size,sizeof(strimg_size), "f_%s_size",imgName);
    ...
    */
	...
	if (ntohl(pimg_header->ih_magic) == IH_MAGIC) {
		...
		img_size = simple_strtoul((char *)getenv(strimg_size),NULL,16); 
		...
		if (memmove_s(srcData_Copy + sizeof(struct conf_header), srcLen, (void *)srcAddr, srcLen)  != EOK) {
			printf("memmove_s failed\n");
			return 1;
		}
#else
		srcData_Copy = malloc(srcLen + sizeof(struct conf_header));
		if (memcpy_s(srcData_Copy+sizeof(struct conf_header), srcLen + sizeof(struct conf_header), (char *)srcAddr,srcLen) != EOK) {
			return 1;
		}
#endif
	...
	sprintf_s(newimg_para, sizeof(newimg_para), "0x%08lx",srcLen);
	setenv(strimg_size,newimg_para);
	upgrade_debug_printf("New variables %s = %s set\n",strimg_size,newimg_para);

	memset(newimg_para,0x00,sizeof(newimg_para));
	sprintf_s(newimg_para,sizeof(newimg_para), "0x%08lx",img_addr);
	...
	if (strcmp(strimg_addr,"f_rootfs_addr") == 0) {
		memset(newimg_para,0x00,sizeof(newimg_para));
		sprintf_s(newimg_para,sizeof(newimg_para),"0x%08lx",img_addr + srcLen);
