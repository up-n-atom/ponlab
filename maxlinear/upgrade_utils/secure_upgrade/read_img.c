...
int main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "sysconfig")) {
		...
		sprintf_s(temp_buf, sizeof(temp_buf), "/usr/sbin/vol_mgmt read_calibration %s %s", argv[1], argv[2]);
		if (system(temp_buf)) 
			return 1;
	...
	for (i = 0; i < nPass; i++) {
		...
		printf("flash_write : Wrote %d bytes\n", bWrote);
		limg_size -= readLen;
		if (i == 0)	
	...
	crc ^= 0xffffffff;
	if (header.crc != crc) {
		printf("Read image fail, crc error.\n");
		rmdir(argv[2]);
		return 1;
	} else			
	{
