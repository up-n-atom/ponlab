...
/**
 * eloop_register_write_sock - Register handler for write events
 * @sock: File descriptor number for the socket
 * @handler: Callback function to be called when socket is ready for write
 * @eloop_data: Callback context data (eloop_ctx)
 * @user_data: Callback context data (sock_ctx)
 * Returns: 0 on success, -1 on failure
 *
 * Register a write socket notifier for the given file descriptor. The handler
 * function will be called whenever socket is ready for write. The handler function
 * is responsible for clearing the event after having processed it in order to
 * avoid eloop from calling the handler again for the same event.
 */
...
/**
 * eloop_unregister_write_sock - Unregister handler for write events
 * @sock: File descriptor number for the socket
 *
 * Unregister a write socket notifier that was previously registered with
 * eloop_register_write_sock().
 */
