...
typedef struct DutRssiOffsetGain_tag
{
	uint32 antMask;
    int Method;
    uint16 rssiResult[DUT_RXANT_ARRAY]; //Out
    uint16 NOS;	
} DutRssiOffsetGain_t;

typedef struct RssiCwPower_tag
{
	CorrRes_t corrResults[DUT_RXANT_ARRAY];
	uint32 antMask;	
	int method;//not used
...
typedef struct RfRssiPower_tag
{
	uint8 antMask;
	uint8 reserved[3];
	int16 power[DUT_RXANT_ARRAY];
} RfRssiPower_t;

typedef enum _FemType_e
{
	FEM_TYPE_LINEAR = 0,
	FEM_TYPE_NON_LINEAR,
	FEM_TYPE_LAST,
...
#define MAXIMUM_BANDWIDTHS_GEN7		(5)
...
typedef enum _Bandwidth_e
{
	...
	BANDWIDTH_EIGHTY_EIGHTY        = 0x5,
	BANDWIDTH_ALL                  = 0x7,
	BANDWIDTH_MAX                  = BANDWIDTH_THREE_HUNDRED_TWENTY + 1,
} Bandwidth_e;
/*--------------------------------------------------------------------------
...
#define TPC_FREQ_STR_NUM        (2 * BANDWIDTH_MAX)
...
typedef struct tpcConfig
{
	...
	int16		regulationLimitMU[BANDWIDTH_MAX];
	int16		regulationLimitBF[BANDWIDTH_MAX];
	int16		regulationLimitBF_AX[BANDWIDTH_MAX];
