...
typedef struct token_fuctionlist_s {
    ...
    CK_RV (*object_mgr_create_final) (STDLL_TokData_t *tokdata, SESSION *sess, OBJECT *obj, CK_OBJECT_HANDLE *handle);
} token_fuctionlist_t;

token_fuctionlist_t ext_funcs = { NULL, NULL, NULL, NULL};

void install_token_fuctionlist( CK_RV (*reload_token_object) (STDLL_TokData_t *tokdata, OBJECT *obj),
                                CK_RV (*delete_token_object) (STDLL_TokData_t *tokdata, OBJECT *obj),
                                CK_RV (*save_token_object) (STDLL_TokData_t *tokdata, OBJECT *obj),
                                CK_RV (*object_mgr_create_final) (STDLL_TokData_t *tokdata, SESSION *sess, OBJECT *obj, CK_OBJECT_HANDLE *handle))
{
    if((reload_token_object == NULL) || (delete_token_object == NULL)
	     || (save_token_object == NULL) || (object_mgr_create_final == NULL)) {
        TRACE_ERROR("Wrong Input params\r\n");
        return;
    }
    ext_funcs.reload_token_object = reload_token_object;
    ext_funcs.delete_token_object = delete_token_object;
    ext_funcs.save_token_object = save_token_object;
    ext_funcs.object_mgr_create_final = object_mgr_create_final;
}
                     ...
                     CK_ULONG ulCount, CK_OBJECT_HANDLE *handle)
{
    ...
    //
    if(ext_funcs.object_mgr_create_final)
        rc = ext_funcs.object_mgr_create_final(tokdata, sess, o, handle);
    else
                      ...
                      CK_OBJECT_HANDLE *new_handle)
{
    ...
    //
    if(ext_funcs.object_mgr_create_final)
        rc = ext_funcs.object_mgr_create_final(tokdata, sess, new_obj, new_handle);
    else
                              ...
                              OBJECT *obj, CK_OBJECT_HANDLE *handle)
{
    ...
    if (sess_obj) {
        ...
        if(ext_funcs.save_token_object)
            rc = ext_funcs.save_token_object(tokdata, obj);
        else
    ...
    if (rc != CKR_OK) {
        ...
        if (sess_obj) {
            ...
            if(ext_funcs.delete_token_object)
                ext_funcs.delete_token_object(tokdata, obj);
            else
                                ...
                                SESSION *sess, CK_OBJECT_HANDLE handle)
{
    ...
    if (map->is_session_obj) {
        ...
        if(ext_funcs.delete_token_object)
            ext_funcs.delete_token_object(tokdata, o);
        else
...
retry:
    ...
    if(ext_funcs.reload_token_object)
        rc = ext_funcs.reload_token_object(tokdata, obj);
    else
...
CK_RV object_mgr_update_publ_tok_obj_from_shm(STDLL_TokData_t *tokdata)
{
    ...
    for (index = 0; index < tokdata->global_shm->num_publ_tok_obj; index++) {
        ...
        if (fa.done == FALSE) {
            ...
            if(ext_funcs.reload_token_object)
                rc = ext_funcs.reload_token_object(tokdata, new_obj);
            else
