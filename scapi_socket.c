...
ipc_create_listener(char *pucto)
{
	...
	iret=sprintf_s(acbuf,MAX_NAME_LEN, IPC_FILE_PATH"%s", pucto);
	if(iret==MAX_NAME_LEN || iret <= 0){
	...
	if (sprintf_s(xaddr.sun_path, sizeof(xaddr.sun_path), "%s", acbuf) <= 0) {
		LOGF_LOG_DEBUG("sprintf_s failed\n");
		if(pxhandle->ifd >= 0)
			close(pxhandle->ifd);
		free(pxhandle);
		return NULL;
...
ipc_send_request(x_ipc_msg_t *pxmsg)
{
	...
	iret=sprintf_s(acbuf,MAX_NAME_LEN, IPC_FILE_PATH"%s",pxmsg->xhdr.aucto);
	if(iret <= 0){
		LOGF_LOG_DEBUG("sprintf_s failed\n");
		return IPC_FAIL;
	}
	...
	memset(&xaddr, 0, sizeof(xaddr));
	xaddr.sun_family = AF_UNIX;
	iret = sprintf_s(xaddr.sun_path, sizeof(xaddr.sun_path), "%s", acbuf);
	if(iret <= 0){
		LOGF_LOG_DEBUG("sprintf_s failed\n");
		if(ifd >= 0)
			close(ifd);
