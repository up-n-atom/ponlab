...
#define BSS_CHANGED_AP_ISOLATE 1ULL <<32
...
enum mac80211_rx_flags {
	...
	RX_FLAG_8023			= BIT(30),
	RX_FLAG_CTRL_PORT_PROTO         = BIT(31),
};
...
struct ieee80211_link_sta {
	...
	struct ieee80211_sta_eht_oper eht_oper;
...
enum ieee80211_hw_flags {
	...
	IEEE80211_HW_DATA_OFFLOAD,
	IEEE80211_HW_SUPPORTS_VENDOR_VHT,
	IEEE80211_HW_SUPPORTS_SSID_LIST,
	IEEE80211_HW_SUPPORTS_WNM_NOTIF,
	IEEE80211_HW_SUPPORTS_OP_MODE_NOTIF,
...
struct ieee80211_ops {
	void (*tx)(struct ieee80211_hw *hw,
		   ...
		   struct sk_buff *skb);
	void (*data_tx)(struct ieee80211_hw *hw,
                        struct ieee80211_vif *vif,
                        struct sk_buff *skb);
	...
	void (*set_default_multicast_key)(struct ieee80211_hw *hw,
                                          struct ieee80211_vif *vif, int idx);
	...
	void (*handle_radar_debug)(struct ieee80211_hw *hw,
                                   struct cfg80211_chan_def *chandef,
                                   u8 radar_bitmap);
	...
	int (*prepare_cac_start)(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif, u32 cac_time_ms);
	void (*notify_cac_finished)(struct ieee80211_hw *hw,
                                    struct ieee80211_vif *vif,
                                    bool cac_canceled);
	...
	int (*cancel_remain_on_channel)(struct ieee80211_hw *hw,
					struct ieee80211_vif *vif);
	int (*set_qos_map)(struct ieee80211_hw *hw,
                           struct ieee80211_vif *vif,
	...
	void (*get_vap_stats)(struct net_device *ndev,
	...
	int (*post_channel_switch)(struct ieee80211_hw *hw,
				   struct ieee80211_vif *vif);
	void (*abort_color_switch)(struct ieee80211_hw *hw,
				   struct ieee80211_vif *vif);
	...
	void (*del_nan_func)(struct ieee80211_hw *hw,
			    struct ieee80211_vif *vif,
			    u8 instance_id);
	int (*driver_free_netdev_allowed)(struct ieee80211_hw * hw,
	...
	int (*net_setup_tc)(struct ieee80211_hw *hw,
			    ...
			    void *type_data);
	struct net_device * (*driver_alloc_netdev)(struct ieee80211_hw * hw);
	struct ieee80211_sta * (*get_sta_by_mld_addr)(struct ieee80211_vif *vif, const unsigned char *mld_addr);
...
/**
 * ieee80211_ml_chan_switch_notif_done - notify mac80211 about ML channel switch
 * @vif: &struct ieee80211_vif pointer from the add_interface callback.
 * @flag: indication type flag
 *
 * During channel switch announcement, ml channel switch notification done event
 * is received on reporting link after channel switch count reaches 1. This API
 * handles the event and post to userspace.
 */
void ieee80211_ml_chan_switch_notif_done(struct ieee80211_vif *vif, uint flag);
...
/*
 * ieee80211_color_coll_notify - queue work to notify about color collision
 * @vif: &struct ieee80211_vif pointer from the add_interface callback.
 *
 * In scan rx callback, check neighbor beacon for color collisions and call this
 * function to queue work for notifying collisions.
 */
void ieee80211_color_coll_notify(struct ieee80211_vif *vif, u64 color_bitmap, const u8 *bssid);
/**
 * ieee80211_get_obss_bsscolor_bitmap - retrieve OBSS bsscolor bitmap
 * @wiphy: &struct wiphy Pointer to wireless hardware description structure.
 *
 * Retrieves the OBSS bsscolor bitmap info which is stored during scan.
 */
u64 ieee80211_get_obss_bsscolor_bitmap(struct wiphy *wiphy);
...
/**
 * ieee80211_radar_detected_rbm - inform that a radar was detected on
 *  part of the channel indicated by radar_bit_map
 *
 * @hw: pointer as obtained from ieee80211_alloc_hw()
 * @chandef: chan_def on which the detection occured
 * @radar_bit_map: bitmap indicating on which channels a radar
 *  was detected on. 0x0 or 0xff means radar deteceted on all
 *  channels in chandef.
 */
void ieee80211_radar_detected_rbm(struct ieee80211_hw *hw,
                                                 struct cfg80211_chan_def *chandef,
                                                 u8 radar_bit_map);
/**
...
/**
 * ieee80211_color_switch_done - Complete color switch process
 * @vif: &struct ieee80211_vif pointer from the add_interface callback.
 * @success: make the color switch successful or not
 *
 * Complete the color switch post-process: set the new bss color params
 * and wake up the suspended queues.
 */
void ieee80211_color_switch_done(struct ieee80211_vif *vif, bool success);
...
/**
 * ieee80211_del_station - delete station
 *
 */
int ieee80211_del_station(struct wiphy *wiphy, struct net_device *dev,
...
/**
 * ieee80211_if_alloc - alloc interfaces net_device.
 *
 * Some drivers may want to alloc the net_device themselves, before the
 * interface is added by the user.
 * @hw: &struct ieee80211_hw pointer from the ieee80211_if_add.
 * @name: name of the net_device.
 * @name_assign_type: Indicates the name assignment type.
 * @type: the (virtual) interface type
 * @params: vif params
 */
struct net_device * ieee80211_if_alloc(struct ieee80211_hw * hw, const char *name,
                     unsigned char name_assign_type,
                     enum nl80211_iftype type,
                     struct vif_params *params);
void ieee80211_dfs_cac_restart(struct ieee80211_hw *hw);
#endif /* MAC80211_H */
