...
/**
 * @brief SCAPI gets MBSSID next MAC address API
 * @details API that generates a MAC address for an 6g interface by properly incrementing the base MAC address without any conflicts (with MAC addresses of other interfaces)
...
/**
 * @brief SCAPI function to set loglevel of this library
 * @details function sets the log level based on the caller daemon(CSD and Servd)
 * @param[in] unLevel log level to be configured
 * @param[in] unType log should be in console or file
 * 
 * @return void 
 */
...
/**
 * @brief SCAPI gets reserved mac address
 * @details API that gets reserved mac address on every call this API its caller response free up the memory.
 * @param[in] ppxMac mac address structure
 * @param[in] nResCnt resevered number f mac address count
 * 
 * @return EXIT_SUCCESS on successful / -ve value (depending on the type of error) on failure
 */
int scapi_getReserevdMac(INOUT ResvMAC **ppxMac, INOUT int *nResCnt);
...
/**
 * @brief SCAPI swap next MAC address API
 * @details API that swaps the MAC address of two WAN inteface from the config file
 *
 * @param[in] pcIfname_newDefWAN Interface name of new Default WAN
 * @param[in] pcIfname_oldDefWAN Interface name of old Default WAN
 * @return EXIT_SUCCESS on successful / -ve value (depending on the type of error) on failure
 */
...
/**
 * @brief SCAPI get PPMD status
 * @details API to get PPMD status from system
 * @param[out] xInfo pointer to struct that contains PPMD info set by API
 * @return UGW_SUCCESS on successful / UGW_FAILURE on failure
 */
int32_t scapi_getPpmdStatus(PPMDInfo_t *pxInfo);

/**
 * @brief SCAPI get Power Monitor status
 * @details API to get Power Monitor status from system
 * @param[out] xInfo pointer to struct that contains Power Monitor info set by API
 * @return UGW_SUCCESS on successful / UGW_FAILURE on failure
 */
int32_t scapi_getPowerMonitorStatus(PowerMonitorInfo_t *pxInfo);

/**
 * @brief SCAPI get CPU Status
 * @details API to get each CPU Status info from system
 * @param[in] sFileName file name from where we get CPU info
 * @param[out] sValue pointer to string that contains CPU status info set by API
 * @return UGW_SUCCESS on successful / UGW_FAILURE on failure
 */
...
/**
 * @brief SCAPI parse MAC table API
 * @details API to parse MAC table of an interface
 *
 * @param[out] macTable Pointer to struct which contains MAC entries set by API
 * @param[in] pIntFace Interface name
 *
