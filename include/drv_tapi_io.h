...
#ifdef TAPI_VERSION3
   #define TAPI_VERSION_CODE TAPI_API_VERSION(3,2,13)
...
#ifdef TAPI_VERSION4
   #define TAPI_VERSION_CODE TAPI_API_VERSION(4,1,11)
...
/** This service sets the configuration of the PCM interface; it must be
   ...
   param.nMCTS         = 0x00;
   nTsSync             = IFX_TAPI_PCM_IF_TS_SYNC_DEFAULT;
   nOffsetSlicTs       = 0;
...
typedef enum
{
   /** Default; u-law decoder algorithm as defined in G.711 standard. */
   IFX_TAPI_DEC_MLAW_STANDARD = 0,
   /** Reencode: u-law decoder algorithm optimised for re-encoding with
       the A-law encoder algorithm. With this setting the decoder algorithm
       is changed from the definition in the standard. */
   IFX_TAPI_DEC_MLAW_REENCODE = 1
} IFX_TAPI_DEC_MLAW_MODE_t;
...
typedef enum
{
   ...
   /** Reserved; voice over data. */
   IFX_TAPI_JB_PKT_ADAPT_VOD           = 5
...
typedef enum
{
   ...
   /** Device with Puma DSP. */
   IFX_TAPI_DEV_TYPE_PUMA_DSP = 15
} IFX_TAPI_DEV_TYPE_t;
...
#ifdef TAPI_VERSION4
   /** Firmware module to play the configuration.
       IFX_TAPI_MODULE_TYPE_COD configuration applied also for DTMF auto-suppression;
...
typedef struct
{
  ...
  /** Select PLC type; defined in codec or special pattern. */
   ...
   /** Select u-law decoder algorithm; Standard according to G.711 or
       optimised for re-encoding with A-law encoder. */
   IFX_TAPI_DEC_MLAW_MODE_t   ulaw_mode;
} IFX_TAPI_DEC_CFG_t;
...
typedef enum
{
   ...
   /** Notification that the system clock was lowered without previous
       callback procedures. This can happen on core overtemperature. */
