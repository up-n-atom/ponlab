...
typedef struct
{
	...
	char sMethods[MAX_UBUS_METHOD_SIZE];/*!< ubus method */
	void *pObjType ; /*!< List ptr refers Objlist | ObjAttrList | ObjAcsList */
}MsgHeader;

/*! 
    \brief Contains struct (object, parameter) count struct
      functions and Callbacks 
*/
typedef struct 
{
   int32_t nObjCnt; /*!< object count */
   int32_t nPramCnt; /*!< parameter count */
}DataModelStat;
...
typedef struct {
	...
	Msg_t			*xMessage;	/*!< Optional Message data from external daemons to ServD */
