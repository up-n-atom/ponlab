...
#define ext_feat_case(name, desc)				\
	...
	ext_feat_case(MFP_PBAC, "MFP PBAC protected block ack capable");
	}
}
...
next:
	...
	if (tb_msg[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_TX] &&
		...
		printf("\tAvailable Antennas: TX 0x%x RX 0x%x\n",
	...
	if (tb_msg[NL80211_ATTR_WIPHY_ANTENNA_TX] &&
		...
		printf("\tConfigured Antennas: TX 0x%x RX 0x%x\n",
	...
	if (tb_msg[NL80211_ATTR_WIPHY_DFS_ANTENNA])
		printf("\tDevice is meant only for DFS\n");
