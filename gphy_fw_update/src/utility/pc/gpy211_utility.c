...
static struct {
	...
	},
	{
		...
		"gpy2xx_ulp_cfg [<ulp_enable=?>] [<ulp_sta_block=?>] [<ulp_exit=?>] [<master_slice=?>]\n"
...
//#endif
		...
		ret = print_sys_time(refTime.tv_sec, refTime.tv_nsec);

		if (ret < 0)
			printf("\nERROR: Print sys time failed.\n");
	...
	} else if (xstrncasecmp(argv[0], "test_gpy2xx_ptp_set_ppsctrl", strlen("test_gpy2xx_ptp_set_ppsctrl")) == 0) {
		int cnt = 0;
		struct gpy2xx_pps_ctrl ppsCfg;
		memset(&ppsCfg, 0, sizeof(struct gpy2xx_pps_ctrl));
		struct timespec64 refTime;
		...
		ppsCfg.sec_tgttime = refTime.tv_sec + 25;
...
static void random_input(void)
{
	...
	phy.mdiobus_data = &phy;
	ret = gpy2xx_init(&phy);
	printf("LOG: GPY211 init returned %d\n", ret);
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		if ((temp_buffer[0] == 'r' || temp_buffer[0] == 'w') && temp_buffer[1] == ' ') {
			...
			CloseHandle(hComm);
			return 0;
		} else if (xstrncasecmp(param_list[0], "random", 6) == 0) {
			random_input();
			return 0;
		} else {
#if LOGM_IN_FILE
