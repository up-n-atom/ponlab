...
#else
#define NUM_ITES_PER_MDIO_FREQ_DIV		50
#endif
...
static int _fmx_reg_read(void *busdata, u32 regnum)
{
	...
	ret = ioctl(priv->fd, GSW_XGMAC_CFG, &param);
	if (!ret)
		return param.val[0];
	else
		return ret;
...
int main(int argc, char **argv)
{
	...
	u32 fw_ver, cfw_ver, tot_ite = 600, clause = 0, mdc_clk_div = 0;
	...
	if (fw_path == NULL) {
		printf("<ffu_appl_name> fw <fw_bin_file_name> <mdio_xgmac_id> <phy_id> <tot_iterations_to_run> <mdio_clause> <fw_version_to_compare> <fmx_mdc_clk_divider>\n");
		printf("example below\n");
		printf("./ffu_auto_erase_flash fw GPY2XXP_FLASH_IMAGE_LTCSS_INPUT_0x3740_unsigned_release.bin 2(must on fmx) <0-31> <0-600> <0(CL45)-1(CL22)> 0x4743 <0-5>\n");
		return -1;
	...
	_fmx_reg_write(phy.mdiobus_data, 0x220, (clause & 1) << phy.phy_addr);
	printf("set clause - %x\n", clause);
	...
	_fmx_reg_write(phy.mdiobus_data, 0x204, mdc_clk_div << 19);
	printf("set mdio clk div - %x\n", mdc_clk_div << 19);
	printf("get mdio clk div is - %x\n", _fmx_reg_read(phy.mdiobus_data, 0x204));

	printf("set clause - %x\n", clause);
	_fmx_set_clause(phy.mdiobus_data, phy.phy_addr, clause & 1);
	printf("get mdio clause is - %x\n", _fmx_get_clause(phy.mdiobus_data, phy.phy_addr));
	printf("get mdio clause reg - %x\n", _fmx_reg_read(phy.mdiobus_data, 0x220));
