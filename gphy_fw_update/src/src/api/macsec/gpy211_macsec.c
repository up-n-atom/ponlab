...
#ifdef __KERNEL__
//This func def is present in drivers\net\ethernet\intel\macsec\aes_crypto.c
int derive_aes_hkey(const u8 *key, u8 *output, u32 keylen);
#else
//This func def is present in GPY211\ISC-DDK-160-BSD_v2.3\SafeXcel-IP-160_DDK_v2.3\Kit\SABuilder_MACsec\Example_AES\src\aes.c
void
...
{
	if (dir == INGRESS_DIR) {
		if (ccw->ca_type == GCM_AES_128)
			*_ccw = ING_32BPN_128KEY_DEF_CCW;
		else if (ccw->ca_type == GCM_AES_XPN_128)
			*_ccw = ING_64BPN_128KEY_DEF_CCW;
		else if (ccw->ca_type == GCM_AES_256)
			*_ccw = ING_32BPN_256KEY_DEF_CCW;
		else if (ccw->ca_type == GCM_AES_XPN_256)
			      ...
			      struct transform_rec *in_tr)
{
	...
	if ((in_tr->ccw.ca_type == GCM_AES_128) ||
	    (in_tr->ccw.ca_type == GCM_AES_256))
		in_tr->ccw.sn_type = SN_32_BIT;
	else
		in_tr->ccw.sn_type = SN_64_BIT;

	ret = _gpy2xx_msec_pop_key(&in_tr->cp, le_key);
	_key_len = ((in_tr->ccw.ca_type == GCM_AES_128) ||
		    (in_tr->ccw.ca_type == GCM_AES_XPN_128)) ? 16 : 32;
#ifdef __KERNEL__
	derive_aes_hkey(in_tr->cp.key, in_tr->cp.hkey, _key_len);
#else
	...
	for (i = 0; i < _key_len; i++)
		sprintf(keystr + 2 * i, "%02X", in_tr->cp.key[i]);

	for (i = 0; i < _key_len; i++)
		sprintf(hkeystr + 2 * i, "%02X", in_tr->cp.hkey[i]);

	LOG_CRIT("key = %s, h-key = %s\n", keystr, hkeystr);
#endif
	...
	/* Build TR */
	if ((in_tr->ccw.ca_type == GCM_AES_128) ||
	    (in_tr->ccw.ca_type == GCM_AES_XPN_128)) {
				 ...
				 struct transform_rec *in_tr)
{
	...
	if ((in_tr->ccw.ca_type == GCM_AES_128) ||
		...
		if (in_tr->ccw.sn_type == SN_32_BIT) {
			//struct egr_tr_32bpn_128bak *target_tr;
			_sa_upd_offset = 15 * 4;
		} else if (in_tr->ccw.sn_type == SN_64_BIT) {
			//struct egr_tr_64bpn_128bak *target_tr;
			_sa_upd_offset = 19 * 4;
		} else
			return -EINVAL;
	} else if ((in_tr->ccw.ca_type == GCM_AES_256) ||
		   (in_tr->ccw.ca_type == GCM_AES_XPN_256)) {
		if (in_tr->ccw.sn_type == SN_32_BIT) {
			//struct egr_tr_32bpn_256bak *target_tr;
			_sa_upd_offset = 19 * 4;
		} else if (in_tr->ccw.sn_type == SN_64_BIT) {
...
#if defined(DEBUG_MSEC_PRINT) && DEBUG_MSEC_PRINT
	LOG_CRIT("Flow-ctrl word = %8x\n",  sam_fca->dec_enc.raw_fca_macsec);
#endif
...
#if defined(DEBUG_MSEC_PRINT) && DEBUG_MSEC_PRINT
	LOG_CRIT("\nSA Counters:\n----------------\n");
	LOG_CRIT("InOctetsDecrypted = %llu\n", sa_stats->InOctetsDecrypted);
	LOG_CRIT("InOctetsDecrypted2 = %llu\n", sa_stats->InOctetsDecrypted2);
	LOG_CRIT("InPktsUnchecked = %llu\n", sa_stats->InPktsUnchecked);
	LOG_CRIT("InPktsDelayed = %llu\n", sa_stats->InPktsDelayed);
	LOG_CRIT("InPktsLate = %llu\n", sa_stats->InPktsLate);
	LOG_CRIT("InPktsOK = %llu\n", sa_stats->InPktsOK);
	LOG_CRIT("InPktsInvalid = %llu\n", sa_stats->InPktsInvalid);
	LOG_CRIT("InPktsNotValid = %llu\n", sa_stats->InPktsNotValid);
	LOG_CRIT("InPktsNotUsingSA = %llu\n", sa_stats->InPktsNotUsingSA);
	LOG_CRIT("InPktsUnusedSA = %llu\n", sa_stats->InPktsUnusedSA);
	LOG_CRIT("InPktsUntaggedHit = %llu\n", sa_stats->InPktsUntaggedHit);
#endif
...
#if defined(DEBUG_MSEC_PRINT) && DEBUG_MSEC_PRINT
	LOG_CRIT("\nSA Counters:\n----------------\n");
	LOG_CRIT("OutOctetsEncrypted = %llu\n", sa_stats->OutOctetsEncrypted);
	LOG_CRIT("OutOctetsEncrypted2 = %llu\n", sa_stats->OutOctetsEncrypted2);
	LOG_CRIT("OutPktsEncrypted = %llu\n", sa_stats->OutPktsEncrypted);
	LOG_CRIT("OutPktsTooLong = %llu\n", sa_stats->OutPktsTooLong);
#endif
...
#if defined(DEBUG_MSEC_PRINT) && DEBUG_MSEC_PRINT
	LOG_CRIT("\nGLOBAL Counters:\n----------------\n");
	LOG_CRIT("TransformErrorPkts = %llu\n", glob_stats->TransformErrorPkts);
	LOG_CRIT("InPktsCtrl = %llu\n", glob_stats->InPktsCtrl);
	LOG_CRIT("InPktsNoTag = %llu\n", glob_stats->InPktsNoTag);
	LOG_CRIT("InPktsUntagged = %llu\n", glob_stats->InPktsUntagged);
	LOG_CRIT("InPktsTagged = %llu\n", glob_stats->InPktsTagged);
	LOG_CRIT("InPktsBadTag = %llu\n", glob_stats->InPktsBadTag);
	LOG_CRIT("InPktsUntaggedMiss = %llu\n", glob_stats->InPktsUntaggedMiss);
	LOG_CRIT("InPktsNoSCI = %llu\n", glob_stats->InPktsNoSCI);
	LOG_CRIT("InPktsUnknownSCI = %llu\n", glob_stats->InPktsUnknownSCI);
	LOG_CRIT("InConsistCheckControlledNotPass = %llu\n", glob_stats->InConsistCheckControlledNotPass);
	LOG_CRIT("InConsistCheckUncontrolledNotPass = %llu\n", glob_stats->InConsistCheckUncontrolledNotPass);
	LOG_CRIT("InConsistCheckControlledPass = %llu\n", glob_stats->InConsistCheckControlledPass);
	LOG_CRIT("InConsistCheckUncontrolledPass = %llu\n", glob_stats->InConsistCheckUncontrolledPass);
	LOG_CRIT("InOverSizePkts = %llu\n", glob_stats->InOverSizePkts);
#endif
				     ...
				     struct egr_global_stats *glob_stats)
{
	...
	LOG_CRIT("\nGLOBAL Counters:\n----------------\n");
	LOG_CRIT("TransformErrorPkts = %llu\n", glob_stats->TransformErrorPkts);
	LOG_CRIT("OutPktsCtrl = %llu\n", glob_stats->OutPktsCtrl);
	LOG_CRIT("OutPktsUnknownSA = %llu\n", glob_stats->OutPktsUnknownSA);
	LOG_CRIT("OutPktsUntagged = %llu\n", glob_stats->OutPktsUntagged);
	LOG_CRIT("OutOverSizePkts = %llu\n", glob_stats->OutOverSizePkts);
#endif
