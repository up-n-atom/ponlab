...
int gpy2xx_ulp_cfg(struct gpy211_device *phy, struct gpy211_ulp_ctrl *ulp)
{
	...
	ret = PHY_MBOX_HWRD(phy, TOP_PDI_REGISTERS_MODULE_BASE + TOP_SLICE_MASTER, &_mstr_slice);

	if (ret < 0) {
		LOG_CRIT("ERROR: TOP_SLICE_MASTER read fails\n");
		goto END;
	}

	if ((phy->phy_addr & 0x3) == _mstr_slice) {
		LOG_WARN("WARN: Can't put MASTER slice in ULP mode\n");
		goto END;
	}
	...
	ret = PHY_MBOX_HWWR(phy, TOP_PDI_REGISTERS_MODULE_BASE + TOP_SLICE_MASTER, ulp->master_slice);

	if (ret < 0) {
		LOG_CRIT("ERROR: TOP_SLICE_MASTER write fails\n");
		goto END;
	}

	if (1 == ulp->ulp_exit) {
		ret = PHY_MBOX_HWRD(phy, TOP_PDI_REGISTERS_MODULE_BASE + TOP_SLICE_IF, &_top_slice_if);

		if (ret < 0) {
			LOG_CRIT("ERROR: TOP_SLICE_IF read fails\n");
			goto END;
		}

		_top_slice_if |= (1 << ((phy->phy_addr & 0x3) + 2));

		ret = PHY_MBOX_HWWR(phy, TOP_PDI_REGISTERS_MODULE_BASE + TOP_SLICE_IF, _top_slice_if);

		if (ret < 0) {
			LOG_CRIT("ERROR: TOP_SLICE_IF write fails\n");
			goto END;
		}
	}
...
int gpy2xx_ulp_get(struct gpy211_device *phy, struct gpy211_ulp_ctrl *ulp)
{
	...
	ulp->master_slice = _mstr_slice & 0x3;

	ret = PHY_MBOX_HWRD(phy, TOP_PDI_REGISTERS_MODULE_BASE + TOP_STATUS, &_top_status);

	if (ret < 0) {
		LOG_CRIT("ERROR: TOP_STATUS read fails\n");
		goto END;
	}

	ulp->ulp_status = (_top_status >> ((phy->phy_addr & 0x3) + 4)) && 1;
		     ...
		     struct gpy211_synce *synce_cfg)
{
	...
	if (synce_cfg->master_sel == SYNCE_MASTER) {
		_synce_gpio_dir = _synce_gpio_dir & ~((u16)0x0001u << (_gpc_pin_num + synce_cfg->gpc_sel - 1));
	} else {
		_synce_gpio_dir = _synce_gpio_dir | ((u16)0x0001u << (_gpc_pin_num + synce_cfg->gpc_sel - 1));
	}

	if (phy->id.family == ID_P31G) {
		_synce_gpio_altsel0 = _synce_gpio_altsel0 & ~((u16)0x0001u << (_gpc_pin_num + synce_cfg->gpc_sel - 1));
	} else {
		_synce_gpio_altsel0 = _synce_gpio_altsel0 | ((u16)0x0001u << (_gpc_pin_num + synce_cfg->gpc_sel - 1));
	}

	_synce_gpio_altsel1 = _synce_gpio_altsel1 | ((u16)0x0001u << (_gpc_pin_num + synce_cfg->gpc_sel - 1));
	...
	if (synce_cfg->master_sel == SYNCE_MASTER) {
		if (phy->id.family == ID_P34X) {
			FIELD_REPLACE(_synce_cfg, synce_cfg->gpc_sel, SLICE_CGU_PDI_SYNCE_CONF_SCLK_SEL);
		} else {
			FIELD_REPLACE(_synce_cfg, synce_cfg->gpc_sel, CGU_PDI_SYNCE_CONF_SCLK_SEL);
		}
	} else {
	...
	if (synce_cfg->master_sel == SYNCE_SLAVE) {
		if (phy->id.family == ID_P34X) {
			_cgu_pdi_gpc_addr = CGU_PDI_CGU_PDI_GPC0_OUT_CONF + ((synce_cfg->gpc_sel - 1) * 4);
		} else {
			_cgu_pdi_gpc_addr = CGU_PDI_CGU_PDI_GPC0_CONF + ((synce_cfg->gpc_sel - 1) * 4);
		}
		     ...
		     struct gpy211_synce *synce_cfg)
{
	...
	if (phy->id.family == ID_P34X) {
		synce_cfg->gpc_sel = FIELD_GET(_gpc_sel, SLICE_CGU_PDI_SYNCE_CONF_SCLK_SEL);
	} else {
		synce_cfg->gpc_sel = FIELD_GET(_gpc_sel, CGU_PDI_SYNCE_CONF_SCLK_SEL);
	}
