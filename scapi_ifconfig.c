...
#define SNPRINTF_IF_CFG(pDest, nBufSize, psFormat, ...) { \
...
int scapi_getIfcIpaddr(char* pcIfname, char* pcIp)
{
	...
	SNPRINTF_IF_CFG(xIfr.ifr_name,IFNAMSIZ, "%s", pcIfname);
	...
	SNPRINTF_IF_CFG(pcIp, INET_ADDRSTRLEN, "%s" , inet_ntoa(((struct sockaddr_in *)&xIfr.ifr_addr)->sin_addr));
	nRet = EXIT_SUCCESS;
returnHandler:
	close(nSkfd);
	return nRet;
...
int scapi_getIfcNetmask(char* pcIfname, char* pcNetmask)
{
	...
	/* Top layer should provide a NULL terminated array of size INET_ADDRSTRLEN */
	SNPRINTF_IF_CFG(pcNetmask, INET_ADDRSTRLEN, "%s", inet_ntoa(((struct sockaddr_in *)&xIfr.ifr_addr)->sin_addr));
	nRet = EXIT_SUCCESS;
returnHandler:
	close(nSkfd);
	return nRet;
...
int scapi_getIfcMacaddr(char* pcIfname, char* pcMac)
{
	...
	/* get result */
	SNPRINTF_IF_CFG(pcMac, SCAPI_MAC_LEN, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
...
int scapi_setIfcMacaddr(char* pcIfname, char* pcMac, int nOctetInc)
{
	...
	sscanf_s(pcMac, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", 
			(unsigned char *)&(xIfr.ifr_hwaddr.sa_data[0]), 
			(unsigned char *)&(xIfr.ifr_hwaddr.sa_data[1]), 
			(unsigned char *)&(xIfr.ifr_hwaddr.sa_data[2]),
			(unsigned char *)&(xIfr.ifr_hwaddr.sa_data[3]),
