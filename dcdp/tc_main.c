 ...
 * @History:
 * Version 1.5.12:
 * 	FEATURE_POWER_DOWN for AC1200
 * */
#define DRV_VERSION	"1.5.12"
...
static u32 txin_num;
static u32 txout_num;
static u32 rxin_num;
static u32 rxout_num;
...
module_param(txin_num, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(txin_num, "txin ring size configuration: 0 - default");
module_param(txout_num, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(txout_num, "txout ring size configuration: 0 - default");
module_param(rxin_num, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(rxin_num, "rxin ring size configuration: 0 - default");
module_param(rxout_num, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(rxout_num, "rxout ring size configuration: 0 - default");
...
void tc_load(struct tc_priv *priv, u32 id, enum dsl_tc_mode mode)
{
	...
	spin_lock_bh(&priv->tc_lock);
	priv->tc_stat = (!ret) ? TC_RUN : TC_ERR;
	priv->tc_idx = id;
	spin_unlock_bh(&priv->tc_lock);
}
