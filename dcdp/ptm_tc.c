...
{
	pr_info("%s: %s\n", __func__,
		priv->tc_priv->tc_mode != TC_PTM_SL_MODE ? "BONDING" : "SINGLE");
...
static int ptm_open(struct net_device *dev)
{
    ...
    if(ppa_drv_datapath_mac_entry_setting)
        ppa_drv_datapath_mac_entry_setting(dev->dev_addr,
        	0, 6, 10, 1, 1);
#endif
	return 0;
}
	...
	int is_bonding)
{
	...
	printk("%s: cfg->txin.soc_cnt_phyaddr[%x]\n",
			__func__, cfg->txin.soc_cnt_phyaddr);
	printk("%s: cfg->txout.soc_cnt_phyaddr[%x]\n",
			__func__, cfg->txout.soc_cnt_phyaddr);
	printk("%s: cfg->rxout.soc_cnt_phyaddr[%x]\n",
			__func__, cfg->rxout.soc_cnt_phyaddr);
	printk("%s: cfg->rxin.soc_cnt_phyaddr[%x]\n",
