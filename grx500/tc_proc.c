...
struct net_device* tc_get_netdev_from_name(char *name)
{
	struct net_device *dev;
	for_each_netdev(&init_net, dev) {
		if (dev) {
			if (vrx_strcmpi(name, dev->name) == 0) {
				return dev;
			}
		}
	}
...
static int proc_read_tc_cfg(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "DMA Scatter-gather feature: %s\n",
	    priv->param.ena_dma_tc_sg == 1 ? "Enable": "Disable");
#endif
...
#ifdef FEATURE_POWER_DOWN
	seq_printf(seq, "Power Down Policy\n");
	for (i =0;i < EP_MAX_NUM; i++) {
	    seq_printf(seq, " Line[%d]: [%d]", i,  priv->dsl_pwr_policy[i]);
	    switch ( priv->dsl_pwr_policy[i]) {
		case 1:
		    seq_printf(seq, " Turn off clocks\n");
		break;
		case 2:
		    seq_printf(seq, " Turn off clocks and PLL\n");
		break;
		default:
		    seq_printf(seq, " No policy\n");
		    break;
	    }
	}
#endif
...
static ssize_t proc_write_cfg(struct file *file, const char __user *buf,
			size_t count, loff_t *data)
{
	struct tc_priv *priv;
	char str[32];
	...
	int num;
	char *param_list[20];
	int conf_PPLSwitchOff, conf_PPLSwitchOff_val, conf_Line;
	...
	num = vrx_split_buffer(str, param_list, ARRAY_SIZE(param_list));
	conf_PPLSwitchOff = 0;
	conf_PPLSwitchOff_val = 0;
	conf_Line = 0;
	if (vrx_strcmpi(param_list[0], "xDSL_Cfg_PLL_SwitchOff") == 0) {
		conf_PPLSwitchOff = 1;
	} else {
		goto proc_cfg_help;
	}
	if (param_list[1]) {
		conf_PPLSwitchOff_val = vrx_atoi(param_list[1]);
	} else
		goto proc_cfg_help;
	if (param_list[2]) {
		conf_Line = vrx_atoi(param_list[2]);
	} else
		goto proc_cfg_help;
	if (conf_PPLSwitchOff) {
	#ifdef FEATURE_POWER_DOWN
	    if (conf_Line < EP_MAX_NUM)
		priv->dsl_pwr_policy[conf_Line] = conf_PPLSwitchOff_val + 1;
	#else
	    pr_info("Does not support Enhance Power Saving Feature\n");
	#endif
	}
	return count;

proc_cfg_help:
	pr_info("echo xDSL_Cfg_PLL_SwitchOff <0|1> <Line> > cfg\n");
	pr_info("\n");
...
static const struct file_operations tc_desc_conf_proc_fops = {
	.owner		= THIS_MODULE,
	.open		= proc_read_desc_conf_seq_open,
	.read		= seq_read,
	.write		= proc_write_desc_conf,
	.llseek		= seq_lseek,
	.release	= single_release,
};
#endif
...
static int proc_read_ver(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "FW built date(MM/DD/YYYY): %d/%d/%d\n",
		(date >> 24) & 0xff,
		(date >> 16) & 0xff,
		(date & 0xffff));
	seq_printf(seq, "Feature support:\n");
#ifdef FIX_DMA_BYTE_ALIGNMENT
	seq_puts(seq, " + Support DMA scatter-gathering feature\n");
#endif
#ifdef FEATURE_CONF_DESC_LENGTH
	seq_puts(seq, " + Support QoS and Configurable descriptor length\n");
#endif
#ifdef FEATURE_PPE_DYNAMIC_FEQ
	seq_puts(seq, " + Support PPE Dynamic Frequency and ERB Recovery\n");
#endif
#ifdef FEATURE_POWER_DOWN
...
static int proc_read_ptm_qosmib_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_ptm_qosmib, PDE_DATA(inode));
}

static const struct file_operations ptm_qosmib_proc_fops = {
	.owner      = THIS_MODULE,
	.open       = proc_read_ptm_qosmib_seq_open,
	.read       = seq_read,
	.write      = proc_write_ptm_qosmib,
	.llseek     = seq_lseek,
	.release    = single_release,
};
...
static int proc_ptm_read_cfg(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "QoS Drop: %s\n",
		ptm_qos_drop(priv, 0, 0) == 1 ? "Enable" : "Disable");
			...
			const char __user *buf, size_t count, loff_t *data)
{
	...
	} else if (vrx_strcmpi(param_list[1], "drop") == 0) {
		ptm_qos_drop(priv, en, 1);
	} else if (clk == 1) {
...
static int proc_ptm_read_soc_fw_seq_open(struct inode *inode,
							struct file *file)
{
	return single_open(file, proc_ptm_read_soc_fw, PDE_DATA(inode));
}
static const struct file_operations ptm_soc_fw_proc_fops = {
	.owner		= THIS_MODULE,
	.open		= proc_ptm_read_soc_fw_seq_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.write		= proc_ptm_write_soc_fw,
	.release	= single_release,
};
...
static struct tc_proc_list ptm_sl_procs[] = {
	...
	{"qosmib",	0644,	&ptm_qosmib_proc_fops,	0},
...
#ifdef FIX_DMA_BYTE_ALIGNMENT
	{"soc_fw",	0644,	&ptm_soc_fw_proc_fops,	0},
#endif
};
				...
				size_t count, loff_t *data)
{
	...
	int len;
	char *p1;
	unsigned int conn = ATM_PVC_NUMBER + 1;
...
static ssize_t proc_write_mpoa(struct file *file,
		const char __user *buf, size_t count, loff_t *data
	)
{
	char local_buf[128];
	...
	char *param_list[20];
	int num, i, len, pos;
	int vpi, vci, mpoa_mode, mpoa_type;
	unsigned int conn = ATM_PVC_NUMBER + 1;
	struct atm_priv *priv;
	struct net_device *atm_net_dev;
	...
	if (vrx_strcmpi(param_list[0], "help") == 0)
		...
		pos = 0;
		while ((i + 2) < num) {
			if (vrx_strcmpi(param_list[2 + i], "if") == 0) {
				/* get net device */
				atm_net_dev = tc_get_netdev_from_name(
					param_list[3 + i]);
				i++;
			} else if (vrx_strcmpi(param_list[2 + i], "mode") == 0) {
				mpoa_mode = get_num_from_str(
					&param_list[3 + i],
					&len,
					1);
				i++;
			} else if (vrx_strcmpi(param_list[2 + i], "type") == 0) {
				mpoa_type = get_num_from_str(
					&param_list[3 + i],
					&len,
					1);
				i++;
	...
	if (!atm_net_dev) {
		pr_info("Usage: missing net interface\n");
		return count;
	}
	if (conn_valid(conn)) {
		if (mpoa_setup_proc(vpi, vci, atm_net_dev, mpoa_type, mpoa_mode))
			tc_err(priv->tc_priv,
				MSG_INIT,
				"Failed to configure vpc[%d.%d]",
				vpi, vci
			);
	} else
		tc_err(priv->tc_priv,
			MSG_INIT,
			"Invalid connection for vpi = %u, vci = %u, conn = %d",
			vpi, vci, conn
		);
	return count;
proc_atm_mpoa_help:
	pr_info("Usage: cat > mpoa\n");
	return count;
}


static int proc_read_mpoa(struct seq_file *seq, void *v)
{
	seq_puts(seq, "Usage:\n");
	seq_puts(seq, "\techo <pvc vpi.vci> [mode m] [type t] > mpoa\n");
	seq_puts(seq, "\t\t[Mode]\n");
	seq_puts(seq, "\t\t\t[0]: VCMux\n");
	seq_puts(seq, "\t\t\t[1]: LLC\n");
	seq_puts(seq, "\t\t[Type]\n");
	seq_puts(seq, "\t\t\t[0]: EoA w/o FCS\n");
	seq_puts(seq, "\t\t\t[1]: EoA w FCS\n");
	seq_puts(seq, "\t\t\t[2]: PPPoA\n");
	seq_puts(seq, "\t\t\t[3]: IPoA\n");
	seq_puts(seq, "Example: echo pvc 0.33 mode 1 type 0 > mpoa\n");
	return 0;
}

static int proc_read_mpoa_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_mpoa, NULL);
}

static const struct file_operations atm_mpoa_fops = {
	.owner		= THIS_MODULE,
	.open		= proc_read_mpoa_seq_open,
	.read		= seq_read,
	.write		= proc_write_mpoa,
	.llseek		= seq_lseek,
	.release	= single_release,
};
