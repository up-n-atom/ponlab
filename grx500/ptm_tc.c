...
static void reset_erb_owner_bit(struct ptm_ep_priv *priv, int bonding)
{
	int i;
	rx_descriptor_t desc;
	tc_dbg(priv->tc_priv, MSG_EVENT, "ERB Reset for[%d] bonding[%d]\n",
		priv->ep_id, bonding);
	...
	if (is_ptm_sl(priv) || (bonding && is_us_bond(priv))) {
		...
		for (i = 0; i < __OUTQ_ERB0_NUM; i++) {
			tc_mem_read(priv, &desc,
				fpi_addr(__OUTQ_ERB0_BASE + i * DESC_DWSZ),
			sizeof(desc));
...
static void ptm_erb_init(struct ptm_ep_priv *priv, int bonding)
{
	...
	reset_erb_owner_bit(priv, bonding);
}
...
static int ptm_framer_request_en(const unsigned char idx)
{
	struct ptm_ep_priv *priv = tc_ep_priv(idx);
	int bonding;
	bonding = priv->tc_priv->tc_mode == TC_PTM_BND_MODE ? 1 : 0;
	reset_erb_owner_bit(priv, bonding);
	ttha_enable(priv);
				...
				u32 ep_id, int bonding)
{
	...
	/* Set VRX518 PPE clock 576MHz */
#ifndef FEATURE_PPE_DYNAMIC_FEQ
	tc_ppe_clkset(priv->ep, PPE_CLK_576MHZ);
#else
	tc_priv->dsl_fw_freq_ind = tc_r32(DSL_FW_FREQ_INC);
	tc_dbg(priv->tc_priv, MSG_SWITCH, "DSL FW Frequency indication %d\n",
		tc_priv->dsl_fw_freq_ind);
	switch (tc_priv->dsl_fw_freq_ind) {
	    case 0:
	    tc_ppe_clkset(priv->ep, PPE_CLK_576MHZ);
	    break;
	    case 1:
	    tc_ppe_clkset(priv->ep, PPE_CLK_494MHZ);
	    break;
	    case 2:
	    tc_ppe_clkset(priv->ep, PPE_CLK_432MHZ);
	    break;
	    case 3:
	    tc_ppe_clkset(priv->ep, PPE_CLK_288MHZ);
	    break;
	    default:
	    tc_ppe_clkset(priv->ep, PPE_CLK_576MHZ);
	    break;
	...
	tc_ppe_rst(priv->ep);
#ifdef FEATURE_PPE_DYNAMIC_FEQ
	tc_w32(tc_priv->dsl_fw_freq_ind, DSL_FW_FREQ_INC);
#endif
...
static void ptm_aca_init(struct ptm_ep_priv *priv, int bonding)
{
	...
	tc_priv = priv->tc_priv;
#endif
	priv->tc_priv->tc_ops.soc_cfg_get(
		&g_ptm_priv->dev->dev,
		&priv->tc_priv->cfg,
		ptm_id(priv), bonding);
...
static void ptm_umt_init(struct ptm_ep_priv *priv, int bonding)
{
	...
	v_dst = dst | (u32)priv->ep->membase;
#ifdef FIX_DMA_BYTE_ALIGNMENT
	if (bonding && (priv->ep_id == DS_BOND) && tcpriv->param.ena_dma_tc_sg) {
		tcpriv->soc_tc_conf.aca_umt_rxin_hd_base_vir =
			dst | (u32)priv->ep->membase;
		tcpriv->soc_tc_conf.aca_umt_rxin_hd_base_phy =
			dst |= priv->ep->phy_membase;

		tcpriv->soc_tc_conf.mips_tc_umt_rxin_hd_base_vir =
				(u32)dma_zalloc_coherent(&ptm_tc->dev->dev,
					sizeof(u32),
					&tcpriv->soc_tc_conf.mips_tc_umt_rxin_hd_base_phy,
					GFP_ATOMIC | GFP_DMA
				);
		dst = tcpriv->soc_tc_conf.mips_tc_umt_rxin_hd_base_phy;
		pr_info("%s: NEW UMT ADDR 0x%08x %p\n", __func__,
					dst, tcpriv->soc_tc_conf.mips_tc_umt_rxin_hd_base_vir
			);
	} else
		dst |= priv->ep->phy_membase;
#else
	...
	pr_info("%s: NEW UMT ADDR 0x%08x ID %d\n", __func__,
		dst, priv->ep_id);


	tcpriv->tc_ops.umt_init(ptm_id(priv), period, dst, v_dst);
			...
			u32 id, enum dsl_tc_mode tc_mode)
{
	...
	if (!bonding) {
		...
		ptm_umt_init(priv, bonding);
		ptm_aca_init(priv, bonding);
		ptm_umt_start(priv);
...
#ifdef FIX_DMA_BYTE_ALIGNMENT
		/* Init resource for SoC's TC to do DMA scatter-gather */
		if (priv->tc_priv->param.ena_dma_tc_sg)
			priv->tc_priv->tc_ops.soc_fw_init(&ptm_tc->dev->dev);
		else
			tc_info(tc_priv, MSG_SWITCH,
			    "DMA Byte-alignment scatter-gather feature is disable\n");
...
void ptm_tc_unload(enum dsl_tc_mode tc_mode)
{
    ...
    if (ptm_tc->tc_priv == NULL)
        return;
    /* Disable US traffic */
	ptm_tc->tc_priv->tc_ops.disable_us(1);
#ifdef FIX_DMA_BYTE_ALIGNMENT
	/* Stop MIPS TC SG */
	while (tc_mode == TC_PTM_BND_MODE) {
		if (!ptm_tc->tc_priv) {
			pr_err("%s-%d: Invalid TC Priv\n", __func__, __LINE__);
			break;
		}
		if (!ptm_tc->tc_priv->tc_ops.soc_fw_deinit) {
			pr_err("%s-%d: Invalid soc_fw_deinit\n", __func__, __LINE__);
			break;
		}
		if (ptm_tc->tc_priv->param.ena_dma_tc_sg)
			ptm_tc->tc_priv->tc_ops.soc_fw_deinit(&ptm_tc->dev->dev);
		break; /* while loop */
	}
#endif
	printk("%s: mode[%d] ep_id[%d]\n", __func__, tc_mode, ptm_tc->ep_id);
...
static void print_qos_mib(struct seq_file *seq, struct ptm_priv *priv)
{
	qosq_mib_t qos;
	...
	for (i = 0; i < QOSQ_NUM; i++) {
		...
		tc_mem_read(priv, &qos,
			fpi_addr(__QOSQ_MIB_BASE + off), sizeof(qos));
		seq_printf(seq, "Queue ID %d\n", i);
...
int proc_read_ptm_qosmib(struct seq_file *seq, void *v)
{
	struct ptm_ep_priv *priv;
	struct ptm_priv *ptm_tc;
	...
	print_qos_mib(seq, ptm_tc);

	return 0;
}

ssize_t proc_write_ptm_qosmib(struct file *file, const char __user *buf,
				size_t count, loff_t *data)
{
	char str[32];
	int len, rlen;
	...
	if ((vrx_strcmpi(param_list[0], "clear") == 0)
		...
		pr_info("echo clear/clean/c > qosmib\n");
...
/**
 *  drop_en:  1 - Enable Drop in QoS, 0 -Disable Drop in QoS
 *  flag:   0 - Get the drop cfg, 1 - Set the drop
 */
int ptm_qos_drop(struct ptm_ep_priv *priv, int drop_en, int flag)
{
	int status;
	if (!flag) {
		status = sb_r32(__QOSQ_NO_DROP);
		if (status)
			return 0;
		return 1;
	} else {
		if (drop_en)
				sb_w32(0, __QOSQ_NO_DROP);
			else
				sb_w32(1, __QOSQ_NO_DROP);
	}
	return 0;
...
#ifdef FIX_DMA_BYTE_ALIGNMENT
#define MIPS_TC_PROC_LEVEL_TAB_MAX 8
u32 state;
struct mips_tc_mib mib;
#define PRINT_TO_CONSOLE 1
#ifndef PRINT_TO_CONSOLE
#define mips_tc_print(seq, fmt, args...) seq_printf(seq, fmt, ##args)
#else
#define mips_tc_print(seq, fmt, args...) printk(fmt, ##args)
#endif

void print_mips_tc_mib(struct seq_file *seq,
	struct mips_tc_mib *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	int i;
	memset(level_tab, 0, MIPS_TC_PROC_LEVEL_TAB_MAX);
	for (i = 0; (i < level) && (i < MIPS_TC_PROC_LEVEL_TAB_MAX); i++)
		level_tab[i] = ' ';
	mips_tc_print(seq, "%s%s at %p\n",
		level_tab, name, mem);
	mips_tc_print(seq, " %s%s:%u\n",
		level_tab, "jobs", mem->jobs);
}

void print_mips_tc_info(struct seq_file *seq,
	struct mips_tc_info *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, "%s%s at %p\n",
		level_tab, name, mem);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "state", mem->state);
	print_mips_tc_mib(seq, &mem->mib, "MIB", level + 1);
}



void print_mips_tc_q_cfg_ctxt(struct seq_file *seq,
	struct mips_tc_q_cfg_ctxt *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, "%s%s at %p\n",
		level_tab, name, mem);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "des_in_own_val", mem->des_in_own_val);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "_res0", mem->_res0);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "des_num", mem->des_num);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "des_base_addr", mem->des_base_addr);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "rd_idx", mem->rd_idx);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "wr_idx", mem->wr_idx);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "_dw_res0", mem->_dw_res0);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "_dw_res0", mem->_dw_res1);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "_dw_res0", mem->_dw_res2);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "rd_frag_cnt", mem->rd_frag_cnt);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "wr_frag_cnt", mem->wr_frag_cnt);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "rd_pkt_cnt", mem->rd_pkt_cnt);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "wr_pkt_cnt", mem->wr_pkt_cnt);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "wr_byte_cnt", mem->wr_byte_cnt);
}
void print_mips_tc_fw_memcpy_ctxt_t(struct seq_file *seq,
	struct mips_tc_fw_memcpy_ctxt_t *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "_res0", mem->_res0);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "sop", mem->sop);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "eop", mem->eop);

	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "byte_off", mem->byte_off);

	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "src_data_ptr", mem->src_data_ptr);

	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "dst_data_ptr", mem->dst_data_ptr);

	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "data_len", mem->data_len);
}

void print_mips_tc_fw_pp_ctxt_t(struct seq_file *seq,
	struct mips_tc_fw_pp_ctxt_t *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "data_len", mem->data_len);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "pkt_data_len", mem->pkt_data_len);
}


void print_mips_tc_mem_conf(struct seq_file *seq,
	struct mips_tc_mem_conf *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, "%s%s at %p\n",
		level_tab, name, mem);
	print_mips_tc_q_cfg_ctxt(seq,
		&mem->soc_rx_out_src_ctxt,
		"soc_rx_out_src_ctxt",
		level + 1);
	print_mips_tc_q_cfg_ctxt(seq,
		&mem->soc_sg_dma_tx_ctxt,
		"soc_sg_dma_tx_ctxt",
		level + 1);
	print_mips_tc_q_cfg_ctxt(seq,
		&mem->soc_sg_dma_rx_ctxt,
		"soc_sg_dma_rx_ctxt",
		level + 1);
	print_mips_tc_q_cfg_ctxt(seq,
		&mem->soc_rx_out_dst_ctxt,
		"soc_rx_out_dst_ctxt",
		level + 1);
	print_mips_tc_fw_memcpy_ctxt_t(seq,
		&mem->fw_memcpy_ctxt,
		"fw_memcpy_ctxt",
		level + 1);
	print_mips_tc_fw_pp_ctxt_t(seq,
		&mem->fw_pp_ctxt,
		"fw_pp_ctxt",
		level + 1);
	mips_tc_print(seq, " %s%s: 0x%p\n",
		level_tab, "cache_rxout_ptr",
		mem->cache_rxout_ptr);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "aca_hw_rxin_hd_acc_addr",
		mem->aca_hw_rxin_hd_acc_addr);
	mips_tc_print(seq, " %s%s: %u\n",
		level_tab, "tc_mode_bonding",
		mem->tc_mode_bonding);
}

void print_mips_tc_ctrl_dbg(struct seq_file *seq,
	struct mips_tc_ctrl_dbg *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, "%s%s at %p\n",
		level_tab, name, mem);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "mips_tc_control",
		mem->mips_tc_control);
	mips_tc_print(seq, " %s%s: %u.%u.%u\n",
		level_tab, "Version:",
		mem->ver_major, mem->ver_mid, mem->ver_minor);
	mips_tc_print(seq, " %s%s: 0x%08x\n",
		level_tab, "dbg_flags",
		mem->dbg_flags);
	mips_tc_print(seq, " %s%s: 0x%08x 0x%08x\n",
		level_tab, "Private Data",
		(u32)mem->priv_data_vir, (u32)mem->priv_data_phy);
	mips_tc_print(seq, " %s%s: %u, ctrl_max_process_pkt: %u\n",
		level_tab, "msg_cur_idx",
		mem->msg_cur_idx, mem->ctrl_max_process_pkt);
	mips_tc_print(seq, " %s%s: %p\n",
		level_tab, "msgs array",
		&mem->msgs[0]);
}

void print_mips_tc_sg_mem(struct seq_file *seq,
	struct mips_tc_sg_mem *mem,
	char *name,
	u32 level)
{
	char level_tab[MIPS_TC_PROC_LEVEL_TAB_MAX];
	...
	mips_tc_print(seq, "%s%s at %p\n",
		level_tab, name, mem);
	print_mips_tc_ctrl_dbg(seq,
		&mem->ctrl, "Control & Debug", level + 1);
	print_mips_tc_mem_conf(seq,
		&mem->conf, "Memory Configuration", level + 1);
	print_mips_tc_info(seq,
		&mem->mib, "MIB Information", level + 1);
}

int proc_ptm_read_soc_fw(struct seq_file *seq, void *v)
{
	struct ptm_ep_priv *priv = (struct ptm_ep_priv *)seq->private;
	struct ptm_priv *ptm_tc;
	struct soc_tc_dma_conf *soc_dma_conf;
	ptm_tc = priv->ptm_tc;
	/* Print DMA memory */
	soc_dma_conf = &priv->tc_priv->soc_tc_conf;
	mips_tc_print(seq, "rch_dbase_phy: 0x%08x\n",
			soc_dma_conf->rch_dbase_phy);
	mips_tc_print(seq, "rch_dbase_vir: %p\n",
			soc_dma_conf->rch_dbase_vir);
	mips_tc_print(seq, "tch_dbase_phy: 0x%08x\n",
			soc_dma_conf->tch_dbase_phy);
	mips_tc_print(seq, "tch_dbase_vir: %p\n",
			soc_dma_conf->tch_dbase_vir);
	mips_tc_print(seq, "gather_dbase_phy: 0x%08x\n",
			soc_dma_conf->rxout_dst_dbase_phy);
	mips_tc_print(seq, "gather_dbase_vir: %p\n",
			soc_dma_conf->rxout_dst_dbase_vir);
	mips_tc_print(seq, "dma_ch_conf.rch_dbase: 0x%08x\n",
			soc_dma_conf->dma_ch_conf.rch_dbase);
	mips_tc_print(seq, "dma_ch_conf.tch_dbase: 0x%08x\n",
			soc_dma_conf->dma_ch_conf.tch_dbase);
	mips_tc_print(seq, "dma_ch_conf.rch_dnum: 0x%08x\n",
			soc_dma_conf->dma_ch_conf.rch_dnum);
	mips_tc_print(seq, "dma_ch_conf.tch_dnum: 0x%08x\n",
			soc_dma_conf->dma_ch_conf.tch_dnum);
	mips_tc_print(seq, "dma_ch_conf.onoff: 0x%08x\n",
			soc_dma_conf->dma_ch_conf.onoff);
	mips_tc_print(seq, "dma_ch_conf.rch_name: %s\n",
			soc_dma_conf->dma_ch_conf.rch_name);
	mips_tc_print(seq, "dma_ch_conf.tch_name: %s\n",
			soc_dma_conf->dma_ch_conf.tch_name);
	print_mips_tc_sg_mem(seq,
		(struct mips_tc_sg_mem *)soc_dma_conf->shared_mem,
		"Shared Memory Information", 0);
	return 0;
}

ssize_t proc_ptm_write_soc_fw(struct file *file, const char __user *buf,
				size_t count, loff_t *data)
{
	struct ptm_ep_priv *priv;
	char local_buf[128];
